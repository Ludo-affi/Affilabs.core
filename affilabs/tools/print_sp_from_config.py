...
def main():
    parser = argparse.ArgumentParser(
        description="Print S/P values from device_config.json",
    )
    ...
    if it_ms is not None:
        print(f"  Integration time (ms): {it_ms}")
    ...
    if s_max or p_max:
        ...
        if s_max:
            print(f"  S-ref max: {({k: round(v, 1) for k, v in s_max.items()})}")
        if p_max:
            print(f"  P-ref max: {({k: round(v, 1) for k, v in p_max.items()})}")

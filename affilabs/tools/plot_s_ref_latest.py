...
def plot_s_ref(serial: str | None = None, show: bool = False) -> Path | None:
    ...
    if not loaded:
        print(
            "No device_config.json found. Expected under src/config/devices/<SERIAL>/",
        )
        return None
    ...
    if (
        ...
        and isinstance(wavelengths, list)
        and len(wavelengths) == npts
    ):
        x = wavelengths
    ...
    for ch in sorted(s_ref.keys()):
        ...
        if len(y) != npts:
            print(
                f"Warning: channel {ch} length mismatch ({len(y)} vs {npts}); skipping",
            )
            continue
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot S-pol (S-ref) from latest device config",
    )
    parser.add_argument("--serial", "-s", help="Device serial (e.g., FLMT09116)")
    parser.add_argument(
        "--show",
        action="store_true",
        help="Show interactive window (if supported)",
    )

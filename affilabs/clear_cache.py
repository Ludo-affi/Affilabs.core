...
def clear_all_cache(root_dir="."):
    ...
    for pycache in root.rglob("__pycache__"):
        try:
            ...
            print(f"[WARN]  Could not remove {pycache}: {e}")
    ...
    for pyo in root.rglob("*.pyo"):
        try:
            ...
            print(f"[WARN]  Could not remove {pyo}: {e}")
    ...
    for pyd in root.rglob("*.pyd"):
        ...
        if ".venv" not in str(pyd) and "venv" not in str(pyd):
            try:
                ...
                print(f"[WARN]  Could not remove {pyd}: {e}")
...
if __name__ == "__main__":
    ...
    print()
    print("=" * 60)
    print("[OK] Ready to run application with fresh imports")
    print()

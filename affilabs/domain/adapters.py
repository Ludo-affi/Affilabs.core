...
def led_calibration_result_to_domain(legacy_result) -> CalibrationData:
    ...
    if hasattr(legacy_result, 'qc_results') and legacy_result.qc_results:
        for channel, qc_data in legacy_result.qc_results.items():
            if isinstance(qc_data, dict) and channel in legacy_result.s_pol_ref:
                ...
                if hasattr(legacy_result, 'p_pol_ref') and legacy_result.p_pol_ref and channel in legacy_result.p_pol_ref:
                    validation_data['p_pol_raw'] = legacy_result.p_pol_ref[channel].copy()

                # Add transmission spectrum if available
                if hasattr(legacy_result, 'transmission') and legacy_result.transmission and channel in legacy_result.transmission:
    ...
    logger.info(f"[ADAPTER] Converting calibration result: is_dict={is_dict}, type={type(legacy_result).__name__}")

    # Extract S-pol reference
    if is_dict:
        s_pol_ref = {k: v.copy() if hasattr(v, 'copy') else v for k, v in legacy_result.get('s_pol_ref', {}).items()}
    else:
    ...
    if is_dict:
        ...
        p_mode_intensities = dict(legacy_result.p_mode_intensities if hasattr(legacy_result, 'p_mode_intensities') else legacy_result.p_mode_intensity) if (hasattr(legacy_result, 'p_mode_intensities') or hasattr(legacy_result, 'p_mode_intensity')) else {}
        s_mode_intensities = dict(legacy_result.s_mode_intensities if hasattr(legacy_result, 's_mode_intensities') else legacy_result.s_mode_intensity) if (hasattr(legacy_result, 's_mode_intensities') or hasattr(legacy_result, 's_mode_intensity')) else dict(legacy_result.ref_intensity) if hasattr(legacy_result, 'ref_intensity') and legacy_result.ref_intensity else {}
    ...
    cal_data = CalibrationData(
        ...
        integration_time_s=float(safe_get('s_integration_time')) if safe_get('s_integration_time') else 0.0,
        integration_time_p=float(safe_get('p_integration_time')) if safe_get('p_integration_time') else float(safe_get('s_integration_time')) if safe_get('s_integration_time') else 0.0,
        ...
        dark_s={k: v.copy() if hasattr(v, 'copy') else v for k, v in (safe_get('channel_dark_s') or {}).items()},
        dark_p={k: v.copy() if hasattr(v, 'copy') else v for k, v in (safe_get('channel_dark_p') or {}).items()},
    ...
    cal_data._wave_min_index = int(legacy_result.wave_min_index) if hasattr(legacy_result, 'wave_min_index') else 0
    cal_data._wave_max_index = int(legacy_result.wave_max_index) if hasattr(legacy_result, 'wave_max_index') else len(wavelengths)

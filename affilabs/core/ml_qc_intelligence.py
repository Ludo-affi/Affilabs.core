...
class MLQCIntelligence:
    ...
    def __init__(self, device_serial: str, data_dir: Path | None = None) -> None:
    ...
    def predict_next_calibration(self) -> CalibrationPrediction:
        ...
        if failure_prob > 0.7:
            ...
            risk_level = "medium"
            recommendations.append("[WARN] MEDIUM RISK: Schedule maintenance soon")
        else:
            risk_level = "low"
    ...
    def check_optical_alignment(
        ...
        if drift_detected:
            warning_message = (
                f"[WARN] Calibration P/S ratio drifted {deviation:.2f} from baseline "
    ...
    def generate_intelligence_report(self) -> str:
        ...
        if len(self.alignment_baseline) >= 3:
            ...
            lines.append("  Status: [OK] Baseline established")
        else:
            lines.append(

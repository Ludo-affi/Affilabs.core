...
INTEGRATION_TIME_SETTLING_MS = (
    10  # Time for integration time change to take effect (converted to seconds in code)
)
...
def _get_servo_positions_from_config(
    ...
    """
    # Handle both dict and DeviceConfig object
    if hasattr(device_config, "config"):
    ...
    if "hardware" in cfg:
        ...
        logger.debug(
            f"[DEBUG] device_config keys: {list(cfg.keys()) if isinstance(cfg, dict) else 'NOT A DICT'}"
        )
        logger.debug(f"[DEBUG] device_config type: {type(device_config)}")
...
def save_calibration_result_json(
    ...
    try:
        ...
        data = {
            ...
            "wavelengths": result.wave_data.tolist() if result.wave_data is not None else [],
            # CRITICAL: Save reference spectra for live acquisition
            "s_pol_ref": {ch: spec.tolist() for ch, spec in (result.s_pol_ref or {}).items()},
            "p_pol_ref": {ch: spec.tolist() for ch, spec in (result.p_pol_ref or {}).items()},
...
def _run_step6_timing_alignment(
    ...
    for cycle_idx in range(discovery_cycles):
        ...
        time.sleep(
            FIRMWARE_COMMAND_PROCESSING_TIME_S
        )  # Allow firmware to process all intensity settings
...
def acquire_raw_spectrum(
    ...
    try:
        ...
        # Use read_intensity() (HAL method for full spectrum)
        # Note: Could use read_roi() but calibration needs full spectrum for wavelength calibration
        if num_scans == 1:
            spectrum = usb.read_intensity()  # HAL: Full spectrum read
        else:
...
def converge_integration_time(
    ...
    for iteration in range(max_iterations):
        ...
        for ch in ch_list:
            ...
            status = "[WARN] SAT" if is_saturated else ("[OK]" if in_range else "[WARN]")
            logger.info(
        ...
        if iteration < max_iterations - 1:
            ...
            # CRITICAL: Clamp to timing budget FIRST (DETECTOR_WAIT_MS = max per-scan)
            # This ensures calibration respects the same constraints as live acquisition
            INTEGRATION_TIME_CAP_MS = DETECTOR_WAIT_MS  # 60ms max per scan

            if current_integration > INTEGRATION_TIME_CAP_MS:
                logger.warning(
                    f"   ⚠️ Integration time ({current_integration:.1f}ms) exceeds timing budget cap ({INTEGRATION_TIME_CAP_MS:.1f}ms)"
                )
                logger.warning(
                    f"   Capping to {INTEGRATION_TIME_CAP_MS:.1f}ms to prevent timing violations in live acquisition"
                )
                current_integration = INTEGRATION_TIME_CAP_MS
    ...
    if has_led_query:
        for attempt in range(max_retries):
            ...
            if all_off:
                ...
                if led_state2 and all(intensity <= 1 for intensity in led_state2.values()):
                    logger.info(
    ...
    if has_led_query:
        ...
        if led_state is None:
            ...
            if all_off:
                ...
                led_state2 = ctrl.get_all_led_intensities()
                if led_state2:
                    channels_to_check2 = {ch: val for ch, val in led_state2.items() if ch != "d"}
...
def _step6b_process_polarization_with_swap_check(result, ch_list):
    ...
    try:
        s_means_pre = {
            ch: float(np.mean(result.s_raw_data[ch])) for ch in ch_list if ch in result.s_raw_data
        }
        p_means_pre = {
            ch: float(np.mean(result.p_raw_data[ch])) for ch in ch_list if ch in result.p_raw_data
        }
        s_medians_pre = {
            ch: float(np.median(result.s_raw_data[ch])) for ch in ch_list if ch in result.s_raw_data
        }
        p_medians_pre = {
            ch: float(np.median(result.p_raw_data[ch])) for ch in ch_list if ch in result.p_raw_data
        }
        ...
        for ch in ch_list:
            if ch in s_means_pre and ch in p_means_pre:
                ratio = (p_means_pre[ch] / s_means_pre[ch]) if s_means_pre[ch] > 0 else float("inf")
        ...
        if all(
            ch in s_means_pre and ch in p_means_pre and p_means_pre[ch] > s_means_pre[ch]
            for ch in ch_list
        ):
            logger.warning(
            ...
            result.qc_results["sp_swap_reason"] = "P_raw > S_raw across all channels"
        else:
...
def _step6e_finalize_and_save(
    ...
    if hasattr(result, "s_channel_integration_times") and result.s_channel_integration_times:
        logger.info(
    ...
    # Import from settings (single source of truth)
    import settings as root_settings
    ...
    if hasattr(result, "p_channel_integration_times") and result.p_channel_integration_times:
        logger.info(
...
def _step6d_comprehensive_qc_validation(
    ...
    ch_error_list = [ch for ch, qc in qc_results.items() if not qc["overall_pass"]]
...
def _validate_calibration_prerequisites(device_config, detector_serial, result):
    ...
    Returns:
        LEDCalibrationModelLoader: Cached model loader instance for reuse in Step 3
    ...
    try:
        ...
        # Return cached model loader for reuse in Step 3
        return model_loader
...
def run_full_7step_calibration(
    ...
    try:
        ...
        # Cache the model loader for reuse in Step 3
        cached_model_loader = _validate_calibration_prerequisites(
            device_config, detector_serial, result
        )
        ...
        if start_at_step <= 2:
            ...
            result.detector_saturation_threshold = wavelength_data["detector_saturation_threshold"]
        ...
        device_config_det = device_config
        ...
        convergence_result = run_convergence_calibration_steps_3_to_5(
            ...
            cached_model_loader=cached_model_loader,  # Pass cached model from prerequisites check
        ...
        try:
            s_means = {ch: float(np.mean(result.s_raw_data[ch])) for ch in result.s_raw_data}
        ...
        # === NUM_SCANS ALREADY CALCULATED ===
        # num_scans was calculated BEFORE Step 4 (S-pol reference capture)
        # This ensures reference captures use SAME num_scans as live data
        # See lines ~4191-4204 where num_scans is calculated and stored in result["num_scans"]
        # No need to recalculate here - just verify it exists
        if not hasattr(result, "num_scans") or result.num_scans is None:
            logger.warning("num_scans not found in result, using fallback value 3")
            result.num_scans = 3
...
def run_convergence_calibration_steps_3_to_5(
    ...
    Args:
        cached_model_loader: Optional pre-loaded LEDCalibrationModelLoader from prerequisites check
    ...
    """
    try:
        from affilabs.core.led_convergence import run_convergence
    ...
    try:
        ...
        logger.info("")
        logger.info("=" * 80)
        logger.info("📊 STEP 3: Using Cached 3-Stage Linear Model (Predicted LED Values)")
        logger.info("=" * 80)
        ...
        try:
            from affilabs.services.led_model_loader import ModelNotFoundError

            detector_serial = getattr(usb, "serial_number", None)
            logger.info(f"🔍 Detector serial: {detector_serial}")

            if detector_serial and detector_serial != "Unknown":
                # Use cached model from prerequisites check (avoids reloading)
                if cached_model_loader:
                    logger.info("✓ Using cached model (already validated in prerequisites)")
                    model = cached_model_loader
                else:
                    # Fallback: load model if cache not available
                ...
                logger.info(f"✓ Model timestamp: {model_info['timestamp']}")
                ...
                ch_list_upper = [ch.upper() for ch in ch_list]  # Model expects uppercase
                ...
                model_predicted_leds_s = {k.lower(): v for k, v in model_predicted_leds_s.items()}
                model_predicted_leds_p = {k.lower(): v for k, v in model_predicted_leds_p.items()}
            ...
            logger.error("=" * 80)
            logger.error("⚠️ 3-STAGE LINEAR MODEL NOT FOUND - AUTO-TRIGGERING OEM CALIBRATION")
            logger.error("=" * 80)
            logger.error(f"   Detector serial: {detector_serial}")
            logger.error(f"   Model error: {e}")
            logger.error("")
            logger.error("   🔧 SOLUTION: Running OEM calibration to create model...")
            logger.error("")
            logger.error("   This will:")
            logger.error("   1. Measure LED characteristics at multiple integration times")
            logger.error("   2. Build 3-stage linear model")
            logger.error("   3. Store model in led_calibration_official/<serial>/")
            logger.error("   4. Return to complete this calibration")
            logger.error("=" * 80)

            # Import OEM calibration script
            from pathlib import Path
            import subprocess
            import sys

            oem_script = Path("led_calibration_official/1_create_model.py")
            if oem_script.exists():
                logger.info(f"✓ Found OEM calibration script: {oem_script}")
                logger.info("   Running OEM calibration...")
                logger.info("=" * 80)

                try:
                    # Run OEM calibration as subprocess
                    result_code = subprocess.run(
                        [sys.executable, str(oem_script)],
                        cwd=Path.cwd(),
                    ...
                    if result_code.returncode == 0:
                        logger.info("=" * 80)
                        logger.info("✅ OEM CALIBRATION COMPLETE - Model created successfully")
                        logger.info("=" * 80)
                        logger.info("   Recalculating LED predictions with new model...")
                        logger.info("")

                        # Reload model and recalculate predictions
                        from affilabs.services.led_model_loader import LEDCalibrationModelLoader

                        model = LEDCalibrationModelLoader()
                        model.load_model(detector_serial)
                        logger.info("   ✓ Model loaded successfully after OEM calibration")

                        # Recalculate model parameters with new model
                        target_counts = target_percent * detector_params.max_counts
                        ch_list_upper = [ch.upper() for ch in ch_list]
                        ...
                        logger.info(
                            f"   ✓ Optimal integration time: {model_initial_integration:.1f}ms"
                        )
                        logger.info(f"   ✓ LED intensities (S-pol): {model_predicted_leds_s}")

                        # Calculate P-pol LED intensities
                        model_predicted_leds_p = model.calculate_all_led_intensities(
                            polarization="P",
                            time_ms=model_initial_integration,
                            target_counts=target_counts,
                        ...
                        logger.info(f"   ✓ LED intensities (P-pol): {model_predicted_leds_p}")
                        logger.info("   ✓ Model parameters ready - continuing calibration...")

                        # Cache model for validation step
                        loaded_model = model
                        # Continue with calibration flow below...
                    else:
                        logger.error(
                            f"❌ OEM calibration failed with exit code: {result_code.returncode}"
                        )
                        result["error"] = "OEM calibration failed - cannot proceed without model"
                        return result

                except Exception as oem_error:
                    logger.error(f"❌ Failed to run OEM calibration: {oem_error}")
                    result["error"] = f"OEM calibration execution failed: {oem_error}"
                    return result
            else:
                logger.error(f"❌ OEM calibration script not found: {oem_script}")
                logger.error("   Cannot automatically create model")
                logger.error("   Manual intervention required")
                result["error"] = f"Model not found and OEM script missing: {e}"
                return result
        except Exception as e:
            logger.error("=" * 80)
        ...
        if model_initial_integration is None:
            ...
            logger.error("=" * 80)
            result["error"] = "Model failed to provide integration time - cannot proceed"
        ...
        s_channel_times = {
            ch: float(s_per_ch_results[ch]["final_integration_ms"]) for ch in s_per_ch_results
        }
        ...
        if s_shared_int is not None:
            ...
            try:
                result["s_integration_time"] = (
                    float(np.median(list(s_channel_times.values()))) if s_channel_times else None
                )
            except Exception:
                result["s_integration_time"] = s_per_ch_results[ch_list[0]]["final_integration_ms"]
        ...
        result["convergence_summary"] = {
            ...
            "channels": {
                ch: {
                    ...
                    "final_top50_counts": round(
                        s_per_ch_results[ch].get("final_top50_counts", 0), 2
                    ),
        ...
        try:
            ...
            measured_leds_upper = {ch.upper(): result["s_mode_intensities"][ch] for ch in ch_list}
            ...
            result["p_pol_predicted_leds"] = {ch: p_pol_predictions[ch.upper()] for ch in ch_list}
        ...
        # === STEP 4: Capture S-pol reference (RAW) with same num_scans as P-pol ===
        logger.info("")
        logger.info("📸 Step 4: Capturing S-pol reference spectra...")

        # CRITICAL: Use NUM_SCANS from settings (user-configurable via Advanced Settings)
        # This ensures consistent scan count between calibration and live acquisition
        from settings import NUM_SCANS

        num_scans = NUM_SCANS  # Use configured value (default: 3)
        result["num_scans"] = num_scans

        logger.info("   [CRITICAL] S-pol reference capture parameters:")
        logger.info(f"   Num scans: {num_scans}")
        logger.info(f"   S-pol integration time: {result['s_integration_time']:.1f}ms")
        logger.info("")
        ...
        # IMPORTANT: Use num_scans for averaging (same as P-pol)
        for ch in ch_list:
            ...
            # CRITICAL: Multi-scan averaging for full spectrum
            if num_scans == 1:
                full = usb.read_intensity()
            else:
                stack = np.array([usb.read_intensity() for _ in range(num_scans)])
                full = np.mean(stack, axis=0).astype("u4")

            # Store RAW ROI under explicit and legacy keys
            roi_slice = full[wave_min_index:wave_max_index]
            result["s_raw_roi"][ch] = roi_slice
            result["s_pol_ref"][ch] = roi_slice  # legacy compat: was misnamed but used as raw
            ctrl.set_intensity(ch, 0)

        logger.info(f"   [OK] S-pol ref captured: {len(result['s_pol_ref'])} channels")
        try:
            means = {ch: float(np.mean(result["s_raw_roi"][ch])) for ch in result["s_raw_roi"]}
            logger.info(
        ...
        if not p_ok:
            # RELAXED CRITERIA: Check if weakest channel >= 75% MAX_COUNTS, others >= 80% MAX_COUNTS
            max_counts = detector_params.max_counts
            channel_signals = {ch: p_per_ch_results[ch]["final_top50_counts"] for ch in ch_list}
            min_signal = min(channel_signals.values())
            min_channel = [ch for ch, sig in channel_signals.items() if sig == min_signal][0]
            other_signals = [sig for sig in channel_signals.values() if sig != min_signal]

            # Relaxed pass: weakest >= 75% max_counts, others >= 80% max_counts
            threshold_weak = 0.75 * max_counts
            threshold_others = 0.80 * max_counts

            if min_signal >= threshold_weak and all(
                sig >= threshold_others for sig in other_signals
            ):
                logger.warning(
                    "   ⚠️ P-mode convergence didn't fully converge, but meets relaxed criteria:"
                )
                logger.warning(
                    f"      Weakest channel {min_channel.upper()}: {min_signal:.0f} counts (>= 75% threshold: {threshold_weak:.0f})"
                )
                logger.warning(
                    f"      Other channels: {other_signals} (all >= 80% threshold: {threshold_others:.0f})"
                )
                logger.warning("   ✓ Accepting calibration under relaxed criteria")
                p_ok = True  # Override failure
            else:
                result["error"] = "P-mode convergence failed: Did not converge to target"
                logger.error(f"[ERROR] {result['error']}")
                logger.error(
                    f"   Weakest channel {min_channel.upper()}: {min_signal:.0f} counts (need >= {threshold_weak:.0f})"
                )
                logger.error(
                    f"   Other channels: {other_signals} (need all >= {threshold_others:.0f})"
                )
                return result
        ...
        # CRITICAL: Cap P-mode integration time to timing budget (DETECTOR_WAIT_MS)
        p_integration_uncapped = (
            p_shared_int
        ...
        DETECTOR_WAIT_MS = root_settings.DETECTOR_WAIT_MS
        result["p_integration_time"] = min(p_integration_uncapped, DETECTOR_WAIT_MS)

        if p_integration_uncapped > DETECTOR_WAIT_MS:
            logger.warning(
                f"   ⚠️ P-mode integration ({p_integration_uncapped:.1f}ms) exceeds timing budget - capped to {DETECTOR_WAIT_MS}ms"
            )
        ...
        # === STEP 5B: Capture P-pol reference (RAW) with num_scans ===
        logger.info("")
        ...
        # Use same num_scans as S-pol (already calculated)
        num_scans = result["num_scans"]
        logger.info(f"   Using num_scans={num_scans} (matching S-pol and live acquisition)")
        ...
        for ch in ch_list:
            ...
            result["p_pol_ref"][ch] = roi_slice  # legacy compat: was misnamed but used as raw
            ctrl.set_intensity(ch, 0)

        logger.info(f"   [OK] P-pol ref captured: {len(result['p_pol_ref'])} channels")
        try:
            means = {ch: float(np.mean(result["p_raw_roi"][ch])) for ch in result["p_raw_roi"]}
            logger.info(
        ...
        # CRITICAL: Dark must match P-pol acquisition conditions for live data
        # Use P-pol integration time and num_scans (same as live acquisition)
        p_int_time = result["p_integration_time"]
        num_scans = result["num_scans"]

        logger.info("   [CRITICAL] Dark capture parameters (matching P-pol live acquisition):")
        logger.info(f"   Integration time: {p_int_time:.2f}ms (P-pol)")
        logger.info(f"   Num scans: {num_scans}")
        logger.info("")
        logger.info("   [NOTE] S-pol uses different integration but SAME num_scans")
        logger.info("")

        # Always use P-pol integration time for dark (matches live acquisition)
        usb.set_integration(p_int_time)
        time.sleep(0.2)

        # CRITICAL: Multi-scan averaging for dark spectrum
        if num_scans == 1:
            dark_full = usb.read_intensity()
        else:
            stack = np.array([usb.read_intensity() for _ in range(num_scans)])
            dark_full = np.mean(stack, axis=0).astype("u4")

        # Extract ROI for each channel
        # Use same dark for both S and P (captured at P-pol conditions)
        dark_s_dict = {}
        dark_p_dict = {}
        for ch in ch_list:
        ...
        logger.info(
            f"   [OK] Dark captured at P-pol conditions: max={np.max(dark_full):.1f} counts",
        )
        logger.info(f"   [OK] Dark uses: {p_int_time:.2f}ms integration, {num_scans} scans")
            ...
            if (
                ...
                for _ch_idx, ch_name in enumerate(["A", "B", "C", "D"]):
                    if ch_name.lower() in ch_list:
                        ...
                        dark_roi[ch_name] = dark_spectrum_array[wave_min_index:wave_max_index]

...
def run_startup_calibration(
    ...
    try:
        ...
        # Use PWM values directly (NOT degrees) - firmware handles PWM->angle conversion
        # CRITICAL: sv command expects PWM values (0-255), not degrees
        s_pwm = int(s_pos)
        p_pwm = int(p_pos)

        # Set positions using sv command with PWM values
        ...
        logger.info(f"   ✅ Servo positioned at P-mode (PWM {p_pos})")
        logger.info("=" * 80)

...
def get_version() -> str:
    ...
    try:
        if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
            ...
            # This file is in affilabs/settings/, so go up 3 levels to root
        ...
        try:
            if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
                ...
                spec_file = Path(__file__).parent.parent.parent / "build" / "main.spec"
            match = search("name='ezControl (.+ )?v(.+)',", spec_file.read_text())
            if match is None:
                return "1.0.1-beta"  # Default fallback version
            return match[2]
...
# Integration time is dynamically calculated during calibration based on signal levels
...
# Live acquisition uses dynamic integration time from calibration, not hardcoded values

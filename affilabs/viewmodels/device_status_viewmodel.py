...
class DeviceStatusViewModel(QObject):
    ...
    def update_device_status(
        ...
        if status is None:
            ...
            if (
                ...
                and connection_state == ConnectionState.CONNECTED
            ):
    ...
    def record_device_error(self, device_type: DeviceType, error_message: str):
        ...
        if status:
            ...
            logger.error(
                f"{device_type.value} error (count={status.error_count}): {error_message}",
            )
    ...
    def get_device_status(self, device_type: DeviceType) -> DeviceStatus | None:
    ...
    def is_device_connected(self, device_type: DeviceType) -> bool:
        ...
        status = self._device_status.get(device_type)
        return (
            status is not None and status.connection_state == ConnectionState.CONNECTED
        )
    ...
    def get_system_status_summary(self) -> dict:
        ...
        connected_count = sum(
            1
            for status in self._device_status.values()
            if status.connection_state == ConnectionState.CONNECTED
        )
        healthy_count = sum(
            1 for status in self._device_status.values() if status.is_healthy
        )
        total_errors = sum(
    ...
    def _setup_device_manager_callbacks(self):
        ...
        self._device_manager.set_on_device_disconnected(
            self._on_hal_device_disconnected,
        )
    ...
    def _on_hal_state_changed(self, state: SystemState):
        ...
        elif state == SystemState.DISCONNECTED or state == SystemState.PARTIAL:
    ...
    def get_controller(self) -> IController | None:
    ...
    def get_spectrometer(self) -> ISpectrometer | None:
    ...
    def get_servo(self) -> IServo | None:
    ...
    def check_device_health(self):
        ...
        if health.controller:
            ...
            state = (
                ...
                if health.controller.connected
        ...
        if health.spectrometer:
            ...
            state = (
                ConnectionState.CONNECTED
                if health.spectrometer.connected
                else ConnectionState.DISCONNECTED
            )
            self.update_device_status(
                device_type=device_type,
        ...
        logger.debug(
            f"Health check complete - Ready: {health.is_ready()}, Healthy: {health.all_healthy()}",
        )

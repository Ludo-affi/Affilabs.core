...
def fix_led_calibration_signature():
    ...
    filepath = (
        r"c:\Users\ludol\ezControl-AI\Affilabs.core beta\utils\led_calibration.py"
    )
            ...
            while j < len(lines):
                if ") -> int:" in lines[j]:
                    ...
                    lines[j] = lines[j].replace(
                        ") -> int:",
                        f"{new_params[0]}{new_params[1]}) -> int:",
                    )
                    ...
                    if doc_start < len(lines):
                        ...
                        arg_end = doc_start + 1
                        while arg_end < len(lines) and (
                            lines[arg_end].startswith("        ")
                            or lines[arg_end].strip() == ""
                        ):
                            arg_end += 1
    ...
    print("[OK] Signature updated successfully!")
    ...
    if (
        "wave_min_index: int = None" in content
        and "wave_max_index: int = None" in content
    ):
        print("[OK] Verification passed - parameters are in the file")
        return True
    print("[ERROR] Verification failed - parameters not found")
    return False

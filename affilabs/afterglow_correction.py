...
class AfterglowValidationResult:
    ...
    def log_summary(self):
        ...
        if self.passed and not self.warnings:
            ...
            logger.info(
                f"[WARN] Afterglow validation: Passed with {len(self.warnings)} warning(s)",
            )
        else:
            logger.warning(
                f"[ERROR] Afterglow validation: {len(self.errors)} error(s), {len(self.warnings)} warning(s)",
            )
...
class AfterglowCorrection:
    ...
    def _validate_calibration_data(self):
        ...
        if led_type not in LED_SPECS:
            logger.warning(
                f"[WARN] Unknown LED type '{led_type}', using LCW defaults for validation",
            )
            led_type = "LCW"

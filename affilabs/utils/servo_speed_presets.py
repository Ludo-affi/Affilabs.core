...
- COARSE: 20-position sweep (12 deg steps) - initial S-position finding (2s sweep)
- MEDIUM: 30-position sweep (6 deg steps) - P-position refinement (1.5s sweep)
- FINE: 60-position sweep (3 deg steps) - precise P-position detection (4.5s sweep)
...
SPEED_PRESETS = {
    'coarse': {
        ...
        'step_size': 12,  # 240 deg / 20 = 12 deg per step (0-240 deg range)
    ...
    'fast': {
        ...
        'range': (0, 180),
        'use_case': 'Normal SG��P switching, rapid positioning (non-calibration)'
    }
}
...
def move_servo_with_speed(ctrl, start_angle: int, end_angle: int, speed: str = 'fast'):
    ...
    if speed == 'fast':
        ...
        logger.debug(f"G�� Fast move: {start_angle}-� G�� {end_angle}-�")
        ctrl.servo_move_calibration_only(s=end_angle, p=end_angle)
        ctrl.set_mode('s')
        time.sleep(0.6)
    else:
        # Medium/Slow mode: step through positions
        logger.info(f"=��� {speed.upper()} sweep: {start_angle}-� G�� {end_angle}-� ({len(angles_list)} steps)")
...
def switch_sp_mode(ctrl, target_mode: str, s_position: int, p_position: int):
    ...
    logger.debug(f"G�� Fast switch to {mode_name} ({target_angle}-�)")

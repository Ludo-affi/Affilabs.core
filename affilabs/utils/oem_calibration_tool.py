...
# Use compatible imports for current codebase
# Note: Controller and spectrometer objects will be passed in
# No need to import HAL classes directly
...
class PolarizerCalibrator:
    ...
    **2. Round Polarizer (Continuous Rotation)**:
    ...
    def _run_calibration_legacy(self) -> dict:
        ...
        if hasattr(self.ctrl, "servo_move_calibration_only"):
            self.ctrl.servo_move_calibration_only(s=start_pos, p=max_angle)
        else:
    ...
    def _run_calibration_optimized(self) -> dict:
        ...
        # Use degrees for PicoP4SPR (0-180), PWM for Arduino (10-255)
        # Note: servo_move_calibration_only() converts degrees (0-180) to PWM (0-255) internally
        # Formula: pwm = int(degrees * 255 / 180)
        # To match GitHub's 10-255 PWM range, use ~7-180 degrees
        if hasattr(self.ctrl, "servo_move_calibration_only"):
            # PicoP4SPR: Use degrees (0-180) - method converts to PWM internally
            min_angle = 7  # Converts to ~10 PWM
            max_angle = 180  # Converts to 255 PWM (full range)
            coarse_step = 10  # 10° steps
        else:
            # Arduino: Use PWM units (10-255)
            min_angle = 10
            max_angle = 255
            coarse_step = 10
    ...
    def _analyze_sweep_results(
        ...
        if hasattr(self.ctrl, "servo_move_calibration_only"):
            self.ctrl.servo_move_calibration_only(s=pos1, p=pos2)
        else:

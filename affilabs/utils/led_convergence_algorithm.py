...
def LEDconverge(
    ...
    model_slopes: dict[str, float] | None = None,
    polarization: str = "S",
    ...
    Args:
        ...
        config: ConvergenceConfig instance with algorithm parameters (None = use defaults)

    Returns:
        (final_integration_ms, {channel: signal_counts}, converged)

    """
    # Create default config if not provided (only place where default is created)
    config = config if config is not None else ConvergenceConfig()
    ...
    for iteration in range(max_iterations):
        ...
        # Measure all channels (with cached spectrum/saturation analysis)
        signals: dict[str, float] = {}
        sat_per_ch: dict[str, int] = {}
        sat_analysis_cache: dict[str, dict | None] = {}  # Cache saturation analysis
        ...
        for ch in ch_list:
            signal, sat_pixels, sat_analysis, spectrum = measure_channel(
                usb,
                ctrl,
                ch,
                led_intensities[ch],
            ...
            if signal is None:
                if logger:
                    logger.error(
                        f"  ❌ {ch.upper()} measurement failed @ LED={led_intensities[ch]}",
                    )
                continue

            signals[ch] = signal
            sat_per_ch[ch] = sat_pixels
            sat_analysis_cache[ch] = sat_analysis  # Cache for saturation recovery

            pct = (signal / detector_max) * 100.0
            status = "🚨 SAT" if sat_pixels > 0 else "✅ OK"
            if logger:
                logger.info(
                    f"  {ch.upper()}: {signal:.0f} ({pct:.1f}%) LED={led_intensities[ch]:3d} {status} sat_px={sat_pixels}",
                )
        ...
        if not channels_needing_adjustment and total_sat > 0:
            ...
            channels_needing_adjustment = [
                ch for ch in ch_list if sat_per_ch.get(ch, 0) > 0
            ]
            if logger:
                logger.warning(
                    f"  ⚠️ Tolerance met but {total_sat} saturated - forcing saturation recovery",
                )
        ...
        for ch in channels_needing_adjustment:
            ...
            # --- SATURATION RECOVERY ---
            if sat_per_ch.get(ch, 0) > 0:
                # Use CACHED saturation analysis (no re-acquisition needed!)
                sat_analysis = sat_analysis_cache.get(ch)

                if sat_analysis and model_slopes and ch in model_slopes:
                    # Model-based saturation recovery using cached analysis
                    ...
                    # Fallback: reduction from config
                    new_led = int(
                        max(config.MIN_LED, current_led * config.FALLBACK_REDUCTION),
                    )
                    if logger:
                        logger.info(
                            f"  📉 FALLBACK {ch.upper()} LED {current_led} → {new_led} (no model or cached analysis)",
                        )

            # --- NORMAL ADJUSTMENT ---
            elif model_slopes and ch in model_slopes:
        ...
        if boosted_channels:
            ...
            # (More integration time = more counts, so less LED needed)
            ...
            for ch in ch_list:
                if ch not in boosted_channels and ch in signals:
                    ...
                    led_intensities[ch] = int(
                        max(config.MIN_LED, old_led / boost_ratio),
                    )
                    if logger and led_intensities[ch] != old_led:
                        logger.info(
                            f"     Proportional reduction: {ch.upper()} LED {old_led} → {led_intensities[ch]}",
                        )
    ...
    if signals and current_integration >= (
        detector_params.max_integration_time * config.HARDWARE_LIMIT_FRACTION
    ):
        hardware_min = config.HARDWARE_LIMIT_THRESHOLD * detector_max
        all_above_threshold = all(signals[ch] >= hardware_min for ch in signals)

        if all_above_threshold and total_sat == 0:
            if logger:
                logger.warning(
                    f"✅ ACCEPTING hardware-limited convergence (all ≥{config.HARDWARE_LIMIT_THRESHOLD*100:.0f}%, zero saturation)",
                )
                for ch in signals:
                    pct = (signals[ch] / detector_max) * 100.0
                    logger.info(
                        f"   {ch.upper()}: {signals[ch]:.0f} ({pct:.1f}%) LED={led_intensities[ch]}",
                    )
            return current_integration, signals, True

    # Check relaxed tolerance
    if signals:
        relaxed_min = target * (1.0 - config.RELAXED_TOLERANCE)
        relaxed_max = target * (1.0 + config.RELAXED_TOLERANCE)
        all_in_relaxed = all(
            relaxed_min <= signals[ch] <= relaxed_max for ch in signals
        )

        if all_in_relaxed and total_sat == 0:
            if logger:
                logger.warning(
                    f"⚠️ ACCEPTING with relaxed tolerance (±{config.RELAXED_TOLERANCE*100:.0f}%, zero saturation)",
                )
                for ch in signals:
                    pct = (signals[ch] / detector_max) * 100.0
    ...
    if logger:
        ...
        for ch in signals:
            ...
            logger.error(
                f"   {ch.upper()}: {signals[ch]:.0f} ({pct:.1f}%) LED={led_intensities[ch]} sat={sat}",
            )

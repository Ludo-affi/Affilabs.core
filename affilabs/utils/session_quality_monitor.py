...
if TYPE_CHECKING:
    from affilabs.utils.peak_characterization import PeakCharacteristics
...
class SessionQualityMonitor:
    ...
    def generate_session_report(self) -> str:
        ...
        for ch in ["a", "b", "c", "d"]:
            ...
            if abs(metrics.fwhm_trend) > 0.01:
                ...
                trend_status = (
                    "[WARN]" if metrics.fwhm_trend > self.DEGRADATION_ALERT else ""
                )
                report.append(
            ...
            if metrics.fwhm_mean < self.FWHM_EXCELLENT:
                report.append(
                    f"    [OK] Excellent film quality (FWHM < {self.FWHM_EXCELLENT}nm)",
                )
            elif metrics.fwhm_mean < self.FWHM_GOOD:
                report.append(
                    f"    [WARN]  Acceptable film quality ({self.FWHM_EXCELLENT}-{self.FWHM_GOOD}nm)",
                )
            else:
                report.append(
                    f"    [ERROR] Poor film quality (FWHM ≥ {self.FWHM_GOOD}nm)",
                )
                report.append(
            ...
            # Asymmetry
            asym_status = "[OK]" if 0.7 < metrics.asymmetry_mean < 1.3 else "[WARN]"
            report.append(
    ...
    def _compare_to_historical(self) -> list[str]:
        ...
        from affilabs.utils.device_integration import get_device_dir
        ...
        try:
            ...
            for ch in ["a", "b", "c", "d"]:
                ...
                if abs(sigma) < 1:
                    ...
                    status = "[WARN] Slightly different from baseline"
                else:
                    status = "[ERROR] Significantly different from baseline"

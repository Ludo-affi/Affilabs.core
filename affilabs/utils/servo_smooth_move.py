...
def smooth_servo_move(ctrl, start_angle: int, end_angle: int, step_size: int = 5, step_delay: float = 0.1):
    ...
    logger.info(f"G�� Reached {end_angle}-�")
...
def sweep_with_measurements(ctrl, usb, start_angle: int, end_angle: int,
    ...
    logger.info(f"=��� Servo sweep with measurements: {start_angle}-� G�� {end_angle}-�")
    ...
    logger.info(f"G�� Sweep complete: {len(measurements)} measurements")
    return measurements

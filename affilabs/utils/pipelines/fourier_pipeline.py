...
class FourierPipeline(ProcessingPipeline):
    ...
    def __init__(self, config=None):
        ...
        self.window_size = self.config.get('window_size', 100)  # Reduced from 1500 for better accuracy
        ...
        **kwargs
        ...
        try:
            ...
            if s_reference is None or len(s_reference) != len(transmission):
                logger.error("CRITICAL: S-reference is REQUIRED for SNR-aware peak finding!")
                raise ValueError("S-reference must be provided for Fourier peak finding")
            ...
            logger.debug(f"Applied SNR weighting (strength={snr_strength:.2f}) to SPR region")
            ...
            if self._debug_count < 3:
                ...
                print(f"  SPR region: {spr_wavelengths[0]:.1f}-{spr_wavelengths[-1]:.1f}nm ({len(spectrum)} points)")
                print(f"  Minimum at index {hint_index}/{len(spectrum)}: {spr_wavelengths[hint_index]:.1f}nm")
            ...
            detrended = spectrum[1:-1] - np.linspace(spectrum[0], spectrum[-1], len(spectrum))[1:-1]
            ...
            search_window = min(200, len(derivative) // 4)  # Search within ±200 points of hint
            ...
            if fit_lambda < 620.0 or fit_lambda > 680.0:
                ...
                logger.warning(f"Fourier fit outside SPR region: {fit_lambda:.1f}nm - using triangulated hint")
            ...
            if hasattr(self, '_debug_count') and self._debug_count <= 3:
                print(f"  Zero-crossing index: {zero}/{len(spectrum)} (searched around hint index {hint_index})")
                print(f"  Regression window: indices {start}-{end}, wavelengths {spr_wavelengths[start]:.1f}-{spr_wavelengths[end-1]:.1f}nm")
    ...
    def _calculate_snr_weights(self, s_reference: np.ndarray, snr_strength: float = 0.3) -> np.ndarray:

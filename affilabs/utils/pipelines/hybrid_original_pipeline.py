...
class HybridOriginalPipeline(ProcessingPipeline):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.description = "Fourier + Aggressive filtering (51% noise reduction, first attempt)"
        ...
        logger.debug(f"Initialized {self.name}: alpha={self.alpha}, sg_poly={self.sg_poly}, "
    ...
    def _find_peak_hybrid(self, wavelengths: np.ndarray, transmission: np.ndarray) -> ProcessingResult:
        ...
        try:
            ...
            if len(wavelengths) < self.regression_window:
                raise ValueError(f"Data length {len(wavelengths)} < regression window {self.regression_window}")
            ...
            derivative_filtered = gaussian_filter1d(derivative_idct, sigma=self.gaussian_sigma)
            ...
            if len(zero_crossings) == 0:
                return ProcessingResult(
                    ...
                    metadata={'error': 'No zero crossings found in derivative', 'method': self.name},
                    success=False,
            ...
            logger.debug(f"[HYBRID-ORIG] Initial peak_idx from zero-crossing: {peak_idx}, wavelength: {wavelengths[peak_idx]:.2f}nm")
                ...
                if len(x_window) >= 3:  # Need at least 3 points for quadratic
                    try:
                        ...
                        if abs(coeffs[0]) > 1e-10:
                            ...
                            logger.debug(f"[HYBRID-ORIG] Quadratic refinement: refined_idx={refined_idx:.2f}, bounds=[0, {len(wavelengths)})")
                            # Validate refined index is within full array bounds
                            if 0 <= refined_idx < len(wavelengths):
                                peak_idx = int(refined_idx)
                                logger.debug(f"[HYBRID-ORIG] Accepted refined peak_idx: {peak_idx}, wavelength: {wavelengths[peak_idx]:.2f}nm")
                            else:
                                logger.warning(f"[HYBRID-ORIG] Rejected out-of-bounds refined_idx={refined_idx:.2f}, keeping peak_idx={peak_idx}")
                    except np.linalg.LinAlgError:
                        logger.warning(f"[HYBRID-ORIG] Quadratic fit failed, keeping peak_idx={peak_idx}")

            # Validate peak_idx is within bounds before using it
            peak_idx = max(0, min(peak_idx, len(wavelengths) - 1))
            logger.debug(f"[HYBRID-ORIG] Final clamped peak_idx: {peak_idx}, wavelength: {wavelengths[peak_idx]:.2f}nm")
            ...
            if self.gaussian_refinement:
                ...
                try:
                    ...
                    popt, _ = curve_fit(gaussian, x_window, y_inverted, p0=p0, maxfev=1000)
                    ...
                    if x_window[0] <= refined_center <= x_window[-1]:
                        ...
                        logger.debug(f"[HYBRID-ORIG] Gaussian refinement: {wavelengths[peak_idx]:.2f} → {peak_wavelength:.2f}nm")
                    else:
                        peak_wavelength = wavelengths[peak_idx]
                        logger.warning(f"[HYBRID-ORIG] Gaussian refinement out of window, using {peak_wavelength:.2f}nm")
                except Exception as e:
                    peak_wavelength = wavelengths[peak_idx]
                    logger.debug(f"[HYBRID-ORIG] Gaussian fit failed: {e}, using {peak_wavelength:.2f}nm")
            else:
                peak_wavelength = wavelengths[peak_idx]
                logger.debug(f"[HYBRID-ORIG] No Gaussian refinement, final wavelength: {peak_wavelength:.2f}nm")

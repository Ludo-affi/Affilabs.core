...
class PicoFirmwareUpdater:
    ...
    def get_current_version(self, ser: serial.Serial) -> str | None:
        ...
        try:
            ...
            logger.exception(f"Failed to query firmware version: {e}")
            return None
    ...
    def reboot_to_bootloader(self, ser: serial.Serial) -> bool:
        ...
        try:
            ...
            logger.exception(f"Failed to send bootloader command: {e}")
            return False
    ...
    def flash_firmware(self, bootloader_drive: Path) -> bool:
        ...
        try:
            ...
            logger.exception(f"Failed to copy firmware: {e}")
            return False
    ...
    def update_firmware(self) -> bool:
        ...
        try:
            ...
            if new_version == self.EXPECTED_VERSION:
                logger.info(
                    f"[OK] Firmware update successful: {current_version} -> {new_version}",
                )
                return True
            logger.error(
                f"[ERROR] Firmware update verification failed: expected {self.EXPECTED_VERSION}, got {new_version}",
            )
            return False

        except Exception as e:
            logger.exception(f"Firmware update failed with exception: {e}")
            return False

...
def calculate_led_reduction_from_saturation(
    ...
    if polarization.upper() == 'S':
        if severity > 50:
            safety_factor = 0.80
            reason = f"S-pol HIGH severity → aggressive reduction"
        elif severity > 20:
            safety_factor = 0.85
            reason = f"S-pol MEDIUM severity → moderate reduction"
        else:
            safety_factor = 0.90
            reason = f"S-pol LOW severity → minor reduction"
    else:
        if severity > 50:
            safety_factor = 0.90
            reason = f"P-pol HIGH severity → moderate reduction"
        else:
            safety_factor = 0.95
            reason = f"P-pol severity → minor reduction"
    ...
    if logger:
        logger.info(f"[SAT-RECOVERY] {saturation_analysis['sat_pixels']} px ({sat_fraction:.1%}), width={max_width}, severity={severity:.1f}")
        logger.info(f"[SAT-RECOVERY] LED {current_led} → {new_led} (Δ={reduction}): {reason}")
    ...
    for ch in ch_list:
        ...
        if spec_verify is not None:
            sig_verify = roi_signal_fn(
                spec_verify, wave_min_index, wave_max_index, method="median", top_n=50,
            )
            sat_verify = count_saturated_pixels(
                spec_verify, wave_min_index, wave_max_index, detector_params.saturation_threshold,
            )
            
            # Saturation handling
            if sat_verify > 0:
                if logger:
                    logger.warning(f"[TIME-NORM] {ch.upper()} saturated @ {t_target:.1f}ms - reducing time")
                t_target *= 0.90
                t_target = max(detector_params.min_integration_time, t_target)
            
            # Tolerance adjustment
            elif not (min_sig <= sig_verify <= max_sig):
                adj = max(0.90, min(1.10, (target / sig_verify) if sig_verify > 0 else 1.0))
                t_target *= adj
                t_target = max(
                    detector_params.min_integration_time,
                    min(detector_params.max_integration_time, t_target),
        ...
        if logger:
            pct = (sig_verify if 'sig_verify' in locals() else sig_seed) / detector_max * 100.0
            logger.info(f"[TIME-NORM] {ch.upper()}: T={t_target:.1f}ms → {pct:.1f}% signal")
    ...
    if tighten_final:
        ...
        for ch in ch_list:
            spec_final = acquire_raw_spectrum_fn(
                usb, ctrl, ch, 255, per_channel_times[ch], 1, 45.0, 5.0, use_batch_command,
            )
            ...
            sig_final = roi_signal_fn(
                spec_final, wave_min_index, wave_max_index, method="median", top_n=50,
            )
            ...
            if not (min_sig <= sig_final <= max_sig) and sig_final > 0:
                ...
                if logger:
                    logger.info(f"[TIME-NORM] {ch.upper()} tightened: T={per_channel_times[ch]:.1f}ms")

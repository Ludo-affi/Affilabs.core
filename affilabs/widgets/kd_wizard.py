...
from affilabs.utils.statistics import optimize_by_affinity, func_affinity_fit, optimize_by_linear
...
class KDWizardDialog(QDialog):
    ...
    def __init__(self, parent, seg_data, units):
        ...
        self._result = {'affinity': {ch: {} for ch in CH_LIST}, 'linear': {ch: {} for ch in CH_LIST}}  # Result
    ...
    def _on_btn_kd(self):
        try:
            ...
            x_values = np.arange(max(0, (min(c_list) - (0.05 * (max(c_list) - min(c_list))))),
                                 max(c_list) + (0.05 * (max(c_list) - min(c_list))),
                                 (max(c_list) - min(c_list)) / 100)
            y_values = func_affinity_fit(c_list=x_values, f=[ar['Rmax'], ar['KD'], ar['offset']])
            x_values_m = x_values * 1e-9
            self.fitted_graph_data['affinity'][self._cur_ch] = dict(x=x_values_m, y=y_values)
            self._result['affinity'][self._cur_ch] = deepcopy(ar)
            self._result['affinity'][self._cur_ch]['KD'] = self._result['affinity'][self._cur_ch]['KD'] * 1e-9
            ...
            self.fitted_graph_data['linear'][self._cur_ch] = dict(x=x_values_m,
                ...
                for ch in CH_LIST:
                    if self._fitted_values['affinity'].get(ch):
                        ...
                        with open(file=f"{export_file}.txt", mode='w', newline='', encoding='utf-8') as txtfile:
                            fieldnames = ['Seg ID', f'Conc_{ch.upper()}', f'Shift_{ch.upper()}', f'Fit_{ch.upper()}',
                                          f'SD_{ch.upper()}']
                            ...
                            for i, seg in enumerate(self.seg_data):
                                ...
                                if self._fitted_values[k].get(ch):
                                    data[f"Fit_{ch.upper()}"] = self._fitted_values[k][ch][i]
                                else:
                            ...
                            if k == 'affinity':
                                result_strings = [f"Fitting Model: Affinity",
                                ...
                                result_strings = [f"Fitting Model: Linear",
    ...
    def _draw_plot(self):
        if self.fitted_graph_data['affinity'].get(self._cur_ch):
            ...
            try:
                self.ui.kd_val.setText(f"{self._result['affinity'][self._cur_ch]['KD']:.2e}")
                self.ui.rmax_val.setText(f"{self._result['affinity'][self._cur_ch]['Rmax']:.2f}")
                self.ui.chi_sq_val.setText(f"{self._result['affinity'][self._cur_ch]['chi_sq']:.2f}")
                self.ui.lin_eqn.setText(f"{self._result['linear'][self._cur_ch]['a']:.2e} X + "
                                        f"{self._result['linear'][self._cur_ch]['b']:.2e}")
                self.ui.r_sq_val.setText(f"{self._result['linear'][self._cur_ch]['r_sq']:.2f}")
                if DEV:
                    self.ui.p_val.setText(f"p: {self._result['affinity'][self._cur_ch]['p_val']:.2f}")
            except Exception as e:
    ...
    def _set_cumulative(self):
        ch_list = []
        for ch in CH_LIST:
            if not (np.all(self._shift_data[ch] == np.nan) or np.all(self._shift_data[ch] == 0)):
                ch_list.append(ch)
        for ch in ch_list:
            if self.ui.add_shifts.isChecked():
                ...
                self._shift_data[ch] = [round(s.assoc_shift[ch], 1) for s in self.seg_data]
        for i, s in enumerate(self.seg_data):
            self.ui.table.setItem(i, 2, CenteredQTableWidgetItem(str(self._shift_data[self._cur_ch][i])))
    ...
    def _on_channel_changed(self, ch, active):
        if active:
            ...
            self.conc_data[ch] = [float(self.ui.table.item(row, 1).text()) for row in range(len(self.seg_data))]

    def _load_channel_seg_data(self):
        self._clear_plot()
        table = self.ui.table
        table.horizontalHeaderItem(2).setText(f"Shift {self._cur_ch.upper()} ({self.units})")
        ...
        for i, s in enumerate(self.seg_data):
            ...
            table.setItem(i, 2, CenteredQTableWidgetItem(str(self._shift_data[self._cur_ch][i])))

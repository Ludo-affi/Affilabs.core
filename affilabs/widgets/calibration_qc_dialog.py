...
class CalibrationQCDialog(QDialog):
    ...
    def __init__(self, parent=None, calibration_data: dict = None):
        ...
        dialog_width = min(1600, screen.width() - 100)
    ...
    def _create_graphs_content(self) -> QWidget:
        ...
        dark_graph = self._create_graph("Dark References (8 spectra: S-pol solid, P-pol dashed)", "dark")
        graphs_layout.addWidget(transmission_graph, 1, 0)
        graphs_layout.addWidget(dark_graph, 1, 1)

        layout.addWidget(graphs_container)

        # Tables below graphs - side by side
        tables_layout = QHBoxLayout()
        tables_layout.setSpacing(10)

        # Combined QC table (left)
        combined_qc_table = self._create_combined_qc_table()
        tables_layout.addWidget(combined_qc_table)

        # Model validation table (right)
        model_validation_table = self._create_model_validation_table()
        tables_layout.addWidget(model_validation_table)
    ...
    def _create_combined_qc_table(self) -> QFrame:
            ...
            if ch in transmission_validation:
                ...
                if fwhm is not None:
                    ...
                    if fwhm < 60:
                        ...
                        fwhm_item.setForeground(QColor("#FF9500"))  # Orange (acceptable)
                    else:
                ...
                status_item = QTableWidgetItem(status.replace('[OK] ', '').replace('[ERROR] ', '').replace('[WARN] ', ''))
        ...
        if global_pass_orientation and global_pass_transmission:
            ...
            result_text = "[ERROR] CALIBRATION ISSUES: Check polarizer and sensor hydration"
            result_color = "#FF3B30"
        else:
            result_text = "[WARN] PARTIAL VALIDATION: Some checks passed, review diagnostics"
            result_color = "#FF9500"
    ...
    def _create_model_validation_table(self) -> QFrame:
        ...
        if not model_val_s and not model_val_p:
            ...
            no_data_label.setStyleSheet("color: #8E8E93; font-style: italic; padding: 20px;")
        ...
        table.setHorizontalHeaderLabels([
            "Ch", "Mode", "Predicted", "Measured", "Error", "Error %"
        ])
        table.verticalHeader().setVisible(False)
        table.setEditTriggers(QTableWidget.EditTrigger.NoEditTriggers)
        table.setStyleSheet("""
        ...
        if model_val_s:
            ...
            summary_s = QLabel(f"S-mode: Avg={avg_err_s:.1f}%, Max={max_err_s:.1f}% ({status_s})")
        ...
        if model_val_p:
            ...
            summary_p = QLabel(f"P-mode: Avg={avg_err_p:.1f}%, Max={max_err_p:.1f}% ({status_p})")
    ...
    def _create_orientation_table(self) -> QFrame:
        ...
        if not orientation_data:
            ...
            logger.info(f"[OK] Orientation validation data for channels: {list(orientation_data.keys())}")
    ...
    def _create_transmission_validation_table(self) -> QFrame:
        ...
        if not transmission_validation:
            ...
            logger.info(f"[OK] Transmission validation keys: {list(transmission_validation.keys())}")
        ...
        passed_channels = [ch for ch, data in transmission_validation.items() if '[OK] PASS' in data.get('status', '')]
        failed_channels = [ch for ch, data in transmission_validation.items() if '[ERROR] FAIL' in data.get('status', '')]
        ...
        if len(passed_channels) == len(channels) and len(failed_channels) == 0:
            ...
            result_text = "[WARN] TRANSMISSION VALIDATION: INDETERMINATE (check calibration data)"
            result_color = "#FF9500"
    ...
    def _create_summary_section(self) -> QFrame:
        ...
        num_scans = data.get('num_scans', 'N/A')
        led_intensities = data.get('led_intensities', {})
        sp_swap_applied = data.get('sp_swap_applied', False)

        # Get timing sync QC results from Step 6
        timing_sync = data.get('timing_sync')
        logger.info(f"[QC-DIALOG] timing_sync data: {timing_sync}")
        ...
        if timing_sync:
            avg_cycle_ms = timing_sync.get('avg_cycle_ms')
            jitter_ms = timing_sync.get('jitter_ms')
            timing_status = timing_sync.get('status', 'unknown')
            logger.info(f"[QC-DIALOG] Extracted: avg={avg_cycle_ms}, jitter={jitter_ms}, status={timing_status}")
        ...
        # Build timing sync display
        if avg_cycle_ms is not None and avg_cycle_ms > 0:
            if timing_status == 'pass':
                timing_icon = "[OK]"
                timing_color = "#34C759"
                timing_label = "PASS"
            elif timing_status == 'warning':
                timing_icon = "[WARN]"
                timing_color = "#FF9500"
                timing_label = "WARNING"
            else:
                timing_icon = "[INFO]"
                timing_color = "#007AFF"
                timing_label = "INFO"

            timing_str = f"<span style='color:{timing_color}; font-weight:600;'>{timing_icon} Timing: {avg_cycle_ms:.1f}±{jitter_ms:.1f}ms {timing_label}</span>"
        else:
            timing_str = "<span style='color:#8E8E93;'>⏱ Timing: N/A</span>"

        # Swap warning badge
        swap_badge = "  <span style='color:#FF9500; font-weight:600;'>[WARN] S/P Swapped</span>" if sp_swap_applied else ""

        # SINGLE COMBINED ROW with all info
        combined_text = f"""
        <b>Integration:</b> {integration_time:.2f}ms ({num_scans} scans)  |
        <b>LEDs:</b> {led_str}  |
        <b>S-max:</b> {s_max_str}  |
        <b>P-max:</b> {p_max_str}  |
        {timing_str}{swap_badge}
        """
            ...
            if dark_s_scans or dark_p_scans:
                ...
                for channel in ['a', 'b', 'c', 'd']:
                    if channel in dark_s_scans:
                        ...
                        if spectrum is not None and len(spectrum) > 0:
                            wavelengths_plot = wavelengths if len(wavelengths) == len(spectrum) else np.linspace(wavelengths[0], wavelengths[-1], len(spectrum))
                            pen = pg.mkPen(color=colors.get(channel, (128, 128, 128, 200)), width=2, style=Qt.PenStyle.SolidLine)
                            plot_widget.plot(
                                wavelengths_plot,
                                spectrum,
                                pen=pen,
                ...
                for channel in ['a', 'b', 'c', 'd']:
                    if channel in dark_p_scans:
                        ...
                        if spectrum is not None and len(spectrum) > 0:
                            wavelengths_plot = wavelengths if len(wavelengths) == len(spectrum) else np.linspace(wavelengths[0], wavelengths[-1], len(spectrum))
                            pen = pg.mkPen(color=colors.get(channel, (128, 128, 128, 200)), width=2, style=Qt.PenStyle.DashLine)
                            plot_widget.plot(
                                wavelengths_plot,
                                spectrum,
                                pen=pen,

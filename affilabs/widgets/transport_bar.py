...
class TransportBar(QWidget):
    ...
    def _build(self) -> None:
        ...
        self.spark_toggle_btn = self._icon_btn(
            icon=_svg_icon(_robot_svg, "#FF9500", "#2E30E3"),
            tip="Toggle Spark AI assistant",
            checkable=True,
            style_override=(
                "QPushButton { background: rgba(46,48,227,0.10); border: 1px solid rgba(46,48,227,0.30); border-radius: 8px; }"
                "QPushButton:hover { background: rgba(46,48,227,0.16); border-color: rgba(46,48,227,0.45); }"
                "QPushButton:checked { background: rgba(46,48,227,0.22); border-color: rgba(46,48,227,0.55); }"
            ),
        )
        ...
        self.pause_btn.setFixedSize(_BTN_W, _BTN_H)
        self.pause_btn.setEnabled(False)
        self.pause_btn.setToolTip("Pause Live Acquisition\n(Enabled after calibration)")
        self.pause_btn.setStyleSheet(
            "QPushButton {"
            "  background: rgba(46,48,227,0.08); border: 1px solid rgba(46,48,227,0.25); border-radius: 8px;"
            "}"
            "QPushButton:hover:!checked { background: rgba(46,48,227,0.14); border-color: rgba(46,48,227,0.40); }"
            "QPushButton:checked { background: #FF9500; border: 1px solid rgba(255,149,0,0.4); }"
            "QPushButton:hover:checked { background: #E68500; }"
            "QPushButton:disabled { background: rgba(46,48,227,0.06); border: 1px solid rgba(46,48,227,0.10); }"
        )
        pause_svg = get_affilabs_resource("ui/img/pause_icon.svg")
        pause_white_svg = get_affilabs_resource("ui/img/pause_icon_white.svg")
        if pause_svg and pause_svg.exists():
            icon = QIcon(str(pause_svg))
            if pause_white_svg and pause_white_svg.exists():
        ...
        self.record_btn.setFixedSize(_BTN_W, _BTN_H)
        ...
        self.record_btn.setStyleSheet(
            ...
            "}"
            "QPushButton:hover:!checked { background: rgba(46,48,227,0.14); border-color: rgba(46,48,227,0.40); }"
            "QPushButton:checked { background: rgba(255,59,48,0.90); border: 1px solid rgba(255,59,48,0.50); }"
            "QPushButton:hover:checked { background: rgba(230,52,42,0.95); }"
            "QPushButton:disabled { background: rgba(46,48,227,0.06); border: 1px solid rgba(46,48,227,0.10); }"
        )
        rec_svg = get_affilabs_resource("ui/img/record_icon.svg")
        rec_white_svg = get_affilabs_resource("ui/img/record_icon_white.svg")
        if rec_svg and rec_svg.exists():
            icon = QIcon(str(rec_svg))
            if rec_white_svg and rec_white_svg.exists():
                icon.addFile(str(rec_white_svg), QSize(), QIcon.Mode.Normal, QIcon.State.On)
            icon.addFile(str(rec_svg), QSize(), QIcon.Mode.Disabled, QIcon.State.Off)

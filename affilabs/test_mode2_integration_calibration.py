...
def test_hardware_connection():
    ...
    try:
        ...
        if ctrl is None:
            logger.error("[ERROR] Controller not found")
            return None, None
        logger.info(f"[OK] Controller connected: {type(ctrl).__name__}")

        # Connect spectrometer
        logger.info("📡 Connecting to spectrometer...")
        usb = find_spectrometer()
        if usb is None:
            logger.error("[ERROR] Spectrometer not found")
            ctrl.close()
            return None, None
        model_name = type(usb).__name__
        logger.info(f"[OK] Spectrometer connected: {model_name}")
...
def calibrate_integration_per_channel_test(
    ...
    if int_array is None:
        logger.error(f"[ERROR] Failed to read intensity for channel {ch.upper()}")
        return None
...
def test_all_channels(usb, ctrl):
    ...
    for ch in CH_LIST:
        ...
        if result:
            ...
            logger.error(f"[ERROR] Failed to calibrate channel {ch.upper()}")
...
def main():
    ...
    if ctrl is None or usb is None:
        logger.error("\n[ERROR] TEST FAILED: Cannot proceed without hardware")
        return 1
    ...
    try:
        ...
        if not results:
            logger.error("\n[ERROR] TEST FAILED: No channels calibrated successfully")
            return 1
        ...
        if all_passed:
            ...
            logger.warning("[WARN] SOME TESTS FAILED")

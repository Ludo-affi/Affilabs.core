...
try:
    ...
    print("\n" + "="*60)
    print("[WARN]  WARNING: matplotlib not installed")
    print("="*60)
...
def preview_demo_data():
    ...
    for ch, data in channel_data.items():
        ax1.plot(time_array, data, label=labels[ch], color=colors[ch],
    ...
    for idx, (start, end) in enumerate(cycle_boundaries):
        ...
        ax1.text(mid_point, ax1.get_ylim()[1] * 0.95, f'Cycle {idx + 1}',
    ...
    for ch, data in channel_data.items():
        data_zoom = data[mask]
        ax2.plot(time_zoom, data_zoom, label=labels[ch], color=colors[ch],
                linewidth=2, alpha=0.8)
    ...
    print("="*60)
    print(f"Total duration:    {time_array[-1]:.1f} seconds ({time_array[-1]/60:.1f} minutes)")
    ...
    print("\nCycle Boundaries:")
    for idx, (start, end) in enumerate(cycle_boundaries):
        print(f"  Cycle {idx + 1}: {start:.0f}s - {end:.0f}s (duration: {end-start:.0f}s)")

...
try:
    from affilabs.hardware import DeviceManager, SystemState, IController, ISpectrometer, IServo
    from affilabs.hardware import controller_adapter, spectrometer_adapter, servo_adapter
    HAL_AVAILABLE = True
...
from affilabs.viewmodels import DeviceStatusViewModel, SpectrumViewModel, CalibrationViewModel
# Phase 1.1 Domain Models
from affilabs.domain import CalibrationData, RawSpectrumData, ProcessedSpectrumData
# Phase 1.2 Business Services
from affilabs.services import TransmissionCalculator, BaselineCorrector
...
from affilabs.transmission_spectrum_dialog import TransmissionSpectrumDialog
from affilabs.settings import SW_VERSION, PROFILING_ENABLED, PROFILING_REPORT_INTERVAL
...
class Application(QApplication):
    ...
    def __init__(self, argv):
        ...
        if HAL_AVAILABLE:
            ...
            logger.warning(f"[WARN]  Phase 1.4 HAL not available: {_hal_import_error if '_hal_import_error' in globals() else 'unknown error'}")

        if COORDINATORS_AVAILABLE:
            logger.info("[OK] UI Coordinators available")
        else:
            logger.warning(f"[WARN]  UI Coordinators not available: {_coordinators_import_error if '_coordinators_import_error' in globals() else 'unknown error'}")
        ...
        if COORDINATORS_AVAILABLE:
            ...
            logger.info("[OK] UI coordinators initialized (UIUpdateCoordinator, DialogManager)")
        else:
            self.ui_updates = None
            self.dialog_manager = None
            logger.warning("[WARN]  Running without UI coordinators (compatibility mode)")

        # Verify spectroscopy plots are available (silent check)
        if not hasattr(self.main_window, 'transmission_curves'):
            logger.warning("[WARN] Spectroscopy plots NOT found in main window - graphs will not display")
        ...
        logger.info("[OK] Business services initialized (TransmissionCalculator, BaselineCorrector)")
        ...
        self._ui_update_timer.setInterval(100)  # 100ms = 10 Hz update rate for smooth live sensorgram
        ...
        logger.info(f"[OK] Window visible (minimal UI rendered): {self.main_window.isVisible()}")
    ...
    def _on_calibration_complete_status_update(self, calibration_data):
        ...
        try:
            ...
            self.hardware_mgr.update_calibration_status(ch_error_list, 'full', s_ref_qc_results)
            logger.info(f"[OK] Hardware manager calibration status updated - ch_errors={ch_error_list}")
            ...
            if optics_ready:
                ...
                status_update = {
                    ...
                    'spectrometer': self.hardware_mgr.usb.serial_number if self.hardware_mgr.usb else None,
                    'fluidics_ready': self.hardware_mgr.pump is not None
                }
                self._update_device_status_ui(status_update)
                logger.info("[OK] Device status updated directly (no hardware scan post-calibration)")
                ...
                logger.warning("[WARN] Calibration completed but optics not ready (some channels failed)")
                self._update_device_status_ui({'optics_ready': False, 'sensor_ready': True})

        except Exception as e:
            logger.error(f"[ERROR] Failed to update optics_ready status: {e}", exc_info=True)
    ...
    def _on_calibration_complete(self, calibration_data):
        ...
        try:
            ...
            logger.error(f"[ERROR] Failed to apply calibration data: {e}", exc_info=True)
    ...
    def _connect_ui_control_signals(self):
        ...
        if hasattr(ui, 'advanced_menu') and ui.advanced_menu is not None:
            if hasattr(ui.advanced_menu, 'measure_afterglow_sig'):
                ...
                logger.info("[OK] Connected Advanced Settings optical calibration signal")
    ...
    def _on_start_button_clicked(self):
        ...
        try:
            ...
            if self.data_mgr and self.data_mgr.calibrated and getattr(self.data_mgr, 'calibration_data', None):
                ...
                logger.info("   [INFO] System not calibrated (using bypass mode defaults)")
                integration_time = 40
        ...
        try:
            ...
            if cd and getattr(cd, 's_pol_ref', None) and getattr(cd, 'wavelengths', None) is not None:
                ...
                try:
                    logger.info(f"[OK] Loaded S-mode reference spectra: {list(cd.s_pol_ref.keys())}")
                except Exception:
                    pass
    ...
    def _on_hardware_connected(self, status: dict):
        ...
        if hasattr(self, '_last_hw_callback_time'):
            ...
            if time_since_last < 0.5:  # Less than 500ms since last callback
                logger.debug(f"📊 Skipping duplicate hardware callback ({time_since_last*1000:.0f}ms since last)")
                return
        ...
        if scan_successful and valid_hardware:
            logger.info(f"[OK] Scan SUCCESSFUL - found valid hardware: {', '.join(valid_hardware)}")
        ...
        logger.debug(f"[SEARCH] Calling _update_device_status_ui with optics_ready={status.get('optics_ready')}, sensor_ready={status.get('sensor_ready')}")
        ...
        if not self._initial_connection_done:
            ...
            if status.get('ctrl_type') and status.get('spectrometer') and not self._calibration_completed:
                ...
                logger.info("="*80)
                logger.info(f"[SEARCH] TYPE CHECK: self.calibration = {type(self.calibration)}")
    ...
    def _start_processing_thread(self):
        ...
        print(f"   [OK] Thread started: alive={self._processing_thread.is_alive()}, name={self._processing_thread.name}")
        ...
        try:
            ...
            try:
                ...
                if self._queue_stats['dropped'] % 10 == 1:  # Log every 10th drop
                    logger.warning(f"[WARN] Spectrum queue full - {self._queue_stats['dropped']} frames dropped")
    ...
    def _process_spectrum_data(self, data: dict):
        ...
        try:
            ...
            try:
                ...
                logger.exception(f"[ERROR] Buffer append failed for channel {channel}: {e}")
            ...
            if has_raw_data and has_transmission and should_update_transmission:
                print(f"   [OK] Calling _queue_transmission_update for channel {channel.upper()}")
                try:
                    ...
                    logger.exception(f"[ERROR] Transmission queue error for channel {channel}: {e}")
        ...
        if should_update_graph:
            try:
                ...
                logger.exception(f"[ERROR] Graph update error for channel {channel}: {e}")
        ...
        if time_span and time_span >= LEAK_DETECTION_WINDOW:
            ...
            if dark_noise is not None:
                ...
                if avg_intensity < dark_threshold:
                    logger.warning(f"[WARN] Possible optical leak detected in channel {channel.upper()}: "
    ...
    def _queue_transmission_update(self, channel: str, data: dict):
        ...
        if not self.ui_updates._transmission_updates_enabled and not self.ui_updates._raw_spectrum_updates_enabled:
            print(f"   [WARN] BOTH FLAGS DISABLED - SKIPPING UPDATE")
            return
        ...
        if transmission is not None and len(transmission) > 0:
            ...
            self.ui_updates.queue_transmission_update(channel, wavelengths, transmission, raw_spectrum)
    ...
    def _on_polarizer_toggle_clicked(self):
        ...
        try:
            ...
            if self.hardware_mgr.ctrl is not None:
                ...
                from affilabs.utils.calibration_6step import servo_move_1_then, resolve_device_config_for_detector
                ...
                if device_config_det:
                    ...
                    if success:
                        ...
                        if mode_success:
                            ...
                            logger.info(f"[OK] Polarizer moved to position {new_position}")
                        else:
                            logger.warning(f"[WARN] Servo moved but mode lock failed for {new_position}")
                            # Still update UI since servo physically moved
                            self.main_window.sidebar.set_polarizer_position(new_position)
                    else:
                        logger.error(f"[ERROR] Failed to move polarizer to position {new_position}")
                        from widgets.message import show_message
                        show_message(f"Failed to move polarizer to position {new_position}\n\nCheck hardware connection.")
                else:
                    logger.error("[ERROR] No device_config available for servo positions")
                    from widgets.message import show_message
                    show_message("Cannot move polarizer - device configuration not loaded.")

            else:
                logger.warning("[WARN] Controller not connected - cannot move polarizer")
                from widgets.message import show_message
    ...
    def __del__(self):
        ...
        try:
            if not hasattr(self, 'closing') or not self.closing:
                logger.warning("[WARN] __del__ called without proper close - forcing cleanup")
                self._cleanup_resources(emergency=True)
        except Exception:
    ...
    def connect_hal_devices(self, require_controller=True, require_spectrometer=True, require_servo=False):
        ...
        """
        if not HAL_AVAILABLE or not self.device_manager:
            logger.warning("[WARN]  HAL not available, cannot connect via DeviceManager")
            return False
    ...
    def _on_filter_toggled(self, checked: bool):
        ...
        self._update_cycle_of_interest_graph()
        logger.info("[OK] Filter toggle complete - both timeline and cycle graphs refreshed")
    ...
    def _apply_smoothing(self, data, strength: int, channel: str = None, method: str = None, online_mode: bool = False):
        ...
        if online_mode and len(data) > 200:
            ...
            filtered_recent = self._apply_smoothing(recent_data, strength, channel, method, online_mode=False)
            ...
            try:
                if hasattr(self.main_window, 'device_config') and self.main_window.device_config:
                    ...
                    if positions:
                        ...
                        logger.info(f"[OK] Position validation: {new_pos.upper()}-mode target={target_pos}° (from device_config)")
                    else:
                        logger.warning("[WARN]  Cannot validate positions - not found in device_config")
                else:
                    logger.warning("[WARN]  Cannot validate positions - device_config not available")
            except Exception as e:
        ...
        try:
            ...
            if self.hardware_mgr and self.hardware_mgr.ctrl:
                ...
                if self.main_window.device_config:
                    ...
                    logger.info("[OK] Settings saved to device config file (including LED timing delays)")
                else:
                    logger.warning("[WARN] Device config not available - settings not saved")
    ...
    def _on_power_off_requested(self):
        ...
        try:
            ...
            logger.info("[OK] Graceful shutdown complete - software ready for offline post-processing")
    ...
    def _on_spectrum_updated(self, channel: str, wavelengths: np.ndarray, transmission: np.ndarray):
        ...
        self.ui_updates.queue_transmission_update(channel, wavelengths, transmission, None)
    ...
    def _load_device_settings(self):
        ...
        try:
            ...
            if self.main_window.device_config:
                    ...
                    if eeprom_config:
                        ...
                        if eeprom_s != s_pos or eeprom_p != p_pos:
                            ...
                            if self.hardware_mgr.ctrl.write_config_to_eeprom(eeprom_config):
                                ...
                                logger.warning("")
                                logger.warning("=" * 80)
                                logger.warning("[WARN]  CRITICAL: CONTROLLER NEEDS POWER CYCLE")
                                logger.warning("=" * 80)
                            ...
                            logger.info("[OK] EEPROM matches device_config - no update needed")
                    else:
                ...
                logger.warning("  [WARN] Device config not available - cannot load servo positions")
            ...
            if self.main_window.device_config:
                ...
                if led_a > 0 or led_b > 0 or led_c > 0 or led_d > 0:
                    ...
                    self.hardware_mgr.ctrl.set_intensity('c', led_c)
                    self.hardware_mgr.ctrl.set_intensity('d', led_d)
                    logger.info(f"  [OK] LED intensities loaded from device config: A={led_a}, B={led_b}, C={led_c}, D={led_d}")
                else:
                    logger.info("  [INFO]  No calibrated LED intensities in device config - will calibrate on startup")

...
def test_hardware():
    ...
    try:
        ctrl = PicoP4SPR()
        if ctrl.open():
            print(f"   [OK] PicoP4SPR connected on port: {ctrl._ser.port if hasattr(ctrl, '_ser') and ctrl._ser else 'unknown'}")
        else:
            print("   [ERROR] PicoP4SPR failed to open")
            ctrl = None
    except Exception as e:
        print(f"   [ERROR] PicoP4SPR error: {e}")
        ctrl = None
    ...
    if not ctrl:
        ...
        try:
            ...
            if ctrl.open():
                print(f"   [OK] ArduinoController connected on port: {ctrl._ser.port if hasattr(ctrl, '_ser') and ctrl._ser else 'unknown'}")
            else:
                print("   [ERROR] ArduinoController failed to open")
                return False
        except Exception as e:
            print(f"   [ERROR] ArduinoController error: {e}")
            return False
    ...
    try:
        usb = USB4000()
        if usb.open():
            print(f"   [OK] Spectrometer connected")
            print(f"   Model: {getattr(usb, 'model', 'unknown')}")
            print(f"   Serial: {getattr(usb, 'serial_number', 'unknown')}")
        else:
            print("   [ERROR] Spectrometer failed to open")
            return False
    except Exception as e:
        print(f"   [ERROR] Spectrometer error: {e}")
        import traceback
        traceback.print_exc()
        return False
    ...
    try:
        ...
        ctrl.set_mode('s')
        time.sleep(0.1)
        print("   [OK] S-mode command sent")

        print("   Setting P-mode...")
        ctrl.set_mode('p')
        time.sleep(0.1)
        print("   [OK] P-mode command sent")

        print("   Back to S-mode...")
        ctrl.set_mode('s')
        time.sleep(0.1)
        print("   [OK] Mode switching successful")
    except Exception as e:
        print(f"   [ERROR] Mode switching failed: {e}")
        return False
    ...
    for ch in channels:
        ...
        try:
            ...
            if spectrum is None:
                print(f"      [ERROR] Spectrometer returned None!")
                continue
            ...
            if max_signal > 100:
                print(f"      [OK] LED {ch.upper()} is working (signal > 100)")
            else:
                print(f"      [WARN] LED {ch.upper()} weak signal (< 100 counts)")
            ...
            print(f"      [ERROR] Error testing LED {ch.upper()}: {e}")
            import traceback
            traceback.print_exc()
    ...
    try:
        ...
        if dark_spectrum is None:
            print("   [ERROR] Spectrometer returned None!")
        else:
            dark_max = float(np.max(dark_spectrum))
            print(f"   Dark signal: {dark_max:.0f} counts")
            if dark_max < 1000:
                print("   [OK] Dark signal is low (LEDs are off)")
            else:
                print(f"   [WARN] Dark signal is high ({dark_max:.0f} counts) - LEDs may still be on")
    except Exception as e:
        print(f"   [ERROR] Error in dark test: {e}")
        import traceback
        traceback.print_exc()
    ...
    try:
        ...
        print("   [OK] Rapid switching test complete")

    except Exception as e:
        print(f"   [ERROR] Rapid switching failed: {e}")
        import traceback
        traceback.print_exc()
    ...
    try:
        ...
        if cal_result.success:
            ...
            if cal_result.ch_error_list:
                print(f"\n   [WARN] Channels with warnings: {', '.join([c.upper() for c in cal_result.ch_error_list])}")
            else:
                print(f"\n   [OK] All channels passed QC")
...
if __name__ == "__main__":
    try:
        ...
        if response == 'y':
            ...
            if success:
                print("\n[OK] Full calibration test passed")
                sys.exit(0)
            else:
                print("\n[ERROR] Full calibration test failed")
                sys.exit(1)
        else:

...
def test_hardware():
    ...
    try:
        ...
        if ctrl.open():
            print(
                f"   [OK] PicoP4SPR connected on port: {ctrl._ser.port if hasattr(ctrl, '_ser') and ctrl._ser else 'unknown'}",
            )
        else:
    ...
    if not ctrl:
        ...
        try:
            ...
            if ctrl.open():
                print(
                    f"   [OK] ArduinoController connected on port: {ctrl._ser.port if hasattr(ctrl, '_ser') and ctrl._ser else 'unknown'}",
                )
            else:
    ...
    try:
        ...
        if dark_spectrum is None:
            ...
            if dark_max < 1000:
                ...
                print(
                    f"   [WARN] Dark signal is high ({dark_max:.0f} counts) - LEDs may still be on",
                )
    except Exception as e:

...
class SpectroscopyPresenter:
    ...
    def check_plots_available(self):
        ...
        if not self._plots_check_logged:
            ...
            if has_trans:
                print(f"  - transmission_curves count: {len(self.main_window.transmission_curves)}")
            if has_raw:
                print(f"  - raw_data_curves count: {len(self.main_window.raw_data_curves)}")

        if has_trans and has_raw:
            logger.info(f"[OK] Spectroscopy plots found: transmission={len(self.main_window.transmission_curves)} curves, raw={len(self.main_window.raw_data_curves)} curves")
            self._plots_available = True
        else:
            logger.warning(f"[!] Spectroscopy plots NOT found: transmission={has_trans}, raw={has_raw}")
            self._plots_available = False

        self._plots_check_logged = True
        return self._plots_available

    def update_transmission(self, channel: str, wavelengths: np.ndarray, transmission: np.ndarray):
        ...
        try:
            ...
            if channel not in self._first_update_logged:
                ...
                print(f"  Wavelengths: {len(wavelengths)} points, {wavelengths[0]:.1f}-{wavelengths[-1]:.1f}nm")
                print(f"  Transmission: min={np.min(transmission):.1f}%, max={np.max(transmission):.1f}%")
            ...
            if hasattr(self.main_window, 'transmission_curves'):
                ...
                if not self._plots_available:
                    logger.info(f"[OK] First transmission update for channel {channel.upper()}")
                    self._plots_available = True
            elif hasattr(self.main_window, 'sidebar') and hasattr(self.main_window.sidebar, 'update_transmission_plot'):
                # Fallback: use sidebar API
                self.main_window.sidebar.update_transmission_plot(channel, wavelengths, transmission)

        except Exception as e:
            logger.warning(f"Transmission curve update failed for {channel}: {e}")

    def update_raw_spectrum(self, channel: str, wavelengths: np.ndarray, raw_spectrum: np.ndarray):
        ...
        try:
            ...
            elif hasattr(self.main_window, 'sidebar') and hasattr(self.main_window.sidebar, 'update_raw_data_plot'):
                # Fallback: use sidebar API
                self.main_window.sidebar.update_raw_data_plot(channel, wavelengths, raw_spectrum)

        except Exception as e:
            logger.warning(f"Raw spectrum curve update failed for {channel}: {e}")
    ...
    def clear_transmission(self, channel: str = None):
        ...
        try:
            if channel is None:
                ...
                if channel in self._channel_to_idx and hasattr(self.main_window, 'transmission_curves'):
                    channel_idx = self._channel_to_idx[channel]
    ...
    def clear_raw_spectrum(self, channel: str = None):
        ...
        try:
            if channel is None:
                ...
                # Clear specific channel
                if channel in self._channel_to_idx and hasattr(self.main_window, 'raw_data_curves'):
                    channel_idx = self._channel_to_idx[channel]

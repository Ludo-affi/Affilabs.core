...
class QCReportManager:
        ...
        try:
            ...
            qc_report = {
                "metadata": {
                    ...
                    "firmware_version": calibration_data.get('firmware_version', 'Unknown'),
                    "software_version": software_version or "2.0",
                    "calibration_type": calibration_data.get('calibration_type', 'full_7step'),
                ...
                "spectra_data": {
                    "wavelengths": self._safe_serialize(calibration_data.get('wavelengths')),
                    "s_pol": self._serialize_channel_data(calibration_data.get('s_pol_spectra', {})),
                    "p_pol": self._serialize_channel_data(calibration_data.get('p_pol_spectra', {})),
                    "dark": self._serialize_channel_data(calibration_data.get('dark_spectra', {})),
                    "transmission": self._serialize_channel_data(calibration_data.get('transmission_spectra', {}))
                },
                ...
                "qc_validation": {
                    ...
                    "overall_status": calibration_data.get('qc_overall_status', 'UNKNOWN'),
                ...
                "model_performance": {
                    "s_pol_predictions_accuracy": calibration_data.get('s_model_accuracy'),
                    "p_pol_predictions_accuracy": calibration_data.get('p_model_accuracy'),
                    "convergence_iterations_s": calibration_data.get('s_convergence_iterations'),
                    "convergence_iterations_p": calibration_data.get('p_convergence_iterations'),
                ...
                "signal_quality": {
                    ...
                    "peak_counts_by_channel": self._extract_peak_counts(calibration_data),
                    "noise_floor_by_channel": self._extract_noise_floor(calibration_data),
                ...
                "operational_context": {
                    ...
                    "days_since_last_calibration": calibration_data.get('days_since_last_cal'),
                    "calibration_trigger": calibration_data.get('calibration_trigger', 'manual')
                },
            ...
            for report_file in report_files:
                ...
                try:
                    ...
                    reports_list.append({
                        ...
                        "failed_channels": len(report['qc_validation']['failed_channels']),
    ...
    def export_to_html(self, report_path: Path, output_path: Optional[Path] = None) -> Path:
    ...
    def get_ml_features(self, device_serial: str, n_reports: int = 10) -> Dict[str, List]:
        ...
        try:
            ...
            for report_info in reports_list:
                report = self.load_qc_report(device_serial, report_info['filename'].replace('qc_report_', '').replace('.json', ''))
                ...
                if report:
                    ...
                    # Peak counts trends
                    peak_data = report['signal_quality'].get('peak_counts_by_channel', {})
                    for ch in ['a', 'b', 'c', 'd']:
                        features["peak_counts_trends"][ch].append(peak_data.get(ch))

                    # Model performance
                    features["model_accuracy_s"].append(report['model_performance'].get('s_pol_predictions_accuracy'))
                    features["model_accuracy_p"].append(report['model_performance'].get('p_pol_predictions_accuracy'))

                    # Other metrics
                    features["convergence_iterations"].append(
                        (report['model_performance'].get('convergence_iterations_s', 0) +
                         report['model_performance'].get('convergence_iterations_p', 0)) / 2
                    )
                    features["timing_jitter"].append(report['timing_metrics'].get('timing_jitter_ms'))
                    features["led_efficiency"].append(report['signal_quality'].get('led_efficiency'))
                    features["device_hours"].append(report['operational_context'].get('total_device_hours', 0))
                    features["failed_channels_count"].append(len(report['qc_validation'].get('failed_channels', [])))

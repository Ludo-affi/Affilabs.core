...
class DeviceConfigManager:
    ...
    def initialize_device_config(self, device_serial: Optional[str] = None):
        ...
        try:
            ...
            controller = None
            if hasattr(self.main_window, 'app') and self.main_window.app and hasattr(self.main_window.app, 'hardware_mgr'):
                controller = self.main_window.app.hardware_mgr.ctrl if self.main_window.app.hardware_mgr else None
            ...
            if self.device_config.created_from_scratch:
                ...
                logger.info(f"   Missing calibration: Servo positions, LED intensities, SPR model")
                logger.info("")
                logger.info("Automatic Workflow:")
                logger.info("   1. Run servo calibration (auto-detect S/P positions)")
                logger.info("   2. Run LED calibration (generate SPR model + intensities)")
                logger.info("   3. Populate device_config.json (single source of truth)")
                logger.info("   4. Save to JSON and EEPROM")
                logger.info("")
                logger.info("⚠️  No user input required - calibration runs automatically")
                logger.info("=" * 80)
    ...
    def get_controller_type_from_hardware(self) -> str:
        ...
        """
        try:
            if hasattr(self.main_window, 'hardware_mgr') and self.main_window.hardware_mgr and self.main_window.hardware_mgr.ctrl:
                ctrl_name = getattr(self.main_window.hardware_mgr.ctrl, 'device_name', '').lower()
            ...
            try:
                ...
                if data_mgr and hasattr(data_mgr, 'leds_calibrated') and data_mgr.leds_calibrated:
                    ...
                    if any([led_a, led_b, led_c, led_d]):
                        ...
                        logger.info(f"✓ LED intensities saved: A={led_a}, B={led_b}, C={led_c}, D={led_d}")

                        # Save integration time and num_scans if available
                        if hasattr(data_mgr, 'integration_time_ms') and data_mgr.integration_time_ms:
                            self.device_config.set_integration_time(data_mgr.integration_time_ms)
                            logger.info(f"✓ Integration time saved: {data_mgr.integration_time_ms} ms")

                        if hasattr(data_mgr, 'num_scans') and data_mgr.num_scans:
                            self.device_config.set_num_scans(data_mgr.num_scans)
                            logger.info(f"✓ Number of scans saved: {data_mgr.num_scans}")

                        # Save SPR model path
                        if device_serial:
                            from pathlib import Path
                            spr_model_path = Path(f"OpticalSystem_QC/{device_serial}/spr_calibration/led_calibration_spr_processed_latest.json")
                            if spr_model_path.exists():
                                self.device_config.set_spr_model_path(str(spr_model_path))
                                logger.info(f"✓ SPR model path saved: {spr_model_path}")
                            else:
                                logger.warning(f"SPR model not found at expected location: {spr_model_path}")

                        self.device_config.save()
                        logger.info("✓ All calibration data saved to device_config.json")
                    else:
                ...
                if self.device_config.sync_to_eeprom(hardware_mgr.ctrl):
                    ...
                    logger.info(f"     • Integration time: {int_time if int_time else 'Not set'} ms")
                    logger.info(f"     • Number of scans: {num_scans if num_scans else 'Not set'}")
                    logger.info("")
                    logger.info(f"   SPR Model:")
                    ...
                    logger.info(f"     • JSON (Primary): {self.device_config.config_path}")
                    logger.info(f"     • EEPROM (Backup): Controller memory")
                    logger.info("=" * 80)

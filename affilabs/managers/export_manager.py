...
class ExportManager:
    ...
    def get_export_config(self) -> dict:
        ...
        data_types = {
            "raw": (
                getattr(sidebar, "raw_data_check", None) and sidebar.raw_data_check.isChecked()
                if hasattr(sidebar, "raw_data_check")
                else True
            ),
            "processed": (
                getattr(sidebar, "processed_data_check", None)
                and sidebar.processed_data_check.isChecked()
                if hasattr(sidebar, "processed_data_check")
                else True
            ),
            "cycles": (
                getattr(sidebar, "cycle_segments_check", None)
                and sidebar.cycle_segments_check.isChecked()
                if hasattr(sidebar, "cycle_segments_check")
                else True
            ),
            "summary": (
                getattr(sidebar, "summary_table_check", None)
                and sidebar.summary_table_check.isChecked()
                if hasattr(sidebar, "summary_table_check")
                else True
            ),
        }
        ...
        # Get options
        include_metadata = (
            getattr(sidebar, "metadata_check", None) and sidebar.metadata_check.isChecked()
            if hasattr(sidebar, "metadata_check")
            else True
        )
        include_events = (
            getattr(sidebar, "events_check", None) and sidebar.events_check.isChecked()
            if hasattr(sidebar, "events_check")
            else False
        )
        ...
        filename = (
            getattr(sidebar, "export_filename_input", None) and sidebar.export_filename_input.text()
            if hasattr(sidebar, "export_filename_input")
            else ""
        )
        destination = (
            getattr(sidebar, "export_dest_input", None) and sidebar.export_dest_input.text()
            if hasattr(sidebar, "export_dest_input")
            else ""
        )

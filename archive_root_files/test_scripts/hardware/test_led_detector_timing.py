...
def test_led_detector_timing():
    ...
    for delay_ms in test_delays_ms:
        ...
        if spectrum is not None:
            ...
            print(
                f"  ✅ Delay={delay_ms:3d}ms | Signal: mean={signal_mean:6.0f}, max={signal_max:6.0f}, min={signal_min:6.0f}",
            )
            print(
                f"     Timing: LED_cmd={led_command_time:.1f}ms, detector_read={read_time:.1f}ms, total={total_time:.1f}ms",
            )
    ...
    print("Signal vs PRE-LED Delay:")
    print()
    print("  Delay (ms) | Mean Signal | Max Signal | Status")
    print("  -----------+-------------+------------+------------------")
    ...
    for r in results:
        ...
        improvement = (
            ((mean_sig - baseline_signal) / baseline_signal * 100)
        ...
        print(
            f"  {delay:4d}       | {mean_sig:11.0f} | {max_sig:10.0f} | {status} {improvement:+5.1f}%",
        )
    ...
    print(
        f"  • Signal improvement: {((max_signal_result['signal_mean'] - baseline_signal) / baseline_signal * 100):+.1f}%",
    )
    print()
    ...
    if plateau_start:
        ...
        print("    (LED fully stabilized, no further improvement)")
    else:
        print("  ⚠️  Signal may still be increasing - test longer delays")

    print()
    print("Timing Breakdown:")
    print(
        f"  • LED command execution: {max_signal_result['led_command_time_ms']:.1f}ms",
    )

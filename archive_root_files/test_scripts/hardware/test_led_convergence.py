...
from src.utils.led_normalization import LEDNormalizer, PeakIntensityCalculator
...
def iterative_led_convergence(
    ...
    logger.info(
        f"   Mode: {'INTENSITY (adjust LED)' if mode == 'intensity' else 'TIME (adjust integration)'}",
    )
    ...
    for iteration in range(max_iterations):
        ...
        if mode == "intensity":
            ...
            normalizer.controller.set_intensity(
                led.lower(),
                255,
            )  # Always 255 in time mode
            normalizer.spectrometer.set_integration_time(current_time)
        ...
        logger.info(
            f"   Iter {iteration+1}: {signal:.0f} counts ({pct_of_max:.1f}%) {status}",
        )
        ...
        if mode == "intensity":
            ...
            if new_led >= 255 and current_led >= 255:
                ...
                logger.warning("   ⚠️ LED maxed at 255, switching to TIME mode")
                mode = "time"
                current_led = 255
                # Adjust time instead
                ...
                logger.info(
                    f"   → LED {current_led} → {new_led} (ratio {desired_ratio:.2f})",
                )
                current_led = new_led
        else:
            ...
            new_time = max(1.0, min(200.0, current_time * desired_ratio))
            logger.info(
                f"   → Time {current_time:.1f}ms → {new_time:.1f}ms (ratio {desired_ratio:.2f})",
            )
            current_time = new_time

    logger.warning(f"   ⚠️ Did not converge after {max_iterations} iterations")
    logger.warning(
        f"   Final: {signal_history[-1] if signal_history else 0:.0f} counts (target {target_count:.0f})",
    )
    return current_led, current_time, signal_history, False


def test_led_convergence_optimized(
    normalizer: LEDNormalizer,
    results: dict,
    led: str = "a",
    mode: str = "intensity",
    ...
    logger.info(
        f"Current intensity at normalized settings: {current_intensity:.1f} counts",
    )
    ...
    logger.info(
        f"Target count ({CONVERGENCE_TARGET_PERCENT*100:.0f}% saturation): {target_count:.0f}",
    )
    logger.info(
        f"Tolerance: ±{CONVERGENCE_TOLERANCE_PERCENT*100:.1f}% = ±{tolerance_count:.0f} counts",
    )
    logger.info(
        f"Range: {target_count - tolerance_count:.0f} - {target_count + tolerance_count:.0f} counts",
    )
    ...
    if initial_time < 1.0:
        ...
        logger.warning(
            f"   ⚠️ Initial integration time too low, setting to {initial_time}ms",
        )
    ...
    if converged:
        ...
        boosted_param_name = (
            "LED intensity" if mode == "intensity" else "integration time (ms)"
        )
        ...
        logger.warning(
            f"   Final signal: {convergence_signals[-1] if convergence_signals else 0:.0f} counts",
        )
    ...
    logger.info(
        f"Phase 2: {num_samples - rapid_samples} steady-state samples (100ms delay)",
    )
    ...
    for i in range(rapid_samples):
        ...
        logger.info(
            f"  Rapid sample {i+1}/{rapid_samples} at t={current_time*1000:.1f}ms: {intensity:.1f} counts",
        )
    ...
    for i in range(num_samples - rapid_samples):
        ...
        logger.info(
            f"  Steady sample {i+1}/{num_samples-rapid_samples} at t={current_time:.2f}s: {intensity:.1f} counts",
        )
    ...
    convergence_time = (
        timestamps_array[converged_idx[0]] if len(converged_idx) > 0 else None
    )
    ...
    if len(rise_start_idx) > 0 and len(rise_end_idx) > 0:
        rise_time = (
            timestamps_array[rise_end_idx[0]] - timestamps_array[rise_start_idx[0]]
        )
    else:
        rise_time = None
    ...
    overshoot = (
        ((peak_intensity - final_value) / final_value) * 100 if final_value > 0 else 0
    )
    ...
    logger.info(
        f"Target: {target_count:.0f} counts ({CONVERGENCE_TARGET_PERCENT*100:.0f}% saturation)",
    )
    logger.info(
        f"Converged: {'✓ YES' if converged else '✗ NO'} after {len(convergence_signals)} iterations",
    )
    logger.info(f"Final settings: LED={final_led_value}, Time={final_time_value:.1f}ms")
    if convergence_time:
        logger.info(
            f"Convergence time (to 95%): {metrics['convergence_time_ms']:.1f}ms",
        )
    if rise_time:
        logger.info(f"Rise time (10%-90%): {metrics['rise_time_ms']:.1f}ms")
    logger.info(
        f"Final intensity: {final_value:.1f} counts ({saturation_achieved:.1f}% saturation)",
    )
...
def plot_optimized_convergence(
    metrics: dict,
    ...
    # Target line
    ax1.axhline(
        y=metrics["target_count"],
        color="r",
        linestyle="--",
        linewidth=1.5,
        label=f"Target ({metrics['target_count']:.0f})",
    )

    # Final value line
    ax1.axhline(
        y=metrics["final_intensity"],
        color="g",
        linestyle=":",
        linewidth=1.5,
        label=f"Final ({metrics['final_intensity']:.0f})",
    )

    # Convergence time marker
    if metrics["convergence_time_ms"]:
        ax1.axvline(
            x=metrics["convergence_time_ms"],
            color="orange",
            linestyle="--",
            linewidth=1,
            label=f"Conv. ({metrics['convergence_time_ms']:.1f}ms)",
        )

    # Peak overshoot marker
    peak_idx = np.argmax(samples)
    ax1.plot(
        timestamps_ms[peak_idx],
        metrics["peak_intensity"],
        "r*",
        markersize=15,
        label=f"Peak ({metrics['overshoot_percent']:.1f}% overshoot)",
    )

    ax1.set_xlabel("Time (ms)")
    ax1.set_ylabel("Intensity (counts)")
    ax1.set_title(
        f"LED {metrics['led'].upper()} Optimized Convergence ({metrics['mode'].upper()} mode, "
    ...
    if metrics["rise_time_ms"]:
        ...
        ax2.plot(
            [timestamps_ms[ten_pct_idx], timestamps_ms[ninety_pct_idx]],
            [10, 90],
            "ro-",
            linewidth=2,
            markersize=8,
    ...
    rolling_std = np.array(
        [np.std(samples[max(0, i - window) : i + 1]) for i in range(len(samples))],
    )
    rolling_cv = (rolling_std / samples) * 100

    ax3.plot(timestamps_ms, rolling_cv, "purple", linewidth=2)
    ax3.axhline(
        y=metrics["steady_state_cv_percent"],
        color="r",
        linestyle="--",
        linewidth=1.5,
        label=f"Steady-state CV: {metrics['steady_state_cv_percent']:.3f}%",
    )
...
def test_all_leds_convergence(
    ...
    logger.info(f"{'='*80}")
    logger.info(
        f"{'LED':<5} {'Conv Time':<15} {'Rise Time':<15} {'Stability':<12} {'Final Sat':<15}",
    )
    logger.info(f"{'-'*80}")

    for led, metrics in all_metrics.items():
        conv_time = (
            f"{metrics['convergence_time_ms']:.1f}ms"
            if metrics["convergence_time_ms"]
            else "N/A"
        )
        rise_time = (
            f"{metrics['rise_time_ms']:.1f}ms" if metrics["rise_time_ms"] else "N/A"
        )
        stability = f"{metrics['steady_state_cv_percent']:.3f}%"
        saturation = f"{metrics['saturation_achieved_percent']:.1f}%"
        logger.info(
            f"{led.upper():<5} {conv_time:<15} {rise_time:<15} {stability:<12} {saturation:<15}",
        )

...
try:
    from src.utils.led_methods import DetectorParams as LedDetectorParams
    from src.utils.led_methods import LEDconverge, LEDnormalizationintensity
except Exception:
    LEDconverge = None
...
def _acquire_raw_spectrum_local(
    usb,
    ctrl,
    channel: str,
    led_intensity: int,
...
def _set_integration_and_log(usb, requested_ms: float):
    try:
        ...
        if applied is None and hasattr(usb, "integration_ms"):
            try:
                applied = float(usb.integration_ms)
            except Exception:
                applied = None
        if applied is not None:
            logger.info(
                f"Integration set: requested={requested_ms:.2f} ms, applied={applied:.2f} ms",
            )
        else:
            logger.info(
                f"Integration set: requested={requested_ms:.2f} ms (applied unknown)",
            )
    except Exception as e:
        ...
        if turn_off_leds:
            ...
            if hasattr(ctrl, "get_all_led_intensities"):
                try:
                    ...
                    channels_to_check = {
                        ch: val
                        for ch, val in led_state.items()
                        if ch != "d" and val is not None
                    }
                    all_off = (
                        all(int(val) <= 1 for val in channels_to_check.values())
                    ...
                    if not all_off:
                        ...
                        channels_to_check2 = {
                            ch: val
                            for ch, val in led_state2.items()
                            if ch != "d" and val is not None
                        }
                        all_off2 = (
                            all(int(val) <= 1 for val in channels_to_check2.values())
                            if channels_to_check2
                            else True
                        )
                        if not all_off2:
                            raise RuntimeError(
                                f"LEDs did not turn off after retries: {led_state2}",
                            )
                except Exception as e:
...
def converge_integration_time(
    ...
    for i in range(max_iterations):
        ...
        for ch in ch_list:
            ...
            sig = (roi_signal or _roi_signal_local)(
                spec,
                wave_min_index,
                wave_max_index,
                method="median",
        ...
        if (
            signals
            and all(min_sig <= signals[ch] <= max_sig for ch in signals)
...
def _init_hardware():
    ...
    try:
        ...
        if isinstance(device_config, dict):
            ...
            logger.info(
                f"device_config.hardware before check: {device_config.get('hardware', {})}",
            )
        hardware = (
            device_config.get("hardware", {}) if isinstance(device_config, dict) else {}
        )
        if not hardware or (
            "servo_s_position" not in hardware or "servo_p_position" not in hardware
        ):
            logger.warning(
                "⚠️ device_config missing 'hardware.servo_s_position'/'servo_p_position' — using safe defaults (S=120°, P=60°)",
            )
            ...
            if isinstance(device_config, dict):
                ...
                logger.info(
                    f"device_config.hardware after fallback: {device_config['hardware']}",
                )
            else:
                # Non-dict config; create a minimal dict
                device_config = {
                    "hardware": {"servo_s_position": 120, "servo_p_position": 60},
                }
    except Exception as e:
...
def run_steps_3_and_4(
    device_type: str = "P4SPR",
    single_mode: bool = False,
    single_ch: str = "a",
    use_per_channel_time: bool = False,
    ...
    logger.info(
        "Controller set_mode('s'/'p') will use these preloaded EEPROM positions",
    )
    logger.info("=" * 80 + "\n")
    ...
    from settings import LED_DELAY, MAX_WAVELENGTH, MIN_WAVELENGTH
    ...
    try:
        ...
        pass
    logger.info(
        f"Detector time bounds (forced min): {det_params.min_integration_time}–{det_params.max_integration_time} ms",
    )
    ...
    for ch in ch_list:
        ...
        logger.info(
            f"{ch.upper()} ranking: mean={mean_int:.0f} max={max_int:.0f} @ LED={test_led}",
        )
    ...
    best_int = max(
        det_params.min_integration_time,
        min(det_params.max_integration_time, best_int),
    )
    usb.set_integration(best_int)
    time.sleep(0.010)
    logger.info(
        f"Step 3B: Computed integration from 3A data → {best_int:.1f} ms (target {target_counts}, predicted at 255 was {predicted_weakest_counts_at_255_70ms:.0f})",
    )
    ...
    if validation_spec is not None:
        sat_px = count_saturated_pixels(
            ...
            wave_min_index,
            wave_max_index,
            det_params.saturation_threshold,
        )
        ...
        if sat_px > 0:
            ...
            logger.warning(
                f"3B saturation detected on weakest @255: {sat_px}/{roi_len} px (ratio={sat_ratio:.3f}) → reducing integration by {reduction*100:.1f}% to {new_int:.1f} ms",
            )
            ...
            if validation_spec2 is not None:
                sat_px2 = count_saturated_pixels(
                    validation_spec2,
                    wave_min_index,
                    wave_max_index,
                    det_params.saturation_threshold,
                ...
                if sat_px2 > 0:
                    ...
                    logger.warning(
                        f"3B still saturated ({sat_px2}/{roi_len}, ratio={sat_ratio2:.3f}) after reduction; consider further time reduction or lowering LED",
                    )
    else:
        logger.warning(
            "3B validation spectrum unavailable; proceeding with computed integration",
        )
    ...
    if use_per_channel_time:
        ...
        for ch, (mean, _max) in ranked:
            ...
            logger.info(
                f"3C {ch.upper()}: ratio {ratio:.2f} → LED={normalized_leds[ch]}",
            )
    ...
    for ch in ch_list:
        ...
        if spectrum is None:
            continue
        roi = spectrum[wave_min_index:wave_max_index]
        signal = (roi_signal or _roi_signal_local)(
            roi,
            0,
            len(roi),
            method="median",
        ...
        if sat_px > 0:
            ...
            logger.warning(
                f"{ch.upper()} saturated ({sat_px}/{roi_len}, ratio={sat_ratio:.3f}) → reducing LED by {reduction*100:.1f}% from {normalized_leds[ch]} to {new_led}",
            )
            ...
            if spectrum is not None:
                ...
                status = "SAT" if sat_px > 0 else "OK"
        logger.info(
            f"S-{ch.upper()} @ LED={normalized_leds[ch]}: top50={signal:.0f} counts ({pct:.1f}%) {status}",
        )

    # If requested, run alternative normalization: per-channel integration times at LED=255
    per_channel_time_results = None
    if use_per_channel_time and hasattr(
        __import__("src.utils.led_methods", fromlist=["LEDnormalizationtime"]),
        ...
        try:
            ...
            per_times = LEDnormalizationtime(
                ...
                acquire_raw_spectrum_fn=(
                    acquire_raw_spectrum or _acquire_raw_spectrum_local
                ),
            ...
            for ch in ch_list:
                ...
                if sat > 0 and abs(Tch - det_params.min_integration_time) <= 0.5:
                    ...
                    scale = max(
                        0.30,
                        min(0.95, (target_counts_local / sig) if sig > 0 else 0.5),
                    )
                    new_led = int(max(10, min(255, round(255 * scale))))
                    logger.info(
                        f"Per-channel time: {ch.upper()} saturated at min time {Tch:.1f}ms — reducing LED 255→{new_led}",
                    )
                    ...
                    if spec is not None:
                        ...
                        if sat2 > 0:
                            ...
                            scale2 = max(
                                ...
                                min(
                                    0.95,
                                    (target_counts_local / sig) if sig > 0 else 0.5,
                                ),
                            )
                            new_led2 = int(max(10, min(255, round(255 * scale2))))
                            logger.info(
                                f"Per-channel time: {ch.upper()} still saturated — further reducing LED {final_led_val}→{new_led2}",
                            )
                ...
                per_channel_time_results[ch] = {
                    ...
                    "final_percentage": float(
                        (sig / detector_max * 100.0) if detector_max else 0.0,
                    ),
                }
            logger.info(
                "Per-channel integration results (LED=255): "
                + str(per_channel_time_results),
            )
        except Exception as e:
    ...
    if per_channel_time_results:
        ...
        converged_int = None
        ch_signals = {
            ch: per_channel_time_results[ch]["final_top50_counts"]
            for ch in ch_list
            if ch in per_channel_time_results
        }
        ok = all(
            0.80 * detector_max * 0.975 <= ch_signals[ch] <= 0.80 * detector_max * 1.025
            for ch in ch_signals
        )
    else:
        ...
        logger.info("=" * 80)
        logger.info(
            f"Starting Step 4 with integration {best_int:.1f} ms and normalized LEDs {normalized_leds}",
        )
        target_percent = 0.80
        tolerance = 0.025
    ...
    if converged_int is not None:
        logger.info(
            f"Final S-mode integration: {converged_int:.1f} ms | Converged={ok}",
        )
    else:
        logger.info(
            f"Per-channel-time strategy used | Within target window for all channels: {ok}",
        )
    ...
    try:
        ...
        for ch in ch_list:
            ...
            sat_summary[ch] = count_saturated_pixels(
                roi,
                0,
                len(roi),
                det_params.saturation_threshold,
        ...
        logger.info(
            f"Saturation summary at final settings: total_saturated_pixels={total_sat} per_channel={sat_summary}",
        )
    except Exception:
        pass
    ...
    for ch in ch_list:
        T_for_measure = (
            float(per_channel_time_results[ch]["final_integration_ms"])
            if per_channel_time_results
            else converged_int
        )
        led_for_measure = (
            int(per_channel_time_results[ch]["final_led"])
            if per_channel_time_results
            else final_leds[ch]
        )
        spec = (acquire_raw_spectrum or _acquire_raw_spectrum_local)(
    ...
    for ch in ch_list:
        ...
        if per_channel_time_results:
            if sig < desired_min:
                exact_T = (
                    final_T_for_ch * (target_counts / sig)
                    if sig > 0
                    else final_T_for_ch
                )
                exact_T = min(
                    det_params.max_integration_time,
                    max(det_params.min_integration_time, exact_T),
                )
                ...
                if spec3 is not None:
                    sig3 = (roi_signal or _roi_signal_local)(
                        spec3,
                        wave_min_index,
                        wave_max_index,
                        method="median",
                ...
                exact_T = final_T_for_ch * max(
                    0.85,
                    min(0.98, (target_counts / sig) if sig > 0 else 0.95),
                )
                ...
                if spec2 is not None:
                    final_sig_for_ch = (roi_signal or _roi_signal_local)(
                        spec2,
                        wave_min_index,
                        wave_max_index,
                        method="median",
                    ...
                    if sat2 > 0 and ch.lower() == "c":
                        ...
                        scale_c = max(
                            ...
                            min(
                                0.95,
                                (target_counts / final_sig_for_ch)
                                if final_sig_for_ch > 0
                                else 0.5,
                            ),
                        )
                        new_led = int(
                            max(10, min(255, round(final_led_for_ch * scale_c))),
                        )
                        logger.info(
                            f"Final fine-tune: C saturated at min time — reducing LED {final_led_for_ch}→{new_led}",
                        )
        ...
        # Shared-integration path: allow LED proportional adjustments, then time if capped
        elif sig < desired_min:
            ratio_needed = (target_counts / sig) if sig > 0 else 1.0
            proposed_led = int(
                min(255, max(10, round(final_led_for_ch * ratio_needed))),
            )
            ...
            if final_sig_for_ch < desired_min and final_led_for_ch >= 255:
                exact_T = (
                    final_T_for_ch * (target_counts / final_sig_for_ch)
                    if final_sig_for_ch > 0
                    else final_T_for_ch
                )
        ...
        per_channel_results[ch] = {
            ...
            "final_percentage": float(
                (final_sig_for_ch / detector_max * 100.0) if detector_max else 0.0,
            ),
        }
    ...
    if per_channel_time_results:
        ...
        try:
            ...
            values = [
                per_channel_time_results[ch]["final_top50_counts"] for ch in ch_list
            ]
            ...
            out_png = os.path.join(
                os.getcwd(),
                "generated-files",
                "step4_plot_per_channel_time.png",
            )
    ...
    try:
        ...
        plt.axhline(
            ...
            color="green",
            linestyle="--",
            label=f"Target {int(target_percent*100)}%",
        )
    ...
    try:
        ...
        for ch in ch_list:
            spec = (acquire_raw_spectrum or _acquire_raw_spectrum_local)(
                ...
                ctrl,
                ch,
                int(per_channel_results[ch]["final_led"]),
                float(per_channel_results[ch]["final_integration_ms"]),
                1,
                LED_DELAY * 1000,
                10.0,
                False,
            ...
            plt.plot(
                wave,
                spec,
                label=f"S-{ch.upper()} (LED={per_channel_results[ch]['final_led']}, T={per_channel_results[ch]['final_integration_ms']:.1f}ms)",
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Isolated LED Calibration Steps 3 and 4 Test",
    )
    parser.add_argument(
        "--per-channel-time",
        action="store_true",
        help="Use per-channel integration time strategy (LEDs frozen at 255)",
    )
    parser.add_argument(
        "--tighten-final",
        action="store_true",
        help="Run one final math-driven tighten iteration in per-channel-time path",
    )
    args = parser.parse_args()
    run_steps_3_and_4(
        use_per_channel_time=bool(args.per_channel_time),
        tighten_final=bool(args.tighten_final),
    )

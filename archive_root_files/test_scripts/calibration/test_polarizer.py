...
try:
    ...
    if usb:
        logger.info(
            f"✅ Spectrometer found: {usb.serial_number if hasattr(usb, 'serial_number') else 'Unknown'}",
        )
    else:
    ...
    if pico_port:
        ...
        if ctrl.open():
            logger.info(
                f"✅ Controller connected: {ctrl.firmware_version if hasattr(ctrl, 'firmware_version') else ctrl.version}",
            )
        else:
...
if usb and ctrl:
    ...
    logger.info(
        "   result = auto_calibrate_polarizer(usb, ctrl, require_water=True, polarizer_type='circular')",
    )

...
def test_async_acquisition():
    ...
    print(f"Synchronized acquisition:")
    ...
    print(f"Asynchronous acquisition:")
    ...
    async_data = []
    for i, (spectrum, t_spec) in enumerate(zip(async_spectra, async_timestamps)):
        ...
        for j, event in enumerate(led_events):
            if event['state'] == 'ON':
                ...
                for k in range(j+1, len(led_events)):
                    if led_events[k]['channel'] == event['channel'] and led_events[k]['state'] == 'OFF':
    ...
    print(f"                          | Synchronized | Asynchronous | Difference")
    print(f"--------------------------+--------------+--------------+------------")
    print(f"Total time                | {sync_time:8.0f}ms   | {async_time:8.0f}ms   | {sync_time - async_time:+7.0f}ms")
    print(f"Valid measurements        | {len(sync_data):12d} | {len(async_data):12d} | {len(async_data) - len(sync_data):+10d}")
    print(f"Time per measurement      | {sync_time/len(sync_data):8.1f}ms   | {async_time/len(async_data):8.1f}ms   | {sync_time/len(sync_data) - async_time/len(async_data):+7.1f}ms")
    print()
    ...
    if len(sync_data) == len(async_data):
        ...
        for i in range(min(len(sync_data), len(async_data))):
            ...
            print(f"  Cycle {sync_data[i]['cycle']}, Ch {ch}: sync={sync_mean:.0f}, async={async_mean:.0f}, diff={diff_percent:.1f}%")

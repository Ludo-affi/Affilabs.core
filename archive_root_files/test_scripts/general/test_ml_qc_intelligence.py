...
def test_ml_qc_intelligence():
    ...
    for i in range(10):
        ...
        avg_fwhm = (
            sum(
                cal_record["transmission_qc"][ch]["fwhm"] for ch in ["a", "b", "c", "d"]
            )
            / 4
        )
        print(
            f"   Calibration {i+1}: FWHM avg = {avg_fwhm:.1f}nm, LED max = {cal_record['p_mode_intensity']['d']}",
        )
    ...
    print("\n" + "=" * 80)
    print("📊 MODEL 1: CALIBRATION QUALITY PREDICTION")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("💡 MODEL 2: LED HEALTH STATUS")
    print("=" * 80)
    ...
    for led in led_statuses:
        ...
        if led.replacement_recommended:
            print("   🚨 REPLACEMENT RECOMMENDED")

    # Test Model 3: Sensor Coating Degradation
    print("\n" + "=" * 80)
    print("🔬 MODEL 3: SENSOR COATING STATUS")
    print("=" * 80)
    ...
    if coating.replacement_warning:
        print("⚠️  REPLACEMENT WARNING: Sensor approaching end of life")

    # Test Model 4: Optical Alignment (Baseline-based)
    print("\n" + "=" * 80)
    print("🔧 MODEL 4: OPTICAL ALIGNMENT (BASELINE MONITOR)")
    print("=" * 80)
    print("Note: Uses CALIBRATION baseline only, not real-time SPR data\n")
    ...
    print(
        f"Baseline Mean: {sum(ml_intel.alignment_baseline)/len(ml_intel.alignment_baseline):.3f}",
    )
    print(
        f"Baseline Std: {(sum((x - sum(ml_intel.alignment_baseline)/len(ml_intel.alignment_baseline))**2 for x in ml_intel.alignment_baseline) / len(ml_intel.alignment_baseline))**0.5:.3f}",
    )
    print("✅ Baseline established - ready for drift detection")
    ...
    print("\n" + "=" * 80)
    print("✅ ML QC INTELLIGENCE TEST COMPLETE")
    print("=" * 80)

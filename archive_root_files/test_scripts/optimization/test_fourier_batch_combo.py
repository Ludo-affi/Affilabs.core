...
def main():
    ...
    for pipeline_name, pipeline_class in pipelines:
        ...
        for strategy_name, strategy_func in strategies:
            ...
            if stats:
                ...
                print(
                    f"  {strategy_name:25s} P2P={stats['peak_to_peak_pm']:7.3f} pm, "
    ...
    for i, row in df_sorted.head(10).iterrows():
        print(
            f"{df_sorted.index.get_loc(i)+1:2d}. {row['name']:60s} {row['peak_to_peak_pm']:7.3f} pm",
        )
    ...
    if "Fourier" in overall_best["name"]:
        ...
        print("   Keep using Fourier pipeline with your batch strategy.")
    else:
        print("\n⚠️  Direct ArgMin performs better with this batch strategy")
        print("   Consider switching from Fourier to Direct ArgMin pipeline.")

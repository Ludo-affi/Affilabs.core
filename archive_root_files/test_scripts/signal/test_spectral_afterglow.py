...
def test_spectral_afterglow():
    ...
    for delay_ms in compare_delays:
        if delay_ms in afterglow_corrected:
            afterglow_norm = afterglow_corrected[delay_ms] / np.max(afterglow_corrected[delay_ms])
    ...
    for i, delay_ms in enumerate(compare_delays):
        if delay_ms in afterglow_corrected:
            ax1.plot(wavelengths, afterglow_corrected[delay_ms],
                    color=colors[i], linewidth=1.5, label=f'{delay_ms}ms delay', alpha=0.7)
    ...
    for i, delay_ms in enumerate(compare_delays):
        if delay_ms in afterglow_corrected:
            afterglow_norm = afterglow_corrected[delay_ms] / np.max(afterglow_corrected[delay_ms])
            ax2.plot(wavelengths, afterglow_norm,
                    color=colors[i], linewidth=1.5, label=f'{delay_ms}ms', alpha=0.7)
    ...
    for (region_name, result), color in zip(decay_results.items(), region_colors):
        times = result['times']
        signals = result['signals']

        # Plot data
        ax3.plot(times, signals, 'o', color=color, markersize=6,
    ...
    region_names = [name.split('(')[0].strip() for name in decay_results.keys()]
    ...
    bars = ax5.bar(range(len(region_names)), baseline_values, color=region_colors, alpha=0.7)
    ...
    # Add value labels
    for bar, baseline in zip(bars, baseline_values):
        height = bar.get_height()
    ...
    for i, delay_ms in enumerate([50, 100, 200]):
        if delay_ms in afterglow_corrected:
            ...
            ax6.plot(wavelengths, diff, color=colors[i+1],
                    linewidth=1.5, label=f'{delay_ms}ms', alpha=0.7)
    ...
    # Save plot
    plot_filename = f"spectral_afterglow_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    ...
    print(f"1. Decay time constant:")
    print(f"   Mean: {tau_mean:.1f} ms")
    print(f"   Std: {tau_std:.1f} ms")
    print(f"   CV: {tau_cv:.1f}%")

    if tau_cv < 10:
        print(f"   → Uniform decay across wavelengths ✓")
    else:
        print(f"   → Wavelength-dependent decay! ⚠")

    print()

    # Check spectral shape preservation
    if 0 in afterglow_corrected and 200 in afterglow_corrected:
        corr_0 = np.corrcoef(led_normalized,
                             afterglow_corrected[0] / np.max(afterglow_corrected[0]))[0, 1]
        corr_200 = np.corrcoef(led_normalized,
                               afterglow_corrected[200] / np.max(afterglow_corrected[200]))[0, 1]

        print(f"2. Spectral shape preservation:")
        ...
        if corr_0 > 0.95 and corr_200 > 0.95:
            print(f"   → Spectral shape preserved ✓")
        elif corr_0 > 0.95:
            print(f"   → Shape changes as afterglow decays ⚠")
        else:
            print(f"   → Afterglow has different spectral shape! ⚠")
    ...
    print(f"3. Persistent baseline:")
    print(f"   Mean: {baseline_mean:.0f} counts")
    print(f"   Std: {baseline_std:.0f} counts")
    print(f"   CV: {baseline_cv:.1f}%")

    if baseline_cv < 20:
        print(f"   → Uniform baseline (likely optical scatter) ✓")
    else:
        print(f"   → Wavelength-dependent baseline ⚠")

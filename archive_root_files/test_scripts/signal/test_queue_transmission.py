...
try:
    ...
    if hasattr(main_window, 'app') and main_window.app:
        ...
        if hasattr(main_window.app, '_queue_transmission_update'):
            ...
            # Try calling it
            print(f"    Calling app._queue_transmission_update('{data['channel']}', data)...")
            ...
            print(f"    Available methods: {[m for m in dir(main_window.app) if 'transmission' in m.lower()]}")
    else:
        print("    [ERROR] main_window.app not set (app not initialized)")
        print("    This is expected in this test - real app sets it in Application.__init__")

...
def load_prediction_model():
    ...
    if not model_files:
        raise FileNotFoundError(
            "No prediction model found! Run led_prediction_model.py first.",
        )
...
def main():
    ...
    for time_ms in test_times:
        ...
        print(
            f"{time_ms:>10} {pred['all_4']:>10.1f} {pred['LED_A']:>10.1f} "
    ...
    for time_ms in test_times:
        ...
        for led_name, led_char in [("A", "a"), ("B", "b"), ("C", "c"), ("D", "d")]:
            ...
            measured, saturated = measure_led(
                controller,
                spectrometer,
                led_char,
                intensity,
            ...
            status = (
                "✓" if abs(error_pct) <= 5 else "~" if abs(error_pct) <= 10 else "✗"
            )
            sat_warn = f" ⚠SAT:{saturated}" if saturated > 0 else ""

            print(
                f"  LED {led_name}: I={intensity:>3} → {measured:>8.0f} counts "
    ...
    for time_ms in test_times:
        ...
        measured, saturated = measure_all_leds(
            controller,
            spectrometer,
            intensity,
            time_ms,
        ...
        print(
            f"{time_ms:>10} {intensity:>10} {measured:>12.0f} {target_counts:>12.0f} "
    ...
    if individual_valid:
        ...
        print(
            f"  Within ±5%:  {within_5pct}/{len(individual_valid)} ({within_5pct/len(individual_valid)*100:.0f}%)",
        )
        print(
            f"  Within ±10%: {within_10pct}/{len(individual_valid)} ({within_10pct/len(individual_valid)*100:.0f}%)",
        )
    ...
    if combined_valid:
        ...
        print(f"  Max absolute error:  {max_error:.1f}%")
        print(
            f"  Within ±5%:  {within_5pct}/{len(combined_valid)} ({within_5pct/len(combined_valid)*100:.0f}%)",
        )
        print(
            f"  Within ±10%: {within_10pct}/{len(combined_valid)} ({within_10pct/len(combined_valid)*100:.0f}%)",
        )
    ...
    if individual_valid:
        ind_mean = sum(abs(r["error_pct"]) for r in individual_valid) / len(
            individual_valid,
        )
        if ind_mean <= 5:
    ...
    if combined_valid:
        comb_mean = sum(abs(r["error_pct"]) for r in combined_valid) / len(
            combined_valid,
        )
        if comb_mean <= 5:
    ...
    # Save results
    output_file = (
        f"led_model_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )

    ...
    for led_name, slopes_data in led_models.items():
        ...
        for v in validation:
            if v['led'] == led_name and abs(v['error_pct']) < 50:  # Exclude saturated
                ...
                correction = 1.0 - (error_pct / 100.0)  # e.g., +5% error → 0.95× correction
...
def predict_led_intensity_corrected(model, led_name, target_counts, integration_time_ms):
    """
    ...
    if integration_time_ms > max_safe_time:
        print(f"   ⚠ LED {led_name} saturates at {integration_time_ms}ms (max safe: {max_safe_time}ms)")
        return None
...
def main():
    ...
    input_file = Path(__file__).parent / "led_calibration_multistage_20251213_184717.json"
    ...
    for time_ms in test_times:
        for led_name in ['A', 'B', 'C', 'D']:
            ...
            new_intensity = predict_led_intensity_corrected(
                corrected_model, led_name, target_counts, time_ms
            )
            ...
            print(f"{time_ms:>6} {led_name:>5} {old_intensity:>12} {new_str:>12} {status:>20}")

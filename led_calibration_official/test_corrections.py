...
def main():
    ...
    if has_corrections:
        ...
        for time_ms, corrections in model_data['correction_factors'].items():
            print(f"  {time_ms}ms: {', '.join(f'{led}={corr:.4f}' for led, corr in corrections.items())}")
    ...
    for time_ms, led in test_cases:
        ...
        print(f"{time_ms:>6} {led:>5} {intensity_no_corr:>14} {intensity_with_corr:>12} {correction:>10.4f} {improvement:>15}")
    ...
    print("=" * 80)
    print(f"Without correction: Predict intensity 135")
    print(f"With correction:    Predict intensity 134 (0.9923× factor)")
    print(f"Result:            More accurate - closer to actual needed value")
    print(f"Impact:            Faster convergence, fewer iterations")

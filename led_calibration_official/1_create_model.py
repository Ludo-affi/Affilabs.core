...
def measure_led_response(controller, spectrometer, led_char, intensity, integration_time_ms, dark_counts, detector_wait_ms=50):
...
def main():
    ...
    for time_ms in integration_times:
        ...
        for led_name, led_char in [('A', 'a'), ('B', 'b'), ('C', 'c'), ('D', 'd')]:
            ...
            for intensity in base_intensities:
                result = measure_led_response(controller, spectrometer, led_char, intensity, integration_time, dark_counts, DETECTOR_WAIT_MS)
                
                # Stop if saturated or near saturation (>60000 counts)
                if result['is_saturated']:
                    if result['saturated_wavelengths'] > 0:
                        print(f"  I={intensity:>3}: SATURATED ({result['saturated_wavelengths']} pixels @ 65535) - stopping scan")
                    else:
                        print(f"  I={intensity:>3}: NEAR SATURATION ({result['raw_counts']:.0f} counts, >60000 threshold) - stopping scan")
                    break
                ...
                print(f"  I={intensity:>3}: {result['corrected_counts']:>8.0f} counts{warning}")
            ...
            if len(led_data) >= 2:
                ...
                if slope:
                    ...
                    if len(led_models[led_name]) > 1:
                        ...
                        print(f"  Model: {slope:.2f} counts/intensity (ratio: {actual_ratio:.3f} vs {expected_ratio:.1f}x expected)")
                    else:
    ...
    for time_ms in validation_times:
        dark_counts = dark_counts_per_time.get(time_ms, dark_counts_per_time[10] * (time_ms / 10))
        ...
        for led_name, led_char in [('A', 'a'), ('B', 'b'), ('C', 'c'), ('D', 'd')]:
            ...
            result = measure_led_response(controller, spectrometer, led_char, predicted_intensity, float(time_ms), dark_counts, DETECTOR_WAIT_MS)
            ...
            if result['is_saturated']:
                status = "⚠️ SAT"
                print(f"{time_ms:>6} {led_name:>5} {predicted_intensity:>12} {actual_counts:>14.0f} {error:>10.0f} {error_pct:>9.1f}% {status}")
            else:
    ...
    for v in validation_results:
        ...
        if abs(error_pct) > 50:
            print(f"   ⚠ Skipping saturated: LED {led_name} @ {time_ms}ms (error {error_pct:.1f}%)")
            continue
        ...
        print(f"   {status} LED {led_name} @ {time_ms}ms: correction factor = {correction:.4f} (error was {error_pct:+.1f}%)")

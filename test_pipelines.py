...
def test_pipelines():
    ...
    for pid in ['fourier', 'centroid', 'polynomial']:
        try:
            ...
            if result.success:
                ...
                print(f"   ✓ {pipeline.get_metadata().name:25} → {result.resonance_wavelength:6.2f} nm (error: {error:5.2f} nm)")
            else:
                print(f"   ✗ {pipeline.get_metadata().name:25} → FAILED: {result.error_message}")
    ...
    if len(results) > 1:
        ...
        print(f"   Max difference: {max(results.values()) - min(results.values()):.2f} nm")

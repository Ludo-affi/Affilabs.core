...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Measure acquisition timing (single channel and A→D cycle)",
    )
    ap.add_argument(
        "--integration-ms",
        type=float,
    ...
    ap.add_argument(
        ...
        default="a",
        choices=CHANNELS,
        help="Channel for single timing",
    )
    ap.add_argument(
        "--intensity",
        type=int,
        default=180,
        help="LED intensity (0-255) for single timing",
    )
    ap.add_argument(
        "--cycle-intensity",
        type=int,
        default=180,
        help="LED intensity for all channels in cycle",
    )
    ap.add_argument(
        "--repeats",
        type=int,
    ...
    print("\n=== Acquisition Timing ===")
    print(
        f"Integration: {args.integration_ms:.1f} ms  Mode: {args.mode.upper()}  Settle: {args.settle_ms:.0f} ms  Repeats: {args.repeats}",
    )
    ...
    print(
        f"  median: {statistics.median(spec_times):6.2f} ms   min: {min(spec_times):6.2f}  max: {max(spec_times):6.2f}",
    )
    ...
    print(
        f"  median: {statistics.median(single_times):6.2f} ms   min: {min(single_times):6.2f}  max: {max(single_times):6.2f}",
    )
    ...
    print(
        f"  median: {statistics.median(cycle_times):7.2f} ms   min: {min(cycle_times):7.2f}  max: {max(cycle_times):7.2f}",
    )

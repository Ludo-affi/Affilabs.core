...
class TestLogging:
    ...
    def test_logging(self):
        ...
        print("\nTest 5: Temperature log")
        time_now = dt.datetime.now(TIME_ZONE)
        dev_timestamp = f"{time_now.hour:02d}:{time_now.minute:02d}:{time_now.second:02d}"
        ...
        ch2_export = self.log_ch2.rename(columns={
            "timestamp": "Timestamp",
            "time": "Experiment Time",
            "event": "Event Type",
            "flow": "Flow Rate",
        ...
        self.temp_log.to_csv("test_temp_log.txt", sep="\t", index=False, encoding="utf-8")

...
class AfterglowValidator:
    ...
    def test_integration_time_dependency(self, channel=ChannelID.D):
        ...
        """
        logger.info(f"\n{'='*60}")
        logger.info(f"TEST 1: INTEGRATION TIME DEPENDENCY")
        logger.info(f"{'='*60}")
        ...
        for int_time_ms in integration_times_ms:
            result = self.measure_decay_at_integration_time(channel, int_time_ms, n_cycles=3)
        ...
        if len(valid_results) >= 3:
            ...
            if tau_cv < 20:
                logger.info(f"   ✅ τ is STABLE across integration times (CV < 20%)")
                logger.info(f"   → Correction is universal!")
            else:
                logger.info(f"   ⚠️ τ VARIES significantly with integration time (CV > 20%)")
                logger.info(f"   → Need integration-time-dependent correction!")

        return results

    def test_rapid_cycling(self, channels=[ChannelID.A, ChannelID.B, ChannelID.C, ChannelID.D],
                           int_time_ms=50, n_cycles=10, inter_channel_delay_ms=5):
        """
        ...
        """
        logger.info(f"\n{'='*60}")
        logger.info(f"TEST 2: RAPID MULTI-CHANNEL CYCLING")
        logger.info(f"{'='*60}")
        ...
        for ch_idx, channel in enumerate(channels):
            signals_per_cycle = [cycle['channels'][ch_idx]['signal'] for cycle in cycle_results]

            first_5 = np.mean(signals_per_cycle[:5])
            last_5 = np.mean(signals_per_cycle[-5:])
            drift = ((last_5 - first_5) / first_5) * 100

            logger.info(f"   Channel {channel.value.upper()}: First 5 cycles = {first_5:.1f}, Last 5 = {last_5:.1f}, Drift = {drift:.2f}%")

            if abs(drift) < 2:
                logger.info(f"      ✅ Stable (drift < 2%)")
            else:
                logger.info(f"      ⚠️ Significant drift detected! Possible cumulative buildup")
    ...
    def test_correction_accuracy(self, channel=ChannelID.D, int_time_ms=50):
        ...
        """
        logger.info(f"\n{'='*60}")
        logger.info(f"TEST 3: CORRECTION ACCURACY VALIDATION")
        logger.info(f"{'='*60}")
        ...
        logger.info(f"   Using model: τ={tau:.2f}ms, A={amplitude:.1f}, baseline={baseline:.1f}")
        ...
        for delay_ms in test_delays_ms:
            ...
            logger.info(f"      {delay_ms:3d} ms: Predicted={predicted:.1f}, Measured={measured:.1f}, Error={error:+.1f} ({error_pct:+.1f}%)")
        ...
        if mape < 5:
            logger.info(f"      ✅ EXCELLENT accuracy (MAPE < 5%)")
        elif mape < 10:
            logger.info(f"      ✅ GOOD accuracy (MAPE < 10%)")
        elif mape < 20:
            logger.info(f"      ⚠️ MODERATE accuracy (MAPE < 20%)")
        else:
            logger.info(f"      ❌ POOR accuracy (MAPE > 20%) - Model may not be universal")
...
def plot_validation_results(results):
    ...
    fig.suptitle('LED Afterglow Validation: Integration Time & Frequency Dependency',
    ...
    ax1.axhline(y=np.mean(taus), color='r', linestyle='--', label=f'Mean τ={np.mean(taus):.2f}ms')
    ax1.fill_between(int_times, np.mean(taus) - np.std(taus), np.mean(taus) + np.std(taus),
                      alpha=0.3, color='red')
    ...
    for idx, data in enumerate(valid_data):
        ...
        ax3.semilogy(times, above_baseline, 'o-', color=colors[idx], markersize=4,
    ...
    ax3.set_title('Decay Curves at Different Integration Times', fontsize=12, fontweight='bold')
    ...
    if 'rapid_cycling_tests' in results and results['rapid_cycling_tests']:
        ...
        for ch_idx, channel in enumerate(channels):
            ...
            ax4.plot(cycles, signals, 'o-', color=colors_ch[channel], markersize=4,
                    linewidth=1.5, label=f'Channel {channel.upper()}', alpha=0.7)

        ax4.set_xlabel('Cycle Number', fontsize=11)
        ax4.set_ylabel('Signal (counts)', fontsize=11)
        ax4.set_title('Rapid Multi-Channel Cycling: Signal Stability', fontsize=12, fontweight='bold')
    ...
    if 'correction_validation' in results and results['correction_validation']:
        ...
        ax5.set_title('Correction Validation: Predicted vs Measured', fontsize=12, fontweight='bold')
    ...
    if 'correction_validation' in results and results['correction_validation']:
        ...
        ax6.bar(range(len(delays)), errors, color=['green' if abs(e) < 5 else 'orange' for e in errors],
               alpha=0.7)
        ...
        ax6.text(0.98, 0.95, f"MAPE = {mape:.1f}%",
    ...
    if valid_data:
        ...
        summary_text += f"Integration Time Dependency:\n"
        summary_text += f"  Mean τ: {tau_mean:.2f} ± {tau_std:.2f} ms\n"
        summary_text += f"  CV: {tau_cv:.1f}%\n"

        if tau_cv < 20:
            summary_text += f"  ✅ τ is STABLE\n"
            summary_text += f"  → Universal correction OK\n\n"
        else:
            summary_text += f"  ⚠️ τ VARIES significantly\n"
            summary_text += f"  → Need time-dependent model\n\n"

    if 'correction_validation' in results and results['correction_validation']:
        mape = results['correction_validation'].get('mape', 0)
        summary_text += f"Correction Accuracy:\n"
        summary_text += f"  MAPE: {mape:.1f}%\n"

        if mape < 5:
            summary_text += f"  ✅ EXCELLENT\n\n"
        elif mape < 10:
            summary_text += f"  ✅ GOOD\n\n"
        else:
            summary_text += f"  ⚠️ Needs improvement\n\n"

    summary_text += "Recommendation:\n"
    if tau_cv < 20 and ('correction_validation' not in results or results['correction_validation'].get('mape', 100) < 10):

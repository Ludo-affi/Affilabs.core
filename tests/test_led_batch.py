...
def test_batch_commands():
    ...
    commands = create_calibration_batch(
        "p",
        {
            "a": 200,
            "b": 180,
    ...
    batch = LEDBatchBuilder()
    batch.set_mode("p").set_intensity("a", 180).set_intensity("b", 190).set_intensity(
        "c",
        200,
    ).set_intensity("d", 210).execute(led_hal)
    ...
    if batch_time < sequential_time:
        ...
        print(
            "   \u26a0\ufe0f Sequential was faster (controller may not support batch)",
        )

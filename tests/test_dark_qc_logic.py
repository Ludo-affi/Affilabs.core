...
def test_dark_qc(dark_mean_values):
    ...
    """
    print("\n" + "=" * 80)
    print("TESTING DARK NOISE QC")
    print(
        f"Expected: {EXPECTED_DARK_MEAN_OCEAN_OPTICS:.0f} counts (Ocean Optics detector)",
    )
    ...
    for attempt, dark_mean in enumerate(dark_mean_values, start=1):
        ...
        if dark_mean > DARK_QC_THRESHOLD:
            ...
            print(
                f"   ({dark_mean/EXPECTED_DARK_MEAN_OCEAN_OPTICS:.1f}× higher than expected)",
            )

            if attempt < MAX_RETRY_ATTEMPTS:
                print("   → Retrying with longer LED settle time...")
                continue
            print(

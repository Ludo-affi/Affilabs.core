...
def display_system_health(fmea):
    ...
    if health["active_failures"]["failures"]:
        ...
        for failure in health["active_failures"]["failures"]:
            ...
            print(
                f"     Severity: {failure['severity']}, Active for: {failure['since']:.0f}s",
            )
    ...
    print("\n✅ Phase Status:")
    print(
        f"   Calibration: {'✓ Complete' if health['phase_status']['calibration_completed'] else '⏳ Pending'}",
    )
    print(
        f"   Afterglow: {'✓ Complete' if health['phase_status']['afterglow_validated'] else '⏳ Pending'}",
    )
    print(
        f"   Live Data: {'🟢 Active' if health['phase_status']['live_data_active'] else '⚫ Inactive'}",
    )
...
def main():
    ...
    print("📊 View JSON file for complete event history and metrics")

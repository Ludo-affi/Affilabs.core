...
def main():
    ...
    # ========== PHASE 1: P REGION ==========
    print("# === PHASE 1: P REGION (1-15 PWM) ===")
    ...
    print(
        f"# P region complete: {p_samples} samples in {segment_end - segment_start:.3f}s",
    )
    ...
    # ========== PHASE 2: S REGION ==========
    print("# === PHASE 2: S REGION (70-105 PWM) ===")
    ...
    print(
        f"# S region complete: {s_samples} samples in {segment_end - segment_start:.3f}s",
    )
    ...
    with open(csv_path, "w") as f:
        ...
        for timestamp, intensity in samples:
            if timestamp >= p_segment_start and timestamp <= p_segment_end:
                ...
                estimated_pwm = p_region[0] + elapsed * (
                    (p_region[-1] - p_region[0]) / (p_segment_end - p_segment_start)
                )
                estimated_pwm = max(p_region[0], min(p_region[-1], estimated_pwm))
        ...
        for timestamp, intensity in samples:
            if timestamp >= s_segment_start and timestamp <= s_segment_end:
                ...
                estimated_pwm = s_region[0] + elapsed * (
                    (s_region[-1] - s_region[0]) / (s_segment_end - s_segment_start)
                )
                estimated_pwm = max(s_region[0], min(s_region[-1], estimated_pwm))

...
def main():
    ...
    p1_std = (sum((x - p1_mean)**2 for x in p1_intensities) / len(p1_intensities))**0.5
    ...
    s1_std = (sum((x - s1_mean)**2 for x in s1_intensities) / len(s1_intensities))**0.5
    ...
    p2_std = (sum((x - p2_mean)**2 for x in p2_intensities) / len(p2_intensities))**0.5
    ...
    s2_std = (sum((x - s2_mean)**2 for x in s2_intensities) / len(s2_intensities))**0.5
    ...
    print(f"  Repeatability: {abs(p1_mean - p2_mean):.0f} counts difference ({abs(p1_mean - p2_mean)/p_overall_mean*100:.2f}%)")
    ...
    print(f"  Repeatability: {abs(s1_mean - s2_mean):.0f} counts difference ({abs(s1_mean - s2_mean)/s_overall_mean*100:.2f}%)")
    ...
    if p_repeatability < 2.0:  # Within 2% (tighter due to directional control)
        print(f"✓ PASS: P position is highly repeatable with directional approach (±{p_repeatability:.2f}%)")
        checks_passed += 1
    else:
        print(f"⚠ WARN: P position repeatability is ±{p_repeatability:.2f}% (expected <2%)")
    ...
    if s_repeatability < 2.0:  # Within 2% (tighter due to directional control)
        print(f"✓ PASS: S position is highly repeatable with directional approach (±{s_repeatability:.2f}%)")
        checks_passed += 1
    else:
        print(f"⚠ WARN: S position repeatability is ±{s_repeatability:.2f}% (expected <2%)")
    ...
    if checks_passed == checks_total:
        ...
        print(f"  S = PWM {s_pwm} ({s_overall_mean:.0f} counts) - approach from PWM {s_approach}")
        print(f"  P = PWM {p_pwm} ({p_overall_mean:.0f} counts) - approach from PWM {p_approach}")

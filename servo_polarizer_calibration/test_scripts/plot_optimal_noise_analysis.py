...
ax1.errorbar(p_data['pwm'], p_data['intensity'], yerr=p_data['std'],
            fmt='o-', color='blue', linewidth=2, markersize=6, capsize=4, capthick=2,
            label='P Region', alpha=0.8)
ax1.errorbar(s_data['pwm'], s_data['intensity'], yerr=s_data['std'],
            fmt='o-', color='green', linewidth=2, markersize=6, capsize=4, capthick=2,
            label='S Region', alpha=0.8)

# Mark optimal positions
ax1.scatter([p_optimal_pwm], [p_optimal['intensity']], s=400, marker='*', 
           color='red', edgecolors='darkred', linewidths=3, zorder=10, label='Optimal P')
ax1.scatter([s_optimal_pwm], [s_optimal['intensity']], s=400, marker='*',
           color='orange', edgecolors='darkorange', linewidths=3, zorder=10, label='Optimal S')

# Shade stable regions
ax1.axvspan(p_good['pwm'].min(), p_good['pwm'].max(), alpha=0.1, color='blue', label='P Stable Range')
ax1.axvspan(s_good['pwm'].min(), s_good['pwm'].max(), alpha=0.1, color='green', label='S Stable Range')

ax1.set_xlabel('PWM Position', fontsize=13, fontweight='bold')
ax1.set_ylabel('Intensity (counts)', fontsize=13, fontweight='bold')
ax1.set_title('Optimal Position Sweep with Noise Analysis (10 scans per position)\nSpectral Analysis: P=min(610-680nm), S=max(full spectrum), ±10 points average', 
             fontsize=14, fontweight='bold')
ax1.grid(True, alpha=0.3)
ax1.legend(loc='upper right', fontsize=10, ncol=2)

# Annotations
ax1.annotate(f"Optimal P\nPWM {p_optimal_pwm}\n{p_optimal['intensity']:.1f} ± {p_optimal['std']:.1f}", 
...
ax2.errorbar(p_data['pwm'], p_data['intensity'], yerr=p_data['std'],
            fmt='o-', color='blue', linewidth=2.5, markersize=8, capsize=5, capthick=2)
ax2.scatter([p_optimal_pwm], [p_optimal['intensity']], s=300, marker='*',
           color='red', edgecolors='darkred', linewidths=3, zorder=10)
ax2.axvspan(p_good['pwm'].min(), p_good['pwm'].max(), alpha=0.2, color='blue')
ax2.axhline(y=p_optimal['intensity'], color='red', linestyle='--', alpha=0.5, linewidth=2)
...
ax3.errorbar(s_data['pwm'], s_data['intensity'], yerr=s_data['std'],
            fmt='o-', color='green', linewidth=2.5, markersize=8, capsize=5, capthick=2)
ax3.scatter([s_optimal_pwm], [s_optimal['intensity']], s=300, marker='*',
           color='orange', edgecolors='darkorange', linewidths=3, zorder=10)
ax3.axvspan(s_good['pwm'].min(), s_good['pwm'].max(), alpha=0.2, color='green')
ax3.axhline(y=s_optimal['intensity'], color='orange', linestyle='--', alpha=0.5, linewidth=2)
...
ax4.scatter([p_optimal_pwm], [p_optimal['std']], s=300, marker='*',
           color='red', edgecolors='darkred', linewidths=3, zorder=10)
...
ax5.scatter([s_optimal_pwm], [s_optimal['std']], s=300, marker='*',
           color='orange', edgecolors='darkorange', linewidths=3, zorder=10)
...
ax6.plot(p_data['pwm'], p_data['cv_percent'], 'o-', color='blue', linewidth=2, markersize=8)
ax6.scatter([p_optimal_pwm], [p_optimal['cv_percent']], s=300, marker='*',
           color='red', edgecolors='darkred', linewidths=3, zorder=10)
...
ax7.plot(s_data['pwm'], s_data['cv_percent'], 'o-', color='green', linewidth=2, markersize=8)
ax7.scatter([s_optimal_pwm], [s_optimal['cv_percent']], s=300, marker='*',
           color='orange', edgecolors='darkorange', linewidths=3, zorder=10)
...
plt.suptitle('Optimal Position Analysis with Noise Characterization', 
...
print(f"\nP REGION:")
print(f"  Stable PWM range: {p_good['pwm'].min()} - {p_good['pwm'].max()} ({len(p_good)} positions)")
...
print(f"  Best noise: PWM {p_data.loc[p_data['std'].idxmin(), 'pwm']:.0f} with {p_data['std'].min():.1f} counts std")

print(f"\nS REGION:")
print(f"  Stable PWM range: {s_good['pwm'].min()} - {s_good['pwm'].max()} ({len(s_good)} positions)")
...
print(f"  Best noise: PWM {s_data.loc[s_data['std'].idxmin(), 'pwm']:.0f} with {s_data['std'].min():.1f} counts std")

...
def sweep_position(hm, center_pwm, approach_pwm, region_name, wavelengths, num_scans=10):
    """
    ...
    for pwm in range(center_pwm - 10, center_pwm + 11):
        ...
        print(f"[PWM {pwm:3d}] Approaching from PWM {approach_pwm}...", end=" ", flush=True)
...
def main():
    ...
    try:
        ...
        p_results = sweep_position(hm, center_pwm=5, approach_pwm=100, region_name="P", wavelengths=wavelengths)
        
        # Sweep S region: PWM 61-81 (center at 71, approach from below)
        s_results = sweep_position(hm, center_pwm=71, approach_pwm=1, region_name="S", wavelengths=wavelengths)
        ...
        print(f"  Minimum intensity: PWM {p_min['pwm']} = {p_min['intensity']:.1f} ± {p_min['std']:.1f} counts")
        print(f"  Maximum intensity: PWM {p_max['pwm']} = {p_max['intensity']:.1f} ± {p_max['std']:.1f} counts")
        ...
        for r in p_good_positions[:5]:  # Show top 5 most stable
            print(f"    PWM {r['pwm']:2d}: {r['intensity']:7.1f} ± {r['std']:5.1f} counts (CV: {r['std']/r['intensity']*100:.2f}%)")
        ...
        if len(p_good_positions) > 0:
            ...
            print(f"    Noise: {p_optimal['std']:.1f} counts (CV: {p_optimal['std']/p_optimal['intensity']*100:.2f}%)")
        ...
        print(f"  Minimum intensity: PWM {s_min['pwm']} = {s_min['intensity']:.1f} ± {s_min['std']:.1f} counts")
        print(f"  Maximum intensity: PWM {s_max['pwm']} = {s_max['intensity']:.1f} ± {s_max['std']:.1f} counts")
        ...
        if len(s_good_positions) > 0:
            ...
            print(f"    Noise: {s_optimal['std']:.1f} counts (CV: {s_optimal['std']/s_optimal['intensity']*100:.2f}%)")
        
        # Final summary
        print("\n" + "="*70)
        print("FINAL OPTIMAL POSITIONS:")
        print("="*70)
        print(f"  P: PWM {p_optimal_pwm} ({p_optimal['intensity']:.1f} ± {p_optimal['std']:.1f} counts)")
        print(f"  S: PWM {s_optimal_pwm} ({s_optimal['intensity']:.1f} ± {s_optimal['std']:.1f} counts)")
        print(f"  S/P Ratio: {s_optimal['intensity'] / p_optimal['intensity']:.2f}x")
        print(f"  Intensity increase: {(s_optimal['intensity'] / p_optimal['intensity'] - 1) * 100:.1f}%")

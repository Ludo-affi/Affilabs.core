...
def main():
    ...
    end_pwm = 255
    num_positions = 5
    target_positions = [int(start_pwm + i * (end_pwm - start_pwm) / (num_positions - 1)) 
                       for i in range(num_positions)]
    
    print(f"# Target positions: {target_positions}")
    print("cycle,direction,segment,target_pwm,timestamp,elapsed_time,estimated_pwm,intensity")
    ...
    for i in range(len(target_positions) - 1):
        ...
        print(f"# Segment {i+1} complete: {segment_samples} samples in {segment_end - segment_start:.3f}s")
    
    print(f"# Forward sweep complete")
    
    # ========== BACKWARD SWEEP ==========
    print(f"# === BACKWARD SWEEP (255->1) ===")
    ...
    print(f"# Backward sweep complete")
    ...
    if samples:
        ...
        for i in range(len(target_positions) - 1):
            ...
            segment_start_time = samples[0][0] + 2.0 + fwd_segment_idx * (segment_duration + 0.5)
            ...
            for timestamp, intensity in samples:
                if segment_start_time <= timestamp <= segment_end_time:
                    ...
                    print(f"forward,forward,{i+1},{end_pos},{timestamp:.6f},{elapsed:.6f},{estimated_pwm:.1f},{intensity:.2f}")
        ...
        for i in range(len(backward_positions) - 1):
            ...
            segment_duration = abs(end_pos - start_pos) / 42.0
            segment_start_time = bwd_start_time + bwd_segment_idx * (segment_duration + 0.5)
            segment_end_time = segment_start_time + segment_duration + 0.5
            ...
            for timestamp, intensity in samples:
                if segment_start_time <= timestamp <= segment_end_time:
                    ...
                    print(f"backward,backward,{i+1},{end_pos},{timestamp:.6f},{elapsed:.6f},{estimated_pwm:.1f},{intensity:.2f}")
    ...
    with open(csv_path, 'w') as f:
        f.write("cycle,direction,segment,target_pwm,timestamp,elapsed_time,estimated_pwm,intensity\n")
        ...
        for i in range(len(target_positions) - 1):
            ...
            for timestamp, intensity in samples:
                if segment_start_time <= timestamp <= segment_end_time:
                    ...
                    f.write(f"forward,forward,{i+1},{end_pos},{timestamp:.6f},{elapsed:.6f},{estimated_pwm:.1f},{intensity:.2f}\n")
        ...
        for i in range(len(backward_positions) - 1):
            ...
            for timestamp, intensity in samples:
                if segment_start_time <= timestamp <= segment_end_time:
                    ...
                    f.write(f"backward,backward,{i+1},{end_pos},{timestamp:.6f},{elapsed:.6f},{estimated_pwm:.1f},{intensity:.2f}\n")

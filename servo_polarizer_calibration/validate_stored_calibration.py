"""
Quick Servo Polarizer Validation Script

Validates stored calibration positions (P and S) without running full calibration.
...
Quick validation takes ~30 seconds vs ~60 seconds for full calibration.
"""
...
def quick_validation(hm, wavelengths, p_pwm, s_pwm, verbose=True):
    """
    Quick validation of stored P and S positions.
    ...
    if verbose:
        print("\n" + "="*70)
        print("QUICK SERVO POLARIZER VALIDATION")
        print("="*70)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Quick validation of stored servo polarizer calibration'
    )
    ...
    if not args.quiet:
        print(f"\nLoaded calibration from {source}")

    ...
    try:
        ...
        if gaussian_refinement_weight > 0:
            try:
                ...
                bounds = ([spr_wavelengths[0], 0, 5, 0], [spr_wavelengths[-1], 100, 80, 100])
                
                popt, _ = curve_fit(peak_model, spr_wavelengths, spectrum, p0=p0, bounds=bounds, maxfev=2000)
                gaussian_peak = popt[0]
                
                if abs(gaussian_peak - peak_wavelength) < 2.0:
                    peak_wavelength = (1 - gaussian_refinement_weight) * peak_wavelength + gaussian_refinement_weight * gaussian_peak
            except:
                pass
...
test_configs = [
    ...
    ("Hybrid Light v1", {
        'sg_window': 11, 'sg_poly': 3, 'gaussian_sigma': 0.5,
        'alpha': 2000, 'regression_window': 50, 'use_quadratic': False, 'gaussian_refinement': 0.0
    }),
    ("Hybrid Light v2", {
        'sg_window': 11, 'sg_poly': 3, 'gaussian_sigma': 1.0,
        'alpha': 2000, 'regression_window': 50, 'use_quadratic': False, 'gaussian_refinement': 0.0
    }),
    ("Hybrid Light v3", {
        'sg_window': 11, 'sg_poly': 4, 'gaussian_sigma': 0.8,
        'alpha': 2000, 'regression_window': 50, 'use_quadratic': False, 'gaussian_refinement': 0.0
    }),
    
    # Medium filtering
    ("Hybrid Medium v1", {
        'sg_window': 11, 'sg_poly': 4, 'gaussian_sigma': 1.0,
        'alpha': 2000, 'regression_window': 60, 'use_quadratic': False, 'gaussian_refinement': 0.0
    }),
    ("Hybrid Medium v2", {
        'sg_window': 11, 'sg_poly': 5, 'gaussian_sigma': 0.8,
        'alpha': 2000, 'regression_window': 60, 'use_quadratic': False, 'gaussian_refinement': 0.0
    }),
    ("Hybrid Medium v3", {
        'sg_window': 11, 'sg_poly': 4, 'gaussian_sigma': 1.2,
        'alpha': 2000, 'regression_window': 70, 'use_quadratic': False, 'gaussian_refinement': 0.0
    }),
    
    # Alpha variations with light filtering
    ("Alpha 3000 Light", {
        'sg_window': 11, 'sg_poly': 3, 'gaussian_sigma': 0.8,
        'alpha': 3000, 'regression_window': 50, 'use_quadratic': False, 'gaussian_refinement': 0.0
    }),
    ("Alpha 4000 Light", {
        'sg_window': 11, 'sg_poly': 3, 'gaussian_sigma': 1.0,
        'alpha': 4000, 'regression_window': 50, 'use_quadratic': False, 'gaussian_refinement': 0.0
    }),
    ("Alpha 5000 Light", {
        'sg_window': 11, 'sg_poly': 3, 'gaussian_sigma': 0.8,
        'alpha': 5000, 'regression_window': 50, 'use_quadratic': False, 'gaussian_refinement': 0.0
    }),
    ...
    ("Reg Window 80", {
        'sg_window': 11, 'sg_poly': 4, 'gaussian_sigma': 1.0,
        'alpha': 2000, 'regression_window': 80, 'use_quadratic': False, 'gaussian_refinement': 0.0
    }),
    
    # Best from alpha optimization with minimal extra filtering
    ("Optimized Alpha Only", {
        'sg_window': 11, 'sg_poly': 3, 'gaussian_sigma': 0,
        'alpha': 2000, 'regression_window': 50, 'use_quadratic': False, 'gaussian_refinement': 0.0
    }),
]
...
print("-" * 100)
print(f"{'Configuration':25s} | {'Baseline (RU)':15s} | {'Position Err (nm)':18s} | {'DC Offset (RU)':15s} | {'Score':10s}")
print("-" * 100)
...
for name, params in test_configs:
    ...
    if not np.isnan(noise):
        ...
        print(f"{name:25s} | {noise:8.2f} RU     | {pos_error:10.4f} nm      | {dc_offset:+8.2f} RU    | {combined_score:8.4f}")
...
for i, result in enumerate(results[:5], 1):
    print(f"\n{i}. {result['name']}")
    print(f"   Baseline Noise: {result['noise']:.2f} RU ({(1 - result['noise']/17.98)*100:+.1f}%)")
    print(f"   Position Error: {result['position_error']:.4f} nm ({(result['position_error']/0.3195 - 1)*100:+.1f}%)")
...
for idx, result in enumerate(results[:3]):
    ...
    ax1.set_title(f"{result['name']}\nNoise: {result['noise']:.2f} RU", fontweight='bold')
...
print(f"   PEAK_FINDING_METHOD = 'hybrid'")
...
print(f"   Baseline Noise: {best['noise']:.2f} RU ({(1 - best['noise']/17.98)*100:+.1f}% vs current)")
print(f"   Position Accuracy: {(best['position_error']/0.3195 - 1)*100:+.1f}% vs current")

...
print(
    f"SPR region: {spr_wavelengths.min():.1f} - {spr_wavelengths.max():.1f} nm ({len(spr_wavelengths)} points)",
)
print()
...
for window in window_sizes:
    for poly in poly_orders:
        ...
        print(
            f"p2p={result['p2p_ru']:5.2f} RU, lag={result['lag_samples']} samples, "
...
print(
    best_noise[["sg_window", "sg_poly", "p2p_ru", "lag_samples", "max_rate"]].to_string(
        index=False,
    ),
)
...
results_df["score"] = (
    results_df["p2p_ru"] + 5 * results_df["lag_samples"] + 100 * results_df["max_rate"]
)
best_balanced = results_df.nsmallest(5, "score")
print("\n⚖  BEST BALANCED (Noise + Temporal Response):")
print(
    best_balanced[
        ["sg_window", "sg_poly", "p2p_ru", "lag_samples", "max_rate", "score"]
...
print(
    low_lag[["sg_window", "sg_poly", "p2p_ru", "lag_samples", "max_rate"]].to_string(
        index=False,
    ),
)
...
scatter = ax.scatter(
    results_df["p2p_ru"],
    results_df["lag_samples"],
    c=results_df["sg_window"],
    s=100,
    alpha=0.6,
    cmap="viridis",
)
...
for _, row in best.iterrows():
    ax.annotate(
        f"w={int(row['sg_window'])},p={int(row['sg_poly'])}",
...
if current:
    ...
    ax.plot(
        current[0]["series"],
        label=f"Current (11,3): {current[0]['p2p_ru']:.2f} RU",
        linewidth=1.5,
    )
...
best_result = [
    r
    for r in results
    if r["sg_window"] == best_config["sg_window"]
    and r["sg_poly"] == best_config["sg_poly"]
][0]
ax = axes[1]
ax.plot(
    best_result["series"],
...
aggressive_result = [
    r
    for r in results
    if r["sg_window"] == aggressive["sg_window"]
    and r["sg_poly"] == aggressive["sg_poly"]
][0]
ax = axes[2]
ax.plot(
    aggressive_result["series"],
...
print("\n   Performance:")
print(
    f"   • Noise: {recommended['p2p_ru']:.2f} RU (vs current {current[0]['p2p_ru']:.2f} RU)",
)
...
if recommended["lag_samples"] <= 1:
    ...
    print(
        f"   [WARN]  Phase lag of {int(recommended['lag_samples'])} samples - may affect fast transients",
    )

...
for config in configurations:
    ...
    print(
        f"{marker} {config['name']:20s}: p2p={config['p2p']:6.2f} RU, std={config['std']:5.2f} RU, mean={config['mean']:+6.2f} RU  ({improvement:+5.1f}%)",
    )
...
print("\n🎯 IMPROVEMENT:")
print(
    f"   Noise:    {baseline['p2p']:.2f} → {optimized['p2p']:.2f} RU  ({(1-optimized['p2p']/baseline['p2p'])*100:.1f}%)",
)
print(
    f"   Std Dev:  {baseline['std']:.2f} → {optimized['std']:.2f} RU  ({(1-optimized['std']/baseline['std'])*100:.1f}%)",
)
...
ax1.fill_between(
    ...
    alpha=0.2,
    color="red",
    label=f'±1σ Baseline ({baseline["std"]:.2f} RU)',
)
ax1.fill_between(
    time_points,
    ...
    alpha=0.3,
    color="green",
    label=f'±1σ Optimized ({optimized["std"]:.2f} RU)',
)
...
ax1.set_title(
    "BASELINE vs OPTIMIZED (NO SG, NO Kalman) - Detrended Overlay",
    fontsize=16,
    fontweight="bold",
    pad=20,
)
...
ax3.hist(
    optimized["series"] - optimized["mean"],
    bins=30,
    alpha=0.6,
...
print(
    f"   • Alpha=2000 alone:       {(1-alpha_only['p2p']/baseline['p2p'])*100:+.1f}%  ({alpha_only['p2p']:.2f} RU)",
)
print(
    f"   • Regression alone:       {(1-reg_only['p2p']/baseline['p2p'])*100:+.1f}%  ({reg_only['p2p']:.2f} RU)",
)
print(
    f"   • Both combined:          {(1-optimized['p2p']/baseline['p2p'])*100:+.1f}%  ({optimized['p2p']:.2f} RU)",
)

...
for alpha in alpha_values:
    ...
    print(
        f"   {symbol} Alpha={alpha:>6}: p2p={p2p_nm:.6f} nm ({p2p_ru:.3f} RU), std={std_nm:.6f} nm",
    )
...
for sg_win, sg_poly in sg_configs:
    try:
        ...
        symbol = "[OK]" if p2p_ru < 2.0 else "  "
        print(
            f"   {symbol} SG({sg_win},{sg_poly}): p2p={p2p_nm:.6f} nm ({p2p_ru:.3f} RU)",
        )
    except:
        pass
...
improvement = (
    ((current_ru_p2p - opt_ru_p2p) / current_ru_p2p * 100)
    if current_ru_p2p > opt_ru_p2p
    else 0
)
...
print("\n💡 IMPLEMENTATION:")
print("Update in affilabs/settings/settings.py:")
print(f"  FOURIER_ALPHA = {best_alpha['alpha']}")
...
plt.axhline(y=2.0, color="g", linestyle="--", label="2 RU Target")
plt.axvline(
    x=best_window["window"],
    color="r",
    linestyle=":",
    label=f"Best: {best_window['window']}",
)

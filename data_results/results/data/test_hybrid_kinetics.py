...
def simulate_kinetics(baseline_spectrum, baseline_wavelengths, 
                     association_time=100, dissociation_time=100,
                     response_amplitude_ru=50, kon=0.05, koff=0.02,
                     noise_level=1.0):
    ...
    for time_col in time_columns[:50]:  # Use 50 timepoints for noise estimation
        ...
        baseline_peaks.append(apply_peak_finding(spr_transmission, spr_wavelengths, 'standard'))
    ...
    response_nm[assoc_indices] = (response_amplitude_ru / 355) * (1 - np.exp(-kon * t_assoc))
    ...
    for i in range(total_time):
        ...
        noise = np.random.normal(0, baseline_std_nm * noise_level / 10, len(shifted_spectrum))
...
print("-" * 80)
print(f"{'Rmax (RU)':20s} | {true_Rmax:12.2f}    | {std_Rmax:12.2f}    | {hyb_Rmax:12.2f}")
print(f"{'kon (1/point)':20s} | {true_kon:12.4f}    | {std_kon:12.4f}    | {hyb_kon:12.4f}")
print(f"{'R0 dissoc (RU)':20s} | {true_R0:12.2f}    | {std_R0:12.2f}    | {hyb_R0:12.2f}")
print(f"{'koff (1/point)':20s} | {true_koff:12.4f}    | {std_koff:12.4f}    | {hyb_koff:12.4f}")
print("-" * 80)
...
print(f"  koff error - Standard: {koff_error_std:6.2f}%, Hybrid: {koff_error_hyb:6.2f}%")
print(f"  Rmax error - Standard: {Rmax_error_std:6.2f}%, Hybrid: {Rmax_error_hyb:6.2f}%")
...
for filter_method, window, label in filter_configs:
    ...
    print(f"{label:25s} | {baseline_noise:9.2f} RU    | {lag:6d} points   | {combined_score:8.4f}")
...
ax1.plot(time, standard_ru, linewidth=1.5, alpha=0.7, label=f'Standard (std={baseline_std:.2f} RU)')
ax1.plot(time, hybrid_ru, linewidth=1.5, alpha=0.7, label=f'Hybrid (std={hybrid_baseline_std:.2f} RU)')
...
ax1.set_title('Kinetics Validation: Standard vs Optimized Hybrid', fontweight='bold', fontsize=14)
ax1.legend()
...
ax4.plot(time[:20], standard_ru[:20], 'o-', linewidth=1.5, alpha=0.7, label=f'Standard ({baseline_std:.2f} RU)', markersize=4)
ax4.plot(time[:20], hybrid_ru[:20], 's-', linewidth=1.5, alpha=0.7, label=f'Hybrid ({hybrid_baseline_std:.2f} RU)', markersize=4)
ax4.axhline(y=0, color='red', linestyle='--', alpha=0.5)
ax4.set_xlabel('Time (points)', fontweight='bold')
ax4.set_ylabel('Response (RU)', fontweight='bold')
ax4.set_title(f'Baseline Noise Detail\n{noise_improvement:+.1f}% improvement', fontweight='bold')
ax4.legend()
ax4.grid(True, alpha=0.3)

# Plot 5: Live filtering comparison
ax5 = fig.add_subplot(gs[2, :2])
ax5.plot(time, hybrid_ru, linewidth=1, alpha=0.4, label='Hybrid (unfiltered)', color='gray')
colors = ['red', 'orange', 'green', 'blue']
for i, result in enumerate(filter_results[:4]):
    ax5.plot(time, result['filtered_data'], linewidth=1.5, alpha=0.7, 
             label=f"{result['label']} (lag={result['lag']}pt)", color=colors[i])
...
ax6_twin = ax6.twinx()
bars1 = ax6.bar(x - width/2, noises, width, label='Noise (RU)', color='blue', alpha=0.7)
bars2 = ax6_twin.bar(x + width/2, lags, width, label='Lag (pts)', color='red', alpha=0.7)
...
print(f"   [OK] Optimized hybrid preserves kinetics with < {max(kon_error_hyb, koff_error_hyb):.1f}% error")
...
print(f"      - Achieves {(1 - best_filter['baseline_noise']/hybrid_baseline_std)*100:.1f}% additional noise reduction")
print(f"      - Minimal lag: {best_filter['lag']} points")
print(f"      - Best balance of smoothness vs responsiveness")

print(f"\n3. IMPLEMENTATION:")
print(f"   For offline analysis (publications, reports):")
print(f"      → Use optimized hybrid method (90% noise reduction)")
print(f"      → No additional filtering needed")
print(f"   ")
print(f"   For live display (real-time monitoring):")
print(f"      → Use optimized hybrid method PLUS {best_filter['label']}")
print(f"      → Total noise reduction: ~{(1 - best_filter['baseline_noise']/baseline_std)*100:.0f}%")
print(f"      → Provides smooth, responsive display")

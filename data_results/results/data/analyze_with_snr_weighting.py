...
def apply_fourier_method(transmission_spectrum, wavelengths, 
    ...
    if use_snr_weighting and has_spol and channel in s_pol_ref:
        ...
        if len(s_ref_full) == len(wavelengths):
            ...
            if s_max > s_min:
                ...
                snr_weights = snr_weights / np.mean(snr_weights)  # Normalize to mean=1.0
    ...
    detrended = weighted_spectrum[1:-1] - np.linspace(weighted_spectrum[0], weighted_spectrum[-1], n)[1:-1]
...
def get_spr_series(alpha=9000, regression_window=50, regression_poly=1, 
                   channel='a', use_snr_weighting=True):
    """Get SPR time series."""
    wavelength_series = []
...
spr = get_spr_series(alpha=9000, regression_window=50, regression_poly=1, 
                     channel=channel, use_snr_weighting=False)
...
if has_spol:
    spr = get_spr_series(alpha=9000, regression_window=50, regression_poly=1, 
                         channel=channel, use_snr_weighting=True)
    configurations.append({
        'name': 'Baseline + SNR',
        'snr': True,
        'alpha': 9000,
...
for config in configurations:
    ...
    print(f"{marker} {config['name']:22s} [SNR:{snr_str}]: p2p={config['p2p']:6.2f} RU  ({improvement:+5.1f}%)")
...
if has_spol and len(configurations) >= 4:
    ...
    snr_impact_optimized = (1 - optimized_with_snr['p2p'] / optimized_no_snr['p2p']) * 100
    ...
    print(f"   {baseline_no_snr['p2p']:.2f} → {optimized_with_snr['p2p']:.2f} RU  ({total_improvement:.1f}%)")
    ...
    for i, config in enumerate(configurations):
        ax.plot(time_points, config['series'], label=config['name'], 
    ...
    ax = axes[0, 1]
    ax.plot(time_points, baseline_no_snr['series'], label=f'No SNR ({baseline_no_snr["p2p"]:.2f} RU)', 
            color='red', linewidth=2, alpha=0.7)
    ax.plot(time_points, baseline_with_snr['series'], label=f'With SNR ({baseline_with_snr["p2p"]:.2f} RU)', 
            color='orange', linewidth=2, alpha=0.7)
    ax.set_xlabel('Time Point', fontweight='bold')
    ax.set_ylabel('SPR (RU)', fontweight='bold')
    ax.set_title(f'SNR Impact on Baseline ({snr_impact_baseline:+.1f}%)', fontweight='bold', fontsize=13)
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # Plot 3: SNR impact on optimized
    ax = axes[1, 0]
    ax.plot(time_points, optimized_no_snr['series'], label=f'No SNR ({optimized_no_snr["p2p"]:.2f} RU)', 
            color='lightgreen', linewidth=2, alpha=0.7)
    ax.plot(time_points, optimized_with_snr['series'], label=f'With SNR ({optimized_with_snr["p2p"]:.2f} RU)', 
            color='darkgreen', linewidth=2, alpha=0.7)
    ax.set_xlabel('Time Point', fontweight='bold')
    ax.set_ylabel('SPR (RU)', fontweight='bold')
    ax.set_title(f'SNR Impact on Optimized ({snr_impact_optimized:+.1f}%)', fontweight='bold', fontsize=13)
    ax.legend()
    ax.grid(True, alpha=0.3)

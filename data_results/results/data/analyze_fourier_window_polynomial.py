...
def apply_fourier_method(transmission_spectrum, wavelengths, alpha=9000,
                         search_window=50, regression_window=50, poly_order=1,
                         apply_sg=True):
...
for window in window_sizes:
    for poly in poly_orders:
        ...
        spr_series = (wavelength_series[valid_mask] - wavelength_series[valid_mask][0]) * 355
        ...
        print(f"  Window={window:3d}, Poly={poly} ({poly_name:9s}): p2p={p2p:5.2f} RU, std={std:5.2f} RU, max_rate={max_rate:5.2f} RU/s")
...
print(f"   Polynomial: {best['poly_order']} ({'Linear' if best['poly_order']==1 else 'Quadratic' if best['poly_order']==2 else 'Cubic'})")
...
for poly in [1, 2, 3]:
    ...
    if len(subset) > 0:
        ...
        poly_name = ['Linear', 'Quadratic', 'Cubic'][poly-1]
        print(f"   {poly_name:9s} (order {poly}): Window={best_poly['window']:3d}, p2p={best_poly['p2p']:5.2f} RU")
...
for window in window_sizes:
    ...
    if len(subset) > 0:
        ...
        print(f"   Window={window:3d}: Poly={best_window['poly_order']} ({poly_name}), p2p={best_window['p2p']:5.2f} RU")
    ...
    for j in range(len(pivot.columns)):
        ...
        if not np.isnan(value):
            text = ax.text(j, i, f'{value:.1f}', ha="center", va="center", 
                          color="white" if value > pivot.values.mean() else "black", fontsize=10)
...
for poly in [1, 2, 3]:
    ...
    poly_name = ['Linear', 'Quadratic', 'Cubic'][poly-1]
    ax.plot(subset['window'], subset['p2p'], marker='o', label=f'Order {poly} ({poly_name})', linewidth=2)

ax.axhline(y=2, color='r', linestyle='--', label='Target (2 RU)', linewidth=2, alpha=0.5)
if current_p2p:
    ax.axhline(y=current_p2p, color='k', linestyle=':', label=f'Current ({current_p2p:.1f} RU)', linewidth=2)
ax.set_xlabel('Regression Window Size')
...
colors = ['green' if row['poly_order'] == 1 else 'orange' if row['poly_order'] == 2 else 'purple' 
...
if current_p2p:
    ...
    ax.plot(current_series, alpha=0.3, label=f'Current (W=50, P=1): {current_p2p:.1f} RU', color='gray', linewidth=2)
    ...
    if len(subset) >= 2:
        ...
        if len(linear) > 0 and len(higher) > 0:
            if higher['p2p'].min() < linear['p2p'].values[0]:
                ...
                print(f"[OK] At Window={window}: {poly_name} (P={best_higher['poly_order']}) gives {improvement:.1f}% improvement over Linear")

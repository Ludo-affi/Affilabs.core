...
for wl_min, wl_max, label in wavelength_bins:
    ...
    if np.sum(mask) > 0:
        ...
        print(f"{label:20s}: {mean_offset:+6.2f} ± {std_offset:4.2f} RU  (n={n_points})")
...
print("-" * 80)
print(f"{'FWHM Range':20s} | {'Current Error (nm)':20s} | {'Hybrid Error (nm)':20s} | {'Improvement':15s}")
print("-" * 80)
...
for fwhm_min, fwhm_max, label in fwhm_bins:
    ...
    if np.sum(mask) > 2:
        ...
        n_points = np.sum(mask)
        print(f"{label:20s} | {curr_err:8.4f} ± {np.std(errors_current[mask]):6.4f} | "
...
print(f"{'Overall':20s} | {overall_curr:8.4f} ± {np.std(errors_current):6.4f} | "
...
print("-" * 80)
print(f"{'Noise Level':15s} | {'Current Std (nm)':20s} | {'Hybrid Std (nm)':20s} | {'Improvement':15s}")
print("-" * 80)
...
for noise_level in noise_levels:
    ...
    if len(peaks_curr) > 0:
        ...
        print(f"{noise_level:6.1f}%        | {std_curr:8.4f} nm          | "
...
ax1.axhline(y=np.mean(offsets), color='red', linestyle='--', linewidth=2, 
            label=f'Mean = {np.mean(offsets):.2f} RU')
ax1.fill_between([620, 680], np.mean(offsets) - np.std(offsets), 
                  np.mean(offsets) + np.std(offsets), alpha=0.2, color='red',
...
ax2.scatter(fwhm_values, errors_current * 1000, alpha=0.5, s=20, label='Current', color='blue')
ax2.scatter(fwhm_values, errors_hybrid * 1000, alpha=0.5, s=20, label='Hybrid', color='orange')
...
ax3.axvline(x=np.mean(fwhm_values), color='red', linestyle='--', linewidth=2,
...
ax4.text(0.05, 0.95, f'R = {correlation:.4f}', transform=ax4.transAxes, 
...
bars1 = ax5.bar(x_pos - width/2, curr_means, width, label='Current', color='blue', alpha=0.7)
bars2 = ax5.bar(x_pos + width/2, hybrid_means, width, label='Hybrid', color='orange', alpha=0.7)
...
print(f"   {'[OK] Wavelength-independent' if np.ptp(offsets) < 2.0 else '[WARN]  Some wavelength dependence'}")

print(f"\n2. PEAK WIDTH ROBUSTNESS:")
print(f"   Overall error reduction: {overall_improvement:+.1f}%")
print(f"   {'[OK] More robust across all widths' if overall_hybrid < overall_curr else '[ERROR] Less robust'}")

print(f"\n3. NOISE ROBUSTNESS:")
print(f"   Tested noise levels: 0-5% transmission")
print(f"   [OK] Hybrid consistently shows lower variability")

...
def get_wavelength_series(alpha=9000, sg_window=11, sg_poly=3, 
                          regression_window=50, regression_poly=1):
...
wl_baseline = get_wavelength_series(alpha=9000, sg_window=11, sg_poly=3, 
                                    regression_window=50, regression_poly=1)

# Stage 2: + SG optimization
wl_sg = get_wavelength_series(alpha=9000, sg_window=11, sg_poly=5, 
                              regression_window=50, regression_poly=1)

# Stage 3: + Regression optimization
wl_reg = get_wavelength_series(alpha=9000, sg_window=11, sg_poly=3, 
                               regression_window=100, regression_poly=2)

# Stage 4: + Both optimizations
wl_both = get_wavelength_series(alpha=9000, sg_window=11, sg_poly=5, 
                                regression_window=100, regression_poly=2)

# Stage 5: + Lower alpha
wl_alpha = get_wavelength_series(alpha=2000, sg_window=11, sg_poly=5, 
                                 regression_window=100, regression_poly=2)
...
print(f"  Baseline:                 {np.mean(spr_baseline_kalman):+.2f} RU  (was {np.mean(spr_baseline):+.2f})")
print(f"  + SG:                     {np.mean(spr_sg_kalman):+.2f} RU  (was {np.mean(spr_sg):+.2f})")
print(f"  + Regression:             {np.mean(spr_reg_kalman):+.2f} RU  (was {np.mean(spr_reg):+.2f})")
print(f"  + Both:                   {np.mean(spr_both_kalman):+.2f} RU  (was {np.mean(spr_both):+.2f})")
print(f"  + Alpha=2000:             {np.mean(spr_alpha_kalman):+.2f} RU  (was {np.mean(spr_alpha):+.2f})")

print("\nKalman-induced DC shift:")
print(f"  Baseline:                 {np.mean(spr_baseline_kalman) - np.mean(spr_baseline):+.2f} RU")
print(f"  + SG:                     {np.mean(spr_sg_kalman) - np.mean(spr_sg):+.2f} RU")
print(f"  + Regression:             {np.mean(spr_reg_kalman) - np.mean(spr_reg):+.2f} RU")
print(f"  + Both:                   {np.mean(spr_both_kalman) - np.mean(spr_both):+.2f} RU")
print(f"  + Alpha=2000:             {np.mean(spr_alpha_kalman) - np.mean(spr_alpha):+.2f} RU")
...
ax.text(10, np.mean(spr_baseline), f'Mean: {np.mean(spr_baseline):.2f} RU', 
        verticalalignment='bottom', color='C0', fontweight='bold')
ax.text(10, np.mean(spr_alpha), f'Mean: {np.mean(spr_alpha):.2f} RU', 
        verticalalignment='bottom', color='C1', fontweight='bold')
...
# Add mean annotations
ax.text(10, np.mean(spr_baseline_kalman), f'Mean: {np.mean(spr_baseline_kalman):.2f} RU', 
        verticalalignment='bottom', color='C0', fontweight='bold')
ax.text(10, np.mean(spr_alpha_kalman), f'Mean: {np.mean(spr_alpha_kalman):.2f} RU', 
        verticalalignment='bottom', color='C1', fontweight='bold')
...
ax.plot(spr_baseline_centered, label=f'Baseline (p2p={np.ptp(spr_baseline_kalman):.2f} RU)', 
        alpha=0.7, linewidth=2, color='red')
ax.plot(spr_optimized_centered, label=f'Optimized (p2p={np.ptp(spr_alpha_kalman):.2f} RU)', 

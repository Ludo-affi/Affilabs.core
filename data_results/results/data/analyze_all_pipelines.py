        ...
        try:
            ...
            try:
                ...
                if coeffs[0] > 0:  # Concave up (dip)
                    ...
                    if (
                        peak_wavelength < spr_wavelengths[0]
                        or peak_wavelength > spr_wavelengths[-1]
                    ):
...
def get_fourier_multifeature_series(
    alpha=2000,
    sg_window=11,
    sg_poly=5,
    regression_window=100,
    use_gaussian_refinement=True,
):
...
spr = get_fourier_multifeature_series(
    alpha=2000,
    sg_window=11,
    sg_poly=5,
    regression_window=100,
    use_gaussian_refinement=False,
)
pipelines.append(
    {
...
for i, pipeline in enumerate(sorted_pipelines):
    ...
    print(
        f"{marker} {pipeline['name']:30s}: {pipeline['p2p']:6.2f} RU  ({improvement:+6.1f}%)  [{pipeline['short']}]",
    )
...
for i, pipeline in enumerate(pipelines):
    alpha_val = (
        0.9
        if "OPTIMIZED" in pipeline["short"]
        else 0.5
        if "Current" in pipeline["short"]
        else 0.6
    )
    linewidth = (
        3
...
ax1.set_title(
    "All Peak Finding Pipelines Comparison (Detrended to Zero)",
    fontsize=16,
    fontweight="bold",
    pad=20,
)
...
colors_bar = [
    "red" if "Current" in n else "darkgreen" if "OPTIMIZED" in n else "lightblue"
    for n in names
]
...
colors_imp = [
    "red" if imp < 0 else "green" if imp > 30 else "orange" for imp in improvements
]
...
ax4.plot(
    time_points[zoom_range],
    current["series"][zoom_range],
    label=f"Current ({current['p2p']:.2f})",
    linewidth=2,
    alpha=0.6,
    color="red",
    linestyle="--",
...
ax5.hist(
    ...
    bins=30,
    alpha=0.5,
    label=f"Current (σ={current['std']:.2f})",
    color="red",
    edgecolor="darkred",
    linewidth=1.2,
)

...
def fourier_peak_finding(wavelength, transmission_spectrum, alpha=3000, 
                        step5_window=50, step6_window=50, 
                        sg_window=15, sg_poly=4, apply_sg=True,
                        pre_median=0):
...
for alpha in alpha_aggressive:
    ...
    for col in time_columns:
        peak = fourier_peak_finding(wavelengths, df[col].values,
...
for sg_win, sg_poly in sg_configs_large:
    try:
        ...
        for col in time_columns:
            peak = fourier_peak_finding(wavelengths, df[col].values,
                                        alpha=1000, step5_window=50, step6_window=50,
        ...
        symbol = "[OK]" if p2p_ru < 2.0 else "  "
        print(f"   {symbol} SG({sg_win},{sg_poly}): p2p={p2p_ru:>6.3f} RU, std={std_ru:>5.3f} RU")
...
for med_size in median_sizes:
    ...
    for col in time_columns:
        peak = fourier_peak_finding(wavelengths, df[col].values,
                                    alpha=1000, step5_window=50, step6_window=50,
                                    sg_window=25, sg_poly=4, apply_sg=True,
                                    pre_median=med_size)
        if peak is not None:
            peaks.append(peak)
    ...
    print(f"   {symbol} Median={med_str:>4}: p2p={p2p_ru:>6.3f} RU, std={std_ru:>5.3f} RU")
...
for col in time_columns:
    peak = fourier_peak_finding(wavelengths, df[col].values,
                                alpha=best_alpha, step5_window=50, step6_window=50,
                                sg_window=best_sg[0], sg_poly=best_sg[1], apply_sg=True)
    if peak is not None:
...
for window_size in [1, 3, 5, 7, 9, 11]:
    if window_size == 1:
        ...
        smoothed = np.convolve(all_peaks, np.ones(window_size)/window_size, mode='valid')
    ...
    symbol = "[OK]" if p2p_ru < 2.0 else "  "
    print(f"   {symbol} Rolling N={window_size:>2}: p2p={p2p_ru:>6.3f} RU, std={std_ru:>5.3f} RU (effective samples: {len(smoothed)})")
...
for n_samples in [1, 3, 5, 7, 9]:
    ...
    symbol = "[OK]" if p2p_ru < 2.0 else "  "
    print(f"   {symbol} Median N={n_samples}: p2p={p2p_ru:>6.3f} RU, std={std_ru:>5.3f} RU (effective samples: {len(medians)})")

print(f"\n{'='*80}")
print("SUMMARY")
print(f"{'='*80}")
print(f"\nKey findings:")
print(f"1. Lower alpha (more smoothing) helps significantly")
print(f"2. Larger SG windows help but have diminishing returns")
print(f"3. Time-domain averaging can achieve <2 RU but reduces temporal resolution")
print(f"\nRecommendation:")
print(f"- For BEST noise: Use time averaging (median N=5-7)")
print(f"- For BALANCED: Alpha=1000, SG(25,4), no time averaging")
print(f"- Trade-off: Temporal resolution vs noise performance")

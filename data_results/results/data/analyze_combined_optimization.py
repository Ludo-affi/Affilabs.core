...
for idx, row in results_df.iterrows():
    ...
    print(
        f"{marker} {row['name']:25s}: {row['p2p']:6.2f} RU  ({improvement:+5.1f}%)  "
...
print(
    f"   • SG Filter: Window={best['sg'].split('/')[0]}, Poly={best['sg'].split('/')[1]}",
)
print(
    f"   • Regression: Window={best['regression'].split('/')[0]}, Type={best['regression'].split('/')[1]}",
)
...
print(
    f"          → {sg_only['p2p']:.2f} RU  ({(1-sg_only['p2p']/baseline_p2p)*100:+.1f}%)",
)
print("\n  Step 2: Add Regression optimization (100/Quadratic)")
print(
    f"          → {reg_only['p2p']:.2f} RU  ({(1-reg_only['p2p']/baseline_p2p)*100:+.1f}%)",
)
print("\n  Step 3: Add Kalman filter")
print(
    f"          → {kalman_only['p2p']:.2f} RU  ({(1-kalman_only['p2p']/baseline_p2p)*100:+.1f}%)",
)
...
colors = [
    "red" if i == 0 else "green" if c["name"].startswith("⭐") else "lightblue"
...
ax.plot(
    baseline["series"],
    alpha=0.5,
    label=f"Baseline: {baseline['p2p']:.2f} RU",
    color="gray",
    linewidth=2,
)
ax.plot(
    best["series"],
    label=f"Optimized: {best['p2p']:.2f} RU",
    color="green",
    linewidth=2,
)
...
ax.plot(
    kalman_only["series"],
    alpha=0.7,
    label=f"+ Kalman: {kalman_only['p2p']:.2f} RU",
    color="lightgreen",
    linewidth=2,
)
ax.plot(
    best["series"],
    label=f"All: {best['p2p']:.2f} RU",
    color="darkgreen",
    linewidth=2.5,
)
ax.set_xlabel("Time Point")
...
ax = axes[2, 0]
ax.hist(
    baseline["series"] - np.mean(baseline["series"]),
    bins=30,
    alpha=0.5,
...
ax.hist(
    best["series"] - np.mean(best["series"]),
    bins=30,
    alpha=0.5,
    label=f"Optimized (σ={best['std']:.2f})",
    color="green",
    edgecolor="black",
)
...
for i, config in enumerate(configurations):
    if config["name"] in [
        ...
        comparison_data.append(
            [config["p2p"], config["std"], np.max(np.abs(np.diff(config["series"])))],
        )

...
if abs(dc_offset) < 1.0:
    print("  [OK] MINIMAL DC BIAS (< 1 RU)")
elif abs(dc_offset) < 3.0:
    print("  [WARN]  MODERATE DC BIAS (1-3 RU)")
else:
    print("  [ERROR] SIGNIFICANT DC BIAS (> 3 RU)")
...
axes[0, 0].axhline(
    ...
    linewidth=1,
    alpha=0.5,
    label=f"Current Mean ({np.mean(current):.2f})",
)
axes[0, 0].axhline(
    y=np.mean(hybrid),
    color="orange",
    ...
    linewidth=1,
    alpha=0.5,
    label=f"Hybrid Mean ({np.mean(hybrid):.2f})",
)
...
axes[0, 1].set_title(
    f"Hybrid - Current (DC Offset = {dc_offset:+.2f} RU)",
    fontweight="bold",
    fontsize=14,
)
...
axes[1, 0].hist(
    ...
    bins=30,
    alpha=0.5,
    label=f"Current (μ={np.mean(current):.2f})",
    color="blue",
    edgecolor="darkblue",
)
axes[1, 0].hist(
    ...
    bins=30,
    alpha=0.5,
    label=f"Hybrid (μ={np.mean(hybrid):.2f})",
    color="orange",
...
axes[1, 1].plot(
    time_points,
    current_centered,
    label=f"Current (σ={np.std(current):.2f})",
    alpha=0.7,
    linewidth=2,
)
axes[1, 1].plot(
    time_points,
    hybrid_centered,
    label=f"Hybrid (σ={np.std(hybrid):.2f})",
    alpha=0.7,
    linewidth=2,
)

...
def apply_fourier_method(transmission_spectrum, wavelengths, 
                         alpha=9000, sg_window=11, sg_poly=3,
    ...
    for time_col in time_columns:
        ...
        try:
            wavelength = apply_fourier_method(
                ...
                alpha=alpha, sg_window=sg_window, sg_poly=sg_poly,
                regression_window=regression_window, regression_poly=regression_poly
            )
...
# Baseline
baseline = get_spr_series(alpha=9000, sg_window=11, sg_poly=3, 
                         regression_window=50, regression_poly=1, 
                         apply_kalman=False)

# Optimized
optimized = get_spr_series(alpha=2000, sg_window=11, sg_poly=5, 
...
if k_on_noisy:
    print(f"  Noisy signal:   {k_on_noisy:.4f} s⁻¹  (error: {abs(k_on_noisy-k_on_true)/k_on_true*100:.1f}%)")
if k_on_filtered:
    print(f"  Kalman filtered: {k_on_filtered:.4f} s⁻¹  (error: {abs(k_on_filtered-k_on_true)/k_on_true*100:.1f}%)")

print(f"\nDissociation Rate Constant (k_off):")
print(f"  True value:     {k_off_true:.4f} s⁻¹")
if k_off_noisy:
    print(f"  Noisy signal:   {k_off_noisy:.4f} s⁻¹  (error: {abs(k_off_noisy-k_off_true)/k_off_true*100:.1f}%)")
if k_off_filtered:
    print(f"  Kalman filtered: {k_off_filtered:.4f} s⁻¹  (error: {abs(k_off_filtered-k_off_true)/k_off_true*100:.1f}%)")
...
for freq in frequencies:
    ...
    correlation = correlate(signal_filtered - np.mean(signal_filtered), 
                           signal_clean - np.mean(signal_clean), mode='same')
    ...
    print(f"  Amplitude: True={amp_true:.1f} RU, Noisy={amp_noisy:.1f} RU, Filtered={amp_filtered:.1f} RU")
...
ax1.plot(time_points, baseline, alpha=0.5, label=f'Baseline (p2p={np.ptp(baseline):.2f} RU)', 
         color='red', linewidth=2)
ax1.plot(time_points, optimized, label=f'Optimized (p2p={np.ptp(optimized):.2f} RU)', 
         color='green', linewidth=2)
ax1.fill_between(time_points, baseline - np.std(baseline), baseline + np.std(baseline),
                 alpha=0.2, color='red', label=f'±1σ Baseline ({np.std(baseline):.2f} RU)')
ax1.fill_between(time_points, optimized - np.std(optimized), optimized + np.std(optimized),
                 alpha=0.2, color='green', label=f'±1σ Optimized ({np.std(optimized):.2f} RU)')
ax1.axhline(y=0, color='black', linestyle=':', alpha=0.5)
ax1.set_xlabel('Time Point (1 Hz sampling)', fontsize=12)
ax1.set_ylabel('SPR Signal (RU)', fontsize=12)
ax1.set_title('BASELINE vs OPTIMIZED: Direct Overlay Comparison', fontsize=14, fontweight='bold')
...
ax7.plot(time_points[zoom_range], baseline[zoom_range], 'o-', alpha=0.7, 
         label='Baseline', color='red', markersize=4)
ax7.plot(time_points[zoom_range], optimized[zoom_range], 's-', alpha=0.7, 
         label='Optimized', color='green', markersize=4)
...
if k_on_filtered and k_on_true:
    print(f"     - Error reduced from {abs(k_on_noisy-k_on_true)/k_on_true*100:.1f}% to {abs(k_on_filtered-k_on_true)/k_on_true*100:.1f}%")
print("   • Dissociation rate preserved: YES")
if k_off_filtered and k_off_true:
    print(f"     - Error reduced from {abs(k_off_noisy-k_off_true)/k_off_true*100:.1f}% to {abs(k_off_filtered-k_off_true)/k_off_true*100:.1f}%")

print("\n[OK] FREQUENCY RESPONSE:")
for r in results_freq:
    if r['freq'] <= 0.1:  # Typical SPR frequencies
        print(f"   • {r['freq']:.3f} Hz: {r['amp_preservation']:.1f}% amplitude, {abs(r['phase_lag'])} sample lag")

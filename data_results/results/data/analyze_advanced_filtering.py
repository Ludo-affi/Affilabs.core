...
colors = [
    "green" if lag <= 1 else "orange" if lag <= 2 else "red"
    for lag in results_df["lag"].values[:10]
]
...
for i in range(4):
    ...
    if i < len(results_df):
        ...
        ax.plot(
            result["series"],
            label=f"{result['method']} ({result['p2p']:.1f} RU)",
            linewidth=2,
        )
...
if "Kalman" in best_low_lag["method"]:
    print("\n   Implementation:")
    print("   • Use Kalman filter in peak processing pipeline")
    print("   • Process wavelength stream, not raw transmission")
    print("   • ~10 lines of code, minimal CPU overhead")
elif "EMA" in best_low_lag["method"]:
    print("\n   Implementation:")
    print("   • Apply EMA to wavelength stream")
    print("   • self.spr_filtered = α * spr_new + (1-α) * spr_prev")
    print("   • 1 line of code per channel")
elif "Wavelet" in best_low_lag["method"]:
    print("\n   Implementation:")
    print("   • Apply wavelet denoising to wavelength stream")
    print("   • Requires pywt library")
    print("   • Slightly higher CPU cost")

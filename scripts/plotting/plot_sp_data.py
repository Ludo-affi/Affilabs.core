...
if latest_file.exists():
    ...
    for ch in ['a', 'b', 'c', 'd']:
        ...
        s_curve = baseline_s - spr_depth * 100 * np.exp(-((wavelengths - spr_wl)**2) / (2 * 20**2))
        s_pol_ref[ch] = s_curve + np.random.normal(0, 500, len(wavelengths))

        # P-pol: should be flat/low (but data shows it's actually higher)
        baseline_p = 46000
        p_curve = baseline_p - spr_depth * 50 * np.exp(-((wavelengths - spr_wl)**2) / (2 * 20**2))
        p_pol_ref[ch] = p_curve + np.random.normal(0, 500, len(wavelengths))

# Create the plot
fig, axes = plt.subplots(2, 2, figsize=(16, 12))
fig.suptitle('S and P Polarization Spectra - SPR Response',
...
for idx, (ax, ch, title) in enumerate(zip(axes.flat, channels, titles)):
    ...
    ax.plot(wavelengths, s_pol_ref[ch], 'b-', label='S-pol (SPR active - shows dip)', linewidth=2, alpha=0.8)
    ax.plot(wavelengths, p_pol_ref[ch], 'r-', label='P-pol (SPR inactive - higher)', linewidth=2, alpha=0.8)

    # Mark SPR wavelength
    spr_wl = qc['spr_wavelength']
    ax.axvline(spr_wl, color='green', linestyle='--', alpha=0.6, linewidth=2,
    ...
    if ps_ratio > 1.15:
        ...
        ax.text(0.98, 0.97, warning_text,

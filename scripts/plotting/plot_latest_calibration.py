...
s_pol_ref = {
    'a': np.array([33538.11387867, 33125.55104035, 33359.66337746, 7188.2678596, 7269.30130156, 7095.91096724]),
    'b': np.array([35264.76313637, 34835.21512245, 34543.84858924, 6752.13809013, 6769.47712357, 6699.41327419]),
    'c': np.array([30758.91452013, 30953.21785191, 30712.80684552, 5472.23433665, 5473.29591013, 5394.91640186]),
    'd': np.array([31904.17537115, 31639.17124591, 31667.16139988, 4102.09683777, 4008.85530085, 4035.21770882])
}

# P-pol reference data (after dark subtraction)
p_pol_ref = {
    'a': np.array([44180.74182715, 43630.17443703, 44021.22271958, 12733.57383714, 12982.51281712, 12792.1373072]),
    'b': np.array([45634.56670167, 44424.23139661, 45494.15591666, 12463.04952984, 12412.09400302, 12362.37697859]),
    'c': np.array([46634.74584439, 45719.35103657, 46184.7094624, 13136.26404214, 13196.06601458, 13089.90866705]),
    'd': np.array([48134.39530727, 47925.12378949, 48357.57343758, 10393.51203959, 10201.72109837, 10352.11067405])
}

# QC data
qc_results = {
    'a': {'s_max': 37101.46, 'p_max': 56367.25, 'p_s_ratio': 5.36, 'spr_wavelength': 645.15},
    'b': {'s_max': 38558.83, 'p_max': 54960.67, 'p_s_ratio': 6.30, 'spr_wavelength': 655.90},
    'c': {'s_max': 34253.08, 'p_max': 56065.59, 'p_s_ratio': 6.75, 'spr_wavelength': 644.82},
    'd': {'s_max': 35159.67, 'p_max': 53961.73, 'p_s_ratio': 6.14, 'spr_wavelength': 651.37}
}
...
# Title
fig.suptitle('SPR Calibration: S-Pol vs P-Pol Analysis\n⚠️ POLARIZER POSITIONS INVERTED (P > S)',
...
for ch, color in zip(channels, colors):
    ...
    s_spectrum = s_max * (0.85 + 0.15 * np.sin(wavelengths / 20) + 0.05 * np.random.randn(len(wavelengths)))
    s_spectrum = np.clip(s_spectrum, 5000, 45000)
    ax1.plot(wavelengths, s_spectrum, label=f'Ch {ch.upper()} (LED={led_intensity[ch]})',
             color=color, alpha=0.8, linewidth=1.5)

ax1.axhline(y=62258, color='red', linestyle='--', alpha=0.3, linewidth=1, label='Saturation threshold')
ax1.set_xlabel('Wavelength (nm)', fontsize=12)
ax1.set_ylabel('Intensity (counts)', fontsize=12)
ax1.set_title('S-Polarization (Reference - SHOULD be HIGHER, but is LOWER!)',
...
for ch, color in zip(channels, colors):
    ...
    p_spectrum = p_max * (0.88 + 0.12 * np.sin(wavelengths / 20) + 0.03 * np.random.randn(len(wavelengths)))
    ...
    ax2.plot(wavelengths, p_spectrum, label=f'Ch {ch.upper()} (LED={led_intensity[ch]})',
             color=color, alpha=0.8, linewidth=1.5)

ax2.axhline(y=62258, color='red', linestyle='--', alpha=0.3, linewidth=1, label='Saturation threshold')
ax2.set_xlabel('Wavelength (nm)', fontsize=12)
ax2.set_ylabel('Intensity (counts)', fontsize=12)
ax2.set_title('P-Polarization (Measurement - SHOULD be LOWER, but is HIGHER!)',
              fontsize=13, fontweight='bold', color='darkred')
...
bars = ax3.bar(ch_names, ps_ratios, color=bar_colors, alpha=0.7, edgecolor='black', linewidth=1.5)
ax3.axhline(y=1.0, color='green', linestyle='--', linewidth=2, label='Expected: P/S < 1.0')
ax3.axhline(y=1.15, color='orange', linestyle='--', linewidth=1.5, label='Warning threshold')
...
for i, (bar, ratio) in enumerate(zip(bars, ps_ratios)):
...
bars1 = ax4.bar(x - width/2, s_maxes, width, label='S-pol (Should be HIGH)',
                color='blue', alpha=0.6, edgecolor='black')
bars2 = ax4.bar(x + width/2, p_maxes, width, label='P-pol (Should be LOW)',
                color='red', alpha=0.6, edgecolor='black')

ax4.axhline(y=62258, color='red', linestyle='--', alpha=0.5, linewidth=1, label='Saturation')
...
print("="*80)
print(f"S-pol max signals: A={s_maxes[0]:.0f}, B={s_maxes[1]:.0f}, C={s_maxes[2]:.0f}, D={s_maxes[3]:.0f}")
print(f"P-pol max signals: A={p_maxes[0]:.0f}, B={p_maxes[1]:.0f}, C={p_maxes[2]:.0f}, D={p_maxes[3]:.0f}")
print(f"P/S ratios:        A={ps_ratios[0]:.2f}, B={ps_ratios[1]:.2f}, C={ps_ratios[2]:.2f}, D={ps_ratios[3]:.2f}")

...
class LEDOnFixedDetectorWaitSweep:
    ...
    def run_sweep(
        self,
        detector_wait_times_ms: list[int],
        led_on_ms: int = 250,
        ...
        for detector_wait_ms in detector_wait_times_ms:
            print(f"\n{'='*70}")
            print(
                f"Testing detector wait time = {detector_wait_ms} ms (LED ON = {led_on_ms} ms)",
            )
            print(f"{'='*70}")
                            ...
                            scan_times.append(
                                {
                                    ...
                                    "led": led_id,
                                    "ready_elapsed": (ready_time - cmd_start_time)
                                    * 1000,
                                    "three_scan_duration": (
                                        three_scan_end - three_scan_start
                                    )
                                    * 1000,
                                    "total_elapsed": (three_scan_end - cmd_start_time)
                                    * 1000,
                                },
                            )
            ...
            timeout = (
                led_on_ms + detector_wait_ms + 100
            ) * expected_samples / 1000 + 10  # Add 10s buffer

            while (
                samples_collected < expected_samples
                        ...
                        if ":READY" in line:
                            ...
                            if led_id in led_sequence:
                                ...
                                for scan_num in range(3):
                                    spectrum = self.read_spectrum()
                                    if spectrum is not None:
                                        mask = (self.wavelengths >= 550) & (
                                            self.wavelengths <= 800
                                        )
                                        peak_value = np.max(spectrum[mask])
                                        peak_values.append(peak_value)
                                three_scan_end = time.time()
                                ...
                                if peak_values:
                                    ...
                                    if samples_collected % 4 == 0:
                                        ...
                                        print(
                                            f"  Cycle {cycle_count}/{cycles_per_test} complete ({samples_collected}/{expected_samples} samples)",
                                        )

                        elif "CYCLE:" in line:
                            pass  # Firmware cycle counter (informational)
                        elif "BATCH_COMPLETE" in line:
                            print("✓ Batch complete signal received")
                            break
            ...
            if samples_collected < expected_samples:
                print(
                    f"⚠ Warning: Only collected {samples_collected}/{expected_samples} samples (timeout)",
                )
            else:
            ...
            if scan_times:
                ...
                print(
                    f"  → 3-scan acquisition duration: {st['three_scan_duration']:.1f}ms",
                )
            ...
            for led_name in ["A", "B", "C", "D"]:
                ...
                if len(readings) >= 3:  # Need at least 3 samples for stats
                    ...
                    print(
                        f"  LED {led_name}: {len(readings)} samples, peak={mean_val:.0f}±{stdev_val:.1f} (CV={cv:.2f}%), range={min_val:.0f}-{max_val:.0f}",
                    )
                else:
                    print(
    ...
    def print_summary(self, results: list[dict]):
        ...
        print(f"\n{'='*70}")
        print(
            f"SUMMARY - Signal Stability vs Detector Wait (LED ON = {results[0]['led_on_ms']}ms)",
        )
        print(f"{'='*70}")
        ...
        for led in ["A", "B", "C", "D"]:
            print(f"\nLED {led}:")
            print(
                f"{'Wait(ms)':<12} {'Samples':<10} {'Peak Mean':<12} {'Std Dev':<10} {'CV%':<8}",
            )
            print("-" * 60)
            for r in results:
                if led in r["led_results"]:
                    ...
                    print(
                        f"{r['detector_wait_ms']:<12} {lr['count']:<10} {lr['peak_mean']:<12.0f} "
        ...
        for r in results:
            print(
                f"{r['detector_wait_ms']:<12} {r['avg_cv']:<10.2f} {r['total_samples']:<15}",
            )
        ...
        if results:
            ...
            print("\n" + "=" * 70)
            print(
                f"✓ Optimal detector wait time: {best['detector_wait_ms']}ms (Avg CV: {best['avg_cv']:.2f}%)",
            )
...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: python scripts/led_on_fixed_detector_wait_sweep.py <COMx> <detector_wait_csv> [led_on_ms] [cycles] [led_intensities]",
        )
        print(

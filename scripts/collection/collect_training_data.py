...
class OptimalProcessor:
    ...
    def find_extremum_centroid_nm(
        ...
        half = float(window_nm) / 2.0
        window_mask = (wl_search >= (center_wl - half)) & (wl_search <= (center_wl + half))
        w_window = wl_search[window_mask]
        t_window = trans_search[window_mask]
    ...
    def compute_fwhm_nm(
        ...
        if smooth:
            ...
            try:
                if smooth_window >= 5 and smooth_window < n:
                    t = savgol_filter(t, smooth_window, min(smooth_poly, smooth_window - 1))
            except Exception:
        ...
        def _crossing_left(idx_center: int, level: float) -> Optional[float]:
        ...
        def _crossing_right(idx_center: int, level: float) -> Optional[float]:
        ...
        for frac in candidate_fracs:
            level = (t_ext - frac * (t_ext - t_base)) if mode == 'max' else (t_ext + frac * (t_base - t_ext))
    ...
    @staticmethod
    def detrend_positions_linear(timestamps: np.ndarray, positions_nm: np.ndarray) -> np.ndarray:
...
class TrainingDataCollector:
    ...
    def __init__(self, device_name: str, sensor_state: str,
        ...
        self.right_decay_gamma = float(right_decay_gamma) if right_decay_gamma is not None else None
        ...
        print(f"  Right decay gamma: {self.right_decay_gamma if self.right_decay_gamma is not None else 0}")
        if self.apply_bias_correction:
            print(f"  Bias correction: ENABLED (baseline window {self.biascorr_baseline_nm:.1f} nm)")
        else:
            print("  Bias correction: DISABLED")
        print(f"  Peak tracking mode: {self.peak_mode.upper()}")
        print(f"  Search range: [{self.search_min_nm:.1f}, {self.search_max_nm:.1f}] nm")
    ...
    def process_and_visualize(self, s_spectra: np.ndarray, p_spectra: np.ndarray,
        ...
        detrended = OptimalProcessor.detrend_positions_linear(s_timestamps, positions_nm)
        ...
        detrended_zc = OptimalProcessor.detrend_positions_linear(s_timestamps, positions_zc_nm)
        ...
        if 'zc_p2p_nm' in metrics:
            print(f"ZC P-P / Std:      {metrics['zc_p2p_nm']:.2f} / {metrics['zc_std_nm']:.2f} nm")
        ...
        if 'fwhm_mean_nm' in metrics:
            print(f"FWHM (mean ± std): {metrics['fwhm_mean_nm']:.2f} ± {metrics['fwhm_std_nm']:.2f} nm")
            print(f"pos~FWHM corr (r): {metrics['pos_fwhm_corr']:.2f}")
            print(f"P-P corrected:     {metrics.get('p2p_nm_corrected', metrics['p2p_nm']):.2f} nm")
        ...
        self._create_visualization(positions_nm, s_timestamps, transmissions, metrics, channel=channel, positions_zc_nm=positions_zc_nm)
    ...
    def _create_visualization(self, positions_nm: np.ndarray, timestamps: np.ndarray,
                             ...
                             positions_zc_nm: Optional[np.ndarray] = None):
        """Create comprehensive visualization (nm-based)."""
        fig = plt.figure(figsize=(16, 10))

        # Sensorgram (nm)
        ax1 = plt.subplot(2, 3, 1)
        ax1.plot(timestamps, positions_nm, 'b-', linewidth=1.5, alpha=0.8, label='Centroid')
        if positions_zc_nm is not None and len(positions_zc_nm) == len(positions_nm):
            ax1.plot(timestamps, positions_zc_nm, color='orange', linestyle='--', linewidth=1.2, alpha=0.9, label='Zero-crossing')
        ...
        ax3.set_ylabel('Absorbance-like' if INVERT_TRANSMISSION_VISUAL else 'Transmission', fontsize=11)
        ax3.set_title((f"Mean Absorbance-like Spectrum\nDepth: {metrics['peak_depth']:.2%}"
        ...
        vis_transmissions = (1.0 - transmissions) if INVERT_TRANSMISSION_VISUAL else transmissions
        im = ax4.imshow(
            vis_transmissions.T,
        ...
        ax4.plot(
            ...
            label=(('Minimum' if self.peak_mode == 'min' else 'Maximum') + ' (Centroid)')
        )
        if positions_zc_nm is not None and len(positions_zc_nm) == len(positions_nm):
            ax4.plot(
                timestamps,
                ...
                label=(('Minimum' if self.peak_mode == 'min' else 'Maximum') + ' (Zero-crossing)')
            )
        ax4.set_xlabel('Time (s)', fontsize=11)
        ax4.set_ylabel('Wavelength (nm)', fontsize=11)
        ax4.set_title(
            'Absorbance-like Time Series' if INVERT_TRANSMISSION_VISUAL else 'Transmission Time Series',
            fontsize=12, fontweight='bold'
        )
        ax4.legend(fontsize=9)
        plt.colorbar(im, ax=ax4, label=('Absorbance-like' if INVERT_TRANSMISSION_VISUAL else 'Transmission'))
    ...
    def save_data(self, channel: str, s_spectra: np.ndarray, p_spectra: np.ndarray,
        ...
        try:
            ...
            hardware_timing = {
                ...
                'min_integration_time_ms': timing_params.get('min_integration_time_ms', None),
                'led_rise_fall_time_ms': timing_params.get('led_rise_fall_time_ms', None),
                'afterglow_correction_enabled': optical_cal.get('afterglow_correction_enabled', None),
            }
        except Exception:
...
Examples:
  ...
  python collect_training_data.py --device "demo P4SPR 2.0" --label used_recycled --notes "After 50 assays"
    ...
    # Physics-aware centroid options
    parser.add_argument('--centroid-window-nm', type=float, default=4.0,
                       help='Total window width (nm) around minimum for centroid (default 4.0)')
    parser.add_argument('--right-decay-gamma', type=float, default=0.0,
    ...
    parser.add_argument('--biascorr-baseline-nm', type=float, default=8.0,
                       help='Baseline window width (nm) used for bias correction when enabled')
    # Peak tracking mode
    parser.add_argument('--peak-mode', type=str, choices=['min', 'max'], default='min',
    ...
    parser.add_argument('--physics-aware-centroid', action='store_true',
    ...
    search_min_nm = args.search_min_nm if args.search_min_nm is not None else SEARCH_NM_MIN
    search_max_nm = args.search_max_nm if args.search_max_nm is not None else SEARCH_NM_MAX
    ...
    collector = TrainingDataCollector(
        ...
        right_decay_gamma=(args.right_decay_gamma if args.right_decay_gamma > 0 else None),
        ...
        fwhm_baseline_percentile=(args.fwhm_baseline_percentile if args.fwhm_baseline_percentile is not None else 95.0),
    )
        ...
        if args.reprocess_latest:
            ...
            for ch in channels:
                ...
                if not s_files or not p_files:
                    print(f"⚠ No saved data found for channel {ch} in {base} — skipping")
                    continue
                s_file = s_files[-1]
                p_file = p_files[-1]
                ...
                if collector.masked_wavelengths is None:
                    ...
                    collector.masked_wavelengths = np.linspace(MIN_WAVELENGTH, MAX_WAVELENGTH, n_pix)

                # Process and visualize
                collector.process_and_visualize(
                    s_spectra, p_spectra, s_dark, p_dark,
                    s_timestamps, p_timestamps, channel=ch
                )

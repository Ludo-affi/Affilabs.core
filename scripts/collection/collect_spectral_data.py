...
class SpectralDataCollector:
    ...
    def _load_calibration_context(self):
        ...
        for ch, settings in calibration.items():
            logger.info(f"  Channel {ch}: {settings['integration_time_ms']}ms, {settings['led_intensity']}% intensity")
    ...
    def collect_channel_data(self, channel: str, mode: str, duration_seconds: int):
        ...
        if hasattr(self.ctrl, 'set_mode'):
            ...
            if result:
                logger.info(f"✅ Polarizer set to {mode.upper()}-mode for Channel {channel}")
            else:
                logger.warning(f"⚠️ Polarizer mode setting returned False - may not have moved")
            time.sleep(0.4)  # 400ms settle time for polarizer servo (ONCE per channel)
        else:
            logger.error(f"❌ Controller does not support set_mode - cannot set polarization!")
            raise RuntimeError(f"Controller {type(self.ctrl).__name__} does not have set_mode method")
        ...
        while (time.time() - start_time) < duration_seconds:
            ...
            if measurement_count % 10 == 0:
                ...
                print(f"  • {elapsed:.1f}s / {duration_seconds}s ({measurement_count} spectra @ {actual_rate:.1f} Hz)")

        print(f"  ✓ Collected {measurement_count} spectra from Channel {channel}")

    def collect_all_channels(self, mode: str, duration_seconds: int, channels: list | None = None):
        """
        ...
        print(f"\n{'='*60}")
        print(f"  {mode}-Signal Collection - Channel{'s' if len(channels) > 1 else ''} {', '.join(channels)}")
        print(f"{'='*60}")

...
class DetectorWaitSweep:
    ...
    def connect_hardware(self) -> bool:
        ...
        try:
            # Connect to firmware
            self.firmware = serial.Serial(self.firmware_port, baudrate=115200, timeout=2)
    ...
    def read_spectrum(self) -> Optional[np.ndarray]:
        ...
        try:
            ...
            elif hasattr(self.spectrometer, 'read_intensity'):
    ...
    def run_sweep(self, led_on_times_ms: List[int], cycles_per_test: int = 20, 
                  led_intensities: tuple = (50, 100, 150, 200), detector_wait_ms: int = 5) -> List[Dict]:
        """
        ...
        for led_on_ms in led_on_times_ms:
            ...
            print(f"Status: {batch_line[:20]}, ACK: {ack_line[:10] if ack_line else 'OK'}")
            ...
            for sample in range(total_samples):
                ...
                if spectrum is not None and len(spectrum) > 0:
                    ...
                    readings_per_led[led_name].append({'peak': peak_val, 'mean': mean_val})
                    samples_collected += 1
                    
                    if samples_collected % 20 == 0:
                        current_cycle = samples_collected // 4
                        print(f"  Cycles: {current_cycle}/{cycles_per_test}, LED {led_name}: {peak_val:.0f} counts")
            ...
            for led_name in ['A', 'B', 'C', 'D']:
                ...
                if len(readings) >= 3:
                    ...
                    print(f"  LED {led_name}: {len(readings)} samples, peak={peak_mean:.0f}±{peak_stdev:.1f} (CV={peak_cv:.2f}%), range={min(peaks):.0f}-{max(peaks):.0f}")
                else:
    ...
    def print_summary(self, results: List[Dict]):
        ...
        for led in ['A', 'B', 'C', 'D']:
            print(f"\nLED {led}:")
            print(f"{'LED_ON(ms)':<12} {'Samples':<10} {'Peak Mean':<12} {'Std Dev':<10} {'CV%':<8}")
            print("-" * 60)
            for r in results:
                if led in r['led_results']:
                    lr = r['led_results'][led]
                    print(f"{r['led_on_ms']:<12} {lr['count']:<10} {lr['peak_mean']:<12.0f} "
        ...
        if results:
            ...
            print("\n" + "="*70)
            print(f"✓ Optimal LED ON time: {best['led_on_ms']}ms (Avg CV: {best['avg_cv']:.2f}%)")
            print("="*70)


def main():
    if len(sys.argv) < 3:
        print("Usage: python scripts/detector_wait_sweep_hardware.py <COMx> <led_on_csv> [cycles] [detector_wait_ms]")

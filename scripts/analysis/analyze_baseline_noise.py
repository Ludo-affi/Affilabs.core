...
print(f"{'Rank':<5} {'Strategy':<20} {'Std (nm)':<12} {'P-P (nm)':<12} {'Improvement':<15}")
print("-" * 80)
for i, (name, stats) in enumerate(ranked[:10], 1):
    print(f"{i:<5} {name:<20} {stats['std']:.4f}      {stats['pp']:.4f}      {stats['improvement']:.1f}%")
...
for idx in top_indices:
    if fft_freq[idx] > 0:  # Skip DC component
        print(f"  {fft_freq[idx]:.4f} Hz (period: {1/fft_freq[idx]:.2f} s), Power: {fft_power[idx]:.2e}")
...
if 'MovAvg' in best_name:
    ...
    print(f"Implementation: Use Savitzky-Golay filter")
    ...
    print(f"Implementation: Use Exponential Moving Average")
...
print(f"  Current alpha: 9000")
print(f"  Current window: 165 points")
print()
...
print()
print("Since baseline wavelengths have std ~{:.4f} nm,".format(orig_std))
...
print(f"  2. Then apply Fourier DST/IDCT with alpha=9000")
print(f"  3. This cascaded filtering will improve zero-crossing clarity")
...
for i, (name, stats) in enumerate(ranked[:3]):
    ...
    ax.plot(time_plot, data, colors[i], alpha=0.8, label=f'{name} (σ={stats["std"]:.4f})', linewidth=1.5)

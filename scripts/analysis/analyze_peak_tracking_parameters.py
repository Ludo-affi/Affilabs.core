...
def load_calibration_data():
    ...
    with open("LED-Counts relationship/led_calibration_spr_S_polarization.json") as f:
        data_S = json.load(f)

    with open("LED-Counts relationship/led_calibration_spr_P_polarization.json") as f:
        data_P = json.load(f)

    with open("LED-Counts relationship/dark_signal_calibration.json") as f:
        ...
        for idx, m in enumerate(measurements):
            ...
            quality = (
                ...
                if counts > 10000
                    ...
                    "Good" if counts > 5000 else ("Fair" if counts > 1000 else "Poor")
                )
            )
            ...
            print(
                f"  [{idx+1:2d}/{len(measurements)}] I={intensity:3d}, T={int_time:4.1f}ms, Counts={counts:6.0f}, SNR={snr:5.1f}, Quality={quality}",
            )
...
def analyze_parameter_impact(results):
    ...
    for pol in ["S", "P"]:
        ...
        for led in ["A", "B", "C", "D"]:
            ...
            if trackable_meas:
                ...
                print("    Optimal ranges (for trackable signals):")
                print(
                    f"      Intensity: {min(intensities)}-{max(intensities)} (avg {np.mean(intensities):.0f})",
                )
                print(
                    f"      Integration: {min(times):.1f}-{max(times):.1f}ms (avg {np.mean(times):.1f}ms)",
                )
                print(
                    f"      Counts: {min(counts):.0f}-{max(counts):.0f} (avg {np.mean(counts):.0f})",
                )
    ...
    for pol in ["S", "P"]:
        ...
        for i in sorted(intensity_groups.keys()):
            ...
            print(
                f"    I={i:3d}-{i+49:3d}: Trackable={trackable_pct:3.0f}%, Avg Counts={avg_counts:6.0f}, Avg SNR={avg_snr:5.1f}",
            )
        ...
        for t in sorted(time_groups.keys()):
            ...
            avg_snr = np.mean(g["snr"])
            print(
                f"    T={t:4.1f}ms: Trackable={trackable_pct:3.0f}%, Avg Counts={avg_counts:6.0f}, Avg SNR={avg_snr:5.1f}",
            )
    ...
    if pol_ratio > 1.2:
        ...
        print("    → Minimal polarization dependence")

    # Find most important parameter
    print("\n  Most Impactful Parameters (for peak tracking):")
    print(
        "    1. Integration Time: Longer times (15-25ms) generally provide better SNR",
    )
    print("    2. LED Intensity: Higher intensities increase counts but may saturate")
    print(
        "    3. Signal Level: Need >5k counts for reliable tracking, >10k for high precision",
    )
    print(
        f"    4. Polarization: {'S-pol preferred' if pol_ratio > 1.1 else ('P-pol preferred' if pol_ratio < 0.9 else 'Either polarization OK')}",
    )
...
def create_analysis_plots(results):
    ...
    for col_idx, led_name in enumerate(["A", "B", "C", "D"]):
        ...
        if measurements_s:
            ...
            ax_s.set_title(
                f"LED {led_name} - S-pol\nSignal Quality vs Counts",
                fontsize=12,
                fontweight="bold",
            )

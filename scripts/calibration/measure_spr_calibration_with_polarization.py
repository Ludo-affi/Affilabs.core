...
def measure_spr_calibration_with_polarization():
    ...
    input(
        "STEP 1: Verify polarizer is at S-POLARIZATION (you should have heard servo move), then press ENTER...",
    )
    ...
    for led_name in ["A", "B", "C", "D"]:
        ...
        for i, point in enumerate(led_plan, 1):
            ...
            try:
                ...
                print(
                    f"{status} [{measurement_count:3d}/{total_per_pol}] "
                ...
                print(
                    f"✗ [{measurement_count:3d}/{total_per_pol}] "
    ...
    input(
        "STEP 2: Verify polarizer is at P-POLARIZATION (you should have heard servo move), then press ENTER...",
    )
    ...
    for i, time_ms in enumerate(integration_times, 1):
        ...
        print(
            f"{i:<4} {time_ms:<12.1f} {avg_counts:<15.1f} {counts_per_ms:<12.2f} "
            f"(+/- {std_counts:.1f})",
        )
    ...
    for led_name in ["A", "B", "C", "D"]:
        ...
        if measurements_S and measurements_P:
            ...
            if avg_P < avg_S:
                ...
                if extinction_pct > 5:
                    print("    ⚠️  High extinction detected - may include SPR effects!")
                    print("        (Expected if measuring through sample)")
            else:
                print("  Note: P counts >= S counts (unexpected for SPR)")

...
def load_calibration_data(detector_serial=None):
    ...
    try:
        with open(
            ...
            data_S = json.load(f)
        print("✓ Loaded S-polarization data")
        ...
        if (
            ...
            print(
                f"⚠️  WARNING: Data is for detector {data_S['detector_serial']}, current is {detector_serial}",
            )
    ...
    try:
        with open(
            ...
            data_P = json.load(f)
        print("✓ Loaded P-polarization data")
        # Verify detector match
        if (
            ...
            print(
                f"⚠️  WARNING: Data is for detector {data_P['detector_serial']}, current is {detector_serial}",
            )
    except FileNotFoundError:
    ...
    try:
        ...
        print("✓ Loaded dark current data")
...
def create_calibration_plots(processed_data, models_S, models_P):
    ...
    for idx, led_name in enumerate(["A", "B", "C", "D"]):
        ...
        ax.set_title(
            f"LED {led_name} - S vs P Calibration\n"

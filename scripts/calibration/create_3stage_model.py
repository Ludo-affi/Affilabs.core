...
def create_3stage_model(input_file: Path = None) -> None:
    ...
    detector_serial = calibration_data.get("calibration_metadata", {}).get(
        "detector_serial",
        "FLMT09116",
    )
    timestamp_str = calibration_data.get("calibration_metadata", {}).get(
        "timestamp",
        "",
    )
    ...
    for time_ms, counts in sorted(
        model_data["dark_counts_per_time"].items(),

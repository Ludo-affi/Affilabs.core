...
def load_calibration_models_ML(detector_serial=None):
    ...
    calibration_file = f'LED-Counts relationship/led_calibration_spr_processed_{detector_serial}.json'
    fallback_file = 'LED-Counts relationship/led_calibration_spr_processed.json'

    try:
        with open(calibration_file, 'r') as f:
            data = json.load(f)
        print(f"✓ Loaded detector-specific calibration")
        print(f"  File: {calibration_file}")
    except FileNotFoundError:
        try:
            with open(fallback_file, 'r') as f:
                data = json.load(f)
            print(f"⚠️  Using generic calibration (detector-specific not found)")
            print(f"  File: {fallback_file}")
        except FileNotFoundError:
            raise FileNotFoundError(f"Calibration not found. Run: python process_spr_calibration.py")

    # Verify detector match
    if 'detector_serial' in data:
        if data['detector_serial'] != detector_serial:
            print(f"⚠️  WARNING: Calibration is for {data['detector_serial']}, current detector is {detector_serial}")
        else:
    ...
    for led_name in ['A', 'B', 'C', 'D']:
        ...
        print(f"  ✓ LED {led_name}: S-pol ({len(measurements_S)} points), P-pol ({len(measurements_P)} points)")
...
def find_optimal_settings_ML(models, led_name, target_counts, polarization='S',
                         intensity_range=(40, 255), time_range=(5.0, 25.0)):
    ...
    x0 = [(intensity_range[0] + intensity_range[1]) / 2,
    ...
    predicted = predict_counts_ML({led_name: model}, led_name, optimal_intensity, optimal_time)
    error_pct = ((predicted - target_counts) / target_counts * 100) if target_counts > 0 else 0
...
def demonstrate_usage():
    ...
    print(f"{'LED':<6} {'Intensity':<12} {'Time (ms)':<12} {'Predicted':<12} {'Error%':<10} {'Status':<10}")
    print("-"*80)
    ...
    for led_name in ['A', 'B', 'C', 'D']:
        ...
        print(f"{led_name:<6} {result['intensity']:<12} {result['time']:<12.1f} "
    ...
    for intensity in range(50, 200, 30):
        for time_ms in [10.0, 15.0, 20.0, 25.0]:
            ...
            if target_min <= predicted <= target_max:
                ...
                print(f"{intensity:<12} {time_ms:<12.1f} {predicted:<12.0f} {quality['quality']:<12}")
    ...
    for led_name in ['A', 'B', 'C', 'D']:
        counts = predict_counts_ML(current_models, led_name, intensity, time_ms, current_polarization)
        quality = validate_measurement_quality(counts, time_ms)

        print(f"{led_name:<6} {counts:<18.0f} {quality['snr']:<10.1f} {quality['quality']:<12}")

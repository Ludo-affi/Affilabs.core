...
class DataWindow(QWidget):
    ...
    # Lightweight UI helpers to avoid reaching through Ui types from callers
    def set_inject_enabled(self: Self, enabled: bool) -> None:
        if isinstance(self.ui, Ui_Sensorgram):
            self.ui.inject_box.setEnabled(enabled)

    def set_flow_rate_now(self: Self, text: str) -> None:
        if isinstance(self.ui, Ui_Sensorgram):
            self.ui.flow_rate_now.setText(text)
    ...
    def setup(self: Self) -> None:
        ...
        self.full_segment_view.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        layout_fs.addWidget(self.full_segment_view, 1)

        self.SOI_view = SegmentGraph("Cycle of Interest", self.unit)
        layout_soi = self._ensure_clean_container(self.ui.SOI)
        self.SOI_view.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        layout_soi.addWidget(self.SOI_view, 1)

    def _ensure_clean_container(self: Self, container_widget):
        """Ensure the frame has a clean, marginless layout without spacer items that push content right.
        Reuse an existing layout by removing all its items, otherwise create a new QVBoxLayout.
        """

...
def find_optimal_intensity(
    ...
    for attempt in range(max_attempts):
        ...
        if spectrum is None:
            print(f"   Failed to read spectrum, trying again...")
            continue
...
def run_hardware_p2p_test(
    ...
    """
    print(f"\n{'='*80}")
    print(f"HARDWARE P2P VARIATION TEST - Averaging Method Comparison")
    print(f"{'='*80}")
    print(f"Port: {port}")
    ...
    if not hardware_mgr.ctrl:
        raise RuntimeError(f"Failed to connect to controller")
    ...
    if test_spectrum is not None:
        ...
        if max_test >= 65000:
            ...
            print(f"   ⚠️  Reduce LED intensity or integration time")
        else:

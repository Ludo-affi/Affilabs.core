...
def test_intelligence_bar_states():
    ...
    print("\n2️⃣ Testing WARNING state (simulated low SNR):")
    ...
    if issues:
        ...
        print(f"   UI Display: '⚠ Warning' → '{issues[0].title}'")

    # Test 3: Error state - simulate LED degradation
    print("\n3️⃣ Testing ERROR state (simulated LED degradation):")
    ...
    if issues:
        print(f"   Most Critical: {issues[0].title}")
        print(f"   UI Display: '❌ Error' → '{issues[0].title}'")

    # Test 4: Show all active issues
    print("\n4️⃣ All Active Issues:")
    ...
    print("\n" + "=" * 70)
    print("✅ Intelligence Bar backend connection verified!")

...
def main():
    ...
    if device_manager.current_device_serial is None:
        logger.warning("⚠️  No device configured in device_manager")
        ...
        if temp_hw.usb and hasattr(temp_hw.usb, "serial_number"):
            ...
            try:
                ...
                logger.info(f"✅ Device configured: {serial}")
            ...
            logger.error("❌ Could not detect device serial from spectrometer")
    ...
    if device_dir is None:
        logger.error("❌ Device directory not available")
        return 1
    ...
    try:
        ...
        if ctrl is None or usb is None:
            logger.error("❌ Hardware not found")
        ...
        if success:
            ...
            try:
                ...
                if len(channels) == 4:
                    logger.info("✅ SUCCESS: All 4 channels calibrated!")
            ...
            logger.error("")
            logger.error("=" * 80)
            logger.error("❌ OPTICAL CALIBRATION FAILED")
            logger.error("=" * 80)
            logger.error("")
        ...
        logger.exception(f"❌ Calibration failed with exception: {e}")
        return 1

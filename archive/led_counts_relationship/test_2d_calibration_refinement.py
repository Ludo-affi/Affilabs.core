...
def find_optimal_settings_2d(
    model,
    target_counts,
    time_fixed=None,
    intensity_fixed=None,
):
    ...
    if time_fixed is not None:
        ...
        def objective(intensity):
            return abs(
                predict_with_2d_model(model, intensity, time_fixed) - target_counts,
            )

        result = minimize_scalar(objective, bounds=(40, 255), method="bounded")
        return (int(round(result.x)), time_fixed)

    if intensity_fixed is not None:
        # Optimize time at fixed intensity
        def objective(time):
            return abs(
                predict_with_2d_model(model, intensity_fixed, time) - target_counts,
            )
...
def run_2d_calibration_refinement(target_counts=52428):
    ...
    try:
        for led_name, led_info in leds.items():
            ...
            for intensity_str, counts in led_calib["characterization"][
                "intensity_data"
            ].items():
            ...
            for idx, (intensity, time_ms) in enumerate(sampling_points):
                ...
                if result["success"]:
                    ...
                    print("  FAILED - skipping point")

            # Build 2D model
            print("\n" + "-" * 80)
            print(
                f"Building 2D interpolation model from {len(all_measurements)} points...",
            )
            print("-" * 80)
            ...
            predicted_35ms = predict_with_2d_model(
                model_2d,
                optimal_intensity_35ms,
                fixed_time,
            )
            ...
            print(
                f"  Predicted counts: {predicted_35ms:.0f} ({predicted_35ms/65535*100:.1f}%)",
            )
            ...
            print(
                f"  Predicted counts: {predicted_255:.0f} ({predicted_255/65535*100:.1f}%)",
            )
            ...
            predicted_optimal = predict_with_2d_model(
                model_2d,
                optimal_intensity,
                optimal_time,
            )
            ...
            print(
                f"  Predicted counts: {predicted_optimal:.0f} ({predicted_optimal/65535*100:.1f}%)",
            )
    ...
    for led_name, data in refined_calibration.items():
        ...
        print(
            f"{led_name:<6} {num_measurements:<15} "

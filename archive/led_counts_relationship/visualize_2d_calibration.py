...
def plot_2d_surface(led_name, measurements, target_counts=52428):
    ...
    # Build 2D model
    points = np.array(list(zip(intensities, times, strict=False)))
    ...
    target_contour = ax2.contour(
        I_mesh,
        T_mesh,
        C_mesh,
        levels=[target_counts],
        colors="red",
        linewidths=3,
    )
...
def analyze_nonlinearity(led_name, measurements):
    ...
    fig.suptitle(
        f"LED {led_name} - Non-linearity Analysis",
        fontsize=14,
        fontweight="bold",
    )
    ...
    for intensity in sorted(intensity_groups.keys()):
        ...
        counts = [p[1] for p in points]
        if len(times) > 1:
            rates = [c / t for t, c in zip(times, counts, strict=False)]

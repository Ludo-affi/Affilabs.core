...
def load_calibration_data(detector_serial=None):
    ...
    try:
        ...
        if Path(grid_s_file).exists():
            with open(grid_s_file, encoding="utf-8") as f:
                raw_data_S = json.load(f)
            print("[OK] Loaded S-polarization data (2d_grid)")
            # Convert format
            data_S = convert_adaptive_to_old_format(raw_data_S)
        # Try adaptive format
        elif Path(adaptive_s_file).exists():
            with open(adaptive_s_file, encoding="utf-8") as f:
                raw_data_S = json.load(f)
            print("[OK] Loaded S-polarization data (adaptive)")
            # Convert adaptive format to old format
            data_S = convert_adaptive_to_old_format(raw_data_S)
        else:
            # Fall back to old format
            with open(
                "LED-Counts relationship/led_calibration_spr_S_polarization.json",
            ) as f:
                data_S = json.load(f)
            print("[OK] Loaded S-polarization data")
        # Verify detector match
        if (
            data_S.get("detector_serial")
            and data_S["detector_serial"] != detector_serial
        ):
            print(
                f"[WARN] Data is for detector {data_S['detector_serial']}, current is {detector_serial}",
            )
    ...
    try:
        ...
        if Path(grid_p_file).exists():
            with open(grid_p_file, encoding="utf-8") as f:
                raw_data_P = json.load(f)
            print("[OK] Loaded P-polarization data (2d_grid)")
            # Convert format
            data_P = convert_adaptive_to_old_format(raw_data_P)
        # Try adaptive format
        elif Path(adaptive_p_file).exists():
            with open(adaptive_p_file, encoding="utf-8") as f:
                raw_data_P = json.load(f)
            print("[OK] Loaded P-polarization data (adaptive)")
            # Convert adaptive format to old format
            data_P = convert_adaptive_to_old_format(raw_data_P)
        else:
            # Fall back to old format
            with open(
                "LED-Counts relationship/led_calibration_spr_P_polarization.json",
            ) as f:
                data_P = json.load(f)
            print("[OK] Loaded P-polarization data")
        # Verify detector match
        if (
            data_P.get("detector_serial")
            and data_P["detector_serial"] != detector_serial
        ):
            print(
                f"[WARN] Data is for detector {data_P['detector_serial']}, current is {detector_serial}",
            )
    except FileNotFoundError:
    ...
    try:
        ...
        if Path(dark_file_new).exists():
            with open(dark_file_new) as f:
                data_dark = json.load(f)
            print("[OK] Loaded dark current data (new format)")
            # New format uses 'model' key
            dark_rate = data_dark["model"]["dark_rate"]
            dark_offset = data_dark["model"]["dark_offset"]
        else:
            # Fall back to legacy format
            with open(dark_file_legacy) as f:
                data_dark = json.load(f)
            print("[OK] Loaded dark current data (legacy format)")
...
def convert_adaptive_to_old_format(adaptive_data):
    ...
    for led_wavelength, measurements_dict in adaptive_data.get("leds", {}).items():
        ...
        if led_letter:
            old_format[led_letter]["measurements"] = measurements_dict.get(
                "measurements",
                [],
            )
...
def validate_intensity_time_matching(data_S, data_P):
    ...
    for led_name in ["A", "B", "C", "D"]:
        ...
        led_valid = (
            len(missing_in_P) == 0
            and len(missing_in_S) == 0
            and len(duplicates_S) == 0
            and len(duplicates_P) == 0
        )
        ...
        if led_valid:
            print("  [OK] Perfect S/P matching")

    print("\n" + "-" * 80)
    if report["overall_valid"]:
        print(
            "[OK] ALL LEDS: Perfect S/P matching - ready for 2D RBF model construction",
        )
    else:
...
def process_calibration():
    ...
    validation_file = (
        f"LED-Counts relationship/spr_processing_validation_{detector_serial}.json"
    )
    ...
    for led_name in ["A", "B", "C", "D"]:
        ...
        measurements_S = apply_dark_correction(
            measurements_S_raw,
            dark_rate,
            dark_offset,
        )
        ...
        if len(measurements_S) > 0:
            ...
            print(
                f"    Raw counts range: {min([m['counts_raw'] for m in measurements_S]):.0f} - "
                f"{max([m['counts_raw'] for m in measurements_S]):.0f}",
            )
            print(
                f"    Corrected counts range: {min(counts_S):.0f} - {max(counts_S):.0f}",
            )
        else:
            counts_S = []
        ...
        measurements_P = apply_dark_correction(
            measurements_P_raw,
            dark_rate,
            dark_offset,
        )
        ...
        if len(measurements_P) > 0:
            ...
            print(
                f"    Raw counts range: {min([m['counts_raw'] for m in measurements_P]):.0f} - "
                f"{max([m['counts_raw'] for m in measurements_P]):.0f}",
            )
            print(
                f"    Corrected counts range: {min(counts_P):.0f} - {max(counts_P):.0f}",
            )
        else:
            counts_P = []
        ...
        if len(counts_S) > 0 and len(counts_P) > 0:
            ...
            print(
                "    [WARN] Cannot calculate polarization ratio - missing measurements",
            )
    ...
    for led_name in ["A", "B", "C", "D"]:
        ...
        if len(measurements_S) < 4:
            print(
                f"  [WARN] Skipping S-pol: only {len(measurements_S)} points (need >=4)",
            )
            ...
            print("  S-pol model:")
            print(
                f"    a={model_S['a']:.4f} (sens. slope), b={model_S['b']:.2f} (sens. intercept)",
            )
            print(
                f"    c={model_S['c']:.4f} (offset slope), d={model_S['d']:.2f} (offset intercept)",
            )
            print(
                f"    R²={model_S['r_squared']:.6f}, RMSE={model_S['rmse']:.1f} counts",
            )
        ...
        if len(measurements_P) < 4:
            print(
                f"  [WARN] Skipping P-pol: only {len(measurements_P)} points (need >=4)",
            )
            ...
            print("  P-pol model:")
            print(
                f"    a={model_P['a']:.4f} (sens. slope), b={model_P['b']:.2f} (sens. intercept)",
            )
            print(
                f"    c={model_P['c']:.4f} (offset slope), d={model_P['d']:.2f} (offset intercept)",
            )
            print(
                f"    R²={model_P['r_squared']:.6f}, RMSE={model_P['rmse']:.1f} counts",
            )
        ...
        validation_results[led_name] = {
            "S": val_S
            if val_S
            else {"mean_error": None, "std_error": None, "max_error": None},
            "P": val_P
            if val_P
            else {"mean_error": None, "std_error": None, "max_error": None},
    ...
    for led_name in ["A", "B", "C", "D"]:
        ...
        print(
            f"{led_name:<6} {avg_S:<15.0f} {avg_P:<15.0f} {ratio:<12.3f} {extinction:<12.1f}%",
        )
    ...
    if avg_ratio > 1.2 or avg_ratio < 0.8:
        ...
        print(
            "   This may include SPR effects if sample was present during calibration",
        )
...
def create_calibration_plots(processed_data, models_S, models_P):
    ...
    for idx, led_name in enumerate(["A", "B", "C", "D"]):
        ...
        if (
            ...
            or len(warning_S) > 0
            or len(warning_P) > 0
            or len(saturating_S) > 0
            or len(saturating_P) > 0
        ):
            all_points = []
            if len(safe_S) > 0:
                all_points.extend(safe_S.tolist())
            if len(safe_P) > 0:
                all_points.extend(safe_P.tolist())
            if len(warning_S) > 0:
                all_points.extend(warning_S.tolist())
            if len(warning_P) > 0:
                all_points.extend(warning_P.tolist())
            if len(saturating_S) > 0:
                all_points.extend(saturating_S.tolist())
            if len(saturating_P) > 0:
                all_points.extend(saturating_P.tolist())
            ...
            if all_points:
                ...
                if len(saturating_S) > 0 or len(saturating_P) > 0:
                    sat_combined = np.vstack(
                        [s for s in [saturating_S, saturating_P] if len(s) > 0],
                    )
                    ax.scatter(
                        sat_combined[:, 0],
                        sat_combined[:, 1],
        ...
        title_color = (
            "green" if safety_pct > 80 else ("orange" if safety_pct > 60 else "red")
        )
        ax.set_title(
            f"LED {led_name} - Safety: {safety_pct:.0f}%\n"
    ...
    sat_all = (
        np.array(all_saturating_points) if all_saturating_points else np.empty((0, 3))
    )
    ...
    if len(safe_all) > 0 or len(warn_all) > 0 or len(sat_all) > 0:
        all_global = []
        if len(safe_all) > 0:
            all_global.extend(safe_all.tolist())
        if len(warn_all) > 0:
            all_global.extend(warn_all.tolist())
        if len(sat_all) > 0:
            all_global.extend(sat_all.tolist())
    ...
    ax_global.set_title(
        f"GLOBAL SYSTEM VIEW - All LEDs & Polarizations\n"
    ...
    output_plot = (
        f"LED-Counts relationship/spr_calibration_comparison_{detector_serial}.png"
    )
    ...
    print(
        f"     Saturation threshold: {saturation_threshold} counts (91% of {detector_max})",
    )
    plt.close()
...
def analyze_system_performance(models_S, models_P):
    ...
    for led_letter in ["A", "B", "C", "D"]:
        ...
        for pol, model in [("S", model_S), ("P", model_P)]:
            ...
            for t_ms in np.arange(5, 201, 0.01):
                ...
                if predicted >= target_80:
                    print(
                        f"      {pol}-pol: {t_ms:.2f}ms (predicted: {predicted:.0f} counts)",
                    )
                    found = True
                    break
            ...
            if not found:
                ...
                print(
                    f"      {pol}-pol: Cannot reach 80% (max: {max_predicted:.0f} @ 200ms)",
                )
    ...
    for led_letter in ["A", "B", "C", "D"]:
        ...
        for pol, model in [("S", model_S), ("P", model_P)]:
            ...
            print(f"      {pol}-pol:", end="")
            for t_ms, sens in zip(test_times, sensitivities, strict=False):
            ...
            if len(sensitivities) > 1:
                ...
                if variation > 10:
                    print(
                        f"        [WARN] Sensitivity varies by {variation:.1f}% - non-linear detector response!",
                    )
    ...
    for led_name in ["A", "B", "C", "D"]:
        ...
        for pol, models in [("S", models_S), ("P", models_P)]:
            ...
            if len(factors) > 0:
                ...
                print(
                    f"  {pol}-pol: Correction range {min_factor:.3f}-{max_factor:.3f} ({range_pct:.1f}% variation)",
                )
            else:
...
if __name__ == "__main__":
    ...
    if result:
        ...
        correction_factors = calculate_sensitivity_correction_factors(
            models_S,
            models_P,
        )
        ...
        print(
            "     This corrects for non-linear detector response across integration times",
        )

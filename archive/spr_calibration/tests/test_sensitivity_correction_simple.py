...
def load_correction_factors(detector_serial):
    ...
    correction_file = (
        f"LED-Counts relationship/sensitivity_correction_factors_{detector_serial}.json"
    )
...
def test_1_no_correction(
    ...
    for led_name, channel in led_mapping.items():
        ...
        print(
            f"  {led_name}: I=255, T={settings['time_ms']:.2f}ms → {settings['counts']:.0f} counts ({pct:.1f}%)",
        )
    ...
    results = {led: [] for led in led_mapping}
    ...
    for led_name in led_mapping:
        ...
        print(
            f"    {led_name}: {count} measurements, avg={avg_counts:.0f} counts ({pct:.1f}%, CV={cv:.2f}%)",
        )

    return results, cycle_times, total_time


def test_2_with_correction(
    controller,
    spec_interface,
    opt_controller,
    calib_data,
    ...
    for led_name in led_mapping:
        ...
        print(
            f"            Raw={avg_raw:.0f} → Corrected={avg_corr:.0f} ({pct_corr:.1f}%, CV={cv:.2f}%)",
        )
...
def run_experiment():
    ...
    detector_serial = (
        spec_hw.serial_number if hasattr(spec_hw, "serial_number") else "FLMT09116"
    )

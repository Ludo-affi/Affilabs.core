...
INTEGRATION_TIMES = [10.00, 20.00, 30.00, 40.00, 50.00, 60.00, 80.00, 100.00, 120.00, 150.00]
...
def load_calibration_models():
    ...
    cal_file = Path('LED-Counts relationship/led_calibration_spr_processed_FLMT09116.json')
...
def measure_counts_at_time(spectrometer, opt_controller, controller_hw, led_channel, 
                           intensity, time_ms, servo_pos, wavelengths, roi_mask, scans=3):
        ...
        for led in LEDS:
            ...
            for time_ms in INTEGRATION_TIMES:
                peak, spectrum = measure_counts_at_time(
                    spectrometer, opt_controller, controller_hw,
                    led_channel, intensity, time_ms, servo_pos,
                    wavelengths, roi_mask, SCANS_TO_AVERAGE
                )
                ...
                error = ((peak_corrected - predicted) / predicted) * 100 if predicted > 0 else 0
                ...
                print(f"    {time_ms:6.2f}ms: Measured={peak_corrected:6.0f}, "
...
def plot_validation_results(results, models):
    ...
    fig.suptitle('Bilinear Model Validation: Fixed Intensity, Variable Time', 
    ...
    for pol_idx, pol in enumerate(POLARIZATIONS):
        for led_idx, led in enumerate(LEDS):
            ...
            ax.plot(times, measured, 'o-', color='blue', linewidth=2, 
                   markersize=6, label='Measured')
            ax.plot(times, predicted, 's--', color='red', linewidth=2, 
            ...
            r_squared = 1 - (np.sum((measured - fit_line(times))**2) / 
                            np.sum((measured - np.mean(measured))**2))
            
            ax.plot(times, fit_line(times), ':', color='green', linewidth=1.5,
                   alpha=0.7, label=f'Linear Fit (R²={r_squared:.4f})')
            
            # Labels and styling
            ax.set_title(f'{pol}-pol: {led_key} (I={intensity})\n'
            ...
            # Show linearity
            ax.text(0.98, 0.05, f'Linearity: R²={r_squared:.5f}',
    ...
    output_file = Path('LED-Counts relationship/validation_fixed_intensity_FLMT09116.png')
...
def print_summary(results):
    ...
    print("="*80)
    print(f"{'LED':<10} {'Pol':<4} {'Intensity':<10} {'RMSE':<10} {'Max Err':<10} {'Mean Err':<10} {'R²':<10}")
    print("-"*80)
    ...
    for pol in POLARIZATIONS:
        for led in LEDS:
            ...
            print(f"{led_key:<10} {pol:<4} {intensity:<10} {rmse:<10.0f} "
...
def main():
    ...
    results_file = Path(f'LED-Counts relationship/validation_fixed_intensity_{detector_serial}.json')

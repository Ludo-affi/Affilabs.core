...
def load_optimal_settings(calib_file: Path):
    ...
    print("="*80)
    print(f"Target counts: {TARGET_COUNTS:,} ({TARGET_COUNTS/DETECTOR_MAX*100:.1f}% detector)")
    ...
    for pol in ['S', 'P']:
        ...
        for led in LEDS:
            ...
            optimal_settings[pol]['leds'][f'LED_{led}'] = {
                ...
                'predicted_counts': float(predicted_counts),  # Convert to native Python float
                'valid': is_valid
            }
            
            print(f"  LED_{led}: I={intensity:5.1f} → {predicted_counts:6.0f} counts {status}")
...
def measure_spectrum(spectrometer, opt_controller, controller_hw, led_name, intensity, time_ms, polarization, scans=3):
...
def measure_all_spectra(optimal_settings):
    ...
    for led in LEDS:
        ...
        wavelengths, spectrum, peak = measure_spectrum(
            spectrometer, opt_controller, controller_hw, led_key, intensity, time_s, 'S', SCANS_TO_AVERAGE
        )
    ...
    for led in LEDS:
        ...
        wavelengths, spectrum, peak = measure_spectrum(
            spectrometer, opt_controller, controller_hw, led_key, intensity, time_p, 'P', SCANS_TO_AVERAGE
        )
...
def plot_validation_results(results, optimal_settings):
    ...
    fig.suptitle('CALIBRATION VALIDATION: Transmission Spectra at Optimal Settings\n'
    ...
    for idx, led in enumerate(LEDS):
        ...
        if led_key in results['S']:
            ...
            ax_s.axhline(TARGET_COUNTS, color='blue', linestyle='--', 
                        linewidth=1, alpha=0.7, label=f'Target ({TARGET_COUNTS:,})')
            ax_s.axhline(peak_s, color='red', linestyle=':', 
                        linewidth=1, alpha=0.7, label=f'Peak ({peak_s:.0f})')
            
            error_pct = ((peak_s - TARGET_COUNTS) / TARGET_COUNTS) * 100
            ax_s.set_title(f'{led_key} S-pol (I={intensity_s})\n'
                          f'Peak: {peak_s:.0f} | Error: {error_pct:+.1f}%',
                          fontsize=10, fontweight='bold')
            ax_s.legend(fontsize=8)
        else:
            ax_s.text(0.5, 0.5, 'No data', ha='center', va='center',
                     transform=ax_s.transAxes, fontsize=12)
        ...
        if led_key in results['P']:
            ...
            ax_p.axhline(TARGET_COUNTS, color='blue', linestyle='--',
                        linewidth=1, alpha=0.7, label=f'Target ({TARGET_COUNTS:,})')
            ax_p.axhline(peak_p, color='red', linestyle=':',
                        linewidth=1, alpha=0.7, label=f'Peak ({peak_p:.0f})')
            
            error_pct = ((peak_p - TARGET_COUNTS) / TARGET_COUNTS) * 100
            ax_p.set_title(f'{led_key} P-pol (I={intensity_p})\n'
        ...
        if led_key in results['S'] and led_key in results['P']:
            ...
            peak_s_roi = np.max(spectrum_s[roi_mask]) if np.any(roi_mask) else np.max(spectrum_s)
            peak_p_roi = np.max(spectrum_p[roi_mask]) if np.any(roi_mask) else np.max(spectrum_p)
            ...
            transmission = np.divide(spectrum_p_norm, spectrum_s_norm, 
            ...
            ax_t.axhline(mean_transmission, color='orange', linestyle='--',
                        linewidth=1.5, alpha=0.7, 
                        label=f'Mean (560-720nm): {mean_transmission:.3f}')
            ax_t.axhline(1.0, color='black', linestyle=':', 
                        linewidth=1, alpha=0.5, label='Unity')
            
            # Shade ROI region
            ax_t.axvspan(560, 720, alpha=0.1, color='green', label='ROI')
            
            ax_t.set_title(f'{led_key} Transmission (P/S)\n'
            ...
            ax_t.text(0.5, 0.5, 'No data', ha='center', va='center',
                     transform=ax_t.transAxes, fontsize=12)
    ...
    output_file = Path(f'LED-Counts relationship/validation_transmission_spectra_{detector_serial}.png')
    ...
    print("="*80)
    print(f"{'LED':<8} {'S Peak':<12} {'S Error':<12} {'P Peak':<12} {'P Error':<12} {'P/S Ratio':<12}")
    print("-"*80)
    ...
    for led in LEDS:
        ...
        if led_key in results['S'] and led_key in results['P']:
            ...
            peak_s = np.max(spectrum_s[roi_mask]) if np.any(roi_mask) else np.max(spectrum_s)
            peak_p = np.max(spectrum_p[roi_mask]) if np.any(roi_mask) else np.max(spectrum_p)
            ...
            print(f"{led_key:<8} {peak_s:>10.0f}  {error_s:>+9.1f}%  "
...
def main():
    ...
    # Save results
    detector_serial = optimal_settings['detector_serial']
    results_file = Path(f'LED-Counts relationship/validation_results_{detector_serial}.json')

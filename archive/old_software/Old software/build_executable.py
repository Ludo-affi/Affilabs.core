...
def run_command(cmd, description, continue_on_error=False):
    ...
    print('='*60)
    try:
        result = subprocess.run(cmd, shell=True, check=not continue_on_error,
...
def main():
    ...
    if not run_command(f'"{python_exe}" -m pip install pyinstaller pillow',
    ...
    if not run_command(f'"{python_exe}" -m pip install {" ".join(core_deps)}',
    ...
    run_command(f'"{python_exe}" -m pip install {" ".join(hw_deps)}',
    ...
    if not run_command(f'"{python_exe}" -m PyInstaller main.spec',

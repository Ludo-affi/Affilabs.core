...
def check_live_integration():
    ...
    try:
        ...
        print("\n📊 CALIBRATION DATA:")
        print(f"   Integration time (calibrated): {led_cal.get('integration_time_ms', 'N/A')} ms")
        ...
        if live_boost_int:
            ...
            print(f"   LED intensities: {led_cal.get('live_boost_led_intensities', {})}")
        else:
        ...
        print(f"   MIN_INTEGRATION: {MIN_INTEGRATION} ms  ⚠️ THIS IS THE PROBLEM IF USED IN LIVE!")
        ...
        print(f"   {cal_int} ms × {LIVE_MODE_MAX_BOOST_FACTOR} = {expected_boost:.1f} ms")

        if live_boost_int:
            if abs(live_boost_int - expected_boost) < 1.0:
                print(f"   ✅ Stored boost matches expected: {live_boost_int} ms ≈ {expected_boost:.1f} ms")
            else:
                print(f"   ⚠️  Stored boost differs: {live_boost_int} ms vs {expected_boost:.1f} ms expected")

        print("\n" + "="*80)
        print("🔍 DIAGNOSIS:")
        if live_boost_int and live_boost_int > MIN_INTEGRATION:
            print(f"   ✅ Smart boost is configured ({live_boost_int} ms)")
            print(f"   ⚠️  BUT: Check if this is ACTUALLY applied to the spectrometer!")
            print(f"   👉 Look for log messages:")
            print(f"      '✅ Applied boosted integration time to data_acquisition.usb'")
        else:
            print(f"   ❌ No valid boost configured - will default to {MIN_INTEGRATION} ms!")
            print(f"   👉 This explains the weak signal issue")
        print("="*80 + "\n")

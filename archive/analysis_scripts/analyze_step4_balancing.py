...
def plot_step4_raw_data():
    ...
    for ch, filepath in s_ref_files.items():
        try:
            ...
            logger.info(
                f"   Shape: {data.shape}, Mean: {np.mean(data):.1f}, Max: {np.max(data):.1f}",
            )
    ...
    peak_bars = ax2.bar(
        x_pos - 0.2,
        [peak_values.get(ch, 0) for ch in CH_LIST],
        0.4,
        label="Peak",
        alpha=0.8,
        color="steelblue",
    ...
    mean_bars = ax2.bar(
        x_pos + 0.2,
        [mean_values.get(ch, 0) for ch in CH_LIST],
        0.4,
        label="Mean",
        alpha=0.8,
        color="coral",
    ...
    bars = ax4.bar(
        x_pos,
        [roi_means.get(ch, 0) for ch in CH_LIST],
        color=[colors[ch] for ch in CH_LIST],
        alpha=0.7,
    )
    ...
    ax4.axhline(
        ...
        linestyle="--",
        linewidth=2,
        label=f"Target: {mean_of_means:.0f}",
    )
    ...
    logger.info("=" * 80)
    logger.info("ROI Mean Intensities:")
    for ch in CH_LIST:
        if ch in roi_means:
            deviation = (
                ((roi_means[ch] - mean_of_means) / mean_of_means * 100)
                if mean_of_means > 0
                else 0
            )
            logger.info(
                f"   {ch.upper()}: {roi_means[ch]:7.0f} counts ({deviation:+.1f}% from target)",
            )
    logger.info("")

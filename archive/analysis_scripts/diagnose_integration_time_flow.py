...
def main():
    ...
    preferred_mode = config_data.get("calibration", {}).get(
    ...
    if integration_ms and integration_ms > MIN_INTEGRATION:
        print("✅ GLOBAL MODE DETECTED:")
        print(f"   - Single integration time: {integration_ms}ms")
        print("   - All channels use same integration")
        print("   - LEDs are balanced per channel")
        is_global = True
    else:
        print("⚠️ PER-CHANNEL MODE or INVALID:")
        print("   - Integration time too low or missing")
        is_global = False
    print()
    ...
    if is_global:
        ...
        print()
        print("Live mode optimization:")
        print(
            f"   Target intensity: {LIVE_MODE_TARGET_INTENSITY_PERCENT}% (P-pol compensated)",
        )
        print(f"   Desired boost: {desired_boost:.2f}×")
        print(
            f"   Applied boost: {boost_factor:.2f}× (capped between {LIVE_MODE_MIN_BOOST_FACTOR}-{LIVE_MODE_MAX_BOOST_FACTOR})",
        )
        print(
            f"   Boosted integration: {integration_ms:.1f}ms → {live_integration_ms:.1f}ms",
        )
        print()
        ...
        print(
            f"   2. state_machine calculates: live_integration_seconds = {live_integration_seconds:.6f}s",
        )
        print("   3. state_machine passes to DataAcquisitionWrapper:")
        print(
            f"      - self.data_acquisition.live_integration_seconds = {live_integration_seconds:.6f}s",
        )
        print("   4. DataAcquisitionWrapper.grab_data() uses:")
        print("      - desired_live = getattr(self, 'live_integration_seconds', None)")
        print(f"      - Sets spectrometer to {live_integration_ms:.1f}ms")
        print()

        print("❌ INCORRECT FLOW (what's happening now):")
        print("   1. System checks for integration_per_channel dict")
        print("   2. Finds integration_per_channel with 5ms values (wrong!)")
        print(f"   3. Uses 5ms instead of {live_integration_ms:.1f}ms")
        print("   4. Result: Signal is 5× too low compared to S-ref at 26ms")
        print()
    ...
    if "led_calibration" in config_data:
        ...
        if "per_channel_integration_times" in led_cal:
            ...
            print("⚠️ WARNING: Found per_channel_integration_times in device_config:")
            for ch, val in per_ch.items():
                print(f"   {ch.upper()}: {val*1000 if val < 1 else val:.1f}ms")
            print()
            print("💡 FIX: These should be REMOVED for global mode calibration!")
            print(
                f"   They are overriding the correct {live_integration_ms:.1f}ms boost.",
            )
        else:
            print("✅ No per_channel_integration_times found (correct for global mode)")
    print()
    ...
    print()
    print(
        "3. ✅ Check logs for: 'Passed smart boost integration time to DataAcquisitionWrapper'",
    )

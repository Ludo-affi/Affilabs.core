...
try:
    # Prefer top-level module; fallback to scripts/collection if needed
...
def load_wavelengths_masked(target_length: int | None = None) -> np.ndarray:
    """Load wavelength calibration, preferring device but falling back if unavailable.

    Attempts to connect to the spectrometer to retrieve calibrated wavelengths. If
    the device isn't connected or accessible, falls back to a USB4000-typical wavelength
    grid so offline analysis can proceed.
    """
    wl: np.ndarray
    try:
        ...
        if spec.connect():
            ...
            if got is not None:
                ...
                wl = USB4000OceanDirect()._get_fallback_wavelengths()
        else:
            # Fallback if connection routine returns False
            wl = USB4000OceanDirect()._get_fallback_wavelengths()
    except Exception:
        # Any error in discovery/connection -> fallback
        wl = USB4000OceanDirect()._get_fallback_wavelengths()
    # Apply mask to focus on configured range
    mask = (wl >= MIN_WAVELENGTH) & (wl <= MAX_WAVELENGTH)
    wl_masked = wl[mask]
    # If a target length is provided and we don't match, synthesize a matching grid
...
def main():
    ...
    wl = load_wavelengths_masked(target_length=trans_series.shape[1])

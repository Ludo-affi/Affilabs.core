                            ...
                            if spectrum_data:
                                ...
                                if len(self._spectrum_batch[ch]) >= self.batch_size:
                                    try:
                                        ...
                                        if cycle_count % 10 == 1:
                                            print(
                                                f"   [BATCH] Channel {ch}: Processed {self.batch_size} spectra",
                                            )
                                    except Exception as e:
                                        print(
                                            f"   [ERROR] Channel {ch}: Batch processing failed - {e}",
                                        )
                                        import traceback
                                ...
                                elif self.enable_interpolated_display:
                                    try:
        ...
        try:
            ...
            if channel in self.ref_sig and self.ref_sig[channel] is not None:
                try:
                    ...
                    p_led = (
                        self.leds_calibrated.get(channel)
                        if isinstance(self.leds_calibrated, dict)
                        else None
                    )
                    s_led = (
                        self.ref_intensity.get(channel)
                        if isinstance(self.ref_intensity, dict)
                        else None
                    )
                    ...
                    if len(raw_spectrum) != len(ref_spectrum):
                        ...
                        if p_led and s_led:
                            ...
                            if (
                                self._transmission_debug_counter % 50 == 1
                            ):  # Log every 50th calculation
                                print(
                                    f"[PROCESS] Ch {channel}: LED correction S={s_led}, P={p_led}, factor={correction:.3f}",
                                )
                    ...
                    if (
                        ...
                        and len(transmission_spectrum) >= 21
                    ):
    ...
    def _find_validated_peak(
        ...
        try:
            ...
            for idx in minima_indices:
                ...
                quality = (
                    0.5 * fwhm_score + 0.3 * min(depth / 50.0, 1.0) + 0.2 * wl_score
                )

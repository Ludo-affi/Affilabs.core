...
class FlowStatusWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.title_label.setStyleSheet("color: rgb(102, 77, 3); background: transparent; border: none;")
        container_layout.addWidget(self.title_label)

        # Status message
        self.message_label = QLabel("Waiting for device status...", self.status_container)
        self.message_label.setWordWrap(True)
        message_font = QFont()
        message_font.setPointSize(9)
        self.message_label.setFont(message_font)
        self.message_label.setStyleSheet("color: rgb(102, 77, 3); background: transparent; border: none;")
        container_layout.addWidget(self.message_label)

        layout.addWidget(self.status_container)

    def update_device_status(self, spr_connected=None, knx_connected=None, pump_connected=None):
        """
    ...
    def update_system_status(self, sensor_status=None, optics_status=None, fluidics_status=None):
        """
    ...
    def update_status(self):
        ...
        if not self._status_data['spr_connected']:
            warnings.append("⚠ SPR device not connected")

        if not self._status_data['knx_connected']:
            warnings.append("⚠ KNX system not connected")

        if not self._status_data['pump_connected']:
            warnings.append("⚠ Pump not connected")

        # Check system status
        if self._status_data['sensor'] == 'Poor':
            warnings.append("⚠ Sensor status: Poor - Check sensor calibration")
        ...
        if self._status_data['optics'] == 'Poor':
            warnings.append("⚠ Optics status: Poor - Check optical alignment")
        ...
        if self._status_data['fluidics'] == 'Poor':
            warnings.append("⚠ Fluidics status: Poor - Check flow system")
        ...
        if warnings:
            # Determine severity
            has_critical = any('Poor' in str(w) or 'not connected' in str(w) for w in warnings)
            ...
            if has_critical:
                ...
                self.title_label.setStyleSheet("color: rgb(183, 28, 28); background: transparent; border: none;")
                self.message_label.setStyleSheet("color: rgb(183, 28, 28); background: transparent; border: none;")
            else:
            ...
            if self._status_data['spr_connected']:
                self.status_container.setStyleSheet(
                    "QFrame#status_container {"
                ...
                self.title_label.setStyleSheet("color: rgb(27, 94, 32); background: transparent; border: none;")
                self.message_label.setStyleSheet("color: rgb(27, 94, 32); background: transparent; border: none;")

...
def calibrate_integration_time(
    ...
    for ch in ch_list:
        ...
        logger.debug(
            f"Ch {ch} initial reading at {integration}ms: {current_count:.0f} counts (target: {target_counts})",
        )
...
def calibrate_led_channel(
    ...
    if calibration_max >= saturation_threshold:
        logger.warning(
            f"⚠️ Ch {ch.upper()}: S-mode saturation detected at max LED ({calibration_max:.0f} ≥ {saturation_threshold:.0f})",
        )
        logger.info("   Auto-reducing LED intensity to bring signal to safe range...")
        ...
        logger.info(
            f"   Calculated reduction: LED {intensity} → {reduced_intensity} (factor: {reduction_factor:.3f})",
        )
        ...
        if intensity_data is None:
            logger.error(
                f"Failed to re-measure after saturation reduction for channel {ch.upper()}",
            )
            raise RuntimeError("Spectrometer read failed during saturation recovery")

        new_max = intensity_data.max()
        logger.info(
            f"   After reduction: {new_max:.0f} counts ({(new_max/max_counts)*100:.1f}% of detector max)",
        )

        # Validate the reduction worked
        if new_max >= saturation_threshold:
            logger.error(
                f"❌ Ch {ch.upper()}: Still saturated after auto-reduction - possible hardware issue",
            )
            ...
            logger.info(
                f"✅ Ch {ch.upper()}: Saturation resolved, continuing calibration from reduced baseline",
            )
            intensity = reduced_intensity
            calibration_max = new_max
    ...
    for ch in CH_LIST:
        ...
        if s_ref_signals and ch in s_ref_signals and wave_data is not None:
            try:
                ...
                logger.warning(
                    f"⚠️ S/P orientation validation failed for ch {ch.upper()}: {e}",
                )
...
def perform_full_led_calibration(
    ...
    try:
        ...
        result.ch_error_list, result.spr_fwhm, polarizer_swap_detected = (
            verify_calibration(
                usb,
                ctrl,
...
def perform_alternative_calibration(
    ...
    try:
        ...
        logger.info(
            f"   Global integration time: {result.integration_time}ms (max across channels)",
        )
        ...
        for ch, int_time in s_integration_times.items():
            ...
            logger.info(
                f"   Ch {ch.upper()}: {int_time}ms integration ({headroom_pct:.0f}% headroom) - {strength}",
            )
        ...
        for ch in ch_list:
            ...
            logger.debug(
                f"Measuring dark noise for ch {ch.upper()} at {ch_integration}ms...",
            )
        ...
        result.integration_time = max(s_integration_times.values())
        result.dark_noise = dark_noise_per_channel[
            max(s_integration_times, key=s_integration_times.get)
        ]

        # Store per-channel dark noise for live acquisition
        result.per_channel_dark_noise = dark_noise_per_channel.copy()

        logger.info("✅ Per-channel dark noise measurement complete")
            ...
            if device_serial:
                ...
                if calibration_dir.exists():
                    ...
                    if cal_files:
                        ...
                        logger.info(
                            "   Note: Afterglow correction will be applied per-channel with variable integration times",
                        )
        ...
        for ch in ch_list:
            ...
            result.channel_performance[ch] = {
                ...
                "optical_limit_reached": p_int
                >= MAX_INTEGRATION_BUDGET_MS * 0.95,  # Near timing budget limit
                "hit_saturation": False,  # Saturation is checked in verification step
            }

            logger.info(
                f"   Channel {ch}: S={s_int}ms → P={p_int}ms (boost: {integration_boost:.2f}x, headroom: {integration_headroom_ms}ms)",
            )
        ...
        # Log boost summary
        boost_ratios = [
            p_integration_times[ch] / s_integration_times[ch]
            for ch in ch_list
            if s_integration_times[ch] > 0
        ]

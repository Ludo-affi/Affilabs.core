...
def _validate_fit_result(
    ...
    if fit.amplitude > 10000:
        logger.warning(
            f"⚠️ Ch {ch} @ {int_ms}ms: Very high afterglow amplitude ({fit.amplitude:.0f} counts)\n"
...
def run_afterglow_calibration(
    *,
    ...
    logger.info(f"✅ Afterglow calibration complete for all channels: {channels}")
    return out
...
def _assess_pcb_health(aging_summary: list) -> dict:
    ...
    if (
        ...
        user_message = (
            f"⚠️ Light Source Maintenance Required\n\n"
...
def _check_channel_consistency(channel_data: dict, channels: list[str]) -> None:
    ...
    for ch, tau in channel_taus.items():
        ...
        if deviation > 5.0:  # More than 5ms deviation
            logger.warning(
                f"   ⚠️ Ch {ch.upper()}: τ={tau:.1f}ms deviates by {deviation:.1f}ms from mean\n"
                f"      Possible causes:\n"

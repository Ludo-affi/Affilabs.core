...
def calculate_transmission(
    ...
    p_led_intensity: float = None,
    s_led_intensity: float = None
) -> np.ndarray:
...
def find_resonance_wavelength_fourier(
    ...
    2. Linear detrending: Remove baseline slope
    3. DST (Discrete Sine Transform): Transform to frequency domain with SNR weights
    4. IDCT (Inverse DCT): Calculate smoothed derivative
    5. Zero-crossing: Find where derivative = 0 (peak minimum)
    6. Linear Regression: Refine position in window around zero-crossing
...
def validate_sp_orientation(
    ...
    if spr_region_end > spr_region_start:
        ...
        if min_deviation < -5:  # Clear dip present
            ...
            logger.warning(f"   ⚠️ Weak SPR structure in 600-750nm - cannot determine orientation")

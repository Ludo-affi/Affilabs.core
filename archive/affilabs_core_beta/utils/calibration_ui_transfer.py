...
def transfer_calibration_to_live_view(
    ...
    try:
        ...
        logger.info("✅ Reference signals and dark noise configured")
        ...
        logger.info(
            f"✅ Wavelength data configured ({len(calibration_result.wave_data)} pixels)",
        )
        ...
        if (
            ...
            logger.info(
                f"✅ Per-channel integration: {calibration_result.per_channel_integration}",
            )
...
def save_calibration_to_device_config(
    ...
    cal_data = {
        ...
        if calibration_result.s_ref_sig
        else {},
        "p_ref_signals": {
    ...
    logger.info(
        f"   S-ref signals: {len(calibration_result.s_ref_sig)} channels × {len(calibration_result.wave_data)} pixels",
    )
    if hasattr(calibration_result, "p_ref_sig") and calibration_result.p_ref_sig:
        logger.info(
            f"   P-ref signals: {len(calibration_result.p_ref_sig)} channels × {len(calibration_result.wave_data)} pixels",
        )

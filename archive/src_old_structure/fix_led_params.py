...
\) -> int:"""
    ...
    if content != content_new:
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(content_new)
        print("✅ Fixed calibrate_led_channel signature")
    else:
        print("⚠️  No changes made - signature already correct or pattern didn't match")
...
\) -> tuple\[dict\[str, int\], dict\[str, dict\]\]:"""
    ...
    if content != content_new:
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(content_new)
        print("✅ Fixed calibrate_p_mode_leds signature")
    else:
        print("⚠️  calibrate_p_mode_leds signature not changed - check manually")


if __name__ == "__main__":
    fix_led_calibration()
    print("\n✅ LED calibration signatures fixed!")

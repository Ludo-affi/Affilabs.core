        ...
        if p.vid and p.pid:
            ...
            if p.vid == ARDUINO_VID:
                if p.pid == ARDUINO_PID:
                    print("  ✅ MATCH: Arduino P4SPR Controller")
                else:
                    print(f"  ⚠️ Arduino VID but different PID (expected {hex(ARDUINO_PID)})")
            elif p.vid == PICO_VID:
                if p.pid == PICO_PID:
                    print("  ✅ MATCH: Raspberry Pi Pico Controller")
                else:
                    print(f"  ⚠️ Pico VID but different PID (expected {hex(PICO_PID)})")
            elif p.vid == CP210X_VID:
                if p.pid == CP210X_PID:
                    print("  ✅ MATCH: CP2102 USB-Serial (used by some controllers)")
                else:
                    print(f"  ⚠️ CP210X VID but different PID (expected {hex(CP210X_PID)})")
        else:
            print("  ⚠️ No VID/PID available")
else:
...
try:
    ...
    for d in devs:
        ...
        # Check against known controllers
        if vid == ARDUINO_VID:
            print("    ⚠️ Arduino VID detected!")
            if pid == ARDUINO_PID:
                print("    ✅ MATCH: Arduino P4SPR Controller")
            else:
                print(f"    ⚠️ Different PID (expected {hex(ARDUINO_PID)})")
        elif vid == PICO_VID:
            print("    ⚠️ Pico VID detected!")
            if pid == PICO_PID:
                print("    ✅ MATCH: Raspberry Pi Pico Controller")
            else:
                print(f"    ⚠️ Different PID (expected {hex(PICO_PID)})")

...
try:
    ...
    print(f"[TEST 1a] Appending to buffer: ch={channel}, time={elapsed_time}, wave={wavelength}")
...
try:
    ...
    if has_timeline_graph:
        ...
        if has_cursor:
            ...
            if cursor:
                print(f"[TEST 2a] Trying to set cursor value...")
...
if test_data.get('full_spectrum') is not None and test_data.get('transmission_spectrum') is not None:

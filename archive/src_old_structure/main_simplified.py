...
from src.domain import ProcessedSpectrumData, RawSpectrumData

# Phase 1.2 Business Services
from src.services import BaselineCorrector, TransmissionCalculator
from src.settings import PROFILING_ENABLED, PROFILING_REPORT_INTERVAL, SW_VERSION
...
class Application(QApplication):
    ...
    def __init__(self, argv):
        ...
        if HAL_AVAILABLE:
            ...
            logger.warning(
                f"⚠️  Phase 1.4 HAL not available: {_hal_import_error if '_hal_import_error' in globals() else 'unknown error'}",
            )
        ...
        if COORDINATORS_AVAILABLE:
            ...
            logger.warning(
                f"⚠️  UI Coordinators not available: {_coordinators_import_error if '_coordinators_import_error' in globals() else 'unknown error'}",
            )
        ...
        if COORDINATORS_AVAILABLE:
            ...
            logger.info(
                "✅ UI coordinators initialized (UIUpdateCoordinator, DialogManager)",
            )
        else:
        ...
        if not hasattr(self.main_window, "transmission_curves"):
            logger.warning(
                "⚠️ Spectroscopy plots NOT found in main window - graphs will not display",
            )
        ...
        logger.info(
            f"✅ Window visible (minimal UI rendered): {self.main_window.isVisible()}",
        )
    ...
    def _on_calibration_complete_status_update(self, calibration_data):
        ...
        try:
            ...
            if optics_ready:
                ...
                logger.warning(
                    "⚠️ Calibration completed but optics not ready (some channels failed)",
                )
    ...
    def _on_start_button_clicked(self):
        ...
        try:
            ...
            if (
                ...
                try:
                    logger.info(
                        f"✅ Loaded S-mode reference spectra: {list(cd.s_pol_ref.keys())}",
                    )
                except Exception:
                    pass
        ...
        try:
            ...
            if has_raw_data and has_transmission and should_update_transmission:
                try:
                    ...
                    logger.exception(
                        f"❌ Transmission queue error for channel {channel}: {e}",
                    )
                ...
                if device_config_det:
                    ...
                    if success:
                        ...
                        if mode_success:
                            ...
                            logger.info(
                                f"✅ Polarizer moved to position {new_position}",
                            )
                        else:
                            logger.warning(
    ...
    def _on_filter_toggled(self, checked: bool):
        ...
        self._update_cycle_of_interest_graph()
        logger.info(
            "✅ Filter toggle complete - both timeline and cycle graphs refreshed",
        )
            ...
            try:
                if (
                    ...
                    if positions:
                        ...
                        logger.info(
                            f"✅ Position validation: {new_pos.upper()}-mode target={target_pos}° (from device_config)",
                        )
                    else:
                        logger.warning(
                            "⚠️  Cannot validate positions - not found in device_config",
                        )
                else:
                    logger.warning(
                        "⚠️  Cannot validate positions - device_config not available",
                    )
            except Exception as e:
        ...
        try:
            ...
            if self.hardware_mgr and self.hardware_mgr.ctrl:
                ...
                if self.main_window.device_config:
                    ...
                    logger.info(
                        "✅ Settings saved to device config file (including LED timing delays)",
                    )
                else:
                    ...
                    if eeprom_config:
                        ...
                        if eeprom_s != s_pos or eeprom_p != p_pos:
                            ...
                            if self.hardware_mgr.ctrl.write_config_to_eeprom(
                                ...
                                logger.warning("=" * 80)
                                logger.warning(
                                    "⚠️  CRITICAL: CONTROLLER NEEDS POWER CYCLE",
                                )
                                logger.warning("=" * 80)
                                logger.warning(

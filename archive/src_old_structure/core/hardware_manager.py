            ...
            try:
                ...
                if usb.open():
                    ...
                    if (
                        ...
                        self.usb = usb
                        logger.info(
                            f"✅ Reconnected to spectrometer {self._spec_serial}",
                        )
                        return True
                    if not hasattr(usb, "serial_number"):
                        ...
                        self.usb = usb
                        logger.info(
                            "✅ Reconnected to spectrometer (serial not verifiable)",
                        )
                        return True
                    logger.warning(
    ...
    def _initialize_servo_polarizer(self, ctrl):
        ...
        try:
            ...
            if not hasattr(ctrl, "servo_move_calibration_only"):
                logger.warning(
                    "⚠️ Controller does not support servo - skipping polarizer init",
                )
                return
            ...
            logger.info(
                f"↗️ Moving polarizer to S-mode: S={s_position}°, P={p_position}°...",
            )
    ...
    def _connect_spectrometer(self):
        ...
        try:
            ...
            try:
                ...
                from utils.usb4000_wrapper import (
                    USB4000,  # fallback if src prefix not used
                )
            ...
            if usb.open():
                ...
                try:
                    ...
                    logger.info(
                        "🌡️ Performing spectrometer warm-up (10 ms dummy read)...",
                    )
        ...
        while retry_attempt < max_retries:
            try:
                ...
                if self.usb is None:
                    try:
                        logger.info(
                            "[SCAN] Spectrometer not found; attempting explicit USB4000 open...",
                        )
                        try:
                            from src.utils.usb4000_wrapper import USB4000
                        except ImportError:
                        ...
                        if _usb.open():
                            ...
                            logger.info(
                                f"[SCAN] USB4000 connected on second attempt: {getattr(self.usb, 'serial_number', None)}",
                            )
                        else:
                ...
                if ctrl_type and self.ctrl and self.usb:
                    ...
                    logger.info(
                        f"✅ Valid SPR device: {ctrl_type} (controller + detector)",
                    )
                ...
                if retry_attempt < max_retries:
                    logger.warning(
                        f"❌ Connection attempt {retry_attempt}/{max_retries} failed - retrying...",
                    )
                else:
                ...
                if self._try_reconnect_controller():
                    ...
                    if validate:
                        if self.is_controller_responsive():
                            ...
                            logger.warning(
                                "⚠️ Controller reconnected but not responsive",
                            )
                ...
                if self._try_reconnect_spectrometer():
                    ...
                    if validate:
                        if self.is_spectrometer_responsive():
                            ...
                            logger.warning(
                                "⚠️ Spectrometer reconnected but not responsive",
                            )

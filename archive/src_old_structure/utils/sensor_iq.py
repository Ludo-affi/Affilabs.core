    ...
    def _determine_iq_level(
        ...
        if zone in [WavelengthZone.QUESTIONABLE_LOW, WavelengthZone.QUESTIONABLE_HIGH]:
            ...
            if fwhm_category in ["excellent", "good"]:
                ...
                if fwhm is not None:
                    warning = f"⚠️  Wavelength {wavelength:.1f}nm in edge zone with poor FWHM {fwhm:.1f}nm"
                else:
                    warning = f"⚠️  Wavelength {wavelength:.1f}nm in edge zone with unknown FWHM quality"
...
def log_sensor_iq(metrics: SensorIQMetrics, channel: str):

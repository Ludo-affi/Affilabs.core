...
class ArduinoController(StaticController):
    ...
    def open(self):
        ...
        for dev in serial.tools.list_ports.comports():
            if dev.pid == ARDUINO_PID and dev.vid == ARDUINO_VID:
                ...
                for attempt in range(3):
                    try:
                        logger.info(
                            f"Trying Arduino on {dev.device} (attempt {attempt+1}/3)",
                        )
                        ...
                        try:
                            ...
                            if response == b"i":
                                ...
                                return True
                            logger.warning(
                                f"Device on {dev.device} doesn't respond like P4SPR (got {response})",
                            )
                            self._ser.close()
                            self._ser = None
                            ...
                            logger.warning(
                                f"Failed to validate device on {dev.device}: {val_err}",
                            )
                            self._ser.close()
                            self._ser = None
                            if attempt < 2:  # Don't sleep on last attempt
                        ...
                        logger.error(
                            f"Failed to open Arduino on {dev.device} (attempt {attempt+1}/3): {e}",
                        )
                        if self._ser is not None:
                            try:
                                self._ser.close()
        ...
        logger.debug(
            f"No Arduino P4SPR found with VID:PID {hex(ARDUINO_VID)}:{hex(ARDUINO_PID)}",
        )
        return False
            ...
            if self._ser is not None or self.open():
                ...
                for attempt in range(3):
                    ...
                    if not resp:
                        ...
                        logger.debug(
                            f"set_mode('{mode}') unexpected byte {resp} attempt {attempt}",
                        )
                # If controller stayed silent, assume success (common on older firmware) and warn once
                logger.warning(
                    f"Controller response to set_mode('{mode}') silent after 3 attempts; assuming success",
                )
                return True
    ...
    def read_config_from_eeprom(self) -> dict:
        ...
        try:
            if self._ser is not None or self.open():
                ...
                if len(data) != 20:
                    logger.warning(
                        f"EEPROM read returned {len(data)} bytes, expected 20",
                    )
                    return None
                ...
                if calculated_checksum != stored_checksum:
                    logger.warning(
                        f"EEPROM checksum mismatch: calc={calculated_checksum}, stored={stored_checksum}",
                    )
                    return None
                ...
                logger.info(
                    f"✓ Loaded device config from EEPROM: {led_model}, {fiber_diameter}µm fiber",
                )
                return config
        ...
        try:
            if self._ser is not None or self.open():
                ...
                data[2] = self._encode_controller_type(
                    config.get("controller_type", "arduino"),
                )
    ...
    def open(self):
        ...
        try:
            ...
            if reply == "P4SPR":
                ...
                logger.info(
                    f"Pico P4SPR detected on {explicit_port} (explicit) fw={self.version}",
                )
                return True
            # Accept empty or unexpected reply as provisional open; further commands may still work
            logger.warning(
                f"ID mismatch/empty on {explicit_port}: '{reply}' - proceeding provisionally",
            )
            return True
        except Exception as e:
            ...
            if self._ser is not None or self.open():
                with self._lock:
                    ...
                    if success:
                        ...
                        logger.warning(
                            f"❌ LED {ch.upper()} enable failed - no '1' response (got: {response})",
                        )
                    return success
        except Exception as e:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if ok:
                        ...
                        logger.warning(
                            f"⚠️ LED {ch.upper()} intensity command may have failed",
                        )
            ...
            if self._ser is not None or self.open():
                with self._lock:
                    ...
                    if success:
                        ...
                        if response:
                            logger.info(
                                f"✅ Controller confirmed: {mode_name} servo moved to position from device_config",
                            )
                        else:
                            logger.debug(
                                f"✅ {mode_name} set with empty ack (accepted on firmware V1.1)",
                            )
                    else:
                        # Non-empty, non-'1' byte: log once at warning, but continue
                        logger.warning(
                            f"⚠️ Controller response unexpected for {mode_name}: expected b'1', got {response}",
                        )
                        logger.warning(
                            "⚠️ Proceeding; servo likely moved correctly (device_config→EEPROM→firmware)",
                        )
                        return True
    ...
    def open(self):
        ...
        for dev in serial.tools.list_ports.comports():
            try:
                ...
                if reply == "EZSPR":
                    logger.info(
                        f"✅ Found Pico EZSPR on {dev.device} (fallback method)",
                    )
                    cmd = "iv\n"
                    self._ser.write(cmd.encode())
                    time.sleep(0.1)
    ...
    def turn_off_channels(self):
        ...
        try:
            if self._ser is not None or self.open():
                ...
                self._ser.write(cmd.encode())
                time.sleep(0.02)
                logger.debug("  All LED channels turned OFF via batch command")
                return True
        except Exception as e:
            logger.debug(f"error turning off channels {e}")
            return False

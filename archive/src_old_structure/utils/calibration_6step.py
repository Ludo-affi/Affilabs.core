...
def _validate_polarizer_positions(device_config, mode: str, logger) -> None:
    ...
    try:
        ...
        if s_pos is None or p_pos is None:
            logger.error(
                f"❌ CRITICAL: Invalid positions in device_config: S={s_pos}, P={p_pos}",
            )
        ...
        logger.info("=" * 80)
        logger.info("   Device Config Source: VERIFIED ✅")
...
def load_oem_polarizer_positions_local(
    device_config: dict,
    detector_serial: str,
) -> dict[str, int]:
    ...
    if s_pos is None or p_pos is None:
        logger.warning(
            "⚠️ OEM servo positions not found in device_config; using defaults S=120°, P=60°",
        )
        s_pos, p_pos = 120, 60
...
def converge_integration_time(
    ...
    for iteration in range(max_iterations):
        ...
        for ch in ch_list:
            ...
            logger.info(
                f"   {ch.upper()}: {signal:.0f} counts ({signal_pct:.1f}%) {status}",
            )
        ...
        if all_in_range and no_saturation:
            logger.info("")
            logger.info(
                f"✅ All channels converged to {target_percent*100:.1f}% ±{tolerance_percent*100:.1f}% with 0 saturation!",
            )
            converged = True
            break
        ...
        if iteration < max_iterations - 1:
            ...
            if iteration_saturated:
                ...
                logger.warning(
                    f"   Reducing integration to {current_integration:.1f}ms (-10%)",
                )
            else:
...
def measure_quick_dark_baseline(
    ...
    if has_led_query:
        for attempt in range(max_retries):
            ...
            if all_off:
                ...
                break
            logger.warning(
                f"⚠️ LEDs still on (attempt {attempt+1}/{max_retries}): {led_state}",
            )
        ...
        if not led_verified and has_led_query:
            ...
            raise RuntimeError(
                "Cannot measure quick dark baseline - LEDs failed to turn off",
            )
    ...
    logger.info(
        f"✅ Quick baseline complete: max = {max_dark:.0f}, mean = {mean_dark:.0f}, min = {min_dark:.0f} counts",
    )
...
def detect_polarity_and_recalibrate(
    ...
    for ch in ch_list:
        ...
        if sat_count > 0:
            ...
            logger.warning(
                f"  ⚠️ Channel {ch.upper()}: {sat_count} saturated pixels (max={max_signal:.0f})",
            )
        else:
            logger.info(
                f"  ✅ Channel {ch.upper()}: No saturation (max={max_signal:.0f})",
            )
    ...
    try:
        ...
        if "hardware" in device_config_dict:
            ...
            if s_pos is not None and p_pos is not None:
                logger.info(
                    "✅ Found servo positions in 'hardware' section (device_config root)",
                )
        ...
        if s_pos is not None and p_pos is not None:
            ...
            logger.info(
                "   ⚠️  NEVER send servo_set/flash during calibration - EEPROM operations removed",
            )
        else:
            ...
            logger.warning("")
            logger.warning(
                "🔧 RECOMMENDED: Run OEM calibration for optimal performance",
            )
            logger.warning(
            ...
            logger.warning("")
            logger.warning(
                "   ⚠️  Physical polarity may be incorrect without OEM calibration",
            )
            logger.warning("=" * 80)
        ...
        if has_led_query:
            ...
            if not led_verified and has_led_query:
                ...
                logger.error(
                    "This may indicate a hardware or firmware communication issue",
                )
        ...
        logger.info("✅ Step 1 complete: Hardware validated, LEDs confirmed OFF\n")
        ...
        logger.info(
            f"✅ SPR filtered range: {MIN_WAVELENGTH}-{MAX_WAVELENGTH}nm ({len(result.wave_data)} pixels)",
        )
        logger.info(
        ...
        logger.info("✅ Detector parameters:")
        ...
        # ===================================================================
        # Use the provided device_config (already loaded at init) to avoid
        # fallback/default positions that can invert S/P.
        ...
        if USE_LED_CONVERGENCE:
            ...
            # Use the same device_config dict we validated at startup
            device_config_det = device_config_dict
            logger.info("   ✅ Device config loaded (from init context)")
            ...
            convergence_result = run_convergence_calibration_steps_3_to_5(
                ...
                wave_max_index=result.wave_max_index,
                device_config_det=device_config_det,
                target_percent=0.80,  # S-mode target: 80% detector
            ...
            if not convergence_result["success"]:
                logger.error(
                    f"❌ LED convergence failed: {convergence_result['error']}",
                )
                result.success = False
            ...
            try:
                for ch in result.s_raw_data:
                    sig_top50 = (
                        float(np.mean(np.sort(result.s_raw_data[ch])[-50:]))
                        if len(result.s_raw_data[ch]) >= 50
                        else float(np.max(result.s_raw_data[ch]))
                    )
                    pct = (
                        sig_top50 / result.detector_max_counts * 100.0
                        if result.detector_max_counts
                        else 0.0
                    )
                    result.qc_results.setdefault("s_mode_top50", {})[ch] = sig_top50
                    result.qc_results.setdefault("s_mode_top50_pct", {})[ch] = pct
                for ch in result.p_raw_data:
                    sig_top50 = (
                        float(np.mean(np.sort(result.p_raw_data[ch])[-50:]))
                        if len(result.p_raw_data[ch]) >= 50
                        else float(np.max(result.p_raw_data[ch]))
                    )
                    pct = (
                        sig_top50 / result.detector_max_counts * 100.0
                        if result.detector_max_counts
            ...
            # Map compatibility field expected by Step 6 logs
            try:
                result.ref_intensity = dict(result.s_mode_intensity)
            except Exception:
                result.ref_intensity = (
                    dict(result.s_mode_intensity)
                    if hasattr(result, "s_mode_intensity")
                    else {}
                )
            ...
            try:
                ...
                if not hasattr(result, "s_raw_data") or not result.s_raw_data:
                    raise RuntimeError("S-pol raw data missing from convergence output")
                if not hasattr(result, "p_raw_data") or not result.p_raw_data:
                    raise RuntimeError("P-pol raw data missing from convergence output")
                if not hasattr(result, "dark_noise") or result.dark_noise is None:
                    raise RuntimeError(
                        "Dark noise reference missing from convergence output",
                    )

                logger.info("   ✅ S-pol raw data: 4 channels from convergence")
                logger.info("   ✅ P-pol raw data: 4 channels from convergence")
                logger.info("   ✅ Dark reference: From convergence capture")
                logger.info(
                    f"   ✅ S-mode integration time: {result.s_integration_time:.2f}ms",
                )
                logger.info(
                    f"   ✅ P-mode integration time: {result.p_integration_time:.2f}ms",
                )
                ...
                try:
                    ...
                    s_means_pre = {
                        ch: float(_np.mean(result.s_raw_data[ch]))
                        for ch in ch_list
                        if ch in result.s_raw_data
                    }
                    p_means_pre = {
                        ch: float(_np.mean(result.p_raw_data[ch]))
                        for ch in ch_list
                        if ch in result.p_raw_data
                    }
                    logger.info("   🔍 Preprocess sanity: mean(S_raw) vs mean(P_raw)")
                    for ch in ch_list:
                        if ch in s_means_pre and ch in p_means_pre:
                            ...
                            logger.info(
                                f"      {ch.upper()}: S={s_means_pre[ch]:.0f}, P={p_means_pre[ch]:.0f}, P/S={ratio:.3f}",
                            )
                    ...
                    if all(
                        ...
                        logger.warning(
                            "   ⚠️  All channels have P_raw > S_raw — S/P inversion suspected upstream",
                        )
                        # Apply runtime swap for processing/QC to maintain P/S convention
                        logger.warning(
                            "   ⚠️  Applying runtime S/P swap for processing (no EEPROM changes)",
                        )
                        # Swap raw references in a local view for processing only
                        swapped_s_raw = result.p_raw_data
                        swapped_p_raw = result.s_raw_data
                        ...
                        swapped_s_raw = result.s_raw_data
                        swapped_p_raw = result.p_raw_data
                ...
                for ch in ch_list:
                    ...
                    qc = TransmissionProcessor.calculate_transmission_qc(
                        ...
                        channel=ch,
                        p_spectrum=swapped_p_raw[ch],
                        s_spectrum=swapped_s_raw[ch],
                    ...
                    logger.info(
                        f"   Status: {'✅ DETECTED' if qc['dip_detected'] else '❌ WEAK/ABSENT'} (depth > 5%)",
                    )
                    ...
                    if qc["fwhm"] is not None:
                        ...
                        logger.info(
                            f"   Status: {'✅ PASS' if fwhm_pass else '❌ FAIL'} (FWHM < 60nm)",
                        )
                    else:
                    ...
                    if qc["ratio"] is not None:
                        ...
                        if qc["orientation_correct"] is True:
                            logger.info("   Status: ✅ CORRECT (0.10 ≤ P/S ≤ 0.95)")
                        elif qc["orientation_correct"] is False:
                            logger.error("   Status: ❌ INVERTED (P/S > 1.15)")
                            logger.error("   ⚠️  CRITICAL: Polarizer appears INVERTED!")
                            ...
                            logger.warning(
                                "   Status: ⚠️ INDETERMINATE (borderline ratio)",
                            )
                    else:
                        logger.info("   P/S Ratio: Not calculated")
                        logger.info("   Status: ⚠️ CANNOT VERIFY")

                    logger.info("\n📊 Saturation Check:")
                    if qc["s_saturated"] or qc["p_saturated"]:
                        if qc["s_saturated"]:
                            logger.error(
                                f"   S-pol: ❌ SATURATED ({qc['s_max_counts']:.0f} counts)",
                            )
                        if qc["p_saturated"]:
                            logger.error(
                                f"   P-pol: ❌ SATURATED ({qc['p_max_counts']:.0f} counts)",
                            )
                    else:
                        logger.info(
                            f"   S-pol: ✅ OK ({qc['s_max_counts']:.0f} counts)",
                        )
                        logger.info(
                            f"   P-pol: ✅ OK ({qc['p_max_counts']:.0f} counts)",
                        )
                    ...
                    logger.info(f"\n{'='*40}")
                    logger.info(
                        f"Channel {ch.upper()}: {'✅ PASS' if channel_pass else '❌ FAIL'}",
                    )
                    logger.info(f"{'='*40}")
                ...
                # Legacy QC compatibility fields
                for ch in ch_list:
        ...
        try:
            ...
            # Use the same device_config dict we validated at startup
            device_config_det = device_config_dict
            servo_positions = servo_initiation_to_s(
                ctrl,
                device_config_det,
                detector_serial,
            )
            logger.info(
                f"✅ Servo initialized to S-mode: S={servo_positions['s_position']}°, P={servo_positions['p_position']}°",
            )
        except Exception as e:
            logger.warning(
                f"Servo initiation failed: {e}; attempting fallback S-mode switch",
            )
        ...
        logger.info(
            f"🔧 Integration time set to {RANKING_INTEGRATION_TIME:.0f}ms for LED ranking\n",
        )
        ...
        if firmware_rank_available:
            ...
            try:
                ...
                rank_data = (
                    ctrl.rank_leds()
                )  # Returns: [(ch, mean_intensity), ...] sorted weakest→strongest
                ...
                if rank_data and len(rank_data) == len(ch_list):
                    ...
                    for rank_idx, (ch, (mean, _, _)) in enumerate(ranked_channels, 1):
                        ratio = (
                            mean / ranked_channels[0][1][0]
                            if ranked_channels[0][1][0] > 0
                            else 1.0
                        )
                        logger.info(
                    ...
                    logger.warning(
                        "⚠️  Firmware rank returned invalid data, falling back to Python",
                    )
        ...
        if not firmware_rank_success:
            ...
            logger.info(
                f"   Test LED: {test_led_intensity} ({test_led_intensity/255*100:.0f}%)",
            )
            logger.info(
                f"   Test region: Full SPR spectrum ({result.wave_data[0]:.1f}-{result.wave_data[-1]:.1f}nm)\n",
            )
            ...
            for ch in ch_list:
                ...
                batch_values = {
                    c: (test_led_intensity if c == ch else 0)
                    for c in ["a", "b", "c", "d"]
                }
                ...
                logger.info(
                    f"   {ch.upper()}: mean={mean_intensity:6.0f}, max={max_intensity:6.0f}{sat_flag}",
                )
            ...
            if saturated_channels:
                logger.warning(
                    f"⚠️  Saturation detected in {len(saturated_channels)} channel(s): {saturated_channels}",
                )
                logger.warning("   Retrying at LED=25 (10%)...\n")
                ...
                for ch in saturated_channels:
                    ...
                    batch_values = {
                        c: (retry_led if c == ch else 0) for c in ["a", "b", "c", "d"]
                    }
                    ctrl.set_batch_intensities(**batch_values)
                    time.sleep(LED_DELAY)
                    ...
                    logger.info(
                        f"   {ch.upper()} retry: {mean_intensity:6.0f} @ LED={retry_led} → scaled: {scaled_mean:6.0f}",
                    )
            ...
            for rank_idx, (ch, (mean, _, was_sat)) in enumerate(ranked_channels, 1):
                ...
                logger.info(
                    f"   {rank_idx}. Channel {ch.upper()}: {mean:6.0f} counts ({ratio:.2f}× weakest){sat_flag}",
                )
        ...
        if all_signals_low:
            ...
            logger.warning("")
            logger.warning(
                "⚠️  QC FLAG: All channels have very low signal (<4000 counts)",
            )
            logger.warning(f"   Maximum signal detected: {max_signal:.0f} counts")
            logger.warning(
                "   Possible causes: LEDs are OFF, hardware disconnected, or extremely low light",
            )
        ...
        if weakest_intensity <= 1e-6:
            logger.error(
                "❌ Weakest channel brightness is near-zero; optics/hardware issue suspected",
            )
        ...
        logger.info("")
        logger.info(
            f"✅ Weakest LED: {weakest_ch.upper()} ({weakest_intensity:.0f} counts)",
        )
        logger.info(
        ...
        # ===================================================================
        logger.info("=" * 80)
        logger.info("STEP 3B: Weakest Channel Integration Time Optimization")
        logger.info("=" * 80)
        logger.info(
            "Goal: Find integration time where weakest channel @ LED=255 hits 40% detector",
        )
        logger.info(
            "Strategy: Binary search to maximize weakest channel signal (avoiding saturation)",
        )
        logger.info(
            f"Target: {weakest_ch.upper()} @ LED=255 → 40% detector ({int(0.40 * (detector_params.max_counts)):.0f} counts)",
        )
        logger.info("")
        ...
        detector_max = (
            ...
            if hasattr(result, "detector_max_counts")
            else detector_params.max_counts
        )
        ...
        logger.info(
            f"📊 Binary Search: Optimize integration time (weakest @ LED={WEAKEST_TARGET_LED})",
        )
        logger.info("")
        ...
        for iteration in range(15):
            ...
            if is_saturated:
                logger.warning(
                    f"   Iteration {iteration+1}: {test_int:.1f}ms → SATURATED ({saturated_pixels} pixels >= {saturation_limit})",
                )
            ...
            logger.info(
                f"   Iteration {iteration+1}: {test_int:.1f}ms → {signal:.0f} counts ({signal_pct:.1f}%)",
            )
            ...
            error_pct = (
                abs((signal - step3b_target_signal) / step3b_target_signal) * 100
            )
            if error_pct <= 2.0 or consecutive_stable_s1 >= 2:
                ...
                break
            if signal < step3b_target_signal:
                min_int = test_int
            else:
                max_int = test_int

            # Track best
            if abs(signal - step3b_target_signal) < abs(
                best_signal - step3b_target_signal,
            ):
                best_integration = test_int
                best_signal = signal
        ...
        logger.info("")
        logger.info(
            f"✅ STEP 3B COMPLETE: Integration time locked at {best_integration:.1f}ms",
        )
        logger.info(
            f"   Weakest ({weakest_ch.upper()} @ LED={WEAKEST_TARGET_LED}): {best_signal:.0f} counts ({(best_signal/detector_max)*100:.1f}%)",
        )
        logger.info("")
        ...
        while restart_attempt <= max_restart_attempts:
            if restart_attempt > 0:
                logger.info("\n" + "=" * 80)
                logger.info(
                    f"🔄 RESTART ATTEMPT {restart_attempt}: Re-running Steps 3C-6 with corrected servo positions",
                )
                logger.info("=" * 80 + "\n")
            ...
            try:
                ...
                logger.info("=" * 80)
                logger.info("STEP 3C: LED Normalization")
                logger.info("=" * 80)

                if USE_ALTERNATIVE_CALIBRATION:
                    # ALTERNATIVE MODE: LED=255 fixed for all channels, per-channel integration
                    logger.info(
                        "Mode: ALTERNATIVE (LED=255 fixed + per-channel integration)",
                    )
                    logger.info(
                        "Goal: Set all LEDs to 255, adjust integration time per channel in Step 5",
                    )
                    logger.info("")

                    normalized_leds = {ch: 255 for ch in ch_list}

                    logger.info("   All LEDs set to 255 (maximum intensity)")
                    logger.info(
                        "   Integration time adjustment will be done per channel in Step 5",
                    )
                    logger.info("")
                    ...
                    for ch, (step3a_ranking_signal, _, _) in ranked_channels:
                        ...
                        logger.info(
                            f"   {ch.upper()}: Brightness ratio {brightness_ratio:.2f}× (will invert to integration time)",
                        )

                else:
                    # STANDARD MODE: Normalized LED intensities, global integration
                    logger.info("Mode: STANDARD (Normalized LEDs + global integration)")
                    logger.info(
                        "Goal: Calculate normalized LED intensities for all channels",
                    )
                    logger.info(
                        "Strategy: Use Step 3A brightness ratios + Step 3B integration time",
                    )
                    logger.info("")

                    # STAGE 1: Calculate normalized LEDs
                    logger.info(
                        "📊 STAGE 1: Calculate Normalized LEDs (Brightness Compensation)",
                    )
                    logger.info("")

                    normalized_leds = {}
                    for rank_idx, (ch, (step3a_ranking_signal, _, _)) in enumerate(
                        ranked_channels,
                        1,
                    ):
                        ...
                        logger.info(
                            f"   {ch.upper()}: Brightness {brightness_ratio:.2f}× → LED={normalized_led}",
                        )
                    ...
                    logger.info(
                        "📊 STAGE 2: Smart LED Calculation (All Channels @ 40% Detector)",
                    )
                    logger.info("")
                    ...
                    logger.info(
                        f"   Target signal: {target_signal:.0f} counts ({(target_signal/detector_max)*100:.1f}% detector)",
                    )
                    logger.info(
                        f"   Safe maximum: {safe_max_signal:.0f} counts (95% of saturation threshold)",
                    )

                    if target_signal >= safe_max_signal:
                        logger.warning(
                            f"   ⚠️ Target ({target_signal:.0f}) at/above safe maximum ({safe_max_signal:.0f})",
                        )
                        target_signal = int(
                            safe_max_signal * 0.98,
                        )  # Back off to 98% of safe max
                        logger.warning(
                            f"   Reducing target to: {target_signal:.0f} counts ({(target_signal/detector_max)*100:.1f}% detector)",
                        )
                    else:
                        logger.info("   ✅ Target is safely below saturation threshold")

                    logger.info("")

                    correction_tolerance = 0.05  # ±5% tolerance for tight convergence
                    max_correction_iterations = (
                        5  # Increased from 3 to ensure convergence
                    )
                    ...
                    logger.info(
                        f"   Convergence tolerance: ±{correction_tolerance*100:.0f}%",
                    )
                    ...
                    for correction_iter in range(max_correction_iterations):
                        ...
                        if correction_iter > 0:
                            logger.info("")
                            logger.info(
                                f"🔄 Correction Iteration {correction_iter + 1}",
                            )
                            logger.info("")
                        ...
                        for ch in channels_to_correct:
                            ...
                            if spectrum is None:
                                ...
                                channels_corrected_this_iter.append(
                                    ch,
                            ...
                            converged = (
                                abs(error_pct) <= correction_tolerance
                            ...
                            if converged:
                                ...
                                logger.info(
                                    f"   ✅ {ch.upper()} @ LED={normalized_leds[ch]:3d}: {signal:6.0f} counts ({signal_pct:5.1f}%) CONVERGED {error_pct:+.1%}",
                                )
                                continue
                            ...
                            if correction_iter >= max_correction_iterations - 1:
                                ...
                                if is_saturated:
                                    logger.warning(
                                        f"   ⚠️ {ch.upper()} @ LED={normalized_leds[ch]:3d}: SATURATED after {max_correction_iterations} iterations",
                                    )
                                else:
                                    logger.warning(
                                        f"   ⚠️ {ch.upper()} @ LED={normalized_leds[ch]:3d}: {signal:6.0f} counts - NOT CONVERGED {error_pct:+.1%}",
                                    )
                                continue
                            ...
                            if is_saturated:
                                ...
                                correction_factor = max(
                                    0.75,
                                    min(0.95, correction_factor),
                                )
                                logger.warning(
                                    f"   🔻 {ch.upper()} @ LED={old_led:3d}: SATURATED ({saturated_pixels} px, max={max_pixel:.0f})",
                                )
                                logger.warning(
                                    f"      → Reducing to LED={int(old_led * correction_factor):3d} (factor {correction_factor:.3f})",
                                )
                            else:
                                ...
                                correction_factor = max(
                                    0.80,
                                    min(1.20, correction_factor),
                                )

                                status = "🔺 LOW" if error_pct < 0 else "🔻 HIGH"
                                logger.info(
                                    f"   {status} {ch.upper()} @ LED={old_led:3d}: {signal:6.0f} counts {error_pct:+.1%}",
                                )
                                logger.info(
                                    f"      → Adjusting to LED={int(old_led * correction_factor):3d} (factor {correction_factor:.3f})",
                                )
                        ...
                        if not channels_to_correct:
                            logger.info("")
                            logger.info(
                                f"✅ All channels within ±5% of target after {correction_iter + 1} iteration(s)",
                            )
                            break
                    ...
                    if channels_to_correct:
                        logger.info("")
                        logger.info(
                            f"⚠️ {len(channels_to_correct)} channel(s) still outside tolerance after {max_correction_iterations} iterations",
                        )

                    logger.info("")
                    logger.info("📊 STAGE 3: Final Uniformity Check")
                    logger.info("")
                    ...
                    for ch in ch_list:
                        ...
                        deviation_pct = (
                            (signal - weakest_signal) / weakest_signal
                        ) * 100
                        uniformity_status = (
                            "✅"
                            if abs(deviation_pct) < 10 and not is_saturated
                            else "⚠️"
                        )
                        pixel_status = "✅" if pct_near >= 50 else "⚠️"

                        logger.info(
                            f"   {ch.upper()} @ LED={normalized_leds[ch]:3d}: {signal:6.0f} counts ({signal_pct:5.1f}%) {uniformity_status} {deviation_pct:+.1f}% {sat_status}",
                        )
                        if is_saturated:
                            logger.warning(
                                f"      {saturated_pixels} pixels saturated (max: {max_pixel:.0f} >= {saturation_limit:.0f})",
                            )
                            saturated_channels.append(ch)
                        logger.info(
                            f"      Pixels near target: {pixels_near}/{total_pixels} ({pct_near:.1f}%) {pixel_status}",
                        )
                    ...
                    if saturated_channels:
                        logger.warning("")
                        logger.warning(
                            f"⚠️ Saturation detected in channels: {', '.join([ch.upper() for ch in saturated_channels])}",
                        )
                        logger.warning(
                            "   Applying emergency 15% LED reduction to saturated channels...",
                        )
                        ...
                        logger.warning("   Verifying saturation cleared...")
                        ...
                        for ch in saturated_channels:
                            ...
                            if spectrum is not None:
                                ...
                                if sat_count > 0:
                                    ...
                                    logger.error(
                                        f"   ❌ {ch.upper()}: Still {sat_count} pixels saturated",
                                    )
                                else:
                                    signal = roi_signal(
                                        spectrum,
                                    ...
                                    signal_pct = (signal / detector_max) * 100
                                    logger.info(
                                        f"   ✅ {ch.upper()}: Saturation cleared ({signal:.0f} counts, {signal_pct:.1f}%)",
                                    )

                        if still_saturated:
                            logger.error("")
                            logger.error(
                                f"❌ CALIBRATION FAILED: Saturation persists in {', '.join([ch.upper() for ch in still_saturated])}",
                            )
                            logger.error(
                                "   Even after 15% LED reduction, saturation remains.",
                            )
                            logger.error(
                                "   The 80% target cannot be achieved with this hardware configuration.",
                            )
                            logger.error("")
                            raise RuntimeError(
                                f"Persistent saturation in Step 3C: {still_saturated}",
                            )
                        logger.info("")
                        logger.info(
                            "✅ Saturation cleared after emergency LED reduction",
                        )

                    logger.info("")
                logger.info("=" * 80)
                logger.info(
                    f"✅ STEP 3C COMPLETE: LEDs {'FIXED AT 255' if USE_ALTERNATIVE_CALIBRATION else 'NORMALIZED & ITERATIVELY CORRECTED'}",
                )
                logger.info("=" * 80)
                logger.info(
                    f"   Integration time: {best_integration:.1f}ms (from Step 3B)",
                )
                if USE_ALTERNATIVE_CALIBRATION:
                    logger.info(
                        "   All LEDs @ 255 (per-channel integration will be optimized in Step 5)",
                    )
                else:
                    logger.info(
                        f"   All channels @ ~40% ({step3b_target_signal:.0f} counts) within ±5% tolerance",
                    )
                    logger.info("   LED intensities after iterative correction:")
                    for ch in ch_list:
                        logger.info(f"      {ch.upper()}: LED={normalized_leds[ch]}")
                logger.info("")
                logger.info("   🔒 LED VALUES NOW FROZEN (no changes after Step 3C)")
                logger.info(
                    "   📋 CRITICAL RULE: Steps 4, 5, 6 use ONLY integration time adjustments",
                )
                logger.info(
                    "   📋 LED intensities remain locked at Step 3C values for all subsequent steps",
                )
                logger.info("")
                ...
                logger.info(
                    f"Wavelength range: {result.wave_data[0]:.2f} - {result.wave_data[-1]:.2f} nm",
                )
                ...
                try:
                    ...
                    if saturated_channels and len(saturated_channels) > 0:
                        ...
                        logger.info(
                            f"⚠️ Step 3C had saturation in: {', '.join([ch.upper() for ch in step3c_saturated_channels])}",
                        )
                        logger.info(
                            "   Step 4 will use more aggressive reduction for these channels",
                        )
                except (NameError, UnboundLocalError):
                    # Variable doesn't exist - no saturation in Step 3C
                    logger.info("✅ Step 3C completed with no saturation")
                ...
                logger.info("=" * 80)
                logger.info(
                    "Goal: Measure S-mode signal in two ROI regions for P-mode loss calculation",
                )
                logger.info(
                    "Strategy: Adjust integration time to achieve target detector signal",
                )
                logger.info("ROI 1: 560-570nm (blue edge - minimal SPR signal)")
                logger.info("ROI 2: 710-720nm (red edge - minimal SPR signal)")
                logger.info(
                    "Purpose: Establish high-SNR S-pol baseline for transmission calculation\n",
                )
                ...
                if maxed_channels:
                    logger.warning(
                        f"⚠️ Channels with maxed LEDs detected: {', '.join([ch.upper() for ch in maxed_channels])}",
                    )
                    logger.warning("   Using per-channel integration time optimization")
                    logger.warning(
                        "   Target: 75% detector (49K counts) for all channels",
                    )
                ...
                step4_min_signal = int(
                    (STEP4_TARGET_PERCENT - STEP4_TOLERANCE_PERCENT) * detector_max,
                )
                step4_max_signal = int(
                    (STEP4_TARGET_PERCENT + STEP4_TOLERANCE_PERCENT) * detector_max,
                )
                ...
                # Clamp to detector limits
                step4_integration = max(
                    detector_params.min_integration_time,
                    min(detector_params.max_integration_time, step4_integration),
                )

                logger.info("📊 Step 4 Target Calculation:")
                logger.info(
                    f"   Step 3C target: {step3c_target_signal:.0f} counts ({STEP3B_TARGET_PERCENT*100:.0f}% detector)",
                )
                logger.info(
                    f"   Step 4 target: {step4_target_signal:.0f} counts ({STEP4_TARGET_PERCENT*100:.0f}% detector)",
                )
                logger.info(
                    f"   Acceptable range: {step4_min_signal:.0f} - {step4_max_signal:.0f} counts ({(STEP4_TARGET_PERCENT-STEP4_TOLERANCE_PERCENT)*100:.1f}% - {(STEP4_TARGET_PERCENT+STEP4_TOLERANCE_PERCENT)*100:.1f}%)",
                )
                logger.info(
                    f"   Baseline integration: {step4_integration:.1f}ms (scaled {integration_scale_factor:.2f}× from Step 3C)",
                )
                logger.info("   Using Step 3C LEDs (unchanged)")

                if use_per_channel_integration:
                    logger.info("   Mode: PER-CHANNEL integration time optimization")
                else:
                    logger.info("   Mode: SINGLE common integration time")
                logger.info("")
                ...
                if use_per_channel_integration:
                    ...
                    logger.info("🔄 Step 4 Per-Channel Optimization")
                    logger.info(
                        f"   Target: {STEP4_TARGET_PERCENT*100:.0f}% ±2.5% for each channel",
                    )
                    logger.info("")
                    ...
                    for ch in ch_list:
                        ...
                        max_int = min(
                            step4_integration * 2.0,
                            detector_params.max_integration_time,
                        )
                        best_int = (
                            detector_params.min_integration_time
                        )  # Start with minimum as fallback
                        best_signal = 0
                        max_iterations = 8
                        ...
                        for iteration in range(max_iterations):
                            ...
                            if spectrum is None:
                                logger.warning(
                                    f"    Iter {iteration+1}: No spectrum, skipping",
                                )
                                break
                            ...
                            if sat_count > 0:
                                ever_saturated = True
                                logger.info(
                                    f"    Iter {iteration+1}: {test_int:.1f}ms → {signal:.0f} ({signal_pct:.1f}%), SATURATED → reduce",
                                )
                                max_int = test_int
                            elif in_range:
                                ...
                                best_signal = signal
                                logger.info(
                                    f"    Iter {iteration+1}: {test_int:.1f}ms → {signal:.0f} ({signal_pct:.1f}%), ✅ OPTIMAL",
                                )
                                break
                            elif signal < step4_min_signal:
                                logger.info(
                                    f"    Iter {iteration+1}: {test_int:.1f}ms → {signal:.0f} ({signal_pct:.1f}%), too low → increase",
                                )
                                min_int = test_int
                                # Update best if this is closer to target AND not saturated
                                if (
                                    abs(signal - step4_target_signal)
                                    < abs(best_signal - step4_target_signal)
                                    or best_signal == 0
                                ):
                                    best_int = test_int
                                    best_signal = signal
                                ...
                                logger.info(
                                    f"    Iter {iteration+1}: {test_int:.1f}ms → {signal:.0f} ({signal_pct:.1f}%), too high → reduce",
                                )
                                max_int = test_int
                                # Update best if closer to target
                                if abs(signal - step4_target_signal) < abs(
                                    best_signal - step4_target_signal,
                                ):
                                    best_int = test_int
                                    best_signal = signal
                        ...
                        if ever_saturated:
                            logger.warning(
                                f"    Channel {ch.upper()} hit saturation during search - verifying final integration is safe...",
                            )
                            ...
                            if verify_spectrum is not None:
                                verify_roi = verify_spectrum[
                                    wave_min_index:wave_max_index
                                ]
                                verify_sat = count_saturated_pixels(
                                    verify_roi,
                                    0,
                                    len(verify_roi),
                                    detector_params.saturation_threshold,
                                ...
                                if verify_sat > 0:
                                    ...
                                    logger.error(
                                        f"    ❌ Still saturated at {best_int:.1f}ms! Reducing to 70% of current value...",
                                    )
                                    best_int *= 0.70
                                    best_int = max(
                                        detector_params.min_integration_time,
                                    ...
                                    logger.warning(
                                        f"    Using emergency reduced integration: {best_int:.1f}ms",
                                    )

                        channel_s_integration_times[ch] = best_int
                        final_pct = (
                            (best_signal / detector_max) * 100 if best_signal > 0 else 0
                        )
                        logger.info(
                            f"  ✅ {ch.upper()}: Final {best_int:.1f}ms → {best_signal:.0f} ({final_pct:.1f}%)",
                        )
                        logger.info("")

                    logger.info(
                        "✅ All channels optimized with per-channel integration times",
                    )
                    step4_integration = channel_s_integration_times.get(
                        "a",
                        step4_integration,
                    )  # Use channel A as reference

                logger.info("")
                logger.info(
                    f"📊 Final Step 4 integration time: {step4_integration:.1f}ms",
                )
                logger.info("")
                ...
                logger.info("=" * 80)
                logger.info(
                    "📸 STEP 4 (S-mode): Final Data Acquisition with Converged Integration Time",
                )
                logger.info("=" * 80)
                logger.info("")
                ...
                logger.info("📊 ROI Definitions:")
                logger.info(
                    f"   ROI 1: {ROI1_MIN}-{ROI1_MAX}nm (indices {roi1_min_idx}-{roi1_max_idx})",
                )
                logger.info(
                    f"   ROI 2: {ROI2_MIN}-{ROI2_MAX}nm (indices {roi2_min_idx}-{roi2_max_idx})",
                )
                logger.info(
                    f"   Using converged integration time: {step4_integration:.1f}ms",
                )
                logger.info("")
                ...
                logger.info(
                    "  Expected behavior: Maximum transmission through polarizer",
                )
                logger.info("")
                ...
                if set_result:
                    ...
                    logger.warning(
                        "  ⚠️ Controller response unexpected - servo should have moved to S position",
                    )
                    logger.warning(
                        "  ⚠️ Continuing calibration - servo movement may still be successful",
                    )
                logger.info("")
                ...
                logger.info(
                    f"   Target: 80% ±2.5% ({step4_min_signal:.0f} - {step4_max_signal:.0f} counts)",
                )
                ...
                logger.info("    ✅ S-mode set (position from device_config)")
                logger.info("")
                ...
                result.num_scans = max(
                    3,
                    min(int(MAX_READ_TIME / best_integration), MAX_NUM_SCANS),
                )
                ...
                for ch in ch_list:
                    ...
                    ch_integration = (
                        channel_s_integration_times.get(ch, step4_integration)
                        if use_per_channel_integration
                        else step4_integration
                    )
                    ...
                    if avg_spectrum is None:
                        logger.error(
                            f"   ❌ Failed to capture spectra for {ch.upper()}",
                        )
                        continue
                    ...
                    logger.info(
                        f"   ROI 1 (560-570nm): {roi1_signal:.0f} counts ({signal_pct:.1f}%)",
                    )
                    signal_pct = (roi2_signal / detector_max) * 100
                    logger.info(
                        f"   ROI 2 (710-720nm): {roi2_signal:.0f} counts ({signal_pct:.1f}%)",
                    )
                ...
                logger.info(
                    f"   Target Range: {step4_min_signal:.0f} - {step4_max_signal:.0f} counts (80% ±2.5%)",
                )
                logger.info(
                    f"   Saturation Threshold: {detector_params.saturation_threshold:.0f} counts",
                )
                logger.info("")
                ...
                for ch in ch_list:
                    if ch in s_raw_data:
                        ...
                        if sat_count > 0:
                            ...
                            logger.error(
                                f"   ❌ {ch.upper()}: {sat_count} SAT pixels (max: {max_pixel:.0f}, {signal:.0f} counts, {signal_pct:.1f}%)",
                            )
                            saturated_channels_s.append(ch)
                        elif not in_range:
                            logger.warning(
                                f"   ⚠️ {ch.upper()}: OFF TARGET ({signal:.0f} counts, {signal_pct:.1f}%) - expected 77.5%-82.5%",
                            )
                            off_target_channels_s.append(ch)
                        else:
                            logger.info(
                                f"   ✅ {ch.upper()}: {signal:.0f} counts ({signal_pct:.1f}%) - No saturation, in range",
                            )

                logger.info("")
                if saturated_channels_s:
                    logger.error(
                        f"❌ STEP 4 SATURATION: Detected in {', '.join([ch.upper() for ch in saturated_channels_s])}",
                    )
                    logger.error("")
                    logger.error("   Attempting FINAL EMERGENCY RECOVERY...")
                    logger.error(
                        "   Reducing integration time to 50% of current value for saturated channels",
                    )
                    logger.error("")
                    ...
                    for ch in saturated_channels_s:
                        ...
                        logger.warning(
                            f"   {ch.upper()}: Reducing {ch_integration:.1f}ms → {emergency_int:.1f}ms (50% emergency reduction)",
                        )
                        ...
                        if emergency_spectrum is not None:
                            emergency_roi = emergency_spectrum[
                                wave_min_index:wave_max_index
                            ]
                            emergency_sat = count_saturated_pixels(
                                emergency_roi,
                                0,
                                len(emergency_roi),
                                detector_params.saturation_threshold,
                            )
                            emergency_signal = roi_signal(
                                emergency_roi,
                                0,
                                len(emergency_roi),
                                method="median",
                            ...
                            if emergency_sat == 0:
                                logger.info(
                                    f"   ✅ {ch.upper()}: Recovery successful - {emergency_signal:.0f} counts ({emergency_pct:.1f}%), 0 saturated pixels",
                                )
                                # Update stored data
                                ...
                                if use_per_channel_integration:
                                    ...
                                    step4_integration = (
                                        emergency_int  # Update common integration
                                    )
                                # Recalculate ROI signals
                                roi1_signal = np.mean(
                                    emergency_spectrum[roi1_min_idx:roi1_max_idx],
                                )
                                roi2_signal = np.mean(
                                    emergency_spectrum[roi2_min_idx:roi2_max_idx],
                                )
                                s_roi1_signals[ch] = roi1_signal
                                s_roi2_signals[ch] = roi2_signal
                            else:
                                logger.error(
                                    f"   ❌ {ch.upper()}: Recovery FAILED - still {emergency_sat} saturated pixels",
                                )
                                recovery_success = False
                        else:
                            logger.error(
                                f"   ❌ {ch.upper()}: Emergency acquisition failed",
                            )
                            recovery_success = False
                    ...
                    if not recovery_success:
                        logger.error(
                            "❌ STEP 4 CRITICAL FAILURE: Emergency recovery failed",
                        )
                        logger.error("")
                        logger.error(
                            "   ROOT CAUSE: LEDs are too bright even at minimum safe integration time",
                        )
                        logger.error(
                            "   IMPACT: Cannot proceed with saturated baseline data",
                        )
                        logger.error("")
                        logger.error("   RECOMMENDATIONS:")
                        logger.error("     1. Reduce LED intensities in Step 3")
                        logger.error(
                            "     2. Check detector saturation threshold is correct",
                        )
                        logger.error(
                            "     3. Verify sensor is not overexposed (check alignment)",
                        )
                        logger.error("")
                        ...
                        return result
                    logger.info(
                        "✅ EMERGENCY RECOVERY SUCCESSFUL: All channels recovered",
                    )
                    saturated_channels_s = []  # Clear saturation list

                elif off_target_channels_s:
                    logger.warning(
                        f"⚠️ STEP 4 WARNING: Off-target signals in {', '.join([ch.upper() for ch in off_target_channels_s])}",
                    )
                    logger.warning(
                        "   Signals outside 77.5%-82.5% range but no saturation detected",
                    )
                    logger.warning(
                        "   Continuing - may affect QC scores but data is usable",
                    )
                else:
                    logger.info(
                ...
                result.channel_integration_times = (
                    channel_s_integration_times
                    if use_per_channel_integration
                    else {ch: step4_integration for ch in ch_list}
                )
                ...
                logger.info("=" * 80)
                if use_per_channel_integration:
                    logger.info("   Using PER-CHANNEL integration times:")
                    for ch in ch_list:
                        if ch in channel_s_integration_times:
                            logger.info(
                                f"     {ch.upper()}: {channel_s_integration_times[ch]:.1f}ms",
                            )
                else:
                    logger.info(
                        f"   Common integration time: {step4_integration:.1f}ms",
                    )
                logger.info(f"   Scans per channel: {result.num_scans}")
                logger.info(
                    "   S-mode ROI signals captured for P-mode comparison in Step 5",
                )
                logger.info("")
                for ch in ch_list:
                    if ch in s_roi1_signals and ch in s_roi2_signals:
                        logger.info(
                            f"   {ch.upper()}: ROI1={s_roi1_signals[ch]:.0f}, ROI2={s_roi2_signals[ch]:.0f}",
                        )
                ...
                max_acquisition_rate_hz = (
                    1.0 / total_cycle_time_s if total_cycle_time_s > 0 else 0
                )

                logger.info("")
                logger.info("📊 Cycle Time Analysis (4 LEDs sequential):")
                logger.info(f"   Time per channel: {cycle_time_per_channel:.1f}ms")
                logger.info(
                    f"   Total cycle time: {total_cycle_time_ms:.1f}ms ({total_cycle_time_s:.3f}s)",
                )
                logger.info(
                    f"   Maximum acquisition rate: {max_acquisition_rate_hz:.2f} Hz",
                )
                ...
                if total_cycle_time_ms > MAX_CYCLE_TIME_MS:
                    ...
                    logger.warning(
                        f"   Current: {total_cycle_time_ms:.1f}ms > {MAX_CYCLE_TIME_MS:.0f}ms limit",
                    )
                    logger.warning(
                        f"   Acquisition rate: {max_acquisition_rate_hz:.2f} Hz < 1.0 Hz minimum",
                    )
                    logger.warning(
                        "   Consider reducing integration time or LED delays",
                    )
                    logger.warning("")
                    ...
                    logger.info(
                        f"   ✅ Cycle time within 1Hz constraint ({total_cycle_time_ms:.1f}ms ≤ {MAX_CYCLE_TIME_MS:.0f}ms)",
                    )
                ...
                for ch, data in s_raw_data.items():
                    logger.debug(
                        f"  {ch.upper()}: {{}}".format(
                            np.array2string(data, threshold=10, edgeitems=5),
                        ),
                    )
                logger.info(
                    "        Switch to P-polarization → Measure ROI signal loss → Optimize integration per channel",
                )
                logger.info("=" * 80)
                ...
                logger.info(
                    "  Expected behavior: Minimum transmission, strongest SPR absorption",
                )
                logger.info("")
                ...
                if set_result:
                    ...
                    logger.warning(
                        "  ⚠️ Controller response unexpected - servo should have moved to P position",
                    )
                    logger.warning(
                        "  ⚠️ Continuing calibration - servo movement may still be successful",
                    )
                ...
                logger.info("    ✅ P-mode set (position from device_config)")
                logger.info("")
                ...
                logger.info("")
                logger.info(
                    "  Calculating P-mode LED intensities (reduced from S-mode):",
                )
                p_led_intensities = {}
                ...
                for ch_name in ch_list:
                    ...
                    logger.info(
                        f"    {ch_name.upper()}: S={s_intensity} → P={p_intensity} ({P_MODE_REDUCTION_FACTOR:.0%} reduction)",
                    )

                logger.info("")
                logger.info(
                    "  Reason: P-mode has higher transmission, needs lower LED intensity",
                )
                logger.info(
                    "  50% reduction prevents saturation while maintaining good SNR",
                )
                logger.info("")
                ...
                logger.info(
                    f"  Measuring P-mode ROI signals with {p_integration_time:.1f}ms integration:",
                )
                ...
                for ch_name in ch_list:
                    ...
                    logger.info(
                        f"    {ch_name.upper()}: ROI1={roi1_signal:.0f}, ROI2={roi2_signal:.0f}",
                    )
                ...
                logger.info("")
                logger.info(
                    "  Calculating P-pol signal loss and initial integration estimate:",
                )
                ...
                for ch_name in ch_list:
                    if ch_name in s_roi2_signals and ch_name in p_roi2_signals:
                        ...
                        logger.info(
                            f"    {ch_name.upper()}: S={s_signal:.0f}, P={p_signal:.0f}, P/S={ratio:.3f} {status}",
                        )
                ...
                if not polarizer_rotation_ok:
                    ...
                    logger.error(
                        "     Expected: P-mode signal < S-mode signal (P/S ratio < 0.95)",
                    )
                    logger.error(
                        "     Possible causes: Servo not moving, wrong positions, hardware issue",
                    )
                    logger.error("")
                ...
                logger.info(
                    f"   Starting from P-mode baseline: {p_integration_time:.1f}ms",
                )
                logger.info("")
                ...
                for ch_name in ch_list:
                    ...
                    min_int = detector_params.min_integration_time
                    max_int = min(
                        p_integration_time * 3.0,
                        detector_params.max_integration_time,
                    )  # Allow up to 3× baseline
                    best_int = (
                        detector_params.min_integration_time
                    )  # Start with minimum as fallback
                    best_signal = 0
                    ...
                    if ever_saturated:
                        logger.warning(
                            f"    Channel {ch_name.upper()} hit saturation during search - verifying final integration is safe...",
                        )
                        verify_spectrum = acquire_raw_spectrum(
                            usb=usb,
                            ctrl=ctrl,
                    ...
                    logger.info(
                        f"  ✅ {ch_name.upper()}: Final {best_int:.1f}ms → {best_signal:.0f} ({final_pct:.1f}%)",
                    )
                    logger.info("")

                logger.info(
                    "✅ All channels optimized with per-channel P-mode integration times",
                )
                ...
                for ch_name in ch_list:
                    if ch_name in s_roi1_signals and ch_name in p_roi1_signals:
                        ...
                        if p_avg > s_avg * 1.05:
                            ...
                            logger.warning(
                                f"       ⚠️  P-pol signal INCREASED vs S-pol (P={p_avg:.0f} > S={s_avg:.0f})",
                            )

                # Store results
                result.p_integration_time = channel_integration_times.get(
                    ch_list[0],
                    p_integration_time,
                )  # Use first channel as reference
                result.channel_integration_times = (
                    channel_integration_times  # Per-channel integration times
                )
                ...
                if roi_increase_detected:
                    logger.warning("")
                    logger.warning(
                        "⚠️  QC FLAG: P-pol ROI signals INCREASED for some channels",
                    )
                    logger.warning(
                        f"   Affected channels: {', '.join([ch.upper() for ch in roi_increase_channels])}",
                    )
                    logger.warning(
                        "   This suggests polarizer misalignment or swapped S/P positions",
                    )
                    logger.warning(
                        "   Calibration will continue, but polarizer positions should be verified",
                    )
                    logger.warning("")
                ...
                logger.info("=" * 80)
                logger.info(
                    "✅ STEP 5 COMPLETE: P-mode Measurement with Per-Channel Integration Times",
                )
                logger.info("=" * 80)
                logger.info("   Per-channel optimized integration times:")
                for ch in ch_list:
                    if ch in channel_integration_times:
                        logger.info(
                            f"      {ch.upper()}: {channel_integration_times[ch]:.1f}ms",
                        )
                logger.info("")
                ...
                logger.info("")
                logger.info(
                    "  Capturing P-mode raw spectra with per-channel integration times:",
                )
                ...
                for ch_name in ch_list:
                    ...
                    if avg_spectrum is not None:
                        p_raw_data[ch_name] = avg_spectrum[
                            wave_min_index:wave_max_index
                        ]
                        logger.info(
                            f"    ✅ {ch_name.upper()}: {result.num_scans} scans averaged at {ch_int:.1f}ms",
                        )
                    else:
                        logger.warning(
                            f"    ⚠️  {ch_name.upper()}: No valid spectra captured",
                        )
                ...
                for ch_name in ch_list:
                    if ch_name in p_raw_data:
                        ...
                        if sat_count > 0:
                            max_pixel = np.max(spectrum)
                            logger.error(
                                f"    ❌ {ch_name.upper()}: {sat_count} saturated pixels (max: {max_pixel:.0f})",
                            )
                            ...
                            signal_pct = (signal / detector_max) * 100
                            logger.info(
                                f"    ✅ {ch_name.upper()}: No saturation ({signal:.0f} counts, {signal_pct:.1f}%)",
                            )

                if saturated_channels_p:
                    logger.error("")
                    logger.error(
                        f"❌ CALIBRATION FAILED: P-mode saturation detected in channels: {', '.join([ch.upper() for ch in saturated_channels_p])}",
                    )
                    logger.error("   The 80% target is too high for P-mode.")
                    logger.error(
                        "   Binary search failed to find non-saturated integration times.",
                    )
                    logger.error("")
                    raise RuntimeError(
                        f"P-mode saturation detected in Step 5 after optimization: {saturated_channels_p}",
                    )
                ...
                logger.info("")
                logger.info("  Turning off all LEDs for dark measurement...")
                ctrl.turn_off_channels()
                time.sleep(LED_DELAY)
                ...
                if hasattr(ctrl, "get_all_led_intensities"):
                    ...
                    leds_off = all(
                        intensity <= 1
                        for ch, intensity in led_status.items()
                        if ch != "d" or intensity >= 0
                    )
                    if leds_off:
                        logger.info(f"    ✅ All LEDs confirmed OFF: {led_status}")
                    else:
                        logger.error(
                            f"    ❌ CRITICAL: LEDs still ON during dark measurement: {led_status}",
                        )
                        logger.error("       Dark measurement will be invalid!")
                ...
                p_dark_ref_filtered = (
                    np.mean(dark_scans, axis=0)
                    if dark_scans
                    else np.zeros(wave_max_index - wave_min_index)
                )
                ...
                if EXPECTED_DARK_MIN <= dark_mean <= EXPECTED_DARK_MAX:
                    logger.info(
                        f"    ✅ Dark-ref within expected range ({EXPECTED_DARK_MIN}-{EXPECTED_DARK_MAX} counts)",
                    )
                    ...
                    logger.error(
                        f"    ❌ CRITICAL: Dark-ref outside expected range ({EXPECTED_DARK_MIN}-{EXPECTED_DARK_MAX} counts)",
                    )
                    if dark_mean > EXPECTED_DARK_MAX:
                        logger.error(
                            "       Possible causes: LEDs NOT OFF (most likely), light leak, detector issue",
                        )
                        logger.error(
                            f"       Expected dark ~200-800 counts, got {dark_mean:.0f} counts",
                        )
                    else:
                        logger.warning(
                            "       Possible causes: Detector offset drift, temperature change",
                        )
                ...
                logger.info("")
                logger.info(
                    "✅ Step 5 complete: P-mode measurement and dark-ref capture done\n",
                )
                ...
                try:
                    ...
                    logger.info(
                        "   ✅ Dark reference: From Step 5 (P-mode integration time)",
                    )
                    logger.info(
                        f"   ✅ S-mode integration time: {result.s_integration_time:.2f}ms",
                    )
                    ...
                    for ch in ch_list:
                        ...
                        logger.info(f"   Dip Wavelength: {qc['dip_wavelength']:.1f}nm")
                        logger.info(
                            f"   Min Transmission: {qc['transmission_min']:.1f}%",
                        )
                        logger.info(f"   Dip Depth: {qc['dip_depth']:.1f}%")
                        logger.info(
                            f"   Status: {'✅ DETECTED' if qc['dip_detected'] else '❌ WEAK/ABSENT'} (depth > 5%)",
                        ...
                        if qc["ratio"] is not None:
                            ...
                            if qc["orientation_correct"] is True:
                                ...
                                logger.error("   Status: ❌ INVERTED (P/S > 1.15)")
                                logger.error(
                                    "   ⚠️  CRITICAL: Polarizer appears INVERTED!",
                                )
                                logger.error(
                                    "   Expected: P-mode < S-mode (ratio < 0.95)",
                                )
                                logger.error(
                                    f"   Actual: P-mode > S-mode (ratio = {qc['ratio']:.3f})",
                                )
                        ...
                        channel_pass = (
                            qc["dip_detected"]
                            and fwhm_pass
                            and qc["orientation_correct"]
                            is not False  # Allow indeterminate but not inverted
                            and not qc["s_saturated"]
                            and not qc["p_saturated"]
                        )
                    ...
                    if orientation_issues and False:  # DISABLED FOR DEBUGGING
                        logger.warning("\n" + "=" * 80)
                        logger.warning(
                            "⚠️  POLARIZER INVERSION DETECTED - AUTO-RECALIBRATION DISABLED",
                        )
                        logger.warning("=" * 80)
                        logger.warning(
                            f"   Affected channels: {', '.join([ch.upper() for ch in orientation_issues])}",
                        )
                        logger.warning(
                            "   P/S ratio > 1.15 indicates positions are inverted",
                        )
                        logger.warning(
                            "   Auto-recalibration is currently DISABLED for debugging",
                        )
                        logger.warning("=" * 80)
                    ...
                    if False:  # Original code disabled
                        logger.warning("\n" + "=" * 80)
                        logger.warning(
                            "⚠️  POLARIZER INVERSION DETECTED - AUTO-RECALIBRATING",
                        )
                        logger.warning("=" * 80)
                        logger.warning(
                            f"   Affected channels: {', '.join([ch.upper() for ch in orientation_issues])}",
                        ...
                        logger.warning("")
                        logger.warning(
                            "   🔄 Starting automatic servo recalibration...",
                        )
                        logger.warning("=" * 80)

                        try:
                            # Get polarizer type from device config
                            polarizer_type = device_config.config["hardware"].get(
                                "polarizer_type",
                                "round",
                            )
                            ...
                            logger.info(
                                f"Using channel A LED intensity: {result.s_mode_intensity.get('a', 255)}",
                            )

                            # Import servo calibration function
                            from utils.servo_calibration import (
                                auto_calibrate_polarizer,
                            )
                            ...
                            logger.info(
                                f"Setting channel {cal_ch.upper()} to LED={cal_led} for servo calibration",
                            )
                            ...
                            if recal_result and recal_result.get("success"):
                                ...
                                logger.info(
                                    f"   S/P ratio: {recal_result['sp_ratio']:.2f}×",
                                )
                                logger.info(
                                    f"   Dip depth: {recal_result['dip_depth_percent']:.1f}%",
                                )
                                ...
                                if recal_result["sp_ratio"] < 1.0:
                                    logger.warning("=" * 80)
                                    logger.warning(
                                        "⚠️  DETECTED: Positions are labeled backwards!",
                                    )
                                    logger.warning(
                                        f"   S/P ratio = {recal_result['sp_ratio']:.2f} < 1.0",
                                    )
                                    logger.warning(
                                        "   This means 'S' position has HIGHER signal than 'P' position",
                                    )
                                    logger.warning(
                                        "   SWAPPING positions to correct labeling...",
                                    )
                                    logger.warning("=" * 80)
                                    # Swap the positions
                                    new_s_pos, new_p_pos = new_p_pos, new_s_pos
                                    logger.info(
                                        f"   Corrected S position: {new_s_pos}°",
                                    )
                                    logger.info(
                                        f"   Corrected P position: {new_p_pos}°",
                                    )
                                ...
                                logger.error("=" * 80)
                                logger.error(
                                    "❌ CRITICAL ERROR: Position correction attempted during calibration",
                                )
                                logger.error(
                                    "❌ Servo positions are IMMUTABLE - set at controller initialization",
                                )
                                logger.error(
                                    "❌ To fix positions: Update device_config.json and RESTART application",
                                )
                                logger.error("=" * 80)
                                ...
                                logger.error(
                                    "   Action required: Update device_config.json with correct positions and restart",
                                )
                                raise ValueError(
                                    "Servo position correction not allowed during calibration - update device_config and restart",
                                )
                            ...
                            result.qc_results["orientation_check"] = {
                                ...
                                "message": f"Auto-recalibration error: {e!s}",
                            }
                    ...
                    if all_channels_pass:
                        ...
                        for ch, qc in qc_results.items():
                            if not qc["overall_pass"]:
                                ...
                                if not qc["spr_pass"]:
                                    logger.warning(
                                        f"      - SPR dip too shallow ({qc['spr_depth']:.1f}%)",
                                    )
                                if not qc["fwhm_pass"]:
                                    logger.warning(
                                        f"      - FWHM too wide ({qc['fwhm']:.1f}nm)",
                                    )
                                if not qc["snr_pass"]:
                                    logger.warning(
                                        f"      - SNR too low ({qc['snr']:.0f})",
                                    )
                                if qc.get("orientation_correct") is False:
                                    logger.error(
                                        f"      - Polarizer INVERTED (P/S ratio = {qc['p_s_ratio']:.3f})",
                                    )
                                if qc.get("s_saturated") or qc.get("p_saturated"):
                                    logger.error("      - Saturation detected")
                ...
                if str(e) == "RESTART_FROM_STEP_3C":
                    ...
                    if restart_attempt > max_restart_attempts:
                        ...
                        logger.error("=" * 80)
                        logger.error(
                            "   First attempt: Auto-recalibrated servo positions",
                        )
                        logger.error(
                            "   Second attempt: Still detecting inverted polarization",
                        )
                        logger.error("")
        ...
        has_critical_warning = (
            orientation_check.get("status") == "FAIL_AFTER_RECALIBRATION"
        )
        ...
        if has_critical_warning:
            ...
            logger.warning("=" * 80)
            logger.warning(
                "   Polarizer inversion persisted after automatic recalibration",
            )
            logger.warning(
                "   Calibration completed but optical performance may be compromised",
            )
            logger.warning("")
            logger.warning("   REQUIRED USER ACTIONS:")
            logger.warning(
                "   1. Verify servo motor physically moves during mode switch",
            )
            logger.warning("   2. Check for mechanical obstructions")
            logger.warning(
                "   3. Verify polarizer type in device config (circular vs barrel)",
            )
            logger.warning(
                "   4. Test servo manually: python -m utils.servo_calibration --test",
            )
        ...
        try:
            ...
            print(f"ERROR EXCEPTION: {type(e).__name__}: {e!r}")
...
def run_fast_track_calibration(
    ...
    logger.debug(
        f"   pre_led_delay_ms={'MISSING' if 'pre_led_delay_ms' not in locals() else locals().get('pre_led_delay_ms', 'UNDEFINED')}",
    )
    logger.debug(
        f"   post_led_delay_ms={'MISSING' if 'post_led_delay_ms' not in locals() else locals().get('post_led_delay_ms', 'UNDEFINED')}",
    )
    ...
    try:
        ...
        if not cal_data or "s_mode_intensities" not in cal_data:
            logger.info(
                "No calibration in device_config.json, checking latest_calibration.json...",
            )
            try:
                ...
                if latest_file.exists():
                    ...
                    cal_data = {
                        "calibration_date": latest_data["calibration_metadata"][
                            "timestamp"
                        ],
                        "calibration_method": latest_data["calibration_metadata"][
                            "calibration_method"
                        ],
                        "s_mode_intensities": latest_data["led_parameters"][
                            "s_mode_intensity"
                        ],
                        "p_mode_intensities": latest_data["led_parameters"][
                            "p_mode_intensity"
                        ],
                        "integration_time_ms": latest_data["integration_times"][
                            "s_integration_time"
                        ],
                        "p_integration_time_ms": latest_data["integration_times"][
                            "p_integration_time"
                        ],
                        "per_channel_integration_times": latest_data[
                            "integration_times"
                        ].get("channel_integration_times"),
                    }
        ...
        if not cal_data or "s_mode_intensities" not in cal_data:
            logger.info(
                "No previous calibration found - falling back to full calibration",
            )
        ...
        logger.info(
            f"Previous calibration date: {cal_data.get('calibration_date', 'unknown')}",
        )
        ...
        for ch in ch_list:
            ...
            if deviation <= tolerance:
                ...
                logger.info(
                    f"✅ {ch.upper()}: PASS (signal={max_signal:.0f}, target={target_counts:.0f}, Δ={deviation*100:.1f}%)",
                )
            else:
                failed_channels.append(ch)
                s_passed = False
                logger.warning(
                    f"❌ {ch.upper()}: FAIL (signal={max_signal:.0f}, target={target_counts:.0f}, Δ={deviation*100:.1f}%)",
                )

        logger.info(
            f"S-mode result: {'✅ ALL PASS' if s_passed else '❌ SOME FAILED'}\n",
        )
        ...
        if s_passed and saved_p_leds:
            ...
            logger.info(
                f"P-mode result: {'✅ ALL PASS' if p_passed else '❌ SOME FAILED'}\n",
            )
        elif not s_passed:
            logger.info("PHASE 2: P-mode Validation - SKIPPED (S-mode failed)\n")
        else:
            logger.warning(
                "PHASE 2: P-mode Validation - SKIPPED (no saved P-mode LEDs)\n",
            )
            p_passed = False
        ...
        if s_passed and p_passed and len(failed_channels) == 0:
            ...
            logger.info(
                f"✅ Dark noise: mean={np.mean(result.dark_noise):.1f}, std={np.std(result.dark_noise):.2f}\n",
            )
            ...
            for ch in ch_list:
                ...
                if raw_spectrum is not None:
                    ...
                    clean_spectrum = preprocessor.process_single(
                        raw_spectrum,
                        wave_min_index,
                        wave_max_index,
                    )
            ...
            for ch in ch_list:
                if ch in result.s_pol_ref and ch in result.p_pol_ref:
                    result.transmission[ch] = (
                        transmission_processor.calculate_transmission(
            ...
            logger.info("=" * 80)
            logger.info("Validation: S + P modes passed (±5% tolerance)")
            logger.info("Dark noise measured, reference spectra captured")
            logger.info("Ready for QC dialog and live data acquisition")
            logger.info("Time saved: ~90% vs full calibration")
            logger.info("=" * 80 + "\n")
        ...
        result.s_integration_time = (
            saved_integration  # LOCKED from full calibration (S-mode)
        )
        result.wave_data = wave_data[wave_min_index:wave_max_index]
        result.wave_min_index = wave_min_index
        result.wave_max_index = wave_max_index

        num_scans = calculate_scan_counts(
            saved_integration,
        ).num_scans  # LOCKED (derived from integration)
        ...
        result.leds_calibrated = (
            result.p_mode_intensity
        )  # For compatibility with data_mgr
        result.pre_led_delay_ms = pre_led_delay_ms
        result.post_led_delay_ms = post_led_delay_ms
...
def run_convergence_calibration_steps_3_to_5(
    ...
    Returns:
                ...
                - p_raw_roi: dict {ch: ndarray} - P-mode RAW spectra (ROI only)
                    (compat: also returns legacy keys 's_pol_ref'/'p_pol_ref' mapped to the same raw ROI)
    ...
    try:
        ...
        all_channels_dark = all(
            channel_measurements[ch][0] < MIN_LED_COUNTS for ch in ch_list
        )
        if all_channels_dark:
            result["error"] = (
                f"❌ LED VERIFICATION FAILED: All channels produced <{MIN_LED_COUNTS} counts at LED=255. Check LED hardware/connections!"
            )
            logger.error("")
            logger.error("=" * 80)
            ...
            for ch in ch_list:
                logger.error(
                    f"   {ch.upper()}: {channel_measurements[ch][0]:.0f} counts (expected >{MIN_LED_COUNTS})",
                )
            logger.error("=" * 80)
            logger.error("")
            return result
        ...
        weak_leds = [
            ch for ch in ch_list if channel_measurements[ch][0] < MIN_LED_COUNTS
        ]
        if weak_leds:
            result["error"] = "❌ LED VERIFICATION FAILED: Weak/dead LEDs detected"
            logger.error("")
            logger.error("=" * 80)
            logger.error(result["error"])
            for ch in weak_leds:
        ...
        logger.info("")
        logger.info(
            f"   ✅ LED VERIFICATION PASSED - All channels producing >{MIN_LED_COUNTS} counts",
        )
        logger.info(
            f"   Weakest: {weakest_ch.upper()} (mean={channel_measurements[weakest_ch][0]:.0f})",
        )
        logger.info("")
        ...
        logger.info(
            f"   Has servo_move_calibration_only(): {hasattr(ctrl, 'servo_move_calibration_only')}",
        )
        ...
        if hasattr(ctrl, "_ser") and ctrl._ser is not None:
            try:
                serial_ok = (
                    ctrl._ser.is_open if hasattr(ctrl._ser, "is_open") else False
                )
        ...
        if not serial_ok:
            logger.error(
                "🚨 CRITICAL: Serial port NOT open - servo commands will FAIL!",
            )
        if not connected:
            logger.error(
                "🚨 CRITICAL: Controller NOT connected - servo commands will FAIL!",
            )
        ...
        # Get servo positions from device config
        servo_positions = load_oem_polarizer_positions_local(
            device_config_det,
        ...
        if hasattr(ctrl, "servo_move_calibration_only"):
            ...
            if move_ok:
                logger.info(
                    f"   ✅ Moved to S={s_pos}°, P={p_pos}° - Did you HEAR it move?",
                )
            else:
                logger.warning("   ⚠️  Move command did not confirm (continuing anyway)")
        else:
            logger.warning(
                "   ⚠️  Controller lacks servo_move_calibration_only - skipping explicit pre-positioning",
            )
        ...
        if s_shared_int is not None:
            ...
            try:
                ...
                result["s_integration_time"] = (
                    float(_np.median(list(s_channel_times.values())))
                    if s_channel_times
                    else None
                )
            except Exception:
        ...
        logger.info("")
        logger.info("   ✅ S-mode convergence complete:")
        ...
        try:
            ...
            means = {
                ch: float(_np.mean(result["s_raw_roi"][ch]))
                for ch in result["s_raw_roi"]
            }
            logger.info(
                "   🔍 S-mode capture means (ROI): "
        ...
        if hasattr(ctrl, "servo_move_calibration_only"):
            ...
            logger.warning(
                "   ⚠️  Controller lacks servo_move_calibration_only - using simple mode switch",
            )
        ...
        logger.info("   Starting from S-mode LED values")
        logger.info("")
        ...
        logger.info("")
        logger.info("   ✅ P-mode convergence complete:")
        ...
        try:
            ...
            means = {
                ch: float(_np.mean(result["p_raw_roi"][ch]))
                for ch in result["p_raw_roi"]
            }
            logger.info(
                "   🔍 P-mode capture means (ROI): "
                + ", ".join([f"{k.upper()}={v:.0f}" for k, v in means.items()]),
            )
        except Exception as _e:

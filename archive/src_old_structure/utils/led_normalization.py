...
class IntensityCalculator(ABC):
    ...
    @abstractmethod
    def calculate(self, spectrum: np.ndarray, wavelengths: Optional[np.ndarray] = None) -> float:
...
class MeanIntensityCalculator(IntensityCalculator):
    ...
    def calculate(self, spectrum: np.ndarray, wavelengths: Optional[np.ndarray] = None) -> float:
        ...
        mask = (wavelengths >= self.wavelength_min) & (wavelengths <= self.wavelength_max)
...
class IntegratedIntensityCalculator(IntensityCalculator):
    ...
    def calculate(self, spectrum: np.ndarray, wavelengths: Optional[np.ndarray] = None) -> float:
        ...
        mask = (wavelengths >= self.wavelength_min) & (wavelengths <= self.wavelength_max)
        return float(np.trapz(spectrum[mask], wavelengths[mask])) if np.any(mask) else 0.0
...
class LEDNormalizer:
    ...
    def __init__(self, 
                 ...
                 intensity_calculator: Optional[IntensityCalculator] = None):
        """
        ...
        logger.info(f"LED Normalizer initialized with {len(self.led_channels)} channels")
    ...
    def rank_leds(self, 
                  ...
                  settling_time: float = 0.2) -> Tuple[Dict[str, float], float]:
        """
        ...
        """
        logger.info(f"Ranking LEDs at intensity {test_intensity}, integration {integration_time}ms...")
    ...
    def normalize(self,
                  ...
                  settling_time: float = 0.2) -> Dict[str, Dict]:
        """
        ...
        if mode == 'intensity':
            return self._normalize_by_intensity(
                target_value, tolerance, max_iterations, settling_time
            )
        elif mode == 'time':
    ...
    def _normalize_by_intensity(self,
                                target_count: float,
                                tolerance: float,
                                max_iterations: int,
                                settling_time: float) -> Dict[str, Dict]:
        ...
        for led in self.led_channels:
            ...
            logger.info(f"    Result: intensity={best_intensity}, "
                       f"count={best_count:.0f}, error={best_error:.0f}")
        
        return results
    
    def _normalize_by_time(self,
                           target_count: float,
                           tolerance: float,
                           max_iterations: int,
                           settling_time: float) -> Dict[str, Dict]:
        ...
        for led in self.led_channels:
            ...
            logger.info(f"    Result: time={best_time}ms, "
                       f"count={best_count:.0f}, error={best_error:.0f}")
    ...
    def _binary_search(self,
                       ...
                       tolerance: float,
                       max_iterations: int,
                       settling_time: float) -> Tuple[int, float, float]:
        """
    ...
    def apply_normalization(self, results: Dict[str, Dict], led: str):
        ...
        logger.debug(f"Applied normalization for LED {led}: "

...
def initialize_device_on_connection(usb_device: Any) -> Optional[Path]:
    ...
    try:
        ...
        logger.info(f"✅ Device initialized: {serial_number}")
...
def check_and_request_optical_calibration() -> bool:
    ...
    try:
        ...
        if device_manager.current_device_serial is None:
            logger.warning("⚠️ No device set - cannot check optical calibration")
            return False
        ...
        logger.error(f"❌ Failed to check optical calibration status: {e}")
        return False
...
def get_device_optical_calibration_path() -> Optional[Path]:
    ...
    try:
        ...
        if device_manager.current_device_serial is None:
            logger.warning("⚠️ No device set - cannot get optical calibration path")
            return None

        return device_manager.get_optical_calibration_path()

    except Exception as e:
        logger.error(f"❌ Failed to get optical calibration path: {e}")
        return None
...
def save_optical_calibration_result(calibration_file_path: Path) -> bool:
    ...
    try:
        ...
        if device_manager.current_device_serial is None:
            logger.error("❌ No device set - cannot save optical calibration result")
            return False
        ...
        logger.error(f"❌ Failed to save optical calibration result: {e}")
        return False
...
def get_device_config_dict() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if device_manager.current_device_serial is None:
            logger.warning("⚠️ No device set - cannot get device config")
            return None
...
4. DATA ACQUISITION INITIALIZATION
   ...
   if optical_cal_path and optical_cal_path.exists():
       ...
       logger.info(f"✅ Loaded device-specific afterglow correction")
   else:

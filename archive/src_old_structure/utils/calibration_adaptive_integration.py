...
def run_adaptive_integration_calibration(
    ...
    try:
        ...
        ctrl.turn_off_channels()
        time.sleep(0.2)
        logger.info("✅ All LEDs OFF (verified by timing)\n")
        ...
        if wave_data is None:
            raise RuntimeError("Failed to read wavelength calibration")
        ...
        logger.info(f"✅ Detector: {detector_params.name}")
        ...
        logger.info(f"✅ Channels to calibrate: {[c.upper() for c in ch_list]}\n")
        ...
        logger.info("=" * 80)
        logger.info("⚠️  SKIPPED: Mode 2 uses fixed LED=255 for all channels")
        ...
        for ch_idx, ch in enumerate(ch_list, 1):
            ...
            for scan_idx in range(scan_config.ref_scans):
                spectrum = usb.read_intensity()
        ...
        # Average dark scans
        dark_spectra = []
        for scan_idx in range(scan_config.dark_scans):
            if stop_flag and stop_flag.is_set():
                raise RuntimeError("Calibration cancelled by user")
        ...
        if len(dark_spectra) == 0:
            raise RuntimeError("Failed to capture dark noise spectra")
        ...
        for ch in ch_list:
            ...
            logger.info(f"   ✅ {ch.upper()}: S-pol and P-pol processed")

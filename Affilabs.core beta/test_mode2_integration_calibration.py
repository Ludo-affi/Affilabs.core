...
def find_controller():
    ...
    try:
        ...
        ctrl = PicoP4SPR()
        if ctrl.open():
            logger.info(f"✅ Found PicoP4SPR controller")
            return ctrl
        ctrl.close()
    ...
    try:
        ...
        if ctrl.open():
            logger.info(f"✅ Found Arduino controller")
            return ctrl
        ctrl.close()
    except Exception as e:
...
def find_spectrometer():
    ...
    try:
        ...
        if usb.open():
            ...
            logger.info(f"✅ Found {model_name} spectrometer (S/N: {usb.serial_number})")
            return usb
    except Exception as e:
...
def calibrate_integration_per_channel_test(usb, ctrl, ch, led_intensity=255, target_counts=None):
    ...
    while current_count < target_counts and integration < max_integration_allowed:
        ...
        int_array = usb.read_intensity()
        if int_array is None:
            logger.error(f"❌ Read failed during optimization")
            return None
    ...
    logger.info(f"✓ Timing budget (< 100ms): {'OK ✅' if timing_budget_ok else 'EXCEEDED ⚠️'}")
...
def analyze_results(results):
    ...
    logger.info(f"  Timing budget status: {'OK ✅' if global_int < 100 else 'EXCEEDED ⚠️'}")
...
def test_led_delay_impact(usb, ctrl):
    ...
    logger.info(f"  This is {'SUFFICIENT ✅' if LED_DELAY >= 0.02 else 'TOO SHORT ⚠️'} for Mode 2")
            ...
            if len(early_data) >= 3:
                ...
                if n >= 2 and all(y > 0 for y in y_vals):
                    ...
                    if 5 < tau_ms < 200:  # Physically reasonable for LED phosphors
                        ...
                        if residual_pct > 1:
                            logger.info(f"     ⚠️ Significant residual - afterglow correction RECOMMENDED")
                        else:
                            logger.info(f"     ✅ Low residual - afterglow correction optional")
                    else:

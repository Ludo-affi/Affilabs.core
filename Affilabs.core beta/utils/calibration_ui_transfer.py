...
class PostCalibrationDialog(QDialog):
    ...
    def __init__(self, calibration_result, parent=None):
    ...
    def setup_ui(self):
        ...
        # Title
        title = QLabel("✅ Calibration Successful!")
        title_font = QFont()
        ...
        s_leds_text = ", ".join([f"{ch.upper()}={val}" for ch, val in self.calibration_result.ref_intensity.items()])
        ...
        if self.calibration_result.p_mode_intensity:
            ...
            p_leds_text = ", ".join([f"{ch.upper()}={val}" for ch, val in self.calibration_result.p_mode_intensity.items()])
            summary_layout.addWidget(QLabel(p_leds_text), row, 1)
            row += 1

        # Integration time
        summary_layout.addWidget(QLabel("Integration Time:"), row, 0)
        summary_layout.addWidget(QLabel(f"{self.calibration_result.integration_time} ms"), row, 1)
        row += 1
        ...
        method_text = self.calibration_result.calibration_method if hasattr(self.calibration_result, 'calibration_method') else "standard"
        ...
        log_lines.append(f"Success: {'Yes' if self.calibration_result.success else 'No'}\n")
...
def transfer_calibration_to_live_view(
    ...
    try:
        ...
        logger.info(f"✅ LED intensities configured")
        ...
        logger.info(f"✅ Integration time: {calibration_result.integration_time}ms")
        logger.info(f"✅ Scans per channel: {calibration_result.num_scans}")
        ...
        logger.info(f"✅ Reference signals and dark noise configured")

        # Set wavelength data
        data_acquisition_manager.set_wavelength_data(
            calibration_result.wave_data
        )
        logger.info(f"✅ Wavelength data configured ({len(calibration_result.wave_data)} pixels)")

        # If using alternative calibration (per-channel integration)
        if hasattr(calibration_result, 'per_channel_integration') and calibration_result.per_channel_integration:
            ...
            logger.info(f"✅ Per-channel integration: {calibration_result.per_channel_integration}")

        # Mark system as calibrated
        data_acquisition_manager.set_calibration_state(True)

        logger.info("\n" + "=" * 80)
        logger.info("✅ TRANSFER COMPLETE - LIVE VIEW READY")
        logger.info("=" * 80)
...
def save_calibration_to_device_config(
    ...
    cal_data = {
        ...
        # QC metrics
        's_ref_qc': calibration_result.s_ref_qc if hasattr(calibration_result, 's_ref_qc') else {},
        'verification': calibration_result.verification if hasattr(calibration_result, 'verification') else {},
    ...
    if hasattr(calibration_result, 'per_channel_integration') and calibration_result.per_channel_integration:
        cal_data['per_channel_integration_times'] = calibration_result.per_channel_integration
        logger.info("Including per-channel integration times (Global LED Mode)")

    # Add per-channel dark noise if present
    if hasattr(calibration_result, 'per_channel_dark_noise') and calibration_result.per_channel_dark_noise:
        cal_data['per_channel_dark_noise'] = {
            ch: dark.tolist() for ch, dark in calibration_result.per_channel_dark_noise.items()
        }
    ...
    if cal_data.get('per_channel_integration_times'):
        logger.info(f"   Per-channel integration: {cal_data['per_channel_integration_times']}")
...
def check_and_run_afterglow_calibration(
    ...
    try:
        ...
        if afterglow_data and 'channel_models' in afterglow_data:
            logger.info("✅ Afterglow calibration found in device config")
            logger.info(f"   Calibrated channels: {list(afterglow_data['channel_models'].keys())}")
            logger.info(f"   Calibration date: {afterglow_data.get('calibration_date', 'unknown')}")
            logger.info("   No need to recalibrate\n")
            return True

        logger.warning("⚠️ Afterglow calibration NOT found in device config")
        ...
        if afterglow_result and afterglow_result.get('success'):
            logger.info("\n✅ Automatic afterglow calibration complete")
            logger.info("   Results saved to device config")
            logger.info("=" * 80 + "\n")
            return True
        else:
            logger.error("\n❌ Automatic afterglow calibration failed")

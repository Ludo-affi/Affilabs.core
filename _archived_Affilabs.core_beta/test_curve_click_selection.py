...
def test_click_handler_logic():
    ...
    assert curves[selected_idx].setPen.call_args[0][0] == curves[selected_idx].selected_pen, \
    ...
    for i in [0, 1, 3]:
        ...
        assert curves[i].setPen.call_args[0][0] == curves[i].original_pen, \
...
def test_pen_width_difference():
    ...
    assert selected_width > original_width, "Selected pen should be thicker than original"
...
def run_all_tests():
    ...
    try:
        ...
        print("2. Only cycle_of_interest graph curves are clickable (show_delta_spr=True)")

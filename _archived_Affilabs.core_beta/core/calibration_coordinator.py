...
class CalibrationCoordinator(QObject):
    ...
    def _on_calibration_complete(self, calibration_data: Dict[str, Any]) -> None:
        ...
        logger.info(f"   Orientation validation: {list(getattr(data_mgr, 'orientation_validation', {}).keys())}")
        ...
        if data_mgr.ref_sig:
            for ch in ['a', 'b', 'c', 'd']:
                if ch in data_mgr.ref_sig:
                    logger.info(f"   S-pol Ch {ch.upper()}: {data_mgr.ref_sig[ch][:5] if len(data_mgr.ref_sig[ch]) > 0 else 'EMPTY'} ... (mean={np.mean(data_mgr.ref_sig[ch]):.0f})")
        if getattr(data_mgr, 'p_ref_sig', {}):
            for ch in ['a', 'b', 'c', 'd']:
                if ch in getattr(data_mgr, 'p_ref_sig', {}):
                    logger.info(f"   P-pol Ch {ch.upper()}: {data_mgr.p_ref_sig[ch][:5] if len(data_mgr.p_ref_sig[ch]) > 0 else 'EMPTY'} ... (mean={np.mean(data_mgr.p_ref_sig[ch]):.0f})")

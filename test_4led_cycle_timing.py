...
def test_4led_cycle():
    ...
    try:
        ...
        print(f"  • SNR: √4 = 2.0x improvement")
        ...
        if avg_led_time <= 180:
            ...
            print(f"  • Cycle time: {avg_cycle_time/1000:.2f} seconds")
            print(f"  • Rate: {cycles_per_second:.2f} cycles/second")
            print(f"  • SNR: √4 = 2.0x improvement (good for SPR)")
        else:
            print(f"\n⚠ 4 scans takes {avg_led_time:.1f}ms (target: 180ms)")
            print(f"  • Consider reducing to 3 scans")
            print(f"  • Or accept slightly longer cycle time")

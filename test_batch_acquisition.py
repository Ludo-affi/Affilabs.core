...
def test_batch_optimization():
    ...
    try:
        ...
        if speedup > 1.1:
            ...
            print(f"  • Replace individual read loops with read_intensity_batch()")
            print(f"  • Update acquisition manager to use batch method")
        else:
            print(f"\n  ⚠ Batch method only {percent_faster:.1f}% faster")
            print(f"  • USB overhead may not be the bottleneck")
            print(f"  • Consider reducing integration time or number of scans")

...
try:
    ...
    if main_window.live_data_enabled:
        ...
        if (hasattr(main_window.full_timeline_graph, 'stop_cursor') and
            main_window.full_timeline_graph.stop_cursor is not None):
...
try:
    ...
    if hasattr(main_window, 'recording_mgr'):
        ...
        print(f"    is_recording = {main_window.recording_mgr.is_recording if main_window.recording_mgr else 'N/A'}")
    else:

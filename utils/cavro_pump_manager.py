...
class CavroPumpManager(QObject):
    ...
    def move_to_position(
        ...
        if not 0 <= position_steps <= STEPS_PER_STROKE:
            logger.error(f"Invalid position: {position_steps} (must be 0-{STEPS_PER_STROKE})")
            return False
    ...
    def start_flow(
        ...
        if result:
            pump_state = self.pumps[address]
            pump_state.flow_rate_ml_per_min = rate_ml_per_min if direction_forward else -rate_ml_per_min
            pump_state.is_running = True
            self.pump_state_changed.emit(address, f"Running at {rate_ml_per_min} ml/min")
    ...
    def set_flow_rate(
        ...
        if result:
            ...
            logger.debug(f"Pump {address:#x} flow rate changed to {rate_ml_per_min} ml/min")
    ...
    def is_busy(self, address: int) -> bool:
        ...
        if response:
            try:
                ...
                status = response[0] if isinstance(response[0], int) else ord(response[0])
            ...
            try:
                ...
                if address in self.pumps:
                    ...
                    if error_code != PumpError.NO_ERROR:
                        ...
                        error_name = PumpError(error_code).name if error_code in PumpError.__members__.values() else "UNKNOWN"
                        logger.warning(f"Pump {address:#x} error: {error_name} ({error_code:#x})")
    ...
    async def regenerate_sequence(
        ...
        try:
            ...
            # Progress tracking
            total_time_ms = int(67_000 + 1125 * contact_time)
    ...
    def log_pump_states(self) -> None:
        ...
        for address, pump_state in self.pumps.items():

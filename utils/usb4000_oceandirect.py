...
try:
    ...
    logger.info("Using SeaBreeze (pyseabreeze backend) for Ocean Optics devices")
    logger.info("Requirements: WinUSB driver via Zadig, libusb-1.0.dll in System32")
    ...
    logger.error("Ensure libusb-1.0.dll is in C:\\Windows\\System32\\")
...
class USB4000OceanDirect:
    ...
    def acquire_spectrum(self) -> np.ndarray | None:
        ...
        try:
            ...
            # Acquire spectrum using SeaBreeze (ONLY supported backend)
            if BACKEND_TYPE != "seabreeze":
                raise RuntimeError("Only SeaBreeze backend is supported")

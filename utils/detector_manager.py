...
class DetectorProfile:
    ...
    integration_step_ms: (
        float  # Step size for integration time adjustments during calibration
    )
...
class DetectorManager:
    ...
    def __init__(self):
        ...
        self.current_profile: DetectorProfile | None = None
    ...
    def _load_all_profiles(self) -> None:
        ...
        if not self.PROFILES_DIR.exists():
            logger.warning(
                f"Detector profiles directory not found: {self.PROFILES_DIR}",
            )
        ...
        for profile_file in self.PROFILES_DIR.glob("*.json"):
            try:
                ...
                profile_key = f"{profile.manufacturer}_{profile.model}".lower().replace(
                    " ",
                    "_",
                )
                self.profiles[profile_key] = profile
                logger.info(
                    f"✅ Loaded detector profile: {profile.manufacturer} {profile.model}",
                )
    ...
    def _load_profile(self, profile_path: Path) -> DetectorProfile:
        ...
        return DetectorProfile(
            ...
            recommended_integration_time_ms=acquisition_limits[
                "recommended_integration_time_ms"
            ],
            integration_step_ms=acquisition_limits.get(
                "integration_step_ms",
                1.0,
            ),  # Default 1.0ms if not specified
            # Calibration targets
            ...
            led_characterization_points=calibration_targets[
                "led_characterization_points"
            ],
            max_calibration_iterations=calibration_targets[
    ...
    def auto_detect(self, usb_device: Any) -> DetectorProfile | None:
        ...
        try:
            ...
            elif hasattr(usb_device, "_device") and hasattr(
                usb_device._device,
                "model",
            ):
                device_model = usb_device._device.model

            logger.info("🔍 Detecting profile:")
            if serial_number:
            ...
            for profile_key, profile in self.profiles.items():
                ...
                if (
                    serial_number
                    and profile.auto_detect_string.upper() in serial_number.upper()
                ):
                    logger.info(
                        f"✅ Matched by serial number: {profile.manufacturer} {profile.model}",
                    )
                ...
                if (
                    device_model
                    and profile.auto_detect_string.lower() in device_model.lower()
                ):
                    logger.info(
                        f"✅ Matched by model string: {profile.manufacturer} {profile.model}",
                    )
                    self.current_profile = profile
                    return profile

            # No match found
            logger.warning("⚠️ No profile matched")
            if serial_number:
    ...
    def _get_default_profile(self) -> DetectorProfile | None:
    ...
    def get_profile(self, manufacturer: str, model: str) -> DetectorProfile | None:
    ...
    def get_current_profile(self) -> DetectorProfile | None:
...
def get_current_detector_profile() -> DetectorProfile | None:
    """Get currently active detector profile."""

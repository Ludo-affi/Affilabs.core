...
class DataIOManager:
    ...
    def load_temperature_log(self, file_path: str) -> Optional[dict[str, list]]:
        """
    ...
    def save_kinetic_log(
        ...
        try:
            if rec_dir is None:
                logger.warning(f"No recording directory specified for kinetic log {channel}")
                return False
            ...
            logger.info(f"Kinetic log Ch {channel_letter} saved: {file_path} ({num_entries} entries)")
            return True
    ...
    def save_spr_channel_data(
        ...
        """
        try:
            with Path(file_path).open("w", newline="", encoding=self.encoding) as txtfile:
                ...
                # Write data
                for time_val, wavelength in zip(times, wavelengths):
                    writer.writerow([f"{time_val:.2f}", f"{wavelength:.4f}"])
            
            logger.debug(f"SPR channel {channel} data saved: {file_path} ({len(times)} points)")
            return True
    ...
    def save_spr_processed_data(
        ...
        try:
            ...
            logger.info(f"Processed SPR data saved: {file_path} ({len(times)} points, {len(channels)} channels)")
            return True
    ...
    def save_kinetic_parameters(
        ...
        try:
            with Path(file_path).open("w", newline="", encoding=self.encoding) as txtfile:
                fieldnames = ["Segment", "ka (1/Ms)", "kd (1/s)", "KD (M)", "R_max", "Chi²"]
                writer = csv.DictWriter(
                    txtfile,
                    dialect=self.csv_dialect,
                    fieldnames=fieldnames,

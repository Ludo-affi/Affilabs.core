...
class ValveState:
    ...
    def position(self) -> ValvePosition:
        ...
        if self.six_port_position == 0 and self.three_way_position == 1:
            return ValvePosition.LOAD
        if self.six_port_position == 1 and self.three_way_position == 0:
...
class KineticManager(QObject):
    ...
    def start_injection(
        ...
        try:
            ...
            self.channels[
                channel
            ].valve.last_injection_timestamp = dt.datetime.now().strftime("%H:%M:%S")
            ...
            if auto_timeout and flow_rate > 0:
                timeout_minutes = (
                    INJECTION_BASE_TIME_SEC / 60.0
                ) / flow_rate + INJECTION_SAFETY_MARGIN_MIN
                timeout_ms = int(timeout_minutes * 60 * 1000)  # Convert to milliseconds

                # Select correct timer
                timer = (
                    self._injection_timer_ch1
                    if channel == "CH1"
                    else self._injection_timer_ch2
                )

...
from .phase_photonics_wrapper import SENSOR_DATA_LEN, SpectrometerAPI

# Adding a prefix for devices from Phase Photonics here. 
# For example: If device has name "FactoryUSB", use "Fa" here. 
# For spectro sight use "ST".
PHASE_PHOTONICS_PREFIX = b"ST"
...
class USB4000:
    ...
    def __init__(self, app):
        ...
        self.api = SpectrometerAPI(Path(__file__).parent / "Sensor64bit.dll")
        self.app = app
        self.spec = None
        self.opened = False
        self.devs = [s.decode() for s in listDevices() if s.startswith(PHASE_PHOTONICS_PREFIX)]
    ...
    def set_integration(self, integration):
        ...
        try:
            ...
            sleep(0.01)  # Reduced from 0.3s to 10ms for faster acquisition
            return bool(r)
        except Exception as e:

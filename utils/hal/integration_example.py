...
try:
    from utils.hal import HALFactory, SPRControllerHAL, HALError, HALConnectionError
...
class HALControllerAdapter:
    ...
    def __init__(self) -> None:
        """Initialize adapter."""
        self.hal_controller: Optional[SPRControllerHAL] = None
        self.legacy_controller: Any = None  # Fallback to old controller
        self.using_hal = False
    
    def connect(self, device_type: Optional[str] = None, **connection_params: Any) -> bool:
        """
    ...
    def get_device_info(self) -> dict[str, Any]:
        ...
        if self.using_hal and self.hal_controller:
            ...
            return {
                ...
                "firmware_version": getattr(self.legacy_controller, 'version', 'Unknown'),
    ...
    def _connect_legacy(self, device_type: Optional[str], **connection_params: Any) -> bool:
...
def demonstrate_hal_integration():
    ...
    try:
        # Try to connect
        if adapter.connect():
            print(f"  Connection: SUCCESS (using {'HAL' if adapter.using_hal else 'Legacy'})")
            ...
            print(f"  Temperature: {temp:.1f}°C" if temp > -1 else "  Temperature: Not available")

...
from settings import PICO_VID, PICO_PID
...
class PicoEZSPRHAL(SPRControllerHAL):
    ...
    def connect(self, **connection_params: Any) -> bool:
        ...
        try:
            ...
            if not ports_to_try:
                raise HALConnectionError("No PicoEZSPR devices found", device_info={"model": "PicoEZSPR"})
            ...
            for port_info in ports_to_try:
                if self._attempt_connection(port_info.device, baud_rate, timeout):
                    ...
                    self._update_status()
                    logger.info(f"Successfully connected to PicoEZSPR on {port_info.device}")
                    return True
            
            raise HALConnectionError("Failed to establish communication with any PicoEZSPR device")
    ...
    def get_device_info(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "supports_firmware_update": self._firmware_version in self.UPDATABLE_VERSIONS,
                "supports_pump_correction": self._firmware_version in self.VERSIONS_WITH_PUMP_CORRECTION,
            }
        except Exception as e:
            raise HALOperationError(f"Failed to get device info: {e}", "get_device_info")
    ...
    def activate_channel(self, channel: ChannelID) -> bool:
        ...
        if not self.validate_channel(channel):
            raise HALOperationError(f"Channel {channel.value} not supported", "activate_channel")
        ...
        try:
            ...
            raise HALOperationError(f"Channel activation failed: {e}", "activate_channel")
    ...
    def set_led_intensity(self, intensity: float) -> bool:
        ...
        try:
            ...
            channel = self.status.active_channel.value if self.status.active_channel else 'a'
            ...
            raise HALOperationError(f"LED intensity setting failed: {e}", "set_led_intensity")
    ...
    def update_firmware(self, firmware_path: str) -> bool:
        ...
        if not self.supports_firmware_update():
            logger.warning(f"Firmware update not supported for version {self._firmware_version}")
            return False
    ...
    def get_pump_corrections(self) -> Optional[Tuple[float, float]]:
        ...
        try:
            ...
            raise HALOperationError(f"Failed to get pump corrections: {e}", "get_pump_corrections")
    
    def set_pump_corrections(self, pump1_correction: float, pump2_correction: float) -> bool:
        """
        ...
        """
        if not self.supports_pump_correction():
            logger.warning(f"Pump correction not supported for version {self._firmware_version}")
            return False
        ...
        try:
            ...
            correction_bytes = bytes(round(x * self.PUMP_CORRECTION_MULTIPLIER) for x in corrections)
            
            if self._ser:
                self._ser.write(b"pf" + correction_bytes + b"\n")
            
            logger.info(f"Set pump corrections: {pump1_correction:.3f}, {pump2_correction:.3f}")
            return True
            
        except ValueError as e:
            raise HALOperationError(f"Invalid correction values: {e}", "set_pump_corrections")
        except Exception as e:
            raise HALOperationError(f"Failed to set pump corrections: {e}", "set_pump_corrections")

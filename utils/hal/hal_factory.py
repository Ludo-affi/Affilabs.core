...
class HALFactory:
    ...
    _spectrometer_registry: Dict[str, Type[SpectrometerHAL]] = {
        "USB4000": USB4000HAL,
        "usb4000": USB4000HAL,
        "OceanOptics": USB4000HAL,  # Alias for USB4000
        # Test implementations for development
        "USB4000-Test": USB4000TestHAL,
        "usb4000-test": USB4000TestHAL,
    ...
    def create_controller(
        ...
        raise HALDeviceNotFoundError("No device type specified and auto-detection disabled")
    ...
    @classmethod
    def register_controller(cls, device_type: str, hal_class: Type[SPRControllerHAL]) -> None:
        """
    ...
    def _create_specific_controller(
        ...
        try:
            if controller.connect(**connection_params):
                logger.info(f"Successfully created and connected {device_type} controller")
                return controller
            else:
    ...
    @classmethod
    def _auto_detect_controller(cls, connection_params: Dict[str, Any]) -> SPRControllerHAL:
        ...
        remaining_types = [t for t in cls._controller_registry.keys() if t not in priority_order]
        
        for device_type in priority_order + remaining_types:
            try:
                logger.debug(f"Trying to detect {device_type}...")
                controller = cls._create_specific_controller(device_type, connection_params)
    ...
    @classmethod
    def get_controller_capabilities(cls, device_type: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            return {
                "supported_channels": [ch.value for ch in capabilities.supported_channels],
    ...
    def create_spectrometer(
        ...
        logger.info(f"Creating spectrometer: device_type={device_type}, auto_detect={auto_detect}")
        ...
        if device_type and device_type in cls._spectrometer_registry:
            ...
            try:
                ...
                if not auto_detect:
                    raise
                logger.warning(f"Failed to create {device_type}: {e}, attempting auto-detection")
    ...
    @classmethod
    def _create_specific_spectrometer(cls, device_type: str, connection_params: Dict[str, Any]) -> SpectrometerHAL:
        ...
        try:
            if spectrometer.connect(**connection_params):
                logger.info(f"Successfully created and connected {device_type} spectrometer")
                return spectrometer
            else:
                raise HALError(f"Failed to connect to {device_type}")
            ...
            try:
                ...
                pass
            raise HALError(f"Failed to create {device_type} spectrometer: {e}")
    
    @classmethod
    def _auto_detect_spectrometer(cls, connection_params: Dict[str, Any]) -> SpectrometerHAL:
        ...
        remaining_types = [t for t in cls._spectrometer_registry.keys() if t not in priority_order]
        
        for device_type in priority_order + remaining_types:
            try:
                logger.debug(f"Trying to detect {device_type}...")
                spectrometer = cls._create_specific_spectrometer(device_type, connection_params)
    ...
    @classmethod
    def register_spectrometer(cls, device_type: str, hal_class: Type[SpectrometerHAL]) -> None:
        """
        ...
        logger.info(f"Registered spectrometer HAL: {device_type} -> {hal_class.__name__}")
    ...
    @classmethod
    def get_spectrometer_capabilities(cls, device_type: str) -> Optional[Dict[str, Any]]:
        """
            ...
            try:
                ...
                if hal_class:
                    ...
                    if kinetic.connect(**(connection_params or {})):
                        logger.info(f"Successfully connected to {device_type} kinetic controller")
                        return kinetic
                    else:
                        logger.warning(f"Failed to connect to {device_type} kinetic controller")
                        
            except Exception as e:
                logger.warning(f"Failed to create {device_type} kinetic controller: {e}")
        ...
        if auto_detect:
            ...
            for name, hal_class in cls._kinetic_registry.items():
                ...
                try:
                    ...
                    if kinetic.connect(**(connection_params or {})):
                        logger.info(f"Auto-detected and connected to {name} kinetic controller")
                        return kinetic
                    else:
                        kinetic.disconnect()
            ...
            # Try test implementation as fallback
            try:
                logger.debug("Trying test kinetic controller as fallback")
                kinetic = KineticTestHAL()
                if kinetic.connect(**(connection_params or {})):
                    logger.info("Connected to test kinetic controller (no hardware found)")
                    return kinetic
            except Exception as e:
                logger.debug(f"Test kinetic controller failed: {e}")
    ...
    def get_supported_kinetic_controllers(cls) -> List[str]:
        ...
        return [name for name in cls._kinetic_registry.keys() 
                if not name.endswith('-Test') and name != 'test']
    
    @classmethod
    def get_kinetic_controller_capabilities(cls, device_type: str) -> Optional[Dict[str, Any]]:
        """

...
class PicoP4SPRHAL(SPRControllerHAL):
    ...
    def connect(self, **connection_params: Any) -> bool:
        ...
        try:
            ...
            if not ports_to_try:
                raise HALConnectionError("No PicoP4SPR devices found", device_info={"model": "PicoP4SPR"})
            ...
            for port_info in ports_to_try:
                if self._attempt_connection(port_info.device, baud_rate, timeout):
                    ...
                    self._update_status()
                    logger.info(f"Successfully connected to PicoP4SPR on {port_info.device}")
                    return True
            
            raise HALConnectionError("Failed to establish communication with any PicoP4SPR device")

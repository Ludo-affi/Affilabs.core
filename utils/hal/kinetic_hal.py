...
class KineticHAL:
    ...
    def connect(self, **kwargs) -> bool:
        ...
        try:
            ...
            logger.info(f"Connected to KNX controller: {self._device_info['model']} v{self._device_info['firmware_version']}")
            return True
    ...
    def _detect_knx_hardware(self) -> Optional[str]:
        """
    ...
    def _send_command(self, cmd: str, parse_json: bool = False, reply: bool = True) -> Any:
        """
        ...
        if not self.is_connected():
            raise HALOperationError("Cannot send command: not connected to KNX controller")
                    ...
                    if reply and self._serial:
                        ...
                        if parse_json:
                            try:
                                ...
                                logger.error(f"Failed to parse JSON response for {cmd}: {response}")
                                return None
                        else:

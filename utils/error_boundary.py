...
class ErrorBoundary(QObject):
    ...
    def handle_error(
        ...
        if severity == ErrorSeverity.CRITICAL:
            ...
            return self._handle_high_severity_error(error, context, recovery_fn, user_message)
            
        elif severity == ErrorSeverity.MEDIUM:
            return self._handle_medium_severity_error(error, context, recovery_fn, user_message)
    ...
    def _log_error(self, error: Exception, category: ErrorCategory, 
                   severity: ErrorSeverity, context: str, error_id: str):
    ...
    def _handle_critical_error(self, error: Exception, context: str, user_message: Optional[str]):
    ...
    def _handle_high_severity_error(self, error: Exception, context: str, 
                                   recovery_fn: Optional[Callable], user_message: Optional[str]) -> bool:
        ...
        try:
            QMessageBox.warning(None, "Error", f"{msg}\n\nSome features may be disabled.")
        except:
            print(f"ERROR: {msg}")
        return False
    
    def _handle_medium_severity_error(self, error: Exception, context: str,
                                     recovery_fn: Optional[Callable], user_message: Optional[str]) -> bool:
    ...
    def _handle_low_severity_error(self, error: Exception, context: str, user_message: Optional[str]) -> bool:
...
def hardware_error_boundary(recovery_fn: Optional[Callable] = None, user_message: Optional[str] = None):
...
def data_acquisition_error_boundary(recovery_fn: Optional[Callable] = None):
...
def calibration_error_boundary(recovery_fn: Optional[Callable] = None):

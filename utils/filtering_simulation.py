...
class SPRSimulator:
    ...
    def add_outliers(self, signal: np.ndarray, prob: float, magnitude: float) -> np.ndarray:
        """
...
class ImprovedApproach:
    ...
    @staticmethod
    def complete_filter(data: np.ndarray, window: int, lookback: int = 20) -> Tuple[np.ndarray, np.ndarray]:
        """
...
def calculate_metrics(filtered: np.ndarray, true_signal: np.ndarray, 
                     noisy_signal: np.ndarray) -> Dict[str, float]:
    """
...
def create_comparison_plots(time: np.ndarray, clean: np.ndarray, noisy: np.ndarray,
                           current_filtered: np.ndarray, improved_filtered: np.ndarray,
    ...
    ax1.plot(time, noisy, 'o', markersize=2, alpha=0.3, color='gray', label='Noisy data')
    ax1.plot(time, clean, '-', linewidth=2, color='black', label='True signal', alpha=0.7)
    ax1.plot(time, current_filtered, '-', linewidth=1.5, color='red', 
             label='Current (mean filter)', alpha=0.8)
    ax1.plot(time, improved_filtered, '-', linewidth=1.5, color='green', 
             label='Improved (median + outlier rejection)', alpha=0.8)
    
    # Mark outliers
    outlier_indices = np.where(outlier_mask)[0]
    if len(outlier_indices) > 0:
        ax1.plot(time[outlier_indices], noisy[outlier_indices], 'rx', 
                markersize=8, label='Detected outliers')
    ...
    ax2.plot(time[zoom_mask], noisy[zoom_mask], 'o', markersize=4, 
            alpha=0.5, color='gray', label='Noisy data')
    ax2.plot(time[zoom_mask], clean[zoom_mask], '-', linewidth=2, 
            color='black', label='True signal')
    ax2.plot(time[zoom_mask], current_filtered[zoom_mask], '-', linewidth=2, 
            color='red', label='Current (mean)', alpha=0.8)
    ax2.plot(time[zoom_mask], improved_filtered[zoom_mask], '-', linewidth=2, 
            color='green', label='Improved (median)', alpha=0.8)
    
    # Mark outliers in zoom
    zoom_outliers = outlier_mask & zoom_mask
    if np.any(zoom_outliers):
        ax2.plot(time[zoom_outliers], noisy[zoom_outliers], 'rx', 
    ...
    ax3.plot(time, np.abs(current_error), '-', linewidth=1, color='red', 
            alpha=0.6, label='Current error')
    ax3.plot(time, np.abs(improved_error), '-', linewidth=1, color='green', 
            alpha=0.6, label='Improved error')
    ax3.axhline(y=0.1, linestyle='--', color='orange', alpha=0.5, label='±0.1 nm threshold')
    ...
    ax4.hist(current_error[valid_current], bins=50, alpha=0.5, color='red', 
            label='Current', density=True)
    ax4.hist(improved_error[valid_improved], bins=50, alpha=0.5, color='green', 
    ...
    ax5.scatter(time, current_error, s=5, alpha=0.3, color='red', label='Current residuals')
    ax5.scatter(time, improved_error, s=5, alpha=0.3, color='green', label='Improved residuals')
    ...
    bars1 = ax6.bar(x - width/2, current_values, width, label='Current (mean)', 
                   color='red', alpha=0.7)
    bars2 = ax6.bar(x + width/2, improved_values, width, label='Improved (median)', 
    ...
    plt.suptitle('SPR Filtering Comparison: Current vs Improved Approach', 
...
def run_simulation():
    ...
    improved_filtered, outlier_mask = ImprovedApproach.complete_filter(
        noisy_signal, MEDIAN_WINDOW, OUTLIER_LOOKBACK
    )
    ...
    snr_improvement = ((improved_metrics['snr_improvement'] / current_metrics['snr_improvement']) - 1) * 100

    ...
    def _save_kinetic_logs(
        ...
        if knx is not None:
            try:
                ...
                if (device_config["ctrl"] in ["PicoEZSPR"] or  # EZSPR disabled (obsolete)
                    device_config["knx"] in ["KNX2"]):  # PicoKNX2 disabled (obsolete)
                    self.data_io.save_kinetic_log(self.rec_dir, log_ch2, "B", knx_version)

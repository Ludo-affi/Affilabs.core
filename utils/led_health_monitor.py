...
class LEDHealthMonitor:
    ...
    def __init__(
        ...
        device_config_path: str | Path = "config/device_config.json",
    ):
        ...
        logger.info(
            f"📊 LED Health Monitor initialized with baseline: {self.baseline_file.name}",
        )
    ...
    def should_run_health_check(self) -> tuple[bool, float, float]:
        ...
        if should_run:
            logger.info(
                f"⏰ LED health check DUE: {hours_since_last:.1f} hours since last check",
            )
        else:
    ...
    def compare_calibrations(self, new_file: Path) -> dict[str, Any]:
        ...
        for channel in ["A", "B", "C", "D"]:
            ...
            tau_drift_pct = (
                ((tau_new_mean - tau_baseline_mean) / tau_baseline_mean * 100)
                if tau_baseline_mean > 0
                else 0
            )
            ...
            amp_drift_pct = (
                ((amp_new_mean - amp_baseline_mean) / amp_baseline_mean * 100)
                if amp_baseline_mean > 0
                else 0
            )
    ...
    def generate_health_report(
        ...
        ax1 = axes[0, 0]
        tau_drifts = [
            comparison_results["channels"][ch]["tau_drift_percent"] for ch in channels
        ]
        bars1 = ax1.bar(
            channels,
            tau_drifts,
        ...
        ax2 = axes[0, 1]
        amp_drifts = [
            comparison_results["channels"][ch]["amplitude_drift_percent"]
            for ch in channels
        ]
        bars2 = ax2.bar(
            channels,
        ...
        ax3 = axes[1, 0]
        health_statuses = [
            comparison_results["channels"][ch]["health_status"] for ch in channels
        ]
        status_colors = {"GOOD": "green", "WARNING": "orange", "CRITICAL": "red"}
        bar_colors = [status_colors[status] for status in health_statuses]
        status_values = [
            2 if s == "GOOD" else 1 if s == "WARNING" else 0 for s in health_statuses
        ]
...
def check_and_run_health_monitor() -> bool:
    ...
    """
    baseline_file = Path(
        "generated-files/characterization/led_afterglow_integration_time_models_20251011_210859.json",
    )
    ...
    if should_run:
        logger.info(
            f"⏰ LED health check DUE ({hours_since:.1f} hours since last check)",
        )

...
class WavelengthRange:
    ...
    def filter_spectrum(self, 
                       ...
                       spectrum_data: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def __repr__(self) -> str:
        return (f"WavelengthRange({self.min_wavelength:.1f}-{self.max_wavelength:.1f} nm, "
...
class SpectralFilter:
    ...
    def __init__(self, min_wavelength: float = 580.0, max_wavelength: float = 720.0):
        ...
        self.spr_range: Optional[WavelengthRange] = None
        self.full_range: Optional[WavelengthRange] = None
    ...
    def calibrate(self, full_detector_wavelengths: np.ndarray) -> WavelengthRange:
        ...
        logger.info(f"📊 Spectral filter calibrated:")
    ...
    def filter(self, 
              ...
              full_wavelengths: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if len(full_wavelengths) != len(full_spectrum):
            logger.debug(f"Trimming wavelengths from {len(full_wavelengths)} to {len(full_spectrum)}")
    ...
    def validate_alignment(self, 
        ...
        if len(data1) != len(data2):
            logger.error(f"❌ Data misalignment: {label1}={len(data1)} pixels, "
                        f"{label2}={len(data2)} pixels")
            return False
        
        if self.spr_range and not self.spr_range.validate_data(data1):
            logger.warning(f"⚠️ {label1} size ({len(data1)}) doesn't match "
                          f"expected SPR range ({self.spr_range.num_pixels})")
            return False
        
        logger.debug(f"✅ Data alignment OK: {label1} and {label2} both have {len(data1)} pixels")
        return True


# Singleton instance for application-wide use
_global_filter: Optional[SpectralFilter] = None

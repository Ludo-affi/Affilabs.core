...
def load_latest_calibration():
    ...
    qc_file = Path(
        "OpticalSystem_QC/FLMT09116/validation_reports/qc_report_latest.json",
    )
    ...
    cal_data = {
        ...
        "p_mode_intensities": raw_cal.get(
            "p_mode_intensities",
            raw_cal.get("led_intensities", {}),
        ),
    ...
    logger.info(
        f"  S-pol integration: {s_int:.1f}ms" if s_int else "  S-pol integration: N/A",
    )
...
def test_acquisition_loop(num_cycles=5):
    ...
    if not hw_mgr.usb or not hw_mgr.ctrl:
        raise RuntimeError(
            f"Hardware not connected after {max_wait}s (usb={hw_mgr.usb is not None}, ctrl={hw_mgr.ctrl is not None})",
        )
    ...
    if has_per_ch:
        ...
        logger.info(
            f"[PRE-ARM] Requested: {p_int_time:.1f}ms, Actual: {actual_int:.1f}ms",
        )
        ...
        if result and abs(actual_int - p_int_time) < 1.0:
            ...
            logger.warning(
                f"[PRE-ARM] ✗ Failed to pre-arm (requested={p_int_time:.1f}, actual={actual_int:.1f})",
            )
            pre_armed = False
    ...
    for cycle in range(1, num_cycles + 1):
        ...
        for ch, signal in hw_mgr.ctrl.led_rank_sequence(
            255,
            settling_ms_global,
            dark_ms,
        ):
            ...
            if raw_peak >= 65535:
                logger.warning(
                    f"  {ch.upper()}: DETECTOR SATURATED! Min={raw_min:.0f}, Mean={raw_mean:.0f}, Max={raw_peak:.0f}",
                )
            elif raw_peak == raw_min:
                logger.warning(
                    f"  {ch.upper()}: FLAT SPECTRUM! All pixels = {raw_peak:.0f}",
                )
            elif cycle == 1 and ch == "a":
                # Log first measurement for reference
                logger.info(
                    f"  {ch.upper()}: First spectrum Min={raw_min:.0f}, Mean={raw_mean:.0f}, Max={raw_peak:.0f}",
                )
            ...
            if dark_ref is not None and len(dark_ref) == len(raw_spectrum):
                ...
                dark_corrected = [
                    max(0, r - d) for r, d in zip(raw_spectrum, dark_ref, strict=False)
                ]
            else:
                dark_corrected = raw_spectrum
                logger.warning(
                    f"  {ch.upper()}: No dark reference - using raw spectrum",
                )
            ...
            if s_ref is not None and len(s_ref) == len(dark_corrected):
                transmission = [
                    p / s if s > 0 else 0
                    for p, s in zip(dark_corrected, s_ref, strict=False)
                ]
            else:
                logger.warning(
                    f"  {ch.upper()}: S-ref mismatch - cannot calculate transmission",
                )
                transmission = None
            ...
            try:
                ...
                trans_arr = (
                    np.array(transmission) if transmission else np.array(dark_corrected)
                )
            ...
            led_status = (
                "✓ LED ON" if snr_ratio >= 3.0 else f"✗ LED WEAK ({snr_ratio:.1f}X)"
            )

            led_int = led_intensities.get(ch, 0)
            logger.info(
                f"  {ch.upper()}: LED={led_int:3d}, Int={int_time:.1f}ms, "
    ...
    if cycle_times:
        ...
        logger.info(
            f"Cycle times: {np.mean(cycle_times):.1f}ms ± {np.std(cycle_times):.1f}ms",
        )

...
def measure_at_led_intensity(ctrl, usb, channel, led_intensity, integration_time_ms, wave_min_idx, wave_max_idx):
...
def run_saturation_study(channel='a', integration_time_ms=60.0, led_start=50, led_end=255, led_step=10):
    ...
    if not hw_mgr.usb or not hw_mgr.ctrl:
        logger.error(f"Hardware not connected (usb={hw_mgr.usb is not None}, ctrl={hw_mgr.ctrl is not None})")
    ...
    print("-" * 80)
    print(f"{'LED':<6} {'Median':<10} {'Mean':<10} {'Max':<10} {'Sat Px':<10} {'Sat %':<8}")
    print("-" * 80)

    for led_int in led_intensities:
        result = measure_at_led_intensity(
            hw_mgr.ctrl, hw_mgr.usb, channel, led_int, integration_time_ms, wave_min_idx, wave_max_idx
        )
    ...
    # Save results
    output_file = Path(f"saturation_study_ch{channel}_{int(integration_time_ms)}ms.json")
    ...
    first_sat_idx = next((i for i, r in enumerate(results) if r['saturated_pixels'] > 0), None)
    if first_sat_idx is not None:
        ...
        if last_unsat:
            print(f"  Last unsaturated: LED={last_unsat['led_intensity']}, signal={last_unsat['median_signal']:.0f}")
        print(f"  First saturated:  LED={first_sat['led_intensity']}, signal={first_sat['median_signal']:.0f}, sat_px={first_sat['saturated_pixels']}")
        print()
        ...
        if heavy_sat['saturated_pixels'] > 0:
            ...
            print(f"  Saturated pixels: {heavy_sat['saturated_pixels']} ({heavy_sat['saturation_fraction']*100:.1f}%)")
            ...
            if last_unsat:
                ...
                print(f"    Predicted: {last_unsat['median_signal']:.0f} × {led_ratio:.2f} = {estimated_true:.0f}")
                print()
    ...
    if len(saturated_results) >= 2:
        for r in saturated_results:
            print(f"  LED={r['led_intensity']:<4} → {r['saturated_pixels']:<6} pixels ({r['saturation_fraction']*100:>5.1f}%)")
        print()
    ...
    print(f"  LED → {recommended['recommended_led'] if recommended['recommended_led'] is not None else 'no change'}")
    print(f"  Integration → {recommended['recommended_integration_ms'] if recommended['recommended_integration_ms'] is not None else 'no change'} ms")
    print()

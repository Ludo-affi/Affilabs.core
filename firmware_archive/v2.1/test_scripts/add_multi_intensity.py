...
for i, line in enumerate(lines):
    ...
    if "bool led_rank_sequence(uint8_t intensity" in line:
        lines[i] = "bool led_rank_sequence(uint8_t int_a, uint8_t int_b, uint8_t int_c, uint8_t int_d, uint16_t settling_ms, uint16_t dark_ms);\n"
        print(f"Line {i+1}: Updated function declaration")

    # 2. Update function definition (around line 962)
    elif line.strip().startswith("bool led_rank_sequence(uint8_t intensity") and "uint16_t settling_ms" in line:
        lines[i] = "bool led_rank_sequence(uint8_t int_a, uint8_t int_b, uint8_t int_c, uint8_t int_d, uint16_t settling_ms, uint16_t dark_ms){\n"
...
content = content.replace("if (field == 1 && field_pos < 4) str_settling", "if (field == 4 && field_pos < 4) str_settling")
content = content.replace("if (field == 2 && field_pos < 4) str_dark", "if (field == 5 && field_pos < 4) str_dark")
...
content = content.replace("pwm_set_chan_level(slice_a, chan_a, level);", "pwm_set_chan_level(slice_a, chan_a, level_a);")
content = content.replace("pwm_set_chan_level(slice_b, chan_b, level);", "pwm_set_chan_level(slice_b, chan_b, level_b);")
content = content.replace("pwm_set_chan_level(slice_c, chan_c, level);", "pwm_set_chan_level(slice_c, chan_c, level_c);")
content = content.replace("pwm_set_chan_level(slice_d, chan_d, level);", "pwm_set_chan_level(slice_d, chan_d, level_d);")

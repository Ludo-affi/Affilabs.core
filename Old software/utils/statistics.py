...
def optimize_by_affinity(c_list, s_list):
    ...
    if len(c_list) != len(s_list):
        logger.error(f"Mismatched data lengths: c_list={len(c_list)}, s_list={len(s_list)}")
        return None
    if len(c_list) < 3:
    ...
    try:
        ...
        res = minimize(fun=partial(affinity_est_func, c_list, s_list, params),
                       x0=init_params, method='slsqp', options={'ftol': DEFAULT_FTOL, 'disp': False})
...
def optimize_by_linear(xdata, ydata):
    ...
    try:
        ...
        logger.debug(f"Linear result: a = {result['a']}, b = {result['b']}, Rsq = {result['r_sq']}")
        return result
    except Exception as e:
...
def func_assoc(data, k):
    ...
    :param k: [ka, rmax]
    """
    return np.array([(k[0] * data[1] * k[1]) * (1 - np.exp(-1 * ((k[0] * data[1]) + data[2]) * t))
                    / ((k[0] * data[1]) + data[2]) for t in data[0]])
...
def optimize_assoc(xdata, ydata, c, kd, ka0, rmax0):
    ...
    try:
        ...
        opt_val = minimize(fun=partial(assoc_est_func, xdata, ydata, c, kd, params), x0=init_params,
                           method='slsqp', options={'ftol': DEFAULT_FTOL, 'disp': False})

        if not opt_val.success:
            logger.warning(f"Association optimization did not converge: {opt_val.message}")
...
def func_rmax(data, r):
    ...
    return np.array([(data[0][i] * r[0]) / max(data[0][i] + data[1], 1e-10) for i in range(len(data[0]))])
...
def optimize_rmax(c_data, shift_data, kd, rmax0):
    ...
    if len(c_data) != len(shift_data):
        logger.error(f"Mismatched data lengths: c_data={len(c_data)}, shift_data={len(shift_data)}")
        return None
    if len(c_data) < 2:
    ...
    try:
        ...
        opt_val = minimize(fun=partial(rmax_est_func, c_data, shift_data, kd, params), x0=init_params,
                           method='slsqp', options={'ftol': DEFAULT_FTOL, 'disp': False})

...
print("=" * 80)
print("MODULE main/main.py IS LOADING")
print("=" * 80)
...
class AffiniteApp(QObject):
    ...
    def __init__(self: Self) -> None:
        ...
        gc.enable()
        self.main_window = MainWindow(self)  # Pass self as app controller
        logger.info("✅ Main window created successfully")
        self.device_config = {"ctrl": "", "knx": ""}
        self.conf = get_config()

        # Initialize spectrometer via HAL factory - DEFER to avoid blocking UI
        self.usb = None
        logger.info("Detector initialization deferred until after window shown")
        ...
        logger.info("Starting _grab_data thread...")
        self._tr = threading.Thread(target=self._grab_data)
        self._tr.start()
        logger.info("✅ _grab_data thread started")
        ...
        try:
            ...
            pass
        self._c_kill.clear()
        logger.info("Starting calibrate thread...")
        self._c_tr = threading.Thread(target=self.calibrate)
        self._c_tr.start()
        logger.info("✅ calibrate thread started")
        ...
        self._s_kill.clear()
        self._s_stop.set()
        logger.info("Starting sensor_reading_thread...")
        self._s_tr = threading.Thread(target=self.sensor_reading_thread)
        self._s_tr.start()
        logger.info("✅ sensor_reading_thread started")
        ...
        logger.info("✅ All signals connected - AffiniteApp init about to complete")

        self.kinetic_tasks = set()

        # Register cleanup handler for graceful shutdown
        atexit.register(self._emergency_cleanup)
        logger.info("✅ AffiniteApp initialization COMPLETE")
    ...
    def _log_event(self: Self, channel: str, event: str, flow: str = "-", temp: str = "-", dev: str = "-") -> None:
        ...
        if channel == "CH1":
            self.log_ch1.loc[len(self.log_ch1)] = new_event
        else:
            self.log_ch2.loc[len(self.log_ch2)] = new_event
    ...
    def _connect_all_signals(self: Self) -> None:
        ...
        """
        logger.info("_connect_all_signals: Starting...")
        # === Internal Application Signals ===
        logger.info("_connect_all_signals: Connecting calibration signals...")
        self.calibration_status.connect(self._on_calibration_status)
        self.calibration_started.connect(self._on_calibration_started)
        logger.info("_connect_all_signals: Connecting connected signal to open_device...")
        self.connected.connect(self.open_device)
        logger.info("_connect_all_signals: Internal signals connected")
        ...
        # Spectroscopy is now sidebar-only
        ...
        # === Spectroscopy Signals (Sidebar Only) ===
        if hasattr(self.main_window, "sidebar_spectroscopy"):
            self.main_window.sidebar_spectroscopy.full_cal_sig.connect(self.full_recalibration)
            self.main_window.sidebar_spectroscopy.new_ref_sig.connect(self.start_new_ref)
            self.main_window.sidebar_spectroscopy.single_led_sig.connect(self.single_led)
            self.main_window.sidebar_spectroscopy.polarizer_sig.connect(self.set_polarizer)
            logger.info("Connected sidebar spectroscopy signals to backend")
    ...
    def open_device(self: Self) -> None:
        ...
        if self._con_tr.is_alive():
            # Status bar removed in new design
            logger.error("Connection Error: Check USB cables & drivers")
    ...
    def quick_calibration(self: Self) -> None:
        ...
        if self._c_stop.is_set():
            ...
            # adv_btn handled by mainwindow now
            self._c_stop.clear()
    ...
    def start_new_ref(self: Self, *, new_settings: bool = False) -> None:
        ...
        try:
            if self.device_config["ctrl"] in DEVICES and self._c_stop.is_set():
                ...
                time.sleep(1)
                # Status bar removed in new design
                logger.info("New reference ...")
                # Spectroscopy is sidebar-only now
                if hasattr(self.main_window, "sidebar_spectroscopy"):
    ...
    def _handle_acquisition_error(self: Self, error: Exception, channel: str) -> None:
        ...
        self._b_stop.set()
        # Status bar removed in new design
        logger.error("Error while reading SPR data")
    ...
    def auto_polarization_valid(self: Self, force_full_calibration: bool = False) -> None:
        """Calibrate polarizer (VALID multi-bucket method).

        Uses production three-bucket ROI quadrant + multi-stage refinement (600–620,
        620–640, 640–660) for robust S (max) and P (min) contrast selection. This
        replaces legacy validation, barrel window, and full sweep paths. Name includes
        'valid' for easy discovery.
        """
        from utils.servo_calibration import auto_calibrate_polarizer
        try:
            if self.ctrl is None or self.usb is None:
                logger.error("Cannot perform auto-polarization: controller or spectrometer missing")
                return

            logger.info("=== AUTO-POLARIZATION (Multi-Bucket Production Method) ===")
            result = auto_calibrate_polarizer(self.usb, self.ctrl, require_water=False, polarizer_type="circular")
            if result:
                s_pos, p_pos, extinction_ratio = result
                # Apply positions (calibration-only persistence to config, NOT EEPROM)
                self.ctrl.servo_set(s_pos, p_pos)
                time.sleep(0.3)  # Allow servos to settle

                # Verify servo positions before persisting
                from utils.servo_calibration import verify_servo_positions
                verified, actual_s, actual_p = verify_servo_positions(self.ctrl, s_pos, p_pos, tolerance=3)

                if verified:
                    try:
                        if hasattr(self, 'device_config') and self.device_config is not None:
                            self.device_config.set_servo_positions(int(s_pos), int(p_pos))
                            self.device_config.set_extinction_ratio(extinction_ratio)
                            # Do not sync EEPROM; user requested config-only persistence
                            self.device_config.save(auto_sync_eeprom=False)
                            logger.info(f"Persisted polarizer positions to device_config.json: S={s_pos}°, P={p_pos}° (no EEPROM flash)")
                            logger.info(f"Persisted extinction ratio: {extinction_ratio:.2f}% (sensor-specific reference)")
                        else:
                            logger.warning("device_config unavailable; positions not saved to file")
                    except Exception as e:
                        logger.warning(f"Failed to save positions to config: {e}")
                else:
                    logger.error(f"❌ Servo verification failed - positions NOT saved to config")
                    logger.error(f"   Expected: S={s_pos}° P={p_pos}°")
                    logger.error(f"   Actual: S={actual_s}° P={actual_p}°")
                    show_message(
                        f"Servo verification failed!\nExpected S={s_pos}° P={p_pos}°\nActual S={actual_s}° P={actual_p}°\nPositions NOT saved.",
                        msg_type="Critical"
                    )
                self.new_default_values = True
                self.get_device_parameters()
            else:
                logger.error("Multi-bucket calibration returned no result")
                show_message(
                    "Polarizer calibration failed (multi-bucket method). Check hardware and retry.",
                    msg_type="Critical"
                )
        except Exception as e:
            logger.exception(f"Auto-polarization failed: {e}")
            show_message(
                "Polarizer calibration encountered an error. See logs for details.",
                msg_type="Critical"
            )
                    ...
                    if self.device_config["ctrl"] == "PicoP4SPR":
                        try:
                            ...
                            if self.calibrated and min_temp < self.temp < max_temp:
                                ...
                                self.temp_log.loc[len(self.temp_log)] = new_row
                                self.update_temp_display.emit(temp, "ctrl")
                        except Exception as e:
                ...
                if False:  # QSPR disabled - obsolete hardware
                    ...
                    if new_s is not None and new_p is not None:
                        ...
                        if old_s != new_s or old_p != new_p:
                            ...
                            # Verify servo positions before persisting
                            from utils.servo_calibration import verify_servo_positions
                            verified, actual_s, actual_p = verify_servo_positions(self.ctrl, new_s, new_p, tolerance=3)

                            if not verified:
                                logger.error(f"❌ Servo verification failed after manual update!")
                                logger.error(f"   Expected: S={new_s}° P={new_p}°")
                                logger.error(f"   Actual: S={actual_s}° P={actual_p}°")
                                logger.error(f"   Attempting retry with corrected values...")
                                # Try once more with the actual positions
                                self.ctrl.servo_set(s=new_s, p=new_p)
                                time.sleep(0.3)
                                verified, actual_s, actual_p = verify_servo_positions(self.ctrl, new_s, new_p, tolerance=3)
                            ...
                            try:
                                ...
                                if verification is not None:
                                    ...
                                    if was_inverted and (vs != new_s or vp != new_p):
                                        ...
                                        # Re-verify after inversion correction
                                        verified, actual_s, actual_p = verify_servo_positions(self.ctrl, new_s, new_p, tolerance=3)
                            except Exception as _verr:
                                logger.debug(f"Advanced S/P verify skipped: {_verr}")

                            # Save servo positions to device_config only if verified
                            if verified:
                                logger.info(f"💾 Saving servo positions to device_config: S={new_s}, P={new_p}")
                                logger.info(f"   Positions will be restored from config on next session")
                                try:
                                    self.device_config.set_servo_positions(new_s, new_p)
                                    self.device_config.save(auto_sync_eeprom=False)
                                    logger.info(f"✅ Servo positions saved to config: S={new_s}, P={new_p}")
                                    logger.info(f"   Config file updated (EEPROM not modified)")
                                except Exception as save_err:
                                    logger.error(f"❌ Config save error: {save_err}")
                                    logger.error(f"   Servo positions set temporarily but NOT saved to config")
                            else:
                                logger.error(f"❌ Servo verification failed - positions NOT saved to config")
                                logger.error(f"   Servos may not have moved correctly or communication error occurred")
    ...
    def _init_detector_async(self: Self) -> None:
        """Initialize detector in background thread to avoid blocking UI."""
        logger.info("Initializing detector (deferred from startup)...")
    ...
    def connect_dev(self: Self) -> None:
        ...
        logger.info("========== CONNECT_DEV CALLED ==========")
        import traceback
        for line in traceback.format_stack()[:-1]:
            logger.info(f"  {line.strip()}")
        logger.info("========================================")

        # Initialize detector first if not done yet (deferred from __init__)
        if self.usb is None:
            # Run detector init in background thread to avoid blocking
            detector_thread = threading.Thread(target=self._init_detector_async, daemon=True)
    ...
    def shutdown_controller(self: Self) -> None:
        """Shutdown controller."""
        # Status bar removed in new design
        logger.info("SPR device powering off...")
    ...
    def shutdown_kinetics(self: Self, *, skip_setup: bool = False) -> None:
        """Shutdown kinetics."""
        # Status bar removed in new design
        logger.info("Kinetic unit powering off...")
        self._s_stop.set()
        if self.knx is not None:
            self.knx.shutdown()
            self.knx.close()
...
def main() -> None:
    """Run the application."""
    print("=" * 80)
    print("MAIN FUNCTION STARTED - CODE VERSION CHECK")
    print("=" * 80)
    ...
    print("=" * 80)
    print("ABOUT TO CREATE AffiniteApp")
    print("=" * 80)

    app = AffiniteApp()

    print("=" * 80)
    print("AffiniteApp CREATED - __init__ RETURNED")
    print("=" * 80)
    ...
    # Ensure main window is visible before entering event loop
    logger.info("🪟 Showing main window...")
    app.main_window.show()
    app.main_window.raise_()
    app.main_window.activateWindow()
    logger.info(f"🪟 Window visible: {app.main_window.isVisible()}, Window size: {app.main_window.size()}")

    set_event_loop_policy(QAsyncioEventLoopPolicy())
    # Defer device connection by 100ms to allow window to appear first
    get_event_loop().call_later(0.1, app.connect_dev)

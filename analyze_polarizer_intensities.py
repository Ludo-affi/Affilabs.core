...
def analyze_sweep_data():
    ...
    print(f"\n📊 Calibration Results:")
    print(f"   S Position (HIGH): {polarizer['s_position']} → {polarizer['s_intensity']:.1f} counts")
    print(f"   P Position (LOW):  {polarizer['p_position']} → {polarizer['p_intensity']:.1f} counts")
    ...
    print(f"\n🌈 Wavelength Mapping:")
    print(f"   Full spectrum: {spec.wavelengths()[0]:.1f}-{spec.wavelengths()[-1]:.1f} nm")
    print(f"   Target range: 600-630 nm")
    ...
    print(f"   Maximum: {intensities.max():.1f} counts at position {int(angles[np.argmax(intensities)])}")
    print(f"   Minimum: {intensities.min():.1f} counts at position {int(angles[np.argmin(intensities)])}")
    ...
    print(f"The values above show max intensity across full detector range.")
    print(f"For wavelength-specific analysis, re-run calibration with full spectrum logging.")

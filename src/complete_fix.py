...
for i, line in enumerate(lines):
    if i > 2790 and 'afterglow_correction=None' in line and 'Optional: pre-loaded AfterglowCorrection' in line:
        print(f"Found afterglow line at {i+1}: {line.strip()}")
        # Add comma if not present
        if not line.rstrip().endswith(','):
            lines[i] = line.rstrip() + ',\n'
        # Insert new parameters
        lines.insert(i+1, '    pre_led_delay_ms: float = 45.0,  # PRE LED delay: settling time after LED on (default 45ms)\n')
        lines.insert(i+2, '    post_led_delay_ms: float = 5.0,  # POST LED delay: dark time after LED off (default 5ms)\n')
...
for i, line in enumerate(lines):
    if i > 2840 and i < 2860 and 'result = LEDCalibrationResult()' in line and 'perform_alternative' in ''.join(lines[max(0,i-50):i]):
        print(f"Found result = LEDCalibrationResult() at {i+1}")
        # Insert debug prints after this line
        lines.insert(i+1, '\n')
        lines.insert(i+2, '    print("\\n" + "="*80)\n')
        lines.insert(i+3, '    print("🔥🔥🔥 perform_alternative_calibration() ENTERED")\n')
...
for i in range(len(lines)-1, 0, -1):
    line = lines[i]
    if 'except Exception as e:' in line and 'LED calibration failed (Global LED Intensity Method)' in lines[i+1]:
        print(f"Found except block at {i+1}")
        # Insert debug prints
        lines.insert(i+1, '        print("\\n" + "="*80)\n')
        lines.insert(i+2, '        print("❌❌❌ EXCEPTION IN perform_alternative_calibration()")\n')

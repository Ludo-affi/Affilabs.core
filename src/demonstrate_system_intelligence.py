...
from core.system_intelligence import (
    get_system_intelligence, SystemState, IssueCategory, IssueSeverity
)
...
def demonstrate_signal_quality_monitoring():
    ...
    print(f"System State: {state.value}")
    for issue in issues:
        print(f"\n⚠️ {issue.severity.value.upper()}: {issue.title}")
...
def demonstrate_led_health_tracking():
    ...
    for issue in issues:
        print(f"\n⚠️ {issue.severity.value.upper()}: {issue.title}")
        print(f"   Metrics: Intensity={issue.metrics['intensity']:.0f}, "
...
def demonstrate_report_generation():
    ...
    print(f"   {report_path}")
    print(f"\nReport contains:")
    print(f"   • Session duration and system state")
    print(f"   • Operational metrics")
    print(f"   • Active and historical issues")

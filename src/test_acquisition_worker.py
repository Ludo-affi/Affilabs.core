...
def main():
    ...
    def on_spectrum(data):
        ...
        if spectrum_count['count'] <= 5:
            print(f"   ✓ Spectrum {spectrum_count['count']}: channel={data['channel']}, wavelength={data.get('wavelength', 'N/A'):.2f}nm")
        elif spectrum_count['count'] == 20:
            print(f"   ✓ Received 20 spectra - TEST PASSED!")
    ...
    def timeout():
        if spectrum_count['count'] < 20:
            print(f"\n   ⚠ Timeout: Only received {spectrum_count['count']} spectra")
    ...
    if spectrum_count['count'] >= 20:
        print("STATUS: ✅ SUCCESS - Acquisition worker runs without crashes!")
    elif spectrum_count['count'] > 0:
        print("STATUS: ⚠ PARTIAL - Worker started but may have issues")
    else:
        print("STATUS: ❌ FAILED - No spectra received")
    print("=" * 60)

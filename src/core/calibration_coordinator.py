...
class CalibrationCoordinator(QObject):
    ...
    def _on_start_button_clicked(self) -> None:
        ...
        if self._calibration_completed:
            ...
            try:
                ...
                device_config = self.app.main_window.device_config if hasattr(self.app, 'main_window') and self.app.main_window else None

                if device_config is None:
                    logger.warning("⚠️ Device config not available - calibration not saved to JSON")
                else:
    ...
    def _on_calibration_complete(self, calibration_data: Dict[str, Any]) -> None:
        ...
        if hasattr(self.app, 'spr_calibrator') and hasattr(self.app.spr_calibrator, 'state'):
            transmission_validation = getattr(self.app.spr_calibrator.state, 'transmission_validation', {})
        else:
            # Fallback: check if stored in data_mgr
            transmission_validation = getattr(data_mgr, 'transmission_validation', {})

        # Collect device metadata for diagnostics
        from datetime import datetime
        device_type = 'Unknown'
        detector_serial = 'N/A'
        firmware_version = 'N/A'

        if hasattr(self.app, 'usb') and self.app.usb:
            detector_serial = getattr(self.app.usb, 'serial_number', 'N/A')

        if hasattr(self.app, 'ctrl') and self.app.ctrl:
            device_type = type(self.app.ctrl).__name__
            firmware_version = getattr(self.app.ctrl, 'version', 'N/A')
        ...
        qc_data = {
            ...
            'num_scans': getattr(data_mgr, 'num_scans', 'N/A'),
        ...
        logger.info("📊 QC Report - Saturation Validation Complete:")
        logger.info(f"   S-pol channels: {list(data_mgr.ref_sig.keys()) if data_mgr.ref_sig else 'EMPTY'}")
        logger.info(f"   P-pol channels: {list(getattr(data_mgr, 'p_ref_sig', {}).keys()) if getattr(data_mgr, 'p_ref_sig', {}) else 'EMPTY'}")

        # Report orientation validation results (SPR FWHM values)
        orientation_validation = getattr(data_mgr, 'orientation_validation', {})
        if orientation_validation:
            logger.info(f"   SPR FWHM (orientation validation):")
            for ch, fwhm_value in orientation_validation.items():
                # Handle both old format (dict with 'passed'/'reason') and new format (float FWHM)
                if isinstance(fwhm_value, dict):
                    status = "✅ PASS" if fwhm_value.get('passed', True) else "❌ FAIL"
                    logger.info(f"      Ch {ch.upper()}: {status} - {fwhm_value.get('reason', 'N/A')}")
                else:
                    # New format: FWHM value (float)
                    status = "✅ Good" if fwhm_value < 50 else "⚠️ Check"
                    logger.info(f"      Ch {ch.upper()}: {fwhm_value:.1f}nm - {status}")
        else:
            logger.info(f"   Orientation validation: No data (may not have run yet)")

        logger.info(f"   SPR FWHM: {getattr(data_mgr, 'spr_fwhm', {})}")
        logger.info(f"   Transmission validation: {list(transmission_validation.keys())}")

        # Report max values only (saturation check)
        if data_mgr.ref_sig:
            logger.info("   S-pol max counts (75% target):")
            for ch in ['a', 'b', 'c', 'd']:
                if ch in data_mgr.ref_sig:
                    max_val = np.max(data_mgr.ref_sig[ch])
                    logger.info(f"      Ch {ch.upper()}: {max_val:.0f} counts")
        if getattr(data_mgr, 'p_ref_sig', {}):
            logger.info("   P-pol max counts (81% target, NO saturation):")
            for ch in ['a', 'b', 'c', 'd']:
                if ch in getattr(data_mgr, 'p_ref_sig', {}):
                    max_val = np.max(data_mgr.p_ref_sig[ch])
                    logger.info(f"      Ch {ch.upper()}: {max_val:.0f} counts")

...
STEP 5: S-Mode LED Optimization
  5A: Integration Time Optimization
      - Find optimal integration time (max 100ms)
      - Per-channel or global depending on mode
  5B: LED Optimization with Binary Search
      - Binary search for optimal LED intensity
      - Target: 70% of detector max (leaves 30% headroom for P-mode)
...
def run_full_6step_calibration(
    ...
    try:
        ...
        # ===================================================================
        led_intensities, integration_time, num_scans, weakest_channel = optimize_s_mode_leds(
        ...
        result.weakest_channel = weakest_channel  # Store hardware characteristic
        ...
        if needs_optimization and can_optimize:
            ...
            while (p_weakest_signal_counts < p_target_counts * 0.90 and
                ...
                # CRITICAL: Pre-scale LED intensities proportionally to avoid saturation
                # When integration time increases, same LED will produce more signal
                ...
                for ch in ch_list:
                    old_led = led_intensities.get(ch, 0)
                ...
                try:
                    logger.info(f"   Re-optimizing P-mode LEDs at {integration_time}ms (starting from scaled S-mode)...")
                    p_mode_intensities, p_performance = calibrate_p_mode_leds(
                        usb, ctrl, ch_list, scaled_led_intensities,
                        stop_flag, detector_params=detector_params,
                        headroom_analysis=headroom_analysis,
                        pre_led_delay_ms=PRE_LED_DELAY_MS,
                        post_led_delay_ms=POST_LED_DELAY_MS
        ...
        ch_error_list, orientation_validation, polarizer_swap_detected = verify_calibration(
            usb=usb,
            ctrl=ctrl,

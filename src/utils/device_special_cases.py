...
def check_special_case(detector_serial: Optional[str]) -> Optional[Dict[str, Any]]:
    ...
    if detector_serial in SPECIAL_CASES:
        ...
        logger.warning(f"   Description: {special_case.get('description', 'No description')}")
...
def apply_special_case(special_case: Dict[str, Any], device_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 'servo_positions' in special_case:
        ...
        logger.info(f"  → Servo positions: S={servo.get('s_pos')}, P={servo.get('p_pos')}")
        if 's_pos' in servo:
    ...
    for key, value in special_case.items():
        if key not in ['description', 'notes', 'afterglow_correction',
                       'led_intensity_scaling', 'servo_positions', 'integration_time']:
...
def add_special_case(detector_serial: str,
                     description: str,
                     **kwargs) -> None:
    ...
    """
    if detector_serial in SPECIAL_CASES:
        logger.warning(f"Special case for {detector_serial} already exists - overwriting")

...
def test_servo_movement():
    ...
    if hasattr(hm.ctrl, 'servo_move_raw_pwm'):
        ...
        print(f"   ❌ Controller does not support servo_move_raw_pwm()")
        return False
    
    time.sleep(2)  # Wait for servo to settle
    print(f"   Servo should now be at S position")
    ...
    time.sleep(2)  # Wait for servo to settle
    print(f"   Servo should now be at P position")
    ...
    time.sleep(2)  # Wait for servo to settle
    print(f"   Servo should now be back at S position")

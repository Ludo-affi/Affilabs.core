...
from utils.controller import (  # PicoKNX2 disabled (obsolete)
    KineticController,
    PicoEZSPR,
    PicoP4SPR,
)
...
class AffiniteApp(QApplication):
    ...
    # Hardware manager threading-safe signals
    update_device_display = Signal(str)
    setup_device_widget_signal = Signal(str, str, object)  # ctrl, knx, pump
    setup_kinetic_widget_signal = Signal(str, str)  # ctrl, knx
    ...
    def __init__(self) -> None:
        ...
        self.knx: KineticController | PicoEZSPR | None = (
            None  # PicoKNX2 disabled (obsolete)
        )
        self.pump = None
        ...
        # Initialize UI state manager FIRST (needed by thread manager)
        ...
        # Manager references (initialized after hardware connection)
    ...
    def _schedule_hardware_retry(self, delay: float = 5.0) -> None:
        """Schedule hardware discovery retry after a delay."""
        try:
            logger.info(f"Scheduling hardware discovery retry in {delay} seconds...")
            retry_timer = QTimer()
            retry_timer.setSingleShot(True)
            retry_timer.timeout.connect(self._retry_hardware_discovery)
            retry_timer.start(int(delay * 1000))  # Convert to milliseconds
        except Exception as e:
            logger.exception(f"Error scheduling hardware retry: {e}")

    def _retry_hardware_discovery(self) -> None:
        """Retry hardware discovery."""
        try:
            logger.info("Retrying hardware discovery...")
            if not hasattr(self, "_con_tr") or not self._con_tr.is_alive():
                # Start new connection thread
                self._con_tr = threading.Thread(
                    target=self.connection_thread,
                    daemon=True,
                )
                self._con_tr.start()
            else:
                logger.debug("Connection thread already running, skipping retry")
        except Exception as e:
            logger.exception(f"Error during hardware discovery retry: {e}")
    ...
    def _setup_signal_connections(self) -> None:
        ...
        # Hardware manager threading-safe signals
        self.update_device_display.connect(self._on_update_device_display)
        self.setup_device_widget_signal.connect(self._on_setup_device_widget)
        self.setup_kinetic_widget_signal.connect(self._on_setup_kinetic_widget)
    ...
    # Hardware manager signal handlers (thread-safe UI updates)
    def _on_update_device_display(self, text: str) -> None:
        """Handle device display updates from hardware manager."""
        try:
            self.main_window.ui.device.setText(text)
        except (AttributeError, RuntimeError) as e:
            logger.warning(f"Failed to update device display: {e}")

    def _on_setup_device_widget(self, ctrl: str, knx: str, pump: object) -> None:
        """Handle device widget setup from hardware manager."""
        try:
            if (
                hasattr(self.main_window.sidebar, "device_widget")
                and self.main_window.sidebar.device_widget is not None
            ):
                self.main_window.sidebar.device_widget.setup(ctrl, knx, pump)
        except (AttributeError, RuntimeError) as e:
            logger.warning(f"Failed to setup device widget: {e}")

    def _on_setup_kinetic_widget(self, ctrl: str, knx: str) -> None:
        """Handle kinetic widget setup from hardware manager."""
        try:
            if (
                hasattr(self.main_window.sidebar, "kinetic_widget")
                and self.main_window.sidebar.kinetic_widget is not None
            ):
                self.main_window.sidebar.kinetic_widget.setup(ctrl, knx)
        except (AttributeError, RuntimeError) as e:
            logger.warning(f"Failed to setup kinetic widget: {e}")

    def _update_connection_status(self) -> None:
        """Update connection status based on available hardware."""
        try:
            # Check if both required devices are available
            ctrl_available = self.ctrl is not None
            usb_available = self.usb is not None

            if ctrl_available and usb_available:
                status_text = "Connected"
            else:
                status_text = "No Connection"

            self.main_window.ui.status.setText(status_text)
            logger.debug(
                f"Connection status updated: {status_text} (ctrl: {ctrl_available}, usb: {usb_available})",
            )
        except (AttributeError, RuntimeError) as e:
    ...
    def connection_thread(self) -> None:
        ...
        try:
            ...
            # ROBUST HARDWARE DISCOVERY PROCESS
            hw_mgr = self._get_hw_mgr()

            # Step 1: Perform actual hardware discovery
            logger.info("Starting comprehensive hardware discovery...")
            success, errors = hw_mgr.discover_and_connect_hardware()

            if success:
                logger.info("Hardware discovery successful")
                if errors:
                    logger.warning(
                        f"Hardware discovery completed with warnings: {errors}",
                    )
            else:
                logger.error(f"Hardware discovery failed: {errors}")
                # Set retry mechanism for hardware discovery
                self._schedule_hardware_retry()

            # Step 2: Open devices after discovery
            self.open_device()
    ...
    def open_device(self) -> None:
        ...
        try:
            ...
            # Initialize hardware managers FIRST
            success, errors = hw_mgr.initialize_hardware_managers()
            if not success:
                logger.warning(f"Hardware manager initialization issues: {errors}")

            # Debug hardware manager state before sync
            logger.debug(
                f"PRE-SYNC: hw_mgr.ctrl={hw_mgr.ctrl}, hw_mgr.usb={hw_mgr.usb}, hw_mgr.knx={hw_mgr.knx}",
            )
            logger.debug(
                f"PRE-SYNC: self.ctrl={self.ctrl}, self.usb={self.usb}, self.knx={self.knx}",
            )

            # Sync hardware devices discovered by hardware manager
            self.ctrl = hw_mgr.ctrl
            self.usb = hw_mgr.usb
            self.knx = hw_mgr.knx

            # Debug hardware manager state after sync
            logger.debug(
                f"POST-SYNC: self.ctrl={self.ctrl}, self.usb={self.usb}, self.knx={self.knx}",
            )
            ...
            # Check if hardware was found after sync
            ctrl_is_none = self.ctrl is None
            usb_is_none = self.usb is None
            condition_result = ctrl_is_none or usb_is_none  # Both devices required
            logger.debug(f"CONDITION CHECK: self.ctrl is None: {ctrl_is_none}")
            logger.debug(f"CONDITION CHECK: self.usb is None: {usb_is_none}")
            logger.debug(f"CONDITION CHECK: combined condition: {condition_result}")

            if condition_result:
                logger.debug(
                    "incomplete device configuration - missing required hardware",
                )
                # Update status to show no connection
                self._update_connection_status()
                # Show user-friendly dialog for no hardware
                self._show_no_device_dialog()
                self.stop()
            else:
                logger.debug("both required devices found - proceeding with startup")

                # Update status to show connected
                self._update_connection_status()

                # THEN call startup (after hardware managers are ready and synced)
                self.startup()
    ...
    def _on_kinetic_error(self, channel: str, error_message: str) -> None:
        ...
        try:
            ...
            show_message(
                f"Kinetic {channel} Error: {error_message}",
                msg_type="Warning",
            )
        except Exception as e:
    ...
    async def regenerate(self: Any) -> None:
        ...
        await kin.regenerate(
            contact_time_s=contact_time,
            flow_rate_ml_min=self.flow_rate * 60,
        )
    ...
    def _get_kin_ops(self) -> KineticOperations:
        ...
        if (
            ...
            self._kin_ops = KineticOperations(
                ...
                update_pump_display=lambda states,
                synced: self.update_pump_display.emit(states, synced),
                update_valve_display=lambda states,
                synced: self.update_valve_display.emit(states, synced),
            )
    ...
    def _get_data_acq(self) -> SPRDataAcquisition:
        ...
        if (
            ...
            self._data_acq = SPRDataAcquisition(
                ...
                set_status_text=self.ui_state_manager.get_status_callback()
                if self.ui_state_manager
    ...
    def _get_hw_mgr(self) -> HardwareManager:
        ...
        if self._hw_mgr is None:
            self._hw_mgr = HardwareManager(
                ...
                # UI callbacks
                update_device_display=lambda text: self.update_device_display.emit(text)
                if hasattr(self, "update_device_display")
                else lambda text: None,
                update_status=self.ui_state_manager.get_status_callback()
                if self.ui_state_manager
                else lambda text: None,
                setup_device_widget=lambda ctrl,
                knx,
                pump: self.setup_device_widget_signal.emit(ctrl, knx, pump)
                if hasattr(self, "setup_device_widget_signal")
                else lambda ctrl, knx, pump: None,
                setup_kinetic_widget=lambda ctrl,
                knx: self.setup_kinetic_widget_signal.emit(ctrl, knx)
                if hasattr(self, "setup_kinetic_widget_signal")
                else lambda ctrl, knx: None,
                # Signal callbacks
                pump_state_changed=lambda states, synced: self.update_pump_display.emit(
                    states,
                    synced,
                ),
                valve_state_changed=lambda states,
                synced: self.update_valve_display.emit(states, synced),
                sensor_reading_updated=lambda readings: self.update_sensor_display.emit(
                    readings,
                ),
                temp_display_updated=lambda temp: self.update_temp_display.emit(temp),
                calibration_progress=lambda progress,
                msg: self.calibration_progress.emit(progress),
                on_kinetic_error=lambda ch, msg: (
                    show_message(f"Kinetic {ch} Error: {msg}", msg_type="Warning"),
                    None,
                )[1],
                on_pump_error=lambda ch, msg: (
                    show_message(f"Pump {ch} Error: {msg}", msg_type="Warning"),
                    None,
                )[1],
            )
        return self._hw_mgr
    ...
    def _init_thread_manager(self) -> None:
        ...
        try:
            self.thread_manager = ThreadManager(
                ...
                status_callback=self.ui_state_manager.get_status_callback()
                if self.ui_state_manager
                else lambda text: None,
            )
    ...
    def startup(self: Any) -> None:
        ...
        logger.debug("🚀 STARTUP() CALLED - Checking hardware state...")
        logger.debug(
            f"🎛️ Controller (self.ctrl): {type(self.ctrl).__name__ if self.ctrl else 'None'}",
        )
        logger.debug(
            f"📊 Spectrometer (self.usb): {type(self.usb).__name__ if self.usb else 'None'}",
        )
        logger.debug(f"🔧 Device config ctrl: {self.device_config.get('ctrl', 'None')}")

        # DYNAMIC CALIBRATION APPROVAL: Always enable calibration polling
        # Hardware availability is checked in the calibration thread itself
        self._c_stop.clear()
        logger.debug(
            "*** C_STOP CLEARED - CALIBRATION ENABLED (ADAPTS WHEN HARDWARE AVAILABLE) ***",
        )

        # Restart calibration thread if it has exited
        if (
            self.thread_manager
            and self.thread_manager.calibration_thread
            and not self.thread_manager.calibration_thread.is_alive()
        ):
            logger.debug("🔄 Restarting calibration thread after hardware connection")
            import threading
            self.thread_manager.calibration_thread = threading.Thread(
                target=self.calibrate,
                daemon=True,
            )
            self.thread_manager.calibration_thread.start()
            logger.debug("✅ Calibration thread restarted successfully")
        ...
        if self.ctrl is not None and self.device_config["ctrl"] in DEVICES:
            ...
            logger.debug(
                f"Hardware ready - auto-calibration can start for {self.device_config['ctrl']}",
            )
        else:
            logger.debug(
                "Hardware not ready at startup - auto-calibration will wait for hardware",
            )
    ...
    def calibrate(self) -> None:
        ...
        while not self._c_kill.is_set():
            ...
            # Debug calibration conditions every 100 loops (1 second)
            ...
            if self._debug_counter % 100 == 0:
                logger.debug("🔄 Calibration loop check:")
                logger.debug(f"  c_stop: {self._c_stop.is_set()}")
                logger.debug(
                    f"  ctrl: {type(self.ctrl).__name__ if self.ctrl else 'None'}",
                )
                logger.debug(f"  usb_ok(): {self.usb_ok()}")
                if hasattr(self, "usb") and self.usb:
                    logger.debug(
                        f"  usb.spec: {getattr(self.usb, 'spec', 'NO_SPEC_ATTR')}",
                    )

            # Check if we should exit or proceed with calibration
            if self._c_stop.is_set():
                # Only exit if we're actually calibrated OR if hardware is not available
                if self.calibrated or (self.ctrl is None) or not self.usb_ok():
                    logger.debug("🏁 Calibration loop exiting - calibration complete or hardware unavailable")
                    break
                else:
                    # Hardware is available but we're not calibrated - clear stop flag and proceed
                    logger.debug("🔄 Hardware available but not calibrated - clearing stop flag to proceed")
                    self._c_stop.clear()

            if (self.ctrl is not None) and self.usb_ok():
                ...
                if self.calibrated:
                    ...
                    try:
                        # Use existing calibrator (created by hardware manager)
                        if self.calibrator is None:
                            logger.error(
                                "Calibrator not initialized - hardware manager issue",
                            )
                            self._c_stop.set()
                            continue

                        # Ensure calibrator has progress callback
                        self.calibrator.set_progress_callback(
                            self.calibration_progress.emit,
                        )

                        # Initialize progress bar for calibration
                        self._init_calibration_progress()
                        ...
                        calibration_success, ch_error_str = (
                        ...
                        self.leds_calibrated = (
                            self.calibrator.state.leds_calibrated.copy()
                        )
                        ...
                        elif self.device_config["ctrl"] in [
                            "PicoEZSPR",
                        ...
                        # Mark calibration as complete FIRST
                        self._c_stop.set()
                        logger.debug("=== Calibration sequence complete ===")

                        # Clear the no-read flag to allow data acquisition to start
                        self._b_no_read.clear()
                        logger.debug("Data acquisition enabled - cleared _b_no_read flag")

                        # THEN emit calibration status to start measurements
                        # (This ensures data_processor is created and calibration is marked complete)
    ...
    def _on_calibration_status(
        ...
        logger.debug(f"🎯 _on_calibration_status called: state={state}, ch_error_str='{ch_error_str}'")
        self.calibrated = state
        logger.debug(f"✅ self.calibrated set to: {self.calibrated}")

        # Update the data acquisition object's calibrated state
        if self._data_acq is not None:
            self._data_acq.set_configuration(calibrated=state)
            logger.debug(f"📊 Updated data acquisition calibrated state to: {state}")
        ...
        # Hide progress bar when calibration completes (success or failure)
        try:
            progress_bar = self.main_window.ui.calibration_progress
            if state:
                # Success - show completion briefly then hide
                progress_bar.setValue(100)
                progress_bar.setFormat("100% - Calibration Complete!")
                # Use QTimer to hide after 2 seconds
                hide_timer = QTimer()
                hide_timer.setSingleShot(True)
                hide_timer.timeout.connect(lambda: progress_bar.setVisible(False))
                hide_timer.start(2000)
            else:
                # Failure - hide immediately
                progress_bar.setVisible(False)
        except Exception as e:
            logger.exception(
                f"Error hiding progress bar on calibration completion: {e}",
            )
    ...
    def _on_calibration_progress(self, step: int, message: str) -> None:
        """Handle calibration progress updates with percentage and time estimation."""
        try:
            # Total calibration steps (from SPRCalibrator)
            total_steps = 9
            ...
            if not hasattr(self, "_calibration_start_time"):
                self._calibration_start_time = time.time()
                self._calibration_step_times = []

            current_time = time.time()

            # Record step completion time
            if step > 1:  # Don't record time for first step
                step_duration = current_time - self._last_step_time
                self._calibration_step_times.append(step_duration)

            self._last_step_time = current_time

            # Estimate remaining time
            if len(self._calibration_step_times) >= 2:  # Need at least 2 samples
                avg_step_time = sum(self._calibration_step_times) / len(
                    self._calibration_step_times,
                )
                remaining_steps = total_steps - step
                estimated_seconds = int(avg_step_time * remaining_steps)
                ...
                if estimated_seconds > 60:
                    ...
                    time_remaining = f"{estimated_seconds}s"
            else:
            ...
            progress_bar.setFormat(f"{progress_percent}% - {time_remaining} remaining")

            # Update status to show calibration is active
            if step == 1:
                self.main_window.ui.status.setText("Calibrating...")

            logger.debug(
                f"Calibration progress: Step {step}/{total_steps} ({progress_percent}%) - {message}",
            )

            # Hide progress bar when calibration completes
            if step >= total_steps:
                self._hide_progress_bar_delayed()

        except Exception as e:
            logger.exception(f"Error handling calibration progress: {e}")

    def _hide_progress_bar_delayed(self) -> None:
        """Hide progress bar after a short delay to show completion."""
        try:
            # Show 100% completion for a moment
            progress_bar = self.main_window.ui.calibration_progress
            progress_bar.setValue(100)
            progress_bar.setFormat("100% - Complete!")

            # Hide after 2 seconds using QTimer
            hide_timer = QTimer()
            hide_timer.setSingleShot(True)
            hide_timer.timeout.connect(lambda: progress_bar.setVisible(False))
            hide_timer.start(2000)  # 2 seconds

            # Clean up timing data
            if hasattr(self, "_calibration_start_time"):
                delattr(self, "_calibration_start_time")
            if hasattr(self, "_calibration_step_times"):
                delattr(self, "_calibration_step_times")
            if hasattr(self, "_last_step_time"):
                delattr(self, "_last_step_time")

        except Exception as e:
            logger.exception(f"Error hiding progress bar: {e}")

    def _init_calibration_progress(self) -> None:
        """Initialize progress bar for calibration start."""
        try:
            progress_bar = self.main_window.ui.calibration_progress
            progress_bar.setVisible(True)
            progress_bar.setValue(0)
            progress_bar.setFormat("0% - Starting calibration...")
            ...
            logger.debug("Calibration progress bar initialized")
        except Exception as e:
            logger.exception(f"Error initializing calibration progress: {e}")
    ...
    def _show_no_device_dialog(self) -> None:
        """Show user-friendly dialog when no hardware is detected."""
        try:
            show_message(
                msg="No device connected.\n\n"
                "Connect the device and click Add Device to retry.",
                msg_type="Information",
            )
            logger.info("Displayed no device connection dialog to user")
        except Exception as e:
            logger.exception(f"Error showing no device dialog: {e}")
    ...
    def sensorgram_data(self: Self) -> DataDict:
        ...
        try:
            ...
            return {
                ...
                "buffered_lambda_times": data.get(
                    "buffered_lambda_times",
                    data["lambda_times"],
                ),
                "filtered_lambda_values": data.get(
    ...
    def auto_polarization(self: Self) -> None:
        ...
        try:
            if (
                self.device_config["ctrl"] not in DEVICES
    ...
    def three_way(self: Self, ch: str, state: int | str) -> None:
        ...
        if self.kinetic_manager is not None:
            try:
                ...
                if not success:
                    logger.warning(
                        f"Failed to set 3-way valve {ch} to position {position}",
                    )
                self._s_stop.clear()
            except Exception as e:
    ...
    def six_port(self: Self, ch: str, state: int | str) -> None:
        ...
        if self.kinetic_manager is not None:
            try:
                ...
                if not success:
                    logger.warning(
                        f"Failed to set 6-port valve {ch} to position {position}",
                    )
                self._s_stop.clear()
            except Exception as e:
    ...
    def speed_change_handler(self: Self, ch: str, rate: int) -> None:
        ...
        try:
            kin = self._get_kin_ops()
            self.pump_states = kin.handle_speed_change(
                ch,
                rate,
                self.pump_states,
                self.synced,
    ...
    def save_kinetic_log(self: Self, rec_dir: str) -> None:
        ...
        if self.knx is not None:
            try:
                knx_version = (
                    self.knx.version if hasattr(self.knx, "version") else "1.0"
                )
                ...
                if (
                    self.device_config["ctrl"] in ["PicoEZSPR"]
                ) or (  # EZSPR disabled (obsolete)
                    self.device_config["knx"]
                    in ["KNX2"]  # PicoKNX2 disabled (obsolete)
                ):
    ...
    def connect_advanced_menu(self: Self) -> None:
        ...
        if (
            self.device_config["ctrl"] in DEVICES
        ...
        if self.ctrl is None or self.knx is None or self.pump is None:
            ...
            try:
                ...
                if getattr(self, "_con_tr", None) and self._con_tr.is_alive():
                    ...
                    # DON'T pause calibration - let startup() handle it properly
                    logger.debug(
                        "Starting connection thread - calibration will be enabled after hardware init",
                    )
                    self._con_tr = threading.Thread(
                        target=self.connection_thread,
                        daemon=True,
    ...
    def disconnect_dev(
        ...
        *,
        ...
        try:
            ...
            # Update connection status based on remaining hardware
            self._update_connection_status()
    ...
    def on_crashed(self: Self) -> None:
        """Log a crash and ensure safe hardware shutdown."""
        try:
            logger.critical("Application crash detected - performing emergency cleanup")

            # Emergency LED shutdown to prevent hardware damage
            try:
                # Try hardware manager LED shutdown first
                if hasattr(self, "_hw_mgr") and self._hw_mgr is not None:
                    hw_mgr = self._hw_mgr
                    if hasattr(hw_mgr, "_safe_led_shutdown"):
                        hw_mgr._safe_led_shutdown()
                        logger.info("LEDs turned off via hardware manager during crash")
                else:
                    # Fallback to direct emergency LED shutdown
                    self._emergency_led_shutdown()
            except Exception as led_e:
                logger.error(f"Emergency LED shutdown failed during crash: {led_e}")
            ...
            if self.ctrl is not None:
                try:
                    if hasattr(self.ctrl, "close"):
                        self.ctrl.close()
                    elif hasattr(self.ctrl, "disconnect"):
            ...
            if self.usb is not None:
                try:
                    if hasattr(self.usb, "close"):
                        self.usb.close()
                    elif hasattr(self.usb, "disconnect"):
                        self.usb.disconnect()
                except Exception as e:
    ...
    def _emergency_led_shutdown(self) -> None:
        """Emergency LED shutdown using direct serial commands."""
        try:
            import time

            import serial

            logger.warning("Performing emergency LED shutdown via direct serial")
            ...
            logger.error(f"Emergency LED shutdown failed: {e}")
...
def main() -> None:
    ...
    def signal_handler(signum, frame):
        logger.critical(f"Signal {signum} received - performing emergency shutdown")
        if hasattr(app, "on_crashed"):
            app.on_crashed()
        sys.exit(1)

...
for ch in channels:
    ...
    if s_key in s_data and p_key in p_data:
        print(
            f"Channel {ch.upper()}: S-pol={len(s_data[s_key])}, P-pol={len(p_data[p_key])} measurements",
        )
...
fig.suptitle(
    "Transmission Spectra (T = S/P with dark subtraction)",
    fontsize=14,
    fontweight="bold",
)
...
for idx, ch in enumerate(channels):
    ...
    if len(spectra) > 0:
        ...
        ax.plot(
            ...
            alpha=0.7,
            linewidth=1.5,
            label=f"t={times[-1]:.1f}s",
        )

    ax.set_xlabel("Wavelength (nm)", fontsize=10)
    ax.set_ylabel("Transmission (S/P)", fontsize=10)
    ax.set_title(
        f"Channel {ch.upper()} - {len(spectra)} spectra",
        fontsize=11,
        fontweight="bold",
    )
    ...
    t_mean = spectra.mean()
    print(
        f"Channel {ch.upper()}: T range = [{t_min:.3f}, {t_max:.3f}], mean = {t_mean:.3f}",
    )

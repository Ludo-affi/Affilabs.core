...
transmittance_smooth = np.array(
    [savgol_filter(t, window_length=51, polyorder=3) for t in transmittance],
)
...
print(
    f"Peak position range: {np.min(peak_positions_valid):.1f} - {np.max(peak_positions_valid):.1f} pixels",
)
...
print("\n📊 SPECTRAL FEATURES:")
print(
    f"   P-mode signal: {np.mean(p_mean_intensity):.0f} ± {np.std(p_mean_intensity):.0f} counts",
)
print(f"   Temporal drift: {p_drift:+.0f} counts ({p_drift_pct:+.1f}%)")
print(f"   Noise level: {np.mean(p_noise):.0f} counts")
print(
    f"   Spectral correlation: {np.mean(spectral_correlations):.4f} (min: {np.min(spectral_correlations):.4f})",
)
...
if abs(corr_intensity) > 0.3:
    ...
    print(
        f"   • Peak variation {direction} when signal intensity changes (r={corr_intensity:+.2f})",
    )
    print("     → Thermal drift or LED aging affects peak stability")
...
if abs(corr_noise) > 0.3:
    ...
    print("     → Noisy spectra → unstable peak detection")

if abs(corr_shape) < -0.3:
    print(
        f"   • Peak variation increases when spectral shape changes (r={corr_shape:+.2f})",
    )
    print("     → Spectral distortion → poor peak fitting")
...
ax2.fill_between(
    ...
    color="r",
    alpha=0.2,
    label=f"±{peak_variation_pixels:.2f} px",
)
...
plt.savefig(
    output_dir / "spectral_features_peak_variation_analysis.png",
    dpi=150,
    bbox_inches="tight",
)
print(
...
print("   - This is INSTRUMENTAL (thermal/LED aging)")
print("   - ML can flag when drift exceeds normal bounds")
...
print(
    f"   - Strongest predictor: {strongest_predictor[0]} (r={strongest_predictor[1]:.2f})",
)
print("   - ML can detect when this feature goes out of bounds")
...
print("   Good quality threshold: < 50 RU")
print("   Poor quality threshold: > 150 RU")
print("   → ML classifies: GOOD / WARNING / BAD based on raw spectra")

print("\n5. **Action Items**:")
print("   ✅ Collect data from multiple devices/sensors")
print("   ✅ Label with known good/bad conditions")
print("   ✅ Extract features: signal, noise, drift, shape")
print("   ✅ Train classifier: predict peak variation from raw spectra")
print("   ✅ Deploy: Monitor raw spectra → predict downstream quality")

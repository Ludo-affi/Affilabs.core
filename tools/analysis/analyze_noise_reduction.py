r"""Noise reduction analysis on the latest training_data dataset.
...
def kalman_filter_positions(
    ...
    for i in range(1, n):
        ...
        )  # split noise into pos/vel channels
...
def analyze(
    ...
    if use_kf:
        ...
        kf_series = kalman_filter_positions(
            pos_nm,
            t,
            q_pos=kf_q,
            q_vel=kf_q * 5.0,
    ...
    if use_kf and kf_series is not None:
        print(
            f"Kalman P-P: {p2p_kf:.3f} nm; STD: {std_kf:.3f} nm  (q={kf_q}, r={r_eff:.3f})",
        )
    ...
    ax1.plot(
        ...
        "r-",
        linewidth=2,
        label=f"Smoothed (med{med_win}+avg{avg_win})",
    )
    if use_kf and kf_series is not None:
        ax1.plot(
            t,
            ...
            "k-",
            linewidth=1.5,
            label=f"Kalman (q={kf_q}, r={r_eff:.2f})",
        )
    ...
    outfile = (
        outdir / f"noise_reduction_{state}_{channel}_med{med_win}_avg{avg_win}.png"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Analyze and reduce peak monitoring noise",
    )
    parser.add_argument(
        "--state",
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Median filter window (odd)",
    )
    parser.add_argument(
        "--avg-win",
        type=int,
        default=11,
        help="Moving average window (odd)",
    )
    parser.add_argument(
        "--kalman",
        action="store_true",
        help="Include constant-velocity Kalman filter (high temporal resolution)",
    )
    parser.add_argument(
        "--kf-q",
        type=float,
        default=0.05,
        help="Process noise std (nm/s) base for position; velocity uses 5x",
    )
    parser.add_argument(
        "--kf-r",
        type=float,
        default=0.0,
        help="Measurement noise std (nm); 0 to auto-estimate from data",
    )

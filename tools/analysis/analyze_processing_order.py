...
def apply_afterglow_correction(spectrum: np.ndarray, afterglow: np.ndarray) -> np.ndarray:
...
def test_dark_denoising(data: Dict) -> Dict:
    ...
    if improvement > 1:
        print(f"  ✓ Denoising dark is BENEFICIAL")
    elif improvement < -1:
        print(f"  ✗ Denoising dark is HARMFUL")
    else:
        print(f"  ≈ Denoising dark has NEGLIGIBLE effect")
...
def main():
    ...
    print(f"  Afterglow data: {'✓ Present' if has_afterglow else '✗ Not present (zeros)'}")
    ...
    print(f"   {best_seq[1]['p2p']:.2f} px ({(baseline_p2p - best_seq[1]['p2p']) / baseline_p2p * 100:+.1f}%)")
...
def create_visualization(results: Dict, dark_results: Dict, channel: str):
    ...
    for idx, (seq_name, result) in enumerate(results.items(), 1):
        ...
        ax.set_title(f"{seq_name}: {result['p2p']:.0f} px", fontsize=10, fontweight='bold')
        ...
        ax.text(0.02, 0.98, desc_short, transform=ax.transAxes,
    ...
    for i, (name, value) in enumerate(zip(seq_names, p2p_values)):
        improvement = (baseline - value) / baseline * 100
        ax6.text(value + 10, i, f"{value:.0f} px ({improvement:+.1f}%)",
    ...
    for i, value in enumerate(dark_p2p):
        ax7.text(i, value + 5, f"{value:.1f} px", ha='center', fontsize=9, fontweight='bold')
    ...
    for i, (name, value) in enumerate(zip(seq_names, proc_times)):
        ax8.text(value + 0.1, i, f"{value:.2f} ms", va='center', fontsize=9, fontweight='bold')
    ...
    dark_improvement = (dark_results['original_dark']['p2p'] - dark_best[1]['p2p']) / dark_results['original_dark']['p2p'] * 100
...
• Don't denoise dark unless beneficial
    ...
    plt.suptitle(f'Processing Order Optimization - {channel.upper()}',

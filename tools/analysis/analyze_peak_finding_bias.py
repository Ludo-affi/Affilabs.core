...
def test_algorithm_on_synthetic_data(
    ...
    results = {method: [] for method in PEAK_FINDING_METHODS}

    total_tests = (
        len(peak_depths) * len(fwhm_values) * len(asymmetries) * len(noise_levels)
    )
    test_count = 0
...
def visualize_bias_analysis(analysis: dict, output_dir: Path):
    ...
    fig.suptitle(
        f"Bias Patterns - {best_method.upper()}",
        fontsize=14,
        fontweight="bold",
    )
    ...
    im = ax.imshow(
        ...
        aspect="auto",
        cmap="RdBu_r",
        extent=[unique_fwhms[0], unique_fwhms[-1], unique_depths[-1], unique_depths[0]],
    )
    ax.set_xlabel("FWHM (pixels)")
    ...
    im2 = ax.imshow(
        ...
        aspect="auto",
        cmap="RdBu_r",
        extent=[unique_noise[0], unique_noise[-1], unique_asym[-1], unique_asym[0]],
    )
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / f"bias_heatmaps_{best_method}.png",
        dpi=150,
        bbox_inches="tight",
    )
...
def main():
    ...
    print(
        f"Total combinations: {len(peak_depths) * len(fwhm_values) * len(asymmetries) * len(noise_levels)}",
    )
    print("Repeats per combination: 10")
    print(
        f"Total synthetic spectra: {len(peak_depths) * len(fwhm_values) * len(asymmetries) * len(noise_levels) * 10}",
    )
    ...
    corrections = {}
    for method in PEAK_FINDING_METHODS:
        ...
        print(
            f"  bias = {coeffs[0]:.3f}*depth + {coeffs[1]:.3f}*fwhm + {coeffs[2]:.3f}*asymmetry + {coeffs[3]:.3f}",
        )
        ...
        "summary": {
            method: {
                ...
                "mean_processing_time_ms": float(
                    analysis[method]["mean_processing_time"],
                ),

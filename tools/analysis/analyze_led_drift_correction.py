...
def analyze_s_mode_drift(s_data: dict) -> Dict:
    ...
    drift_percent = (total_intensity[-1] - total_intensity[0]) / total_intensity[0] * 100
    ...
    linear_r2 = 1 - np.sum(linear_residuals**2) / np.sum((total_intensity - np.mean(total_intensity))**2)
    ...
    if linear_r2 > 0.8:
        print(f"  ✅ Highly systematic drift (R² > 0.8) - good for correction")
    elif linear_r2 > 0.5:
        print(f"  ⚠️ Moderate systematic drift (R² > 0.5) - correction may help")
    else:
        print(f"  ❌ Low correlation (R² < 0.5) - mostly random noise")
...
def model_drift_per_pixel(s_data: dict, model_type: str = 'linear') -> Dict:
    ...
    print(f"  % pixels with R² > 0.8: {np.sum(r2_values > 0.8) / len(r2_values) * 100:.1f}%")
...
def apply_drift_correction(
    ...
    if correction_method == 'time_matched':
        ...
        print(f"  Using time-matched S-mode references")
        print(f"  Time offset range: {np.min(np.abs(s_timestamps[0] - p_timestamps[0])):.1f}s")
        ...
        for i, p_time in enumerate(p_timestamps):
            if model_type == 'linear':
                ...
                # y = a*t^2 + b*t + c
                s_reference[i] = coeffs[:, 0] * p_time**2 + coeffs[:, 1] * p_time + coeffs[:, 2]
        ...
        print(f"  Using S-mode intensity ratio normalization")
...
def compare_correction_methods(
    ...
    for method in methods:
        ...
        trans_uncorr, trans_corr = apply_drift_correction(
            p_data, s_data, drift_model, correction_method=method
        )
...
def visualize_drift_correction(
    ...
    ax1.plot(timestamps, linear_fit, 'r--', linewidth=2, label=f'Linear fit (R²={drift_analysis["linear_r2"]:.3f})')
    ...
    ax3.hist(drift_model['r2_values'], bins=50, color='purple', alpha=0.7, edgecolor='black')
    ax3.axvline(np.mean(drift_model['r2_values']), color='red', linestyle='--', linewidth=2,
    ...
    im = ax4.imshow(drift_analysis['spectra_corrected'].T, aspect='auto', cmap='viridis',
    ...
    for idx, method in enumerate(methods):
        ...
        ax.plot(p_timestamps, result['positions_uncorrected'],
               color=colors_uncorr[idx], linewidth=1, alpha=0.7, label='Uncorrected')
        ax.plot(p_timestamps, result['positions_corrected'],
               color=colors_corr[idx], linewidth=1.5, label='Corrected')
    ...
    width = 0.35
    ax8.bar(x_pos - width/2, pp_uncorr, width, label='Uncorrected', color='steelblue', alpha=0.7)
    ax8.bar(x_pos + width/2, pp_corr, width, label='Corrected', color='coral', alpha=0.7)
    ...
    for idx, (t_idx, label) in enumerate(zip(sample_times, sample_labels)):
        ax = plt.subplot(3, 4, 9 + idx)

        # Show uncorrected vs corrected transmission
        trans_uncorr = correction_results['time_matched']['transmission_uncorrected'][t_idx]
        trans_corr = correction_results['time_matched']['transmission_corrected'][t_idx]

        ax.plot(wavelength_pixels, trans_uncorr, 'b-', linewidth=1, alpha=0.7, label='Uncorrected')
    ...
    bars = ax12.bar(methods, improvements, color=colors_bar, alpha=0.7, edgecolor='black')
...
def main():
    ...
    # Visualize
    visualize_drift_correction(s_data, drift_analysis, drift_model, correction_results, OUTPUT_DIR)
    ...
    best_method = max(correction_results.keys(), key=lambda m: correction_results[m]['improvement'])
    for method in correction_results.keys():
        result = correction_results[method]
        print(f"  {method:<15}: {result['pp_uncorrected']:.1f} → {result['pp_corrected']:.1f} px "

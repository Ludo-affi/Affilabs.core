...
has_saturation = any(
    np.max(channel_data[ch]["spectra"]) >= 65535 for ch in ["A", "B", "C", "D"]
)
...
if has_saturation and mean_max_signal > 50000:
    ...
    if labeled_mode == "S":
        print(
            "⚠️  MISLABELED: Folder says S-mode but signal shows P-mode characteristics!",
        )
        print("   Likely cause: Polarizer didn't move from previous P-mode position")
elif mean_max_signal < 35000:
    actual_mode = "S"
    print("\n✅ ACTUAL MODE: S-mode (no sensor resonance → low signal)")
    if labeled_mode == "P":
        print(
            "⚠️  MISLABELED: Folder says P-mode but signal shows S-mode characteristics!",
        )
else:
    actual_mode = "Unknown"
...
for channel in ["A", "B", "C", "D"]:
    ...
    if drift_pct > 1.0:
        issues_found.append(
            f"Channel {channel}: {drift_pct:.2f}% drift (thermal or LED aging)",
        )
    ...
    if temporal_std > 100:
        issues_found.append(
            f"Channel {channel}: High temporal noise (σ={temporal_std:.0f})",
        )
    ...
    if spectral_correlation < 0.95:
        issues_found.append(
            f"Channel {channel}: Spectral shape changed (r={spectral_correlation:.3f})",
        )
...
print("\n✅ Data Quality:")
print("  • 4 channels × 480 spectra = 1,920 raw spectra")
print(f"  • {num_pixels} pixels per spectrum (full wavelength range)")
print("  • 4 Hz acquisition rate (production-matched)")
print("  • ~2 min per channel (good for proof of concept)")
...
fig.suptitle(
    f'{actual_mode}-Mode Spectral Data Analysis - {metadata["device_serial"]} ({metadata["sensor_quality"]} sensor)',
    fontsize=14,
    fontweight="bold",
)
...
for idx, channel in enumerate(["A", "B", "C", "D"]):
    ...
    ax.plot(pixels, spectra[0, :], "b-", alpha=0.7, linewidth=1, label="t=0s")
    ax.plot(
        pixels,
        ...
        alpha=0.7,
        linewidth=1,
        label=f"t={timestamps[len(spectra)//2]:.0f}s",
    )
    ax.plot(
        pixels,
        spectra[-1, :],
        ...
        alpha=0.7,
        linewidth=1,
        label=f"t={timestamps[-1]:.0f}s",
    )
    ax.plot(
        pixels,
        channel_data[channel]["dark"].flatten(),
        "k--",
        alpha=0.5,
        linewidth=1,
        label="Dark",
...
plt.savefig(
    output_dir / f"{actual_mode.lower()}_mode_spectral_analysis.png",
    dpi=150,
    bbox_inches="tight",
)
...
fig, axes = plt.subplots(2, 2, figsize=(14, 10))
fig.suptitle(
    f"{actual_mode}-Mode Temporal Analysis - Signal Stability",
    fontsize=14,
    fontweight="bold",
)
...
for idx, channel in enumerate(["A", "B", "C", "D"]):
    ...
    ax.axhline(
        ...
        linestyle="--",
        alpha=0.5,
        label=f"Mean: {np.mean(temporal_means):.0f}",
    )
...
plt.tight_layout()
plt.savefig(
    output_dir / f"{actual_mode.lower()}_mode_temporal_analysis.png",
    dpi=150,
    bbox_inches="tight",
)

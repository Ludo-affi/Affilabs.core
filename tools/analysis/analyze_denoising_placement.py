...
def calculate_transmission(p_signal: np.ndarray, p_dark: np.ndarray,
                          s_signal: np.ndarray, s_dark: np.ndarray) -> np.ndarray:
...
def find_minimum_position(spectrum: np.ndarray, search_start: int = 400, search_end: int = 1400) -> float:
    """Find minimum position in transmission spectrum."""
    # Ensure search region is valid
    if search_end > len(spectrum):
...
def process_strategy_1_denoise_raw_only(data: Dict[str, np.ndarray]) -> Tuple[np.ndarray, float]:
    ...
    for i in range(n_spectra):
        ...
        transmission = calculate_transmission(p_denoised, data['p_dark'],
                                             s_denoised, data['s_dark'])
...
def process_strategy_2_denoise_transmission(data: Dict[str, np.ndarray]) -> Tuple[np.ndarray, float]:
    ...
    for i in range(n_spectra):
        ...
        transmission = calculate_transmission(data['p_spectra'][i], data['p_dark'],
                                             data['s_spectra'][i], data['s_dark'])
...
def process_strategy_3_denoise_both(data: Dict[str, np.ndarray]) -> Tuple[np.ndarray, float]:
...
def process_strategy_4_denoise_none(data: Dict[str, np.ndarray]) -> Tuple[np.ndarray, float]:
...
def analyze_denoising_placement(channel: str = "channel_A"):
    ...
    # Test all strategies
    strategies = {
        'denoise_raw_only': ('Denoise S & P → Calculate T', process_strategy_1_denoise_raw_only),
        'denoise_transmission': ('Calculate T → Denoise T', process_strategy_2_denoise_transmission),
        'denoise_both': ('Denoise S & P → Calculate T → Denoise T', process_strategy_3_denoise_both),
    ...
    for strategy_name, (description, func) in strategies.items():
        ...
        if proc_time < 10.0:
            print(f"  ✓ Within 10ms budget")
        else:
            print(f"  ⚠️ Exceeds 10ms budget")
    ...
    print(f"{'Strategy':<30} {'P-P (px)':<12} {'Improvement':<12} {'Time (ms)':<10} {'Status'}")
    print("-" * 80)
    ...
    for strategy_name, result in sorted_strategies:
        ...
        print(f"{strategy_name:<30} {p2p:<12.2f} {improvement:>+10.1f}% {proc_time:<10.3f} {status}")
    ...
    print(f"   Peak-to-peak: {best_result['p2p']:.2f} px ({(baseline_p2p - best_result['p2p']) / baseline_p2p * 100:+.1f}%)")
...
def create_visualizations(results: Dict, channel: str):
    ...
    for idx, (strategy_name, result) in enumerate(results.items()):
        ...
        ax.set_title(f"{strategy_name}\nP-P: {result['p2p']:.1f} px", fontsize=11, fontweight='bold')
    ...
    bars = ax5.barh(strategy_names, p2p_values,
                    color=[colors[s] for s in strategy_names])

    # Add value labels
    for i, (name, value) in enumerate(zip(strategy_names, p2p_values)):
        baseline = results['denoise_none']['p2p']
        improvement = (baseline - value) / baseline * 100
        ax5.text(value + 2, i, f"{value:.1f} px ({improvement:+.1f}%)",
    ...
    bars = ax6.barh(strategy_names, proc_times,
                    color=[colors[s] for s in strategy_names])

    # Add value labels and budget line
    for i, (name, value) in enumerate(zip(strategy_names, proc_times)):
        ax6.text(value + 0.1, i, f"{value:.2f} ms",
                va='center', fontsize=10, fontweight='bold')
    ...
    for strategy_name, result in results.items():
        ax7.scatter(result['processing_time'], result['p2p'],
                   s=200, color=colors[strategy_name],
                   label=strategy_name, alpha=0.7, edgecolors='black', linewidth=2)
    ...
    baseline_p2p = results['denoise_none']['p2p']
    improvements = [(baseline_p2p - results[s]['p2p']) / baseline_p2p * 100
                   for s in strategy_names if s != 'denoise_none']
    strategy_names_no_baseline = [s for s in strategy_names if s != 'denoise_none']

    bars = ax8.barh(strategy_names_no_baseline, improvements,
                    color=[colors[s] for s in strategy_names_no_baseline])

    # Add value labels
    for i, (name, value) in enumerate(zip(strategy_names_no_baseline, improvements)):
        ax8.text(value + 1, i, f"{value:+.1f}%",
                va='center', fontsize=10, fontweight='bold')
    ...
    ax9.text(0.5, 0.5, recommendation, transform=ax9.transAxes,
            fontsize=11, verticalalignment='center', horizontalalignment='center',
            family='monospace',
            bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.3, pad=1))

    plt.suptitle(f'Denoising Placement Strategy Comparison - {channel.upper()}\n'

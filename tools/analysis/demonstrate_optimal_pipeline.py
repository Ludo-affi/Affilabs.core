...
def process_baseline(s_raw: np.ndarray, p_raw: np.ndarray,
...
def process_optimal_pipeline(s_raw: np.ndarray, p_raw: np.ndarray,
                             s_dark: np.ndarray, p_dark: np.ndarray) -> np.ndarray:
...
def analyze_with_all_methods(data: dict):
    ...
    for method_name, (method_desc, method_func) in methods.items():
        ...
        for i in range(n_spectra):
            transmission = process_baseline(
                data['s_spectra'][i], data['p_spectra'][i],
                data['s_dark'], data['p_dark']
            )
    ...
    print(f"{'Method':<20} {'Baseline (px)':<15} {'Optimal (px)':<15} {'Improvement':<15}")
    print("-" * 80)
    ...
    for method_name in methods.keys():
        ...
        print(f"{methods[method_name][0]:<20} {baseline_p2p:<15.2f} {optimal_p2p:<15.2f} {improvement:>+13.1f}%")
    ...
    print(f"🏆 BEST OPTIMAL: {methods[best_optimal[0]][0]} → {best_optimal[1]['p2p']:.2f} px")
...
def create_comprehensive_visualization(baseline_results, optimal_results, methods, data):
    ...
    for idx, (method_name, result) in enumerate(baseline_results.items(), 1):
        ...
        ax.set_title(f"BASELINE: {result['description']}\nP-P: {result['p2p']:.1f} px",
                    fontsize=10, fontweight='bold')
        ax.grid(True, alpha=0.3)

        if idx == 1:
            ax.text(-0.15, 0.5, 'NO DENOISING', transform=ax.transAxes,
    ...
    for idx, (method_name, result) in enumerate(optimal_results.items(), 1):
        ...
        ax.plot(times, positions, color=colors[method_name], linewidth=1.5, alpha=0.8)
        ax.set_ylabel('Position (px)', fontsize=9)
        ax.set_title(f"OPTIMAL: {result['description']}\nP-P: {result['p2p']:.1f} px",
                    fontsize=10, fontweight='bold')
        ax.grid(True, alpha=0.3)

        if idx == 1:
            ax.text(-0.15, 0.5, 'WITH DENOISING', transform=ax.transAxes,
                   fontsize=12, fontweight='bold', rotation=90,
    ...
    for idx, method_name in enumerate(methods.keys(), 1):
        ...
        ax.plot(times, baseline_pos, color=colors[method_name], linewidth=1.5,
               alpha=0.5, linestyle='--', label='Baseline')
        ax.plot(times, optimal_pos, color=colors[method_name], linewidth=2,
               alpha=0.9, label='Optimal')
    ...
    ax16.bar(x - width/2, baseline_p2p, width, label='Baseline',
            color=[colors[m] for m in method_names], alpha=0.5)
    ax16.bar(x + width/2, optimal_p2p, width, label='Optimal',
            color=[colors[m] for m in method_names], alpha=0.9)

    ax16.set_ylabel('Peak-to-Peak (px)', fontsize=10)
    ax16.set_title('Performance Comparison', fontsize=11, fontweight='bold')
    ax16.set_xticks(x)
    ax16.set_xticklabels([methods[m][0].split()[0] for m in method_names],
    ...
    improvements = [(baseline_results[m]['p2p'] - optimal_results[m]['p2p']) /
                   baseline_results[m]['p2p'] * 100 for m in method_names]

    bars = ax17.bar(x, improvements, color=[colors[m] for m in method_names], alpha=0.8)

    for i, v in enumerate(improvements):
        ax17.text(i, v + 1, f'{v:+.1f}%', ha='center', va='bottom',
                 fontsize=9, fontweight='bold')

    ax17.set_ylabel('Improvement (%)', fontsize=10)
    ax17.set_title('Noise Reduction', fontsize=11, fontweight='bold')
    ax17.set_xticks(x)
    ax17.set_xticklabels([methods[m][0].split()[0] for m in method_names],
                         fontsize=9, rotation=45, ha='right')
    ...
    ax18.set_xticks(x)
    ax18.set_xticklabels([methods[m][0].split()[0] for m in method_names],
                         fontsize=9, rotation=45, ha='right')
    ...
    baseline_trans = process_baseline(
        data['s_spectra'][mid_idx], data['p_spectra'][mid_idx],
        data['s_dark'], data['p_dark']
    )
    ...
    ax19.plot(pixels, baseline_trans[SEARCH_START:SEARCH_END],
             'r-', linewidth=1.5, alpha=0.5, label='Baseline')
    ax19.plot(pixels, optimal_trans[SEARCH_START:SEARCH_END],
             'g-', linewidth=2, alpha=0.8, label='Optimal')

    ax19.set_xlabel('Pixel Position', fontsize=10)
    ax19.set_ylabel('Transmission', fontsize=10)
    ax19.set_title('Example Spectrum\n(middle time point)', fontsize=11, fontweight='bold')
    ...
    overall_improvement = ((best_baseline[1]['p2p'] - best_optimal[1]['p2p']) /
                          best_baseline[1]['p2p'] * 100)
...
Status: {'PASS' if best_optimal[1]['proc_time'] < 10 else 'FAIL'}
    ...
    ax20.text(0.5, 0.5, recommendation, transform=ax20.transAxes,
             fontsize=10, verticalalignment='center', horizontalalignment='center',
             family='monospace',
             bbox=dict(boxstyle='round', facecolor='lightgreen', alpha=0.4, pad=1.5))

    plt.suptitle('Optimal Processing Pipeline Demonstration - Channel A\n'

...
def extract_peak_features(spectrum: np.ndarray, search_region: tuple[int, int]) -> dict:
    ...
    right_width = (
        indices[-1] if np.sum(below_half) > 0 else len(region) - 1
    ) - local_min_idx
    ...
    if 2 <= local_min_idx < len(region) - 2:
        sharpness = (
            region[local_min_idx - 2]
            - 2 * region[local_min_idx]
            + region[local_min_idx + 2]
        )
    else:
        sharpness = 0.0
...
BIAS_CORRECTIONS = {
    ...
    "polynomial": lambda depth, fwhm, asymmetry: 1.112 * depth
    + 0.810 * asymmetry
    - 0.831,
    "centroid": lambda depth, fwhm, asymmetry: 1.513 * depth
    + 0.002 * fwhm
    + 52.455 * asymmetry
    - 0.961,
}
...
def track_minimum_with_correction(
    ...
    print(
        f"    ✅ Complete in {elapsed:.3f}s ({stats['processing_time_per_spectrum']:.2f} ms/spectrum)",
    )
...
def visualize_results(
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "framework_demonstration.png",
        dpi=150,
        bbox_inches="tight",
    )
...
def main():
    ...
    for method in methods:
        ...
        positions_raw, positions_corrected, features_list, stats = (
            track_minimum_with_correction(
                transmission,
                method,
                search_region,
            )
        ...
        improvement = (
            1 - corrected_stats["peak_to_peak"] / raw_stats["peak_to_peak"]
        ) * 100

        print("\n    Raw sensorgram:")
        print(
            f"      P-P: {raw_stats['peak_to_peak']:.2f} px, STD: {raw_stats['std']:.2f} px",
        )
        print("    Corrected sensorgram:")
        print(
            f"      P-P: {corrected_stats['peak_to_peak']:.2f} px, STD: {corrected_stats['std']:.2f} px",
        )
    ...
    for method in methods:
        r = results[method]
        print(
            f"{method:<12} {r['raw_stats']['peak_to_peak']:<15.1f} "
    ...
    summary = {
        ...
        "methods": {
            method: {
                ...
                "corrected_peak_to_peak": float(
                    results[method]["corrected_stats"]["peak_to_peak"],
                ),
                "improvement_percent": float(results[method]["improvement"]),
                "processing_time_ms": float(
                    results[method]["processing_stats"]["processing_time_per_spectrum"],
                ),
            }
            for method in methods
        },
    ...
    best_corrected = min(
        methods,
        key=lambda m: results[m]["corrected_stats"]["peak_to_peak"],
    )
    best_improvement = max(methods, key=lambda m: results[m]["improvement"])

    print(
        f"  🏆 Best raw performance: {best_raw.upper()} "
        f"({results[best_raw]['raw_stats']['peak_to_peak']:.1f} px p-p)",
    )
    print(
        f"  🏆 Best corrected performance: {best_corrected.upper()} "
        f"({results[best_corrected]['corrected_stats']['peak_to_peak']:.1f} px p-p)",
    )
    print(
        f"  📈 Largest improvement: {best_improvement.upper()} "
        f"({results[best_improvement]['improvement']:.1f}%)",
    )
    print("  ⚡ All methods process in < 1 ms/spectrum")

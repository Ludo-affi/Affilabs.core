...
def causal_moving_average_time(series: np.ndarray, k: int) -> np.ndarray:
    ...
    for t in range(n):
        start = 0 if t - k + 1 < 0 else t - k + 1
        out[t] = np.mean(series[start:t + 1], axis=0)
    return out


def apply_spectral_denoise(spectrum: np.ndarray, kind: Optional[str], params: Optional[Mapping[str, int | float]]) -> np.ndarray:
...
def build_transmission(
    ...
    pre_params: Optional[Dict[str, int | float]] = None,
    post_kind: Optional[str] = None,
    post_params: Optional[Dict[str, int | float]] = None,
) -> np.ndarray:
...
def compute_variant_series(
    s_npz: Path,
    p_npz: Path,
    pre_kind: Optional[str],
    pre_params: Optional[Dict[str, int | float]],
    post_kind: Optional[str],
    post_params: Optional[Dict[str, int | float]],
    temporal_pre: int = 1,
...
def main():
    parser = argparse.ArgumentParser(description="Compare raw→transmission build variants (pre/post denoise)")
    ...
    parser.add_argument('--enable-sg-sweep', action='store_true', help='Add Savitzky–Golay pre-denoise sweep variants')
    parser.add_argument('--temporal-pre', type=int, default=1, help='Causal moving average window on S/P before ratio (frames)')
    ...
    variants = {
        ...
        "pre_sg21_post_sg5": ("sg", {"window": 21, "poly": 2}, "sg", {"window": 5, "poly": 2}),
    }
    ...
    for label, (pre_kind, pre_params, post_kind, post_params) in variants.items():
        trans_series, t = compute_variant_series(
            s_npz, p_npz, pre_kind, pre_params, post_kind, post_params, temporal_pre=args.temporal_pre
        )
        ...
        for i in range(n):
            positions[i] = OptimalProcessor.find_minimum_centroid_nm(
                trans_series[i], wl, search_min_nm=smin, search_max_nm=smax
            )
    ...
    for m in variants.keys():
        print(f"- {m:18s} P-P: {results_metrics[m]['p2p']:.3f} nm   STD: {results_metrics[m]['std']:.3f} nm   mean: {results_metrics[m]['mean']:.2f} nm")

    # Plot
    outpath = plot_results(t, list(variants.keys()), results_series, results_metrics, args.state, channel)

...
print(f"   Min position: {np.mean(min_positions):.1f} ± {np.std(min_positions):.2f} pixels")
print(f"   Peak-to-peak variation: {np.max(min_positions) - np.min(min_positions):.1f} pixels")
...
print(f"   Min position: {np.mean(median_positions):.1f} ± {np.std(median_positions):.2f} pixels")
print(f"   Peak-to-peak variation: {np.max(median_positions) - np.min(median_positions):.1f} pixels")
print(f"   Processing time: {np.mean(median_times):.3f} ± {np.std(median_times):.3f} ms")
...
def parabolic_peak_pipeline(p_spectrum, s_spectrum, p_dark, s_dark):
    ...
    if 1 <= min_idx < len(transmittance) - 1:
        y1, y2, y3 = transmittance[min_idx-1], transmittance[min_idx], transmittance[min_idx+1]
...
print(f"   Min position: {np.mean(parabolic_positions):.3f} ± {np.std(parabolic_positions):.3f} pixels")
print(f"   Peak-to-peak variation: {np.max(parabolic_positions) - np.min(parabolic_positions):.3f} pixels")
print(f"   Processing time: {np.mean(parabolic_times):.3f} ± {np.std(parabolic_times):.3f} ms")
print(f"   Improvement: {np.std(min_positions) - np.std(parabolic_positions):.3f} pixels")
...
def savgol_pipeline(p_spectrum, s_spectrum, p_dark, s_dark):
    ...
    if 1 <= min_idx < len(transmittance_sg) - 1:
        y1, y2, y3 = transmittance_sg[min_idx-1], transmittance_sg[min_idx], transmittance_sg[min_idx+1]
        denom = y1 - 2*y2 + y3
        if abs(denom) > 1e-10:
            offset = 0.5 * (y1 - y3) / denom
            min_pixel_interp = min_idx + offset
...
print(f"   Min position: {np.mean(savgol_positions):.3f} ± {np.std(savgol_positions):.3f} pixels")
print(f"   Peak-to-peak variation: {np.max(savgol_positions) - np.min(savgol_positions):.3f} pixels")
print(f"   Processing time: {np.mean(savgol_times):.3f} ± {np.std(savgol_times):.3f} ms")
...
print(f"   Min position: {np.mean(centroid_positions):.3f} ± {np.std(centroid_positions):.3f} pixels")
print(f"   Peak-to-peak variation: {np.max(centroid_positions) - np.min(centroid_positions):.3f} pixels")
print(f"   Processing time: {np.mean(centroid_times):.3f} ± {np.std(centroid_times):.3f} ms")
print(f"   Improvement: {np.std(min_positions) - np.std(centroid_positions):.3f} pixels")
...
print("\n{:<25} {:>10} {:>10} {:>12}".format("Method", "Std (px)", "P2P (px)", "Time (ms)"))
print("-" * 60)
for name, data in methods.items():
    print("{:<25} {:>10.3f} {:>10.1f} {:>12.3f}".format(
        name, data['std'], data['p2p'], data['time']
    ))
...
print(f"   ✅ Well under 10 ms limit!")

# Visualization
print("\n📊 Generating comparison plots...")

fig, axes = plt.subplots(3, 2, figsize=(16, 12))
fig.suptitle('SPR Processing Pipeline Comparison - Channel A', fontsize=14, fontweight='bold')
...
ax = axes[1, 1]
idx = 100  # Mid-point
for name, data in [('Production', production_results),
...
plt.savefig(output_dir / 'processing_pipeline_comparison.png', dpi=150, bbox_inches='tight')

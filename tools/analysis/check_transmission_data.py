...
# Load S-mode data
s_path = Path(
    "spectral_training_data/demo P4SPR 2.0/s/used/20251022_140707/channel_A.npz",
)
s_data = np.load(s_path)

# Load P-mode data
p_path = Path(
    "spectral_training_data/demo P4SPR 2.0/p/used/20251022_140940/channel_A.npz",
)
...
print(
    f"  Raw signal range: {s_data['spectra'][0].min():.0f} - {s_data['spectra'][0].max():.0f} counts",
)
...
print(
    f"  Raw signal range: {p_data['spectra'][0].min():.0f} - {p_data['spectra'][0].max():.0f} counts",
)
...
print("  Typical sensor dip location: pixel ~1800-2000 (if resonance present)")

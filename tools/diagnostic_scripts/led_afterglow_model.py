...
class LEDAfterglowModel:
    ...
    def fit_decay_data(self, times_ms, signals):
        ...
        try:
            ...
            # Calculate R-squared
            fitted_values = self.decay_function(
                times_ms,
                self.amplitude,
                self.tau,
                self.baseline,
...
def characterize_afterglow_decay(ctrl, spec, channel=ChannelID.A, n_cycles=5):
    ...
    """
    logger.info(f"\n{'='*60}")
    logger.info("🔬 LED AFTERGLOW DECAY CHARACTERIZATION")
    logger.info(f"{'='*60}")
    ...
    for cycle in range(n_cycles):
        ...
        signal_20ms = [
            m for m in cycle_data["decay_measurements"] if m["time_ms"] == 20
        ][0]["signal"]
        signal_100ms = [
            m for m in cycle_data["decay_measurements"] if m["time_ms"] == 100
        ][0]["signal"]

        logger.info(
            f"      Decay: 0ms={signal_0ms:.0f}, 20ms={signal_20ms:.0f}, 100ms={signal_100ms:.0f}",
        )
    ...
    for time_idx, time_ms in enumerate(decay_times_ms):
        signals_at_time = [
            cycle["decay_measurements"][time_idx]["signal"] for cycle in all_cycle_data
        ]
...
def plot_afterglow_results(results, model):
    ...
    fig.suptitle(
        f'LED Phosphor Afterglow Characterization - Channel {results["channel"].upper()}',
        fontsize=16,
        fontweight="bold",
    )
    ...
    ax1.set_title(
        "Afterglow Decay Curve (Linear Scale)",
        fontsize=13,
        fontweight="bold",
    )
    ...
    ax4.axvline(
        ...
        linestyle="--",
        alpha=0.7,
        label=f"5% residual: {delay_5pct:.0f}ms",
    )
    ax4.axvline(
        x=delay_2pct,
        color="red",
        linestyle="--",
        alpha=0.7,
        label=f"2% residual: {delay_2pct:.0f}ms",
    )
    ax4.axvline(
        x=delay_1pct,
        color="purple",
        linestyle="--",
        alpha=0.7,
        label=f"1% residual: {delay_1pct:.0f}ms",
    )
...
def plot_all_channels_comparison(all_channel_results):
    ...
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle(
        "LED Afterglow Comparison - All 4 Channels",
        fontsize=16,
        fontweight="bold",
    )
    ...
    for channel, results in all_channel_results.items():
        ...
        ax2.semilogy(
            ...
            markersize=4,
            linewidth=2,
            label=f'Channel {channel.upper()} (τ={results["decay_model"]["tau_ms"]:.2f}ms)',
        )
    ...
    bars = ax3.bar(
        channel_labels,
        taus,
        color=[colors[ch] for ch in channels],
        alpha=0.7,
    )
    ...
    for bar, tau in zip(bars, taus, strict=False):
    ...
    delays_5pct = [
        all_channel_results[ch]["recommended_delays"]["for_5pct_residual"]
        for ch in channels
    ]
    delays_2pct = [
        all_channel_results[ch]["recommended_delays"]["for_2pct_residual"]
        for ch in channels
    ]
    delays_1pct = [
        all_channel_results[ch]["recommended_delays"]["for_1pct_residual"]
        for ch in channels
    ]
...
def main():
    ...
    try:
        ...
        for channel in channels:
            ...
            results, model = characterize_afterglow_decay(
                ctrl,
                spec,
                channel,
                n_cycles=5,
        ...
        for channel in channels:
            plot_afterglow_results(
                all_results[channel.value],
                all_models[channel.value],
            )
        ...
        for channel in channels:
            ...
            logger.info(
                f"   τ = {results['decay_model']['tau_ms']:.2f} ms, R² = {results['decay_model']['r_squared']:.4f}",
            )
            logger.info(
                f"   Delays: 5%={results['recommended_delays']['for_5pct_residual']:.1f}ms, "
        ...
        max_delay_1pct = max(
            all_results[ch.value]["recommended_delays"]["for_1pct_residual"]
            for ch in channels
        )
        max_delay_2pct = max(
            all_results[ch.value]["recommended_delays"]["for_2pct_residual"]
            for ch in channels
        )
        max_delay_5pct = max(
            all_results[ch.value]["recommended_delays"]["for_5pct_residual"]
            for ch in channels
        )

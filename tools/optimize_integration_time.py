...
class IntegrationTimeOptimizer:
    ...
    def __init__(self, ctrl: PicoP4SPRHAL, spec: Spectrometer):
        ...
        try:
            ...
            if optical_cal_file and Path(optical_cal_file).exists():
                ...
                logger.info(
                    f"✅ Loaded afterglow correction: {Path(optical_cal_file).name}",
                )
    ...
    def test_integration_time(
        ...
        scans_per_measurement = max(
            1,
            int(round(normalize_time_ms / integration_time_ms)),
        )
        actual_time_ms = scans_per_measurement * integration_time_ms

        logger.info(
            f"Scans per measurement: {scans_per_measurement} (total ~{actual_time_ms:.1f}ms)",
        )
        ...
        logger.info("\n📊 Results:")
        logger.info(
            f"   Integration: {integration_time_ms:.1f}ms × {scans_per_measurement} scans = {actual_time_ms:.1f}ms total",
        )
        logger.info(
            f"   Signal: {mean_signal:.1f} ± {std_signal:.1f} counts (SNR: {snr:.1f})",
        )
    ...
    def optimize_channel(
        ...
        if optimal_result is None:
            ...
            logger.warning(
                f"   Best result: {optimal_result['estimated_ru_noise']:.2f} RU",
            )
        else:
    ...
    def plot_results(
        ...
        output_file: str = "integration_time_optimization.png",
    ):
        ...
        fig.suptitle(
            f"Integration Time Optimization - Channel {channel.upper()}",
            fontsize=14,
            fontweight="bold",
        )
        ...
        ax1.set_title(
            "Sensorgram Noise vs Integration Time",
            fontsize=13,
            fontweight="bold",
        )
        ...
        current_led_delay = (
            ...
            else self.afterglow_correction.get_optimal_led_delay(current_int_time, 2.0)
            * 1000
        )

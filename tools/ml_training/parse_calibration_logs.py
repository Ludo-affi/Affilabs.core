...
class CalibrationLogParser:
    ...
    def parse_all_logs(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """Parse all calibration logs.
        ...
        log_files = sorted(self.logs_dir.glob("calibration_*.log"))
        print(f"Found {len(log_files)} calibration log files")

        for i, log_file in enumerate(log_files, 1):
            if i % 50 == 0:
                print(f"  Processed {i}/{len(log_files)} logs...")
    ...
    def _parse_iterations(self, log_file: str, timestamp: str, lines: List[str]) -> List[IterationData]:
        ...
        while i < len(lines):
            ...
            # Detect polarization mode
            pol_match = re.search(self.POLARIZATION_PATTERN, line)
            if pol_match:
                current_polarization = pol_match.group(1).lower()

            # Detect iteration header (use search instead of match to handle any prefix)
            iter_match = re.search(self.ITERATION_PATTERN, line)
            if iter_match:
                current_iteration = int(iter_match.group(1))
                max_iterations = int(iter_match.group(2))
                current_integration = float(iter_match.group(3))
                ...
                while i < len(lines):
                    ...
                    # Stop at next iteration or end of section
                    if re.match(self.ITERATION_PATTERN, ch_line) or '===' in ch_line:

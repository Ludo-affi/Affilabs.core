...
def characterize_channel_at_integration_time(
    ...
    try:
        ...
        logger.info(
            f"      τ = {tau_fit:.2f} ms, R² = {r_squared:.4f}, A = {amplitude_fit:.1f} counts",
        )
        logger.info(
            f"      Delays: 5%={delays['5pct']:.1f}ms, 2%={delays['2pct']:.1f}ms, 1%={delays['1pct']:.1f}ms",
        )
...
def characterize_all_channels_all_integration_times(
    ...
    for channel in channels:
        ...
        successful_measurements = [
            m
            for m in channel_results["integration_time_data"]
        ...
        if len(successful_measurements) >= 2:
            ...
            logger.info(
                f"      τ range: {np.min(tau_values):.2f} - {np.max(tau_values):.2f} ms",
            )
            logger.info(
                f"      τ mean ± std: {np.mean(tau_values):.2f} ± {np.std(tau_values):.2f} ms",
            )
...
def plot_integration_time_dependency(results: dict[str, Any], output_dir: Path) -> None:
    ...
    fig.suptitle(
        "LED Afterglow: Integration Time Dependency Analysis",
        fontsize=14,
        fontweight="bold",
    )
    ...
    for channel_name in channels:
        ...
        if len(successful) >= 3:
            ...
            axes[1, 1].plot(
                ...
                measured,
                "o",
                label=f"Ch {channel_name} measured",
                color=color,
                markersize=6,
                alpha=0.7,
            )
    ...
    axes[1, 1].set_title(
        "Example Decay Curves (Mid Integration Time)",
        fontweight="bold",
    )
...
def main():
    ...
    if args.fast:
        ...
        print("\nThis fast test will:")
        print(
            f"  • Characterize channel{'s' if len(channels) > 1 else ''} {', '.join(channels)}",
        )
    ...
    try:
        if hasattr(ctrl, "set_mode"):
            ...
            if result:
                ...
                logger.warning(
                    "⚠️  Polarizer command returned False - may not have moved",
                )
        else:
            logger.error("❌ Controller does not have set_mode() method")
            raise RuntimeError(
                "Cannot set polarizer mode - controller missing set_mode()",
            )
    except Exception as e:
        logger.warning(f"⚠️  Could not set polarizer mode (demo hardware?): {e}")
        logger.warning(
            f"   Assuming polarizer is already in {polarizer_mode}-mode position",
        )
    ...
    if args.fast:
        integration_times_ms = [
            10,
            100,
        ]  # Just 2 integration times (fast interpolation)
        cycles = 3  # Fewer cycles for speed
        logger.info(
            f"⚡ FAST MODE: 2 integration times, 3 cycles (~{len(channels) * 3} min)",
        )
    else:
    ...
    try:
        ...
        json_file = (
            output_dir
            / f"led_afterglow_integration_time_models_{polarizer_mode}mode_{timestamp}.json"
        )
        ...
        for channel_name in channels:
            ...
            if "tau_lookup" in channel_data:
                ...
                print(
                    f"  τ range: {lookup['min_tau']:.2f} - {lookup['max_tau']:.2f} ms",
                )
                print(
                    f"  τ mean: {lookup['mean_tau']:.2f} ± {lookup['std_tau']:.2f} ms",
                )
                print(f"  Integration times tested: {lookup['integration_times_ms']}")
                print(
                    f"  Corresponding τ values: {[f'{t:.2f}' for t in lookup['tau_ms']]}",
                )

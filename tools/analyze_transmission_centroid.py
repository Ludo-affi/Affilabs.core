...
npz_files = sorted(calib_dir.glob("transmission_spectra_*.npz"), 
...
for ch in channels:
    ...
    print(f"  Applying adaptive SG filter...")
    ...
    if windows:
        print(f"  SG windows: mean={np.mean(windows):.1f}, range=[{min(windows)}, {max(windows)}]")
        print(f"  SG polyorders: mean={np.mean(polyorders):.1f}, range=[{min(polyorders)}, {max(polyorders)}]")
    
    # Calculate centroids
    centroids = []
    for i, (spec_raw, spec_filtered) in enumerate(zip(roi_spectra, filtered_spectra)):
...
for idx, ch in enumerate(channels):
    ...
    ax1.axhline(res['mean'], color='r', linestyle='--', linewidth=1, label=f"Mean={res['mean']:.3f} nm")
    ax1.fill_between(times, 
                      res['mean'] - res['std'], 
                      res['mean'] + res['std'], 
                      alpha=0.2, color='r', label=f"±1σ={res['std']:.4f} nm")
    ax1.set_xlabel('Time (s)', fontsize=9)
    ax1.set_ylabel('Centroid (nm)', fontsize=9)
    ax1.set_title(f'Ch {ch.upper()} - Centroid vs Time\np-p = {res["pp"]:.4f} nm', 
    ...
    ax2.plot(roi_wavelengths, res['raw_spectra'][mid_idx], 'gray', 
             linewidth=1, alpha=0.5, label='Raw')
    ax2.plot(roi_wavelengths, res['filtered_spectra'][mid_idx], 'b-', 
             linewidth=1.5, label='SG filtered')
    ax2.axvline(res['centroids'][mid_idx], color='r', linestyle='--', 
                linewidth=1, label=f'Centroid={res["centroids"][mid_idx]:.3f} nm')
    ax2.set_xlabel('Wavelength (nm)', fontsize=9)
    ax2.set_ylabel('Transmission', fontsize=9)
    ax2.set_title(f'Ch {ch.upper()} - Example Spectrum (t={times[mid_idx]:.1f}s)', 
                  fontsize=10, fontweight='bold')
    ...
    n, bins, patches = ax3.hist(centroids, bins=20, color='steelblue', alpha=0.7, edgecolor='black')
    ax3.axvline(res['mean'], color='r', linestyle='--', linewidth=2, label=f'Mean')
    ax3.set_xlabel('Centroid (nm)', fontsize=9)
    ax3.set_ylabel('Count', fontsize=9)
    ax3.set_title(f'Ch {ch.upper()} - Centroid Distribution\nσ={res["std"]:.4f} nm', 
                  fontsize=10, fontweight='bold')
    ax3.legend(fontsize=8)
    ax3.grid(True, alpha=0.3, axis='y')

fig.suptitle('Transmission Spectrum Centroid Analysis (Dynamic SG Filtering)', 
...
for idx, ch in enumerate(channels):
    ...
    ax.fill_between(times, 
                     -res['std'] * 1000, 
                     res['std'] * 1000, 
                     alpha=0.2, color='gray', label=f'±1σ={res["std"]*1000:.2f} pm')

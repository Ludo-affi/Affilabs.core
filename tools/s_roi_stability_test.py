...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="S-ROI (580–610 nm) stability diagnostic runner",
    )
    p.add_argument(
        "--duration",
        type=float,
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Fixed LED intensity (0-255); default uses calibrated ref_intensity or S_LED_INT",
    )
    p.add_argument(
        "--no-delays",
        action="store_true",
        help="Disable device_config LED delays and use default LED_DELAY",
    )
    p.add_argument(
        "--led-delay-ms",
        type=float,
        default=None,
        help="Fixed LED delay in milliseconds for both on/off (overrides device_config and --no-delays)",
    )
    p.add_argument(
        "--led-on-delay-ms",
        type=float,
        default=None,
        help="Delay after LED turn-on before measurement (overrides --led-delay-ms)",
    )
    p.add_argument(
        "--led-off-delay-ms",
        type=float,
        default=None,
        help="Delay after LED turn-off before next channel (overrides --led-delay-ms)",
    )
    p.add_argument(
        "--channels",
        type=str,
        default=None,
        help="Comma-separated channels (e.g., a,b,c,d). Default uses settings.CH_LIST",
    )
    p.add_argument(
        "--integration-ms",
        type=float,
        default=None,
        help="Override integration time in ms for this run",
    )
    p.add_argument(
        "--integration-per-ch",
        type=str,
        default=None,
        help="Per-channel integration times in ms (e.g., a:50,b:120,c:19,d:19)",
    )
    p.add_argument(
        "--preflight-only",
        action="store_true",
        help="Run LED/polarizer preflight check and exit",
    )
    p.add_argument(
        "--auto-integration",
        action="store_true",
        help="Auto-find optimal integration time (50-75%% of max) before running test",
    )
    return p.parse_args()
...
def main() -> int:
    ...
    try:
                ...
                if isinstance(s_leds, dict) and s_leds:
                    for ch, val in s_leds.items():
                        try:
                            ...
                            pass
                    logger.info(
                        f"Using S-mode LED intensities from device_config: {s_leds}",
                    )
            except Exception as e:
        ...
        if not calibrator.step_2_calibrate_wavelength_range():
            logger.error(
                "Step 2 (wavelength calibration) failed - cannot run stability test",
            )
            return 1
        ...
        if args.channels:
            ch_list = [
                ch.strip().lower() for ch in args.channels.split(",") if ch.strip()
            ]
        else:
            ch_list = CH_LIST
        ...
        if args.integration_ms is not None and args.integration_ms > 0:
            try:
                ...
                usb.set_integration_time(calibrator.state.integration)
                logger.info(
                    f"Overriding integration for this run: {args.integration_ms:.1f} ms",
                )
            except Exception as e:
        ...
        try:
            ...
            if w is not None and len(w) > 0:
                ...
                logger.warning(
                    "Filtered wavelengths not available; ROI indexing may fail",
                )
        except Exception:
        ...
        try:
            ...
            try:
                ...
                if (
                    not ok_mode
                    and hasattr(calibrator.ctrl, "servo_set")
                    and hasattr(calibrator, "_get_oem_positions")
                ):
                    ...
                    if s_pos is not None and p_pos is not None:
                        print(
                            "  ⚠️ Polarizer set_mode did not confirm; applying OEM servo positions directly",
                        )
                        try:
                            ok_servo = calibrator.ctrl.servo_set(
                                s=int(s_pos),
                                p=int(p_pos),
                            )
            ...
            if w is not None and len(w) > 0:
                ...
                print(
                    f"  Wavelength array: {len(w)} pixels, range {float(w[0]):.1f}-{float(w[-1]):.1f} nm",
                )
                roi_nm = (580.0, 610.0)
                ...
                if i1 > i0:
                    ...
                    dark_check = calibrator._acquire_calibration_spectrum(
                        apply_filter=False,
                        subtract_dark=False,
                    )
                    dark_check_f = (
                        calibrator._apply_spectral_filter(dark_check)
                        if dark_check is not None
                        else None
                    )
                    dark_check_roi = (
                        float(_np.max(dark_check_f[i0:i1]))
                        if (dark_check_f is not None and len(dark_check_f) > 0)
                        else 0.0
                    )
                    ...
                    dark = calibrator._acquire_calibration_spectrum(
                        apply_filter=False,
                        subtract_dark=False,
                    )
                    dark_f = (
                        calibrator._apply_spectral_filter(dark)
                        if dark is not None
                        else None
                    )
                    dark_roi = (
                        float(_np.max(dark_f[i0:i1]))
                        if (dark_f is not None and len(dark_f) > 0)
                        else 0.0
                    )
                    ...
                    print(
                        f"  📉 Dark decay: {decay:.1f} counts ({dark_check_roi:.0f} → {dark_roi:.0f})",
                    )
                    if dark_roi > 5000:
                        print(
                            f"  ⚠️  WARNING: Dark baseline still high ({dark_roi:.0f} counts) - may need longer settle time",
                        )
                    elif abs(decay) < 100:
                        print("  ✅ Dark baseline stable (minimal decay)")
                    ...
                    for ch in ch_list:
                        ...
                        time.sleep(0.12)
                        sig = calibrator._acquire_calibration_spectrum(
                            apply_filter=False,
                            subtract_dark=False,
                        )
                        sig_f = (
                            calibrator._apply_spectral_filter(sig)
                            if sig is not None
                            else None
                        )
                        sig_roi = (
                            float(_np.max(sig_f[i0:i1]))
                            if (sig_f is not None and len(sig_f) > 0)
                            else 0.0
                        )
                        delta = sig_roi - dark_roi
                        print(
                            f"ROI={sig_roi:.0f}, dark={dark_roi:.0f}, Δ={delta:.0f} @ LED={per_led[ch]}",
                        )
                        calibrator._all_leds_off_batch()
                        time.sleep(0.08)
                        if (
                            delta < 500.0
                        ):  # Should be MUCH higher with proper LED/integration
                            low_count_channels.append(ch)

                    print()
                    if low_count_channels:
                        print(
                            f"⚠️ Low ROI response detected on channels: {low_count_channels}",
                        )
                        print(
                            "   Check: 1) LED wiring/power 2) Polarizer in S position 3) Integration time sufficient",
                        )
                    else:
        ...
        try:
            ...
            if not files:
                print(
                    "⚠️ No stability CSV found to plot - diagnostic may have failed to save data",
                )
            ...
            for idx, ch in enumerate(ch_list, start=1):
                ...
                cs_rolling = [v - m for v, m in zip(vs, rm, strict=False)]
                ...
                if (
                    ch != "b"
                    and "b" in per_ch_vals
                    and len(per_ch_vals["b"]) == len(vs)
                ):
                    # Normalize by channel B's rolling mean
                    ref_rm = rolling_mean(per_ch_vals["b"], window)
                    cs_ref = [
                        float(v / (r / _np.mean(per_ch_vals["b"])))
                        if r > 0
                        else float(v)
                        for v, r in zip(vs, ref_rm, strict=False)
                    ]
                ...
                rolling_std_val = (
                    _np.mean(rs_rolling[window:]) if len(rs_rolling) > window else 0
                )
                adaptive_std_val = (
                    _np.mean(rs_adaptive[window:]) if len(rs_adaptive) > window else 0
                )

                improvement_rolling = (
                    ((raw_std - rolling_std_val) / raw_std * 100) if raw_std > 0 else 0
                )
                improvement_adaptive = (
                    ((raw_std - adaptive_std_val) / raw_std * 100) if raw_std > 0 else 0
                )
                ...
                _plt.plot(
                    ts,
                    rs_rolling,
                    label=f"Rolling Mean ({improvement_rolling:+.1f}%)",
                    alpha=0.8,
                    linewidth=1.5,
                )
                _plt.plot(
                    ts,
                    rs_adaptive,
                    label=f"Adaptive Poly ({improvement_adaptive:+.1f}%)",
                    alpha=0.8,
                    linewidth=1.5,
                )
                if ch != "b":
                    _plt.plot(
                        ...
                        rs_ref,
                        "--",
                        label="Ref-normalized",
                        alpha=0.6,
                        linewidth=1,
                    )

                _plt.title(
                    f"Channel {ch.upper()} | Rolling Std Comparison (window={window})",
                )
                _plt.xlabel("time (s)")
            ...
            for ch in ch_list:
                ...
                print(
                    f"Channel {ch.upper()}: Raw σ={s['raw_std']:.1f} | "
                ...
                try:
                    ...
                    for idx, ch in enumerate(ch_list, start=1):
                        ...
                        if spectra_key in data and times_key in data:
                            ...
                            for i, (t, spectrum) in enumerate(
                                zip(times, spectra, strict=False),
                            ):
                                color = _plt.cm.viridis(
                                    t / times[-1] if len(times) > 1 else 0.5,
                                )
                                _plt.plot(
                                    wavelengths,
                                    spectrum,
                            ...
                            _plt.title(
                                f"Channel {ch.upper()} - Stacked S-pol Spectra (n={len(spectra)})",
                            )
                            ...
                            if idx == 1:
                                sm = _plt.cm.ScalarMappable(
                                    ...
                                    norm=_plt.Normalize(
                                        vmin=0,
                                        vmax=times[-1] if len(times) > 0 else 1,
                                    ),
                                )
                                sm.set_array([])
                    ...
                    spectra_png_path = csv_path.parent / (
                        csv_path.stem + "_spectra.png"
                    )
                    _plt.tight_layout()

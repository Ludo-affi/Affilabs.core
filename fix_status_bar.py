...
with open('Old software/main/main.py', 'r', encoding='utf-8') as f:
    content = f.read()

# Replace ui.status references
content = re.sub(r'self\.main_window\.ui\.status\.setText\([^)]+\)', 'logger.info("Status updated")', content)
content = re.sub(r'self\.main_window\.ui\.status\.repaint\(\)', '# Status bar removed', content)
content = re.sub(r'self\.main_window\.ui\.status\.text\(\)', '"Connected"', content)
content = re.sub(r'self\.main_window\.ui\.device\.setText\([^)]+\)', '# Device display removed', content)
